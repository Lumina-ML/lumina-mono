...
@lumina/ui/src/widgets/        # Widget 框架
  ...
  GridStackLayout.vue  # 可拖拽的 GridStack 布局
...
```vue
<GridStackLayout v-model:layout="layout" :editable="editable">
  <template #default="{ item }">
...
- 拖拽把手：`.widget-drag-handle`
- 响应式：小于 768px 自动单列
- 编辑模式：通过 `editable` 开关拖拽/缩放
