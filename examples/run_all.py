#!/usr/bin/env python3
"""Run every Lumina example script against a local backend, in isolated
subprocesses, and print a pass / skip / fail summary.

Usage:
    python examples/run_all.py                 # runs against $LUMINA_API_URL or localhost:8000

An example is counted as SKIPped (not failed) when it prints a line starting
with "SKIP:" — this is how examples signal a missing optional dependency
(numpy, Pillow, torch).
"""
...
def check_server() -> bool:
    try:
        ...
        print("Start it first, e.g. `docker compose up` or the server dev process.")
        return False
...
def main() -> int:
    ...
    scripts = sorted(p for p in EXAMPLES_DIR.glob("*_experiment.py"))
