...
def _env_as_bool(var: str, default: str | None=None, env: MutableMapping | None=None) -> bool:
    if env is None:
        env = os.environ
...
def get_error_reporting(default: bool | str=True, env: MutableMapping | None=None) -> bool | str:
    if env is None:
        env = os.environ
    return env.get(ERROR_REPORTING, default)

def get_run(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
    return env.get(RUN_ID, default)

def get_args(default: list[str] | None=None, env: MutableMapping | None=None) -> list[str] | None:
    if env is None:
        env = os.environ
    if env.get(ARGS):
        try:
...
def get_docker(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
...
def get_file_pusher_timeout(default: int | None=None, env: MutableMapping | None=None) -> int | None:
    if env is None:
        env = os.environ
    timeout = env.get(FILE_PUSHER_TIMEOUT, default)
    return int(timeout) if timeout else None

def get_ignore(default: list[str] | None=None, env: MutableMapping | None=None) -> list[str] | None:
    if env is None:
        env = os.environ
...
def get_project(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
    return env.get(PROJECT, default)

def get_username(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
    return env.get(USERNAME, default)

def get_user_email(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
    return env.get(USER_EMAIL, default)

def get_entity(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
    return env.get(ENTITY, default)

def get_organization(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
    return env.get(ORGANIZATION, default)

def get_base_url(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
    return env.get(BASE_URL, default)

def get_app_url(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
...
def get_description(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
...
def get_dir(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
    return env.get(DIR, default)

def get_config_paths(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
    return env.get(CONFIG_PATHS, default)

def get_agent_report_interval(default: str | None=None, env: MutableMapping | None=None) -> int | None:
    if env is None:
        env = os.environ
...
def get_agent_kill_delay(default: str | None=None, env: MutableMapping | None=None) -> int | None:
    if env is None:
        env = os.environ
...
def get_crash_nosync_time(default: str | None=None, env: MutableMapping | None=None) -> int | None:
    if env is None:
        env = os.environ
...
def get_magic(default: str | None=None, env: MutableMapping | None=None) -> str | None:
    if env is None:
        env = os.environ
...
def get_agent_max_initial_failures(default: int | None=None, env: MutableMapping | None=None) -> int | None:
    if env is None:
        env = os.environ
