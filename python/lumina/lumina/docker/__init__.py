...
from lumina.docker import names
...
entrypoint = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'wandb-entrypoint.sh')
...
def build(tags: list[str], file: str, context_path: str, platform: str | None=None) -> str:
    ...
    stdout = run_command_live_output(args)
    return stdout
...
def run(args: list[Any], capture_stdout: bool=True, capture_stderr: bool=True, input: bytes | None=None, return_stderr: bool=False, env: dict[str, str] | None=None) -> str | tuple[str, str]:
    ...
    completed_process = subprocess.run(args, input=input, stdout=stdout_dest, stderr=stderr_dest, env=subprocess_env)
    if completed_process.returncode != 0:
        raise DockerError(args, completed_process.returncode, completed_process.stdout, completed_process.stderr)
    if return_stderr:
        return (_post_process_stream(completed_process.stdout), _post_process_stream(completed_process.stderr))
    else:
...
def image_id_from_registry(image_name: str) -> str | None:
    ...
    format_args = ['--format', '{{.Name}}@{{.Manifest.Digest}}']
...
__all__ = ['shell', 'build', 'run', 'image_id', 'image_id_from_registry', 'is_docker_installed', 'parse', 'parse_repository_tag', 'default_image', 'get_image_uid', 'push', 'login', 'tag']
