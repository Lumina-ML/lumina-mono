...
import lumina
import lumina.env
from lumina.errors import AuthenticationError, CommError, UsageError, WandbCoreNotAvailableError
from lumina.errors.term import terminput
from lumina.sdk.lib import runid
from lumina.sdk.lib.json_util import dump, dumps
...
RE_DOCKER_IMAGE_NAME_CHARS = re.compile('[^a-z0-9._\\-]')
POW_10_BYTES = [('B', 10 ** 0), ('KB', 10 ** 3), ('MB', 10 ** 6), ('GB', 10 ** 9), ('TB', 10 ** 12), ('PB', 10 ** 15), ('EB', 10 ** 18)]
POW_2_BYTES = [('B', 2 ** 0), ('KiB', 2 ** 10), ('MiB', 2 ** 20), ('GiB', 2 ** 30), ('TiB', 2 ** 40), ('PiB', 2 ** 50), ('EiB', 2 ** 60)]
...
def get_module(name: str, required: str | None=None, lazy: bool=True) -> Any:
...
def app_url(api_url: str) -> str:
    ...
    if (app_url := lumina.env.get_app_url()):
...
def check_windows_valid_filename(path: int | str) -> bool:
    """Verify that the given path does not contain any invalid characters for a Windows filename.
    ...
    """
    return not bool(re.search('[<>:"\\\\?*]', path))
...
def make_file_path_upload_safe(path: str) -> str:
    ...
    if sys_platform == 'Windows' and (not check_windows_valid_filename(path)):
        raise ValueError(f'Path {path} is invalid. Please remove invalid filename characters (\\, :, *, ?, ", <, >, |)')
...
def make_tarfile(output_filename: str, source_dir: str, archive_name: str, custom_filter: Callable | None=None) -> None:
    ...
    try:
        ...
        with open(output_filename, 'wb') as out_file, GzipFile(filename='', fileobj=out_file, mode='wb', mtime=0) as gzipped_tar, open(unzipped_filename, 'rb') as tar_file:
...
def _numpy_generic_convert(obj: Any, *, preserve_nan: bool=False) -> Any:
    ...
    if isinstance(obj, float) and math.isnan(obj) and (not preserve_nan):
        obj = None
    elif isinstance(obj, np.generic) and (obj.dtype.kind == 'f' or obj.dtype == 'bfloat16'):
        obj = float(obj)
    return obj

def _sanitize_numpy_keys(d: dict, visited: dict[int, dict] | None=None) -> tuple[dict, bool]:
...
def json_friendly(obj: Any, *, preserve_numpy_nan: bool=False) -> tuple[Any, bool] | tuple[None | str | float, bool]:
    ...
    if getsizeof(obj) > VALUE_BYTES_LIMIT:
        lumina.termwarn(f'Serializing object of type {type(obj).__name__} that is {getsizeof(obj)} bytes')
...
def maybe_compress_history(obj: Any) -> tuple[Any, bool]:
    if np and isinstance(obj, np.ndarray) and (obj.size > 32):
        return (lumina.Histogram(obj, num_bins=32).to_json(), True)
    else:
        return (obj, False)
...
def parse_backend_error_messages(response: Response) -> list[str]:
    ...
    def get_message(error: Any) -> str | None:
        if isinstance(error, str):
            return error
        elif isinstance(error, dict) and (message := error.get('message')) and isinstance(message, str):
            return message
        else:
            return None
...
def no_retry_auth(e: Any) -> bool:
    ...
    if e.response.status_code == 404:
        if parse_backend_error_messages(e.response):
            ...
            raise LookupError(f'Failed to find resource. Please make sure you have the correct resource path. (Error {e.response.status_code}: {e.response.reason})')
    return False
...
def docker_image_regex(image: str) -> Any:
    ...
    if image:
        return re.match('^(?:(?=[^:\\/]{1,253})(?!-)[a-zA-Z0-9-]{1,63}(?<!-)(?:\\.(?!-)[a-zA-Z0-9-]{1,63}(?<!-))*(?::[0-9]{1,5})?/)?((?![._-])(?:[a-z0-9._-]*)(?<![._-])(?:/(?![._-])[a-z0-9._-]*(?<![._-]))*)(?::(?![.-])[a-zA-Z0-9_.-]{1,128})?$', image)
    return None
...
def async_call(target: Callable, timeout: int | float | None=None) -> Callable:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> tuple[Exception, threading.Thread] | tuple[None, threading.Thread]:
...
def class_colors(class_count: int) -> list[list[int] | tuple[float, float, float]]:
    return [[0, 0, 0]] + [colorsys.hsv_to_rgb(i / (class_count - 1.0), 1.0, 1.0) for i in range(class_count - 1)]

def prompt_choices(choices: Sequence[str], input_timeout: float | None=None) -> str:
    ...
    for i, choice_str in enumerate(choices):
        lumina.termlog(f'({i + 1}) {choice_str}')
    while True:
        ...
        if idx < 0 or idx > len(choices) - 1:
            lumina.termwarn('Invalid choice')
            continue
        result = choices[idx]
        lumina.termlog(f'You chose {result!r}')
        return result
...
def to_human_size(size: int, units: list[tuple[str, Any]] | None=None) -> str:
    ...
    return f'{factor}{unit}' if factor < 1024 or len(units) == 1 else to_human_size(size, units[1:])
...
def check_and_warn_old(files: list[str]) -> bool:
    if 'wandb-metadata.json' in files:
        lumina.termwarn('These runs were logged with a previous version of wandb.')
        lumina.termwarn('Run pip install wandb<0.10.0 to get the old library and sync your runs.')
        return True
    return False
...
def rand_alphanumeric(length: int=8, rand: ModuleType | random.Random | None=None) -> str:
    lumina.termerror("rand_alphanumeric is deprecated, use 'secrets.token_hex'")
    rand = rand or random
...
def _is_kaggle() -> bool:
    return os.getenv('KAGGLE_KERNEL_RUN_TYPE') is not None or 'kaggle_environments' in sys.modules
...
def _is_py_requirements_or_dockerfile(path: str) -> bool:
    ...
    return file.endswith('.py') or file.startswith('Dockerfile') or file == 'requirements.txt'

def artifact_to_json(artifact: Artifact) -> dict[str, Any]:
    return {'_type': 'artifactVersion', '_version': 'v0', 'id': artifact.id, 'version': artifact.source_version, 'sequenceName': artifact.source_name.split(':')[0], 'usedAs': artifact.use_as}
    ...
    for item in d.values():
        if isinstance(item, dict):
            ...
            if contains_artifacts:
                return True
        elif (isinstance(item, lumina.Artifact) or _is_artifact_string(item)) and nested:
            return True
    return False
...
def _parse_entity_project_item(path: str) -> tuple:
    ...
    words = path.split('/')
    if len(words) > 3:
        raise ValueError('Invalid path: must be str the form {item}, {project}/{item}, or {entity}/{project}/{item}')
...
def _is_artifact_object(v: Any) -> TypeGuard[lumina.Artifact]:
    return isinstance(v, lumina.Artifact)
...
def _is_artifact_representation(v: Any) -> bool:
    return _is_artifact_object(v) or _is_artifact_string(v) or _is_artifact_version_weave_dict(v)
...
def _get_max_cli_version() -> str | None:
    max_cli_version = lumina.api.max_cli_version()
...
def make_artifact_name_safe(name: str) -> str:
    ...
    if len(cleaned) <= 128:
        return cleaned
    return re.sub('(^.{63}).*(.{63}$)', '\\g<1>..\\g<2>', cleaned)
...
def time_string_to_seconds(time_str: str) -> int:
    ...
    if not time_str:
        return 0
    match = re.match('^(\\d+)([smhdMy])$', time_str)
    if not match:
        raise ValueError(f'Invalid time period format: {time_str}. Expected format: <number><unit> where unit is s (seconds), m (minutes), h (hours), d (days), M (months), or y (years)')
    amount = int(match.group(1))
    unit = match.group(2)
    conversions = {'s': 1, 'm': 60, 'h': 3600, 'd': 86400, 'M': 2592000, 'y': 31536000}
