...
def _display_by_wandb_path(path: str, *, height: int) -> None:
    ...
    try:
        ...
        IPython.display.display_html(obj.to_html(height=height), raw=True)
    except lumina.Error:
        traceback.print_exc()
...
def _display_wandb_run(run: lumina.Run, *, height: int) -> None:
    ...
    Args:
        ...
        height: Height of the iframe in pixels.
    """
    IPython.display.display_html(run.to_html(height=height), raw=True)
...
class WandBMagics(Magics):
    ...
    @magic_arguments()
    @argument('path', default=None, nargs='?', help='The path to a resource you want to display.')
    ...
    def wandb(self, line: str, cell: str | None=None) -> None:
        ...
        args = parse_argstring(self.lumina, line)
...
def notebook_metadata_from_jupyter_servers_and_kernel_id():
    try:
        ...
        if ipython is not None:
            ...
            if notebook_path:
                return {'root': os.path.dirname(notebook_path), 'path': notebook_path, 'name': os.path.basename(notebook_path)}
    ...
    for s in servers:
        ...
        for nn in res:
            if isinstance(nn, dict) and nn.get('kernel') and ('notebook' in nn) and (nn['kernel']['id'] == kernel_id):
...
def notebook_metadata(silent: bool) -> dict[str, str]:
    ...
    lumina.termerror(error_message)
    return {}
...
def jupyter_servers_and_kernel_id():
    ...
    try:
        ...
        serverapp = lumina.util.get_module('jupyter_server.serverapp')
        notebookapp = lumina.util.get_module('notebook.notebookapp')
        servers = []
...
def attempt_colab_load_ipynb():
    colab = lumina.util.get_module('google.colab')
    if colab:
...
def attempt_kaggle_load_ipynb():
    kaggle = lumina.util.get_module('kaggle_session')
    if not kaggle:
        return None
    try:
        ...
        lumina.termerror('Unable to load kaggle notebook.')
...
class Notebook:
    ...
    def save_ipynb(self) -> bool:
        ...
        try:
            ...
            lumina.termerror('Failed to save notebook.')
    ...
    def save_history(self, run: lumina.Run):
        ...
        try:
            ...
            lumina.termerror('The nbformat package was not found. It is required to save notebook history.')
            return
        ...
        try:
            ...
            nb = v4.new_notebook(cells=cells, metadata={'kernelspec': {'display_name': f'Python {sys.version_info[0]}', 'name': f'python{sys.version_info[0]}', 'language': 'python'}, 'language_info': language_info})
            ...
            with open(os.path.join(self.settings.files_dir, state_path), 'w', encoding='utf-8') as f:
                write(nb, f, version=4)
        except (OSError, validator.NotebookValidationError):
            lumina.termerror('Unable to save notebook session history.')
