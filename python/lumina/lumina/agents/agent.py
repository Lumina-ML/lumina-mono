...
THREAD_NAME = "_terminated"

def _terminate_thread(thread: Thread):
    if not thread.is_alive():
        logger.warning("agent terminate thread is not alive.")
        return
    if hasattr(thread, THREAD_NAME):
        return    
    ...
    if tid is None:
        logger.warning("cannot find thread id.")
        return
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    thread = Thread(target=self._run_job, args=(job,))
