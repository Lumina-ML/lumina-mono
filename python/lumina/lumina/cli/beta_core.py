...
import logging
import click
from lumina import env as wandb_env
...
from lumina.sdk.lib.service import service_process, service_token
...
def start(*, idle_timeout: str) -> None:
    ...
    if token:
        raise click.UsageError(f'{wandb_env.SERVICE} is already set. Clear it or run `wandb beta core stop` before starting another detached service.')
    ...
    click.echo(f'Set {wandb_env.SERVICE} to this value before starting worker processes: {token_value}', err=True)
    click.echo('Any Python process launched with that environment variable will connect to the existing service instead of spawning its own.', err=True)
...
def stop(*, exit_code: int=0) -> None:
    ...
    try:
        ...
        raise click.UsageError(str(e)) from None
    if not token:
        raise click.UsageError(f'{wandb_env.SERVICE} is not set. Set it to the detached service you want to stop and rerun the command.')
    ...
    try:
        ...
        async def publish_teardown_and_close() -> None:
            await client.publish(spb.ServerRequest(inform_teardown=spb.ServerInformTeardownRequest(exit_code=exit_code)))
            await client.close()
        ...
        raise click.ClickException(f'Failed to connect to wandb-core using {wandb_env.SERVICE}: {e}') from e
    ...
    click.echo(f'Clear {wandb_env.SERVICE} from any shells or process environments that still set it.', err=True)
