...
from lumina.env import error_reporting_enabled, is_debug
...
@click.group(cls=DefaultCommandGroup, default_cmd='run', invoke_without_command=True, context_settings={'help_option_names': ['-h', '--help']})
def leet() -> None:
...
@click.option('--pprof', default='', hidden=True, help='Serve /debug/pprof/* on this address (e.g. 127.0.0.1:6060).')
...
@leet.command()
@click.option('--pprof', default='', hidden=True, help='Serve /debug/pprof/* on this address (e.g. 127.0.0.1:6060).')
@click.option('--interval', default='', metavar='DURATION', help='Sampling interval for system metrics (e.g. 500ms, 2s, 1m).')
@click.help_option('-h', '--help')
...
def _create_remote_launch_config(path: str) -> RemoteLaunchConfig:
    ...
    auth = wbauth.authenticate_session(host=base_url, source='wandb-cli', no_offline=True, input_timeout=wandb_setup.singleton().settings.login_timeout)
...
def _parse_remote_url(path: str) -> tuple[str, str]:
    ...
    if parsed_url.scheme not in {'http', 'https'} or not parsed_url.netloc:
        _fatal(f'Invalid remote URL: {path!r}. Expected format: https://<host>/<entity>/<project>/runs/<run_id>')
