...
from lumina.sandbox import CWSandboxError, Sandbox, SandboxStatus
from lumina.sandbox._auth import override_sandbox_entity
...
class SandboxCommand(click.Command):
    ...
    def __init__(self, *args: object, **kwargs: object) -> None:
        ...
        self.params = [click.Option(['-e', '--entity'], default=None, help="Set the W&B entity for sandbox. Default is user's default entity."), *self.params]
...
@sandbox.command('ls')
@click.option('--status', '-s', default=None, type=click.Choice(_STATUS_CHOICES, case_sensitive=False), help='Filter by status.')
@click.option('--all', '-a', 'include_stopped', is_flag=True, default=False, help='Include stopped sandboxes in results.')
@click.option('--tag', '-t', 'tags', multiple=True, help='Filter by tag (repeatable).')
@click.option('--output', '-o', 'output_format', default='table', type=click.Choice(['table', 'json'], case_sensitive=False), help='Output format.')
def list_sandboxes(status: str | None, include_stopped: bool, tags: tuple[str, ...], output_format: str) -> None:
    """List sandboxes.
    ...
    """
    sandboxes = Sandbox.list(tags=list(tags) if tags else None, status=status, include_stopped=include_stopped).result()
    if output_format == 'json':
        data = [{'sandbox_id': sb.sandbox_id, 'status': sb.status.value if sb.status else None, 'started_at': sb.started_at.isoformat() if sb.started_at else None} for sb in sandboxes]
...
@click.option('--cmd', default='/bin/bash', callback=_cwsandbox_validate_cmd, help='Command to run (default: /bin/bash). Accepts full command strings.')
...
@sandbox.command('exec', context_settings={'ignore_unknown_options': True})
@click.argument('sandbox_id')
@click.argument('command_args', nargs=-1, required=True, type=click.UNPROCESSED)
@click.option('--cwd', '-w', default=None, help='Working directory for the command.')
@click.option('--timeout', '-t', 'timeout_seconds', type=click.FloatRange(min=0, min_open=True), default=None, help='Timeout in seconds.')
def exec_in_sandbox(sandbox_id: str, command_args: tuple[str, ...], cwd: str | None, timeout_seconds: float | None) -> None:
    ...
    callback(sandbox_id=sandbox_id, command=command_args, cwd=cwd, timeout_seconds=timeout_seconds)

@sandbox.command('logs')
@click.argument('sandbox_id')
@click.option('--follow', '-f', is_flag=True, default=False, help='Follow log output (like tail -f).')
@click.option('--tail', 'tail_lines', type=click.IntRange(min=0), default=None, help='Number of recent lines to show.')
@click.option('--since', 'since_time', type=click.DateTime(), default=None, help="Show logs since timestamp (e.g. 'YYYY-MM-DD' or 'YYYY-MM-DD HH:MM:SS').")
@click.option('--timestamps', '-t', is_flag=True, default=False, help='Show timestamps.')
def logs(sandbox_id: str, follow: bool, tail_lines: int | None, since_time: datetime | None, timestamps: bool) -> None:
    ...
    callback(sandbox_id=sandbox_id, follow=follow, tail_lines=tail_lines, since_time=since_time, timestamps=timestamps)
