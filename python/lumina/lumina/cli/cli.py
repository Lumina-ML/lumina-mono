...
import lumina.sdk.verify.verify as wandb_verify
from lumina import Config, Error, env, util, wandb_agent
from lumina.analytics import get_sentry
from lumina.apis import InternalApi, PublicApi
from lumina.cli import beta_sync
from lumina.errors.links import url_registry
from lumina.sdk import wandb_setup, wandb_sweep
...
from lumina.sdk.internal.internal_api import Api as SDKInternalApi
from lumina.sdk.launch import utils as launch_utils
from lumina.sdk.launch._launch_add import _launch_add
from lumina.sdk.launch.errors import ExecutionError, LaunchError
from lumina.sdk.launch.sweeps import SweepNotFoundError
from lumina.sdk.launch.sweeps import utils as sweep_utils
from lumina.sdk.launch.sweeps.scheduler import Scheduler
...
def _get_wandb_dir(root_dir: str | None=None) -> str:
    if root_dir is None or root_dir == '':
        try:
            ...
            lumina.termwarn('os.getcwd() no longer exists, using system temp directory')
    ...
    if not os.access(root_dir, os.W_OK):
        lumina.termwarn(f"Path {path} wasn't writable, using system temp directory", repeat=False)
...
def _setup_logger() -> None:
    ...
    logger_handler.setFormatter(logging.Formatter(fmt='%(asctime)s %(levelname)s %(message)s', datefmt='%Y-%m-%d %H:%M:%S'))
    logger.addHandler(logger_handler)
    logging.root.addHandler(logger_handler)
_HAS_DOCKER = bool(shutil.which('docker'))
_HAS_NVIDIA_DOCKER = bool(shutil.which('nvidia-docker'))
CONTEXT = {'default_map': {}}
RUN_CONTEXT = {'default_map': {}, 'allow_extra_args': True, 'ignore_unknown_options': True}

def cli_unsupported(argument):
    lumina.termerror(f'Unsupported argument `{argument}`')
    sys.exit(1)
...
class ClickWandbException(ClickException):
    ...
    def format_message(self):
        ...
        if issubclass(self.orig_type, Error):
            ...
            return f'An Exception was raised, see {_wandb_log_path} for full traceback.\n{orig_type}: {self.message}'

def parse_service_config(ctx: click.Context | None, param: click.Parameter | None, value: tuple[str, ...] | None) -> dict[str, str]:
    ...
    for config in value:
        if '=' not in config:
            raise click.BadParameter(f"Service must be in format 'serviceName=policy', got '{config}'")
        ...
        if policy not in ['always', 'never']:
            raise click.BadParameter(f"Policy must be 'always' or 'never', got '{policy}'")
...
def display_error(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        except lumina.Error as e:
            exc_type, exc_value, exc_traceback = sys.exc_info()
            lines = traceback.format_exception(exc_type, exc_value, exc_traceback)
            logger.exception(''.join(lines))
            lumina.termerror(f'Find detailed error logs at: {_wandb_log_path}')
...
def prompt_for_project(ctx, entity):
    ...
    try:
        if len(result) == 0:
            ...
            lumina.termlog('Which project should we use?')
            ...
            if project == 'Create New':
                ...
                project = api.upsert_project(project, entity=entity)['name']
    except lumina.errors.CommError as e:
...
@click.version_option(version=lumina.__version__)
@click.pass_context
def cli(ctx):
    _setup_logger()
...
@click.option('--cloud', is_flag=True, help='Log in to the W&B public cloud\n    (https://api.wandb.ai).\n    Mutually exclusive with --host.')
@click.option('--host', '--base-url', default=None, help='Log in to a specific W&B server\n    instance by URL\n    (e.g. https://my-wandb.example.com).\n    Mutually exclusive with --cloud.')
@click.option('--relogin', default=None, is_flag=True, help='Force a new login prompt, ignoring any existing credentials.')
@click.option('--anonymously', default=False, hidden=True, is_flag=True, help='Deprecated. Has no effect and will be removed in a future version.')
@click.option('--verify/--no-verify', default=False, is_flag=True, help='Verify the API key with W&B after storing it. If verification\n    is successful, display the source of the credentials and the\n    default team.')
@display_error
def login(key, host, cloud, relogin, anonymously, verify, no_offline=False):
    ...
    """
    if anonymously:
        lumina.termwarn('The --anonymously parameter has no effect and will be removed' + ' in a future version.', repeat=False)
    if host and cloud:
        lumina.termerror('Cannot use --host and --cloud together.')
        sys.exit(1)
    if cloud:
    ...
    lumina.login(force=True, host=host, key=key, relogin=relogin, verify=verify, referrer='models')
...
@click.option('--reset', is_flag=True, help='Reset existing W&B configuration\n    for the directory.')
@click.option('--mode', '-m', help="Set the W&B mode. One of 'online', 'offline', or 'disabled'.")
@click.pass_context
@display_error
def init(ctx, project, entity, reset, mode):
    ...
    if os.path.exists(global_settings.settings_workspace):
        click.confirm(click.style('This directory has been configured previously, should we re-configure it?', bold=True), abort=True)
    else:
    ...
    if len(viewer['teams']['edges']) > 1:
        team_names = [e['node']['name'] for e in viewer['teams']['edges']] + ['Manual entry']
        lumina.termlog('Which team should we use?')
    ...
    with open(os.path.join(global_settings.wandb_dir, '.gitignore'), 'w') as file:
        file.write('*\n!settings')
    click.echo(click.style('This directory is configured!  Next, track a run:\n', fg='green') + textwrap.dedent('        * In your training script:\n            {code1}\n            {code2}\n        * then `{run}`.\n        ').format(code1=click.style('import wandb', bold=True), code2=click.style(f'wandb.init(project="{project}")', bold=True), run=click.style('python <train.py>', bold=True)))

@cli.command(context_settings=CONTEXT)
@click.pass_context
@click.argument('path', nargs=-1, type=click.Path(exists=True))
@click.option('--view', is_flag=True, default=False, help='(legacy only) View runs. Try `wandb leet` instead!', hidden=True)
@click.option('--verbose', is_flag=True, default=False, help='Enable verbose output.', hidden=True)
@click.option('--yes', 'skip_confirmation', is_flag=True, default=False, help="(beta only) Don't prompt for confirmation.")
@click.option('--id', 'run_id', help='Upload to an existing run ID.')
@click.option('--project', '-p', help='Set the project to upload the run to.')
@click.option('--entity', '-e', help='Set the entity to scope the project to.')
@click.option('--job_type', 'job_type', help='Set the job type to group related runs.')
@click.option('--sync-tensorboard/--no-sync-tensorboard', is_flag=True, default=None, help='(legacy only) Sync TensorBoard tfevent files.\n    On by default for specific paths,\n    off for --sync-all.')
@click.option('--include-globs', help='\n        (legacy only) Include only runs matching these glob patterns\n        (comma-separated).\n    ')
@click.option('--exclude-globs', help='\n        (legacy only) Exclude runs matching these glob patterns\n        (comma-separated).\n    ')
@click.option('--include-online/--no-include-online', '--no-skip-online/--skip-online', is_flag=True, default=None, help='Include runs created in online mode.')
@click.option('--include-offline/--no-include-offline', is_flag=True, default=None, help='(legacy only) Include runs created in offline mode.')
@click.option('--include-synced/--no-include-synced', '--no-skip-synced/--skip-synced', is_flag=True, default=None, help='Include runs that are already synced.')
@click.option('--mark-synced/--no-mark-synced', is_flag=True, default=True, help='(legacy only) Mark runs as synced after upload.')
@click.option('--sync-all', is_flag=True, default=False, help='(legacy only) Sync all unsynced runs in the local wandb directory.')
@click.option('--clean', is_flag=True, default=False, help='(legacy only) Delete local data for runs that are already synced.')
@click.option('--clean-old-hours', default=24, help='Delete only synced runs older than this many\n    hours (use with --clean).', type=int)
@click.option('--clean-force', is_flag=True, default=False, help='Skip the confirmation prompt if --clean is specified.')
@click.option('--ignore', hidden=True)
@click.option('--show', default=5, help='Set the number of runs to show in the summary.')
@click.option('--append', is_flag=True, default=False, help='\n        (legacy only) Append data to an existing run instead of creating\n        a new run.\n    ')
@click.option('--skip-console', is_flag=True, default=False, help='(legacy only) Skip uploading console logs.')
@click.option('--replace-tags', help="Rename tags during sync. Use 'old=new' pairs separated by commas.")
@click.option('--legacy', is_flag=True, help='Force legacy behavior instead of rerouting to `wandb beta sync`.')
@display_error
def sync(ctx: click.Context, path: tuple[str, ...], view: bool, verbose: bool, skip_confirmation: bool, run_id: str | None, project: str | None, entity: str | None, job_type: str | None, sync_tensorboard: bool | None, include_globs: str | None, exclude_globs: str | None, include_online: bool | None, include_offline: bool | None, include_synced: bool | None, mark_synced: bool, sync_all: bool, ignore: str | None, show: int, clean: bool, clean_old_hours: int, clean_force: bool, append: bool, skip_console: bool, replace_tags: str | None, legacy: bool):
    ...
    """
    if not legacy and (not any((TFEVENT_SUBSTRING in p for p in path))) and (not view) and (not sync_tensorboard) and (not include_globs) and (not exclude_globs) and (include_offline is not False) and mark_synced and (not sync_all) and (not clean) and (not append) and (not skip_console):
        lumina.termlog('Using wandb beta sync. Turn this off with --legacy.')
        beta_sync.sync([pathlib.Path(p) for p in path], live=False, entity=entity or '', project=project or '', run_id=run_id or '', job_type=job_type or '', replace_tags=replace_tags or '', dry_run=False, skip_confirmation=skip_confirmation, skip_synced=not include_synced, skip_online=not include_online, verbose=verbose, parallelism=5)
        return
    if view:
        lumina.termwarn('--view is deprecated. Consider using `wandb leet`.')
    if include_globs or exclude_globs:
        lumina.termwarn('--include-globs and --exclude-globs are deprecated.' + ' Provide explicit paths instead.')
    if include_offline is False:
        lumina.termwarn('--no-include-offline is deprecated and will be removed.')
    if not mark_synced:
        lumina.termwarn('--no-mark-synced is deprecated and will be removed.')
    if sync_all:
        lumina.termwarn('--sync-all is deprecated. It is equivalent to' + ' `wandb sync --yes --include-online`.')
    if skip_console:
        lumina.termwarn('--skip-console is deprecated and will be removed.')
    bad_options: list[str] = []
    if skip_confirmation:
        bad_options.append('--yes')
    if bad_options:
        if not legacy:
            lumina.termlog('Legacy mode was selected due to presence of legacy options.' + ' See --help for more info or use `wandb beta sync` directly.')
        bad_opts_str = ', '.join(bad_options)
        raise ClickException(f'Not allowed in legacy mode: {bad_opts_str}')
    api = _get_cling_api()
    if not api.is_authenticated:
        lumina.termlog('Login to W&B to sync runs')
    ...
    def _summary():
        all_items = get_runs(include_online=True, include_offline=True, include_synced=True, include_unsynced=True)
        ...
        if sync_items:
            lumina.termlog(f'Number of runs to be synced: {len(sync_items)}')
            if show and show < len(sync_items):
                lumina.termlog(f'Showing {show} runs to be synced:')
            for item in sync_items[:show or len(sync_items)]:
                lumina.termlog(f'  {item}')
        else:
            lumina.termlog('No runs to be synced.')
        if synced:
            clean_cmd = click.style('wandb sync --clean', fg='yellow')
            lumina.termlog(f'NOTE: use {clean_cmd} to delete {len(synced)} synced runs from local directory.')
        if unsynced:
            sync_cmd = click.style('wandb sync --sync-all', fg='yellow')
            lumina.termlog(f'NOTE: use {sync_cmd} to sync {len(unsynced)} unsynced runs from local directory.')

    def _sync_path(_path, _sync_tensorboard):
        if run_id and len(_path) > 1:
            lumina.termerror('id can only be set for a single run.')
            sys.exit(1)
        sm = SyncManager(project=project, entity=entity, run_id=run_id, job_type=job_type, mark_synced=mark_synced, app_url=api.app_url, view=view, verbose=verbose, sync_tensorboard=_sync_tensorboard, log_path=_wandb_log_path, append=append, skip_console=skip_console, replace_tags=replace_tags_dict)
        for p in _path:
            sm.add(p)
        sm.start()
    ...
    def _sync_all():
        sync_items = get_runs(include_online=include_online if include_online is not None else True, include_offline=include_offline if include_offline is not None else True, include_synced=include_synced if include_synced is not None else False, include_unsynced=True, exclude_globs=exclude_globs, include_globs=include_globs)
        if not sync_items:
            lumina.termerror('Nothing to sync.')
        else:
    ...
    def _clean():
        if path:
            ...
            if not clean_force:
                click.confirm(click.style(f'Are you sure you want to remove {len(runs)} runs?', bold=True), abort=True)
            for run in runs:
                shutil.rmtree(run.path)
            click.echo(click.style('Success!', fg='green'))
            return
        runs = get_runs(include_online=include_online if include_online is not None else True, include_offline=include_offline if include_offline is not None else True, include_synced=include_synced if include_synced is not None else True, include_unsynced=False, exclude_globs=exclude_globs, include_globs=include_globs)
        ...
        if old_runs:
            click.echo(f'Found {len(runs)} runs, {len(old_runs)} are older than {clean_old_hours} hours')
            for run in old_runs:
                click.echo(run.path)
            if not clean_force:
                click.confirm(click.style(f'Are you sure you want to remove {len(old_runs)} runs?', bold=True), abort=True)
            for run in old_runs:
                shutil.rmtree(run.path)
            click.echo(click.style('Success!', fg='green'))
        else:
            click.echo(click.style(f'No runs older than {clean_old_hours} hours found', fg='red'))
    if sync_all:
        _sync_all()
    elif clean:
        _clean()
...
def _parse_sync_replace_tags(replace_tags: str) -> dict[str, str] | None:
    ...
    for pair in replace_tags.split(','):
        if '=' not in pair:
            lumina.termerror(f"Invalid replace-tags format: {pair}. Use 'old_tag=new_tag' format.")
            return None
...
@cli.command(context_settings=CONTEXT)
@click.option('--project', '-p', default=None, help="Set the project for sweep runs. Use 'Uncategorized' if not set.")
@click.option('--entity', '-e', default=None, help="Set the entity for sweep. Use the current user's default entity\n    if not set.")
@click.option('--controller', is_flag=True, default=False, help='Start a local sweep controller after creating the sweep.')
@click.option('--verbose', is_flag=True, default=False, help='Display verbose output.')
@click.option('--name', default=None, help='Set a display name for the sweep. Use the sweep ID\n    if not specified.')
@click.option('--program', default=None, help='Override the training program specified in the sweep config.')
@click.option('--settings', default=None, help='Set sweep settings.', hidden=True)
@click.option('--update', default=None, help='Update an existing sweep configuration. Pass the sweep ID.')
@click.option('--stop', is_flag=True, default=False, help='Stop a sweep. Let active runs finish but do not start new runs.')
@click.option('--cancel', is_flag=True, default=False, help='Cancel a sweep. Kill active runs and stop starting new ones.')
@click.option('--pause', is_flag=True, default=False, help='Pause a sweep. Temporarily stop starting new runs.')
@click.option('--resume', is_flag=True, default=False, help='Resume a paused sweep.')
@click.option('--prior_run', '-R', 'prior_runs', multiple=True, default=None, help='Attach an existing run to this sweep by ID. Specify multiple times\n    to attach multiple runs.')
@click.argument('config_yaml_or_sweep_id')
@click.pass_context
@display_error
def sweep(ctx, project, entity, controller, verbose, name, program, settings, update, stop, cancel, pause, resume, prior_runs, config_yaml_or_sweep_id):
    ...
    if is_state_change_command > 1:
        ...
        api = _get_cling_api()
        if not api.is_authenticated:
            lumina.termlog('Login to W&B to use the sweep feature')
            ctx.invoke(login, no_offline=True)
            api = _get_cling_api(reset=True)
        ...
        ings = {'stop': 'Stopping', 'cancel': 'Cancelling', 'pause': 'Pausing', 'resume': 'Resuming'}
        lumina.termlog(f'{ings[state]} sweep {entity}/{project}/{sweep_id}')
    ...
    def _parse_settings(settings):
        ...
        if settings.find('=') > 0:
            for item in settings.split(','):
                kv = item.split('=')
                if len(kv) != 2:
                    lumina.termwarn('Unable to parse sweep settings key value pair', repeat=False)
                ret.update(dict([kv]))
            return ret
        lumina.termwarn('Unable to parse settings parameter', repeat=False)
        return ret
    api = _get_cling_api()
    if not api.is_authenticated:
        lumina.termlog('Login to W&B to use the sweep feature')
    ...
    if update:
        ...
        if not has_entity:
            lumina.termerror(termerror_msg % (('entity',) * 2))
            return
        if not has_project:
            lumina.termerror(termerror_msg % (('project',) * 2))
            return
        found = api.sweep(sweep_id, '{}', entity=entity, project=project)
        if not found:
            lumina.termerror(f'Could not find sweep {entity}/{project}/{sweep_id}')
            return
        sweep_obj_id = found['id']
    action = 'Updating' if sweep_obj_id else 'Creating'
    lumina.termlog(f'{action} sweep from: {config_yaml}')
    ...
    if is_local:
        from lumina import controller as wandb_controller
        tuner = wandb_controller()
        err = tuner._validate(config)
        if err:
            lumina.termerror(f'Error in sweep file: {err}')
            return
    env = os.environ
    entity = entity or env.get('WANDB_ENTITY') or config.get('entity') or api.settings('entity')
    project = project or env.get('WANDB_PROJECT') or config.get('project') or api.settings('project') or util.auto_project_name(config.get('program'))
    sweep_id, warnings = api.upsert_sweep(config, project=project, entity=entity, obj_id=sweep_obj_id, prior_runs=prior_runs)
    sweep_utils.handle_sweep_config_violations(warnings)
    styled_id = click.style(sweep_id, fg='yellow')
    lumina.termlog(f'{action} sweep with ID: {styled_id}')
    sweep_url = wandb_sweep._get_sweep_url(api, sweep_id)
    if sweep_url:
        styled_url = click.style(sweep_url, underline=True, fg='blue')
        lumina.termlog(f'View sweep at: {styled_url}')
    ...
    lumina.termlog(f'Run sweep agent with: {styled_path}')
    if controller:
        lumina.termlog('Starting wandb controller...')
        from lumina import controller as wandb_controller
...
@click.option('--project', '-p', default=None, help='Name of the project which the agent will watch. If passed in,\n    will override the project value passed in using a config file.')
...
@click.option('--prior_run', '-R', 'prior_runs', multiple=True, default=None, help='ID of an existing run to add to this sweep.')
@click.argument('config', required=False, type=click.Path(exists=True))
@click.pass_context
@display_error
def launch_sweep(ctx, project, entity, queue, config, resume_id, prior_runs):
    api = _get_cling_api()
    env = os.environ
    if not api.is_authenticated:
        lumina.termlog('Login to W&B to use the sweep feature')
    ...
    if entity is None:
        lumina.termerror('Must specify entity when using launch')
        return
    project = project or env.get('WANDB_PROJECT') or api.settings('project')
    if project is None:
        lumina.termerror('A project must be configured when using launch')
        return
    author = api.viewer().get('username') or entity
    if not (config or resume_id):
        lumina.termerror("'config' and/or 'resume_id' required")
        return
    ...
    if scheduler_job:
        lumina.termwarn('Using a scheduler job for launch sweeps is *experimental* and may change without warning')
    ...
    if not resume_id:
        ...
        if scheduler_job and (not method):
            ...
            lumina.termwarn("Use 'method': 'custom' in the sweep config when using scheduler jobs, or omit it entirely. For jobs using the wandb optimization engine (WandbScheduler), set the method in the sweep config under scheduler.settings.method ")
        ...
        if not found:
            lumina.termerror(f'Could not find sweep {entity}/{project}/{resume_id}')
            return
        if found.get('state') == 'RUNNING':
            lumina.termerror(f'Cannot resume sweep {entity}/{project}/{resume_id}, it is already running')
            return
        sweep_obj_id = found['id']
        sweep_config = yaml.safe_load(found['config'])
        lumina.termlog(f'Resuming from existing sweep {entity}/{project}/{resume_id}')
        if len(parsed_user_config.keys()) > 0:
            lumina.termwarn('Sweep parameters loaded from resumed sweep, ignoring provided config')
        ...
        if scheduler_job and run_spec.get('job') and (run_spec.get('job') != scheduler_job):
            lumina.termerror(f"Resuming a launch sweep with a different scheduler job is not supported. Job loaded from sweep: {run_spec.get('job')}, job in config: {scheduler_job}")
            return
    ...
    if not queue:
        lumina.termerror("Launch-sweeps require setting a 'queue', use --queue option or a 'queue' key in the 'launch' section in the config")
        return
    ...
    if resource:
        if resource == 'local-process' and scheduler_job:
            lumina.termerror("Scheduler jobs cannot be run with the 'local-process' resource")
            return
        if resource == 'local-process' and scheduler_args.get('docker_image'):
            lumina.termerror("Scheduler jobs cannot be run with the 'local-process' resource and a docker image")
            return
    else:
    ...
    sweep_id, warnings = api.upsert_sweep(sweep_config, project=project, entity=entity, obj_id=sweep_obj_id, launch_scheduler=launch_scheduler_with_queue, state='PENDING', prior_runs=prior_runs, template_variable_values=scheduler_args.get('template_variables'))
    sweep_utils.handle_sweep_config_violations(warnings)
    styled_id = click.style(sweep_id, fg='yellow')
    lumina.termlog(f"{('Resumed' if resume_id else 'Created')} sweep with ID: {styled_id}")
    sweep_url = wandb_sweep._get_sweep_url(api, sweep_id)
    if sweep_url:
        styled_url = click.style(sweep_url, underline=True, fg='blue')
        lumina.termlog(f'View sweep at: {styled_url}')
    lumina.termlog(f'Scheduler added to launch queue ({queue})')

@cli.command(help=f"Launch or queue a W&B Job. See {url_registry.url('wandb-launch')}")
@click.option('--uri', '-u', metavar='(str)', default=None, help='Local path or git repo uri to launch. If provided this\n    command will create a job from the specified uri.')
@click.option('--job', '-j', metavar='(str)', default=None, help='Name of the job to launch. If passed in, launch\n    does not require a uri.')
@click.option('--entry-point', '-E', metavar='NAME', default=None, help="Entry point within project. [default: main]. If the entry point\n    is not found, attempts to run the project file with the specified name\n    as a script, using 'python' to run .py files and the default shell\n    (specified by environment variable $SHELL) to run .sh files. If\n    passed in, will override the entrypoint value passed in using a\n    config file.")
@click.option('--git-version', '-g', metavar='GIT-VERSION', hidden=True, help='Version of the project to run, as a Git commit\n    reference for Git projects.')
@click.option('--build-context', metavar='(str)', help='Path to the build context within the source code. Defaults to the\n    root of the source code. Compatible only with -u.')
@click.option('--job-name', '-J', metavar='(str)', default=None, hidden=True, help='Name for the job created if the -u,--uri flag is passed in.')
@click.option('--name', envvar='WANDB_NAME', help='Name of the run under which to launch the run. If not\n    specified, a random run name will be used to launch run. If passed in,\n    will override the name passed in using a config file.')
@click.option('--entity', '-e', metavar='(str)', default=None, help='Name of the target entity which the new run will be sent to.\n    Defaults to using the entity set by local wandb/settings folder.\n    If passed in, will override the entity value passed in using a config\n    file.')
@click.option('--project', '-p', metavar='(str)', default=None, help='Name of the target project which the new run will be sent to.\n    Defaults to using the project name given by the source uri or for github\n    runs, the git repo name. If passed in, will override the project value\n    passed in using a config file.')
@click.option('--resource', '-r', metavar='BACKEND', default=None, help="Execution resource to use for run. Supported\n    values: 'local-process', 'local-container', 'kubernetes', 'sagemaker',\n    'gcp-vertex'. This is now a required parameter if pushing to a queue\n    with no resource configuration. If passed in, will override the\n    resource value passed in using a config file.")
@click.option('--docker-image', '-d', default=None, metavar='DOCKER IMAGE', help="Specific docker image you'd like to use. In the form name:tag.\n    If passed in, will override the docker image value passed in using a\n    config file.")
@click.option('--base-image', '-B', default=None, metavar='BASE IMAGE', help='Docker image to run job code in. Incompatible\n    with --docker-image.')
@click.option('--config', '-c', metavar='FILE', help="Path to JSON file (must end in '.json') or JSON string which\n    will be passed as a launch config. Dictation how the launched run will\n    be configured.")
@click.option('--set-var', '-v', 'cli_template_vars', default=None, multiple=True, help='Set template variable values for queues with allow listing enabled,\n    as key-value pairs e.g. `--set-var key1=value1 --set-var key2=value2`')
@click.option('--queue', '-q', is_flag=False, flag_value='default', default=None, help="Name of run queue to push to. If none, launches single run\n    directly. If supplied without an argument (`--queue`), defaults to\n    queue 'default'. Else, if name supplied, specified run queue must\n    exist under the project and entity supplied.")
@click.option('--async', 'run_async', is_flag=True, help='Flag to run the job asynchronously. Defaults to false, in\n    other words, unless --async is set, wandb launch will wait for\n    the job to finish. This option is incompatible with --queue; asynchronous\n    options when running with an agent should be set\n    on wandb launch-agent.')
@click.option('--resource-args', '-R', metavar='FILE', help="Path to JSON file (must end in '.json') or JSON string which\n    will be passed as resource args to the compute resource. The exact\n    content which should be provided is different for each execution\n    backend. See documentation for layout of this file.")
@click.option('--build', '-b', is_flag=True, hidden=True, help='Flag to build an associated job and push to queue as an image job.')
@click.option('--repository', '-rg', is_flag=False, default=None, hidden=True, help='Name of a remote repository. Will be used to push a built image to.')
@click.option('--project-queue', '-pq', default=None, hidden=True, help='Name of the project containing the queue to push to.\n    If none, defaults to entity level queues.')
@click.option('--dockerfile', '-D', default=None, help="Path to the Dockerfile used to build the job, relative to\n    the job's root.")
@click.option('--priority', '-P', default=None, type=click.Choice(['critical', 'high', 'medium', 'low']), help='When --queue is passed, set the priority of the job. Launch\n    jobs with higher priority are served first.  The order, from highest to\n    lowest priority, is: critical, high, medium, low.')
@display_error
def launch(uri, job, entry_point, git_version, build_context, name, resource, entity, project, docker_image, base_image, config, cli_template_vars, queue, run_async, resource_args, build, repository, project_queue, dockerfile, priority, job_name):
    ...
    """
    logger.info(f'=== Launch called with kwargs {locals()} CLI Version: {lumina.__version__}===')
    from lumina.sdk.launch._launch import _launch
    from lumina.sdk.launch.create_job import _create_job
    from lumina.sdk.launch.utils import _is_git_uri
    api = _get_cling_api()
    ...
    if queue and docker_image and (not project):
        raise LaunchError('Cannot use --queue and --docker together without a project. Please specify a project with --project or -p.')
    ...
    try:
        ...
        lumina.termerror(f'Error running job: {traceback.format_exc()}')
    run_id = config.get('run_id')
    if uri:
        if entry_point is None:
            raise LaunchError('Cannot provide a uri without an entry point. Please provide an entry point with --entry-point or -E.')
        if job is not None:
        ...
        artifact, _, _ = _create_job(api, job_type, uri, entrypoint=' '.join(entry_point), git_hash=git_version, name=job_name, project=project, base_image=base_image, build_context=build_context, dockerfile=dockerfile, entity=entity)
        if artifact is None:
    ...
    if queue is None:
        try:
            run = asyncio.run(_launch(api, job, project=project, entity=entity, docker_image=docker_image, name=name, entry_point=entry_point, version=git_version, resource=resource, resource_args=resource_args, launch_config=config, synchronous=not run_async, run_id=run_id, repository=repository))
            if asyncio.run(run.get_status()).state in ['failed', 'stopped', 'preempted']:
                lumina.termerror('Launched run exited with non-zero status')
                sys.exit(1)
        ...
        try:
            _launch_add(api, job, config, template_variables, project, entity, queue, resource, entry_point, name, git_version, docker_image, project_queue, resource_args, build=build, run_id=run_id, repository=repository, priority=priority)
...
@click.pass_context
@click.option('--queue', '-q', 'queues', default=None, multiple=True, help='The name of a queue for the agent to watch. Multiple -q flags\n    are supported.')
@click.option('--entity', '-e', default=None, help='The entity to use. Defaults to current logged-in user')
...
@click.option('--max-jobs', '-j', default=None, help='The maximum number of launch jobs this agent can run in parallel.\n    Defaults to 1. Set to -1 for no upper limit.')
...
@display_error
def launch_agent(ctx, entity=None, queues=None, max_jobs=None, config=None, url=None, log_file=None, verbose=0):
    logger.info(f'=== Launch-agent called with kwargs {locals()}  CLI Version: {lumina.__version__} ===')
    if url is not None:
        raise LaunchError('--url is not supported in this version, upgrade with: pip install -u wandb')
    import lumina.sdk.launch._launch as _launch
    if log_file is not None:
        _launch.set_launch_logfile(log_file)
    api = _get_cling_api()
    get_sentry().configure_scope(process_context='launch_agent')
    agent_config, api = _launch.resolve_agent_config(entity, max_jobs, queues, config, verbose)
    ...
    launch_utils.check_logged_in(api)
    lumina.termlog('Starting launch agent ✨')
    try:
...
@cli.command(context_settings=CONTEXT)
@click.pass_context
@click.option('--project', '-p', default=None, help='Set the project to upload runs to.')
@click.option('--entity', '-e', default=None, help='Set the entity to scope the project to.')
@click.option('--count', default=None, type=int, help='Maximum number of runs this agent will execute. Continues until\n    the sweep completes if not set.')
@click.option('--forward-signals', '-f', is_flag=True, default=False, help='Forward signals (e.g. SIGINT/SIGTERM) to child runs so they can\n    shut down cleanly.')
...
def agent(ctx, project, entity, count, forward_signals, sweep_id):
    ...
    api = _get_cling_api()
    if not api.is_authenticated:
        lumina.termlog('Login to W&B to use the sweep agent feature')
        ctx.invoke(login, no_offline=True)
        api = _get_cling_api(reset=True)
    lumina.termlog('Starting wandb agent 🕵️')
    try:
        ...
        lumina.termerror('Stopping agent.')
        sys.exit(1)

@cli.command(context_settings=RUN_CONTEXT, help='Run a W&B launch sweep scheduler (Experimental).')
@click.pass_context
@click.argument('sweep_id')
@display_error
def scheduler(ctx, sweep_id):
    api = InternalApi()
    if not api.is_authenticated:
        lumina.termlog('Login to W&B to use the sweep scheduler feature')
        ctx.invoke(login, no_offline=True)
        api = InternalApi(reset=True)
    get_sentry().configure_scope(process_context='sweep_scheduler')
    lumina.termlog('Starting a Launch Scheduler 🚀')
    from lumina.sdk.launch.sweeps import load_scheduler
    kwargs = {}
    ...
    try:
        ...
        _scheduler = load_scheduler(scheduler_type=sweep_type)(api, sweep_id=sweep_id, **kwargs)
        _scheduler.start()
    except Exception as e:
        get_sentry().exception(e)
        raise
...
@click.option('--project', '-p', envvar=env.PROJECT, help='The project you want to list jobs from.')
@click.option('--entity', '-e', default='models', envvar=env.ENTITY, help='The entity the jobs belong to.')
def _list(project, entity):
    lumina.termlog(f'Listing jobs in {entity}/{project}')
    public_api = PublicApi()
    try:
        jobs = public_api.list_jobs(entity=entity, project=project)
    except lumina.errors.CommError as e:
        lumina.termerror(f'{e}')
        return
    if len(jobs) == 0:
        lumina.termlog('No jobs found')
        return
    for job in jobs:
        aliases = []
        if len(job['edges']) == 0:
        ...
        lumina.termlog(f'{name} -- versions ({len(aliases)}): {aliases_str}')

@job.command(help='Describe a launch job. Provide the launch job in the form\n    of: entity/project/job-name:alias-or-version.')
@click.argument('job')
def describe(job):
    public_api = PublicApi()
    try:
    ...
    for key in job._job_info:
        if key.startswith('_'):
            continue
        lumina.termlog(f'{key}: {job._job_info[key]}')

@job.command(no_args_is_help=True)
@click.option('--project', '-p', envvar=env.PROJECT, help='The project you want to list jobs from.')
@click.option('--entity', '-e', envvar=env.ENTITY, help='The entity the jobs belong to.')
@click.option('--name', '-n', help='Name for the job.')
@click.option('--description', '-d', help='Description for the job.')
@click.option('--alias', '-a', 'aliases', help='Alias for the job.', multiple=True, default=tuple())
@click.option('--entry-point', '-E', 'entrypoint', help='Entrypoint to the script, including an executable and an entrypoint\n    file. Required for code or repo jobs. If --build-context is provided,\n    paths in the entrypoint command will be relative to the build context.')
@click.option('--git-hash', '-g', 'git_hash', type=str, help='Commit reference to use as the source for git jobs.')
@click.option('--runtime', '-r', type=str, help='Python runtime to execute the job.')
@click.option('--build-context', '-b', type=str, help='Path to the build context from the root of the job source code.\n    If provided, this is used as the base path for the Dockerfile and\n    entrypoint.')
@click.option('--base-image', '-B', type=str, help='Base image to use for the job. Incompatible with image jobs.')
@click.option('--dockerfile', '-D', type=str, help='Path to the Dockerfile for the job. If --build-context is provided,\n    the Dockerfile path will be relative to the build context.')
@click.argument('job_type', type=click.Choice(('git', 'code', 'image')))
@click.option('--service', '-s', 'services', multiple=True, callback=parse_service_config, help='Service configurations in format serviceName=policy. Valid\n    policies: always, never.', hidden=True)
@click.option('--schema', type=str, help='Path to the schema file for the job.', hidden=True)
@click.argument('path')
def create(path, project, entity, name, job_type, description, aliases, entrypoint, git_hash, runtime, build_context, base_image, dockerfile, services, schema):
    ...
    if not entity:
        lumina.termerror('No entity provided, use --entity or set WANDB_ENTITY')
        return
    project = project or os.getenv('WANDB_PROJECT')
    if not project:
        lumina.termerror('No project provided, use --project or set WANDB_PROJECT')
        return
    if entrypoint is None and job_type in ['git', 'code']:
        lumina.termwarn(f'No entrypoint provided for {job_type} job, defaulting to main.py')
        entrypoint = 'main.py'
    if job_type == 'image' and base_image:
        lumina.termerror('Cannot provide --base-image/-B for an `image` job')
        return
    if schema:
        schema_dict = util.load_json_yaml_dict(schema)
        if schema_dict is None:
            lumina.termerror(f'Invalid format for schema file: {schema}')
            return
    artifact, action, aliases = _create_job(api=api, path=path, entity=entity, project=project, name=name, job_type=job_type, description=description, aliases=list(aliases), entrypoint=entrypoint, git_hash=git_hash, runtime=runtime, build_context=build_context, base_image=base_image, dockerfile=dockerfile, services=services, schema=schema_dict if schema else None)
    if not artifact:
        lumina.termerror('Job creation failed')
        return
    ...
    lumina.termlog(f"View all jobs in project '{project}' here: {url}\n")
...
def docker_run(ctx, docker_run_args):
    ...
    resolved_image = None
    if image:
        resolved_image = lumina.docker.image_id(image)
    if resolved_image:
        args = ['-e', f'WANDB_DOCKER={resolved_image}'] + args
    else:
        lumina.termlog("Couldn't detect image argument, running command without the WANDB_DOCKER env variable")
    if api.api_key:
        args = ['-e', f'WANDB_API_KEY={api.api_key}'] + args
    else:
        lumina.termlog('Not logged in, run `wandb login` from the host machine to enable result logging')
...
@click.option('--nvidia/--no-nvidia', default=_HAS_NVIDIA_DOCKER, help='Use the nvidia runtime, defaults to nvidia\n    if nvidia-docker is present.')
...
def docker(ctx, docker_run_args, docker_image, nvidia, digest, jupyter, dir, no_dir, shell, port, cmd, no_tty):
    ...
    if not util.docker_image_regex(image.split('@')[0]):
        if image:
            args = args + [image]
        image = lumina.docker.default_image(gpu=nvidia)
        subprocess.call(['docker', 'pull', image])
    _, repo_name, tag = lumina.docker.parse(image)
    resolved_image = lumina.docker.image_id(image)
    if resolved_image is None:
        raise ClickException(f"Couldn't find image locally or in a registry, try running `docker pull {image}`")
    if digest:
        sys.stdout.write(resolved_image)
        exit(0)
    existing = lumina.docker.shell(['ps', '-f', f'ancestor={resolved_image}', '-q'])
    if existing and click.confirm('Found running container with the same image, do you want to attach?'):
        subprocess.call(['docker', 'attach', existing.split('\n')[0]])
        exit(0)
    cwd = os.getcwd()
    command = ['docker', 'run', '-e', 'LANG=C.UTF-8', '-e', f'WANDB_DOCKER={resolved_image}', '--ipc=host', '-v', lumina.docker.entrypoint + ':/wandb-entrypoint.sh', '--entrypoint', '/wandb-entrypoint.sh']
    if nvidia:
    ...
    if api.api_key:
        ...
        lumina.termlog("Couldn't find WANDB_API_KEY, run `wandb login` to enable streaming metrics")
    if jupyter:
    ...
    if no_tty:
        ...
        lumina.termlog('Launching docker container 🚢')
...
def local(ctx, *args, **kwargs):
    lumina.termwarn('`wandb local` has been replaced with `wandb server start`.')
...
@click.option('--env', '-e', default=[], multiple=True, help='Environment variables to pass to wandb/local Docker image.')
@click.option('--daemon/--no-daemon', default=True, help='Run the server in the background. Use --no-daemon\n    to run in the foreground.')
@click.option('--upgrade', is_flag=True, default=False, help='Pull the latest wandb/local Docker image before\n    starting. Stop any existing container.', hidden=True)
@click.option('--edge', is_flag=True, default=False, help='Use the bleeding edge', hidden=True)
@display_error
def start(ctx, port, env, daemon, upgrade, edge):
    ...
    if not _HAS_DOCKER:
        raise ClickException('Docker not installed, install it from https://docker.com')
    import lumina.docker
    local_image_sha = lumina.docker.image_id('wandb/local').split('wandb/local')[-1]
    registry_image_sha = lumina.docker.image_id_from_registry('wandb/local').split('wandb/local')[-1]
    if local_image_sha != registry_image_sha:
        if upgrade:
            subprocess.call(['docker', 'pull', 'wandb/local'])
        else:
            lumina.termlog('A new version of the W&B server is available, upgrade by calling `wandb server start --upgrade`')
    running = subprocess.check_output(['docker', 'ps', '--filter', 'name=^wandb-local$', '--format', '{{.ID}}'])
    if running != b'':
        if upgrade:
            subprocess.call(['docker', 'stop', 'wandb-local'])
        else:
            lumina.termerror('A container named wandb-local is already running, run `docker stop wandb-local` if you want to start a new instance')
            exit(1)
    ...
    if daemon:
        if code != 0:
            lumina.termerror('Failed to launch the W&B server container, see the above error.')
            exit(1)
        else:
            lumina.termlog(f'W&B server started at http://localhost:{port} 🚀')
            lumina.termlog('You can stop the server by running `wandb server stop`')
            if not api.api_key:
                time.sleep(2)
...
@click.argument('path')
@click.option('--name', '-n', help='Artifact name in project/artifact_name format. Defaults to\n    the basename of the path.')
@click.option('--description', '-d', help='A description of this artifact.')
@click.option('--type', '-t', default='dataset', help="The type of the artifact. Defaults to 'dataset'.")
@click.option('--alias', '-a', default=['latest'], multiple=True, help="An alias to apply to this artifact. Can be specified multiple\n    times. Defaults to 'latest'.")
@click.option('--id', 'run_id', help='Upload to an existing run with this ID.')
@click.option('--resume', is_flag=True, default=None, help='Resume the last run from your current directory.')
@click.option('--skip_cache', is_flag=True, default=False, help='Skip caching while uploading artifact files.')
@click.option('--policy', default='mutable', type=click.Choice(['mutable', 'immutable']), help="Set the storage policy for artifact files. Either\n    'mutable' (default) or 'immutable'.")
@display_error
def put(path, name, description, type, alias, run_id, resume, skip_cache, policy):
    ...
    artifact = lumina.Artifact(name=artifact_name, type=type, description=description)
    artifact_path = f'{entity}/{project}/{artifact_name}:{alias[0]}'
    if os.path.isdir(path):
        lumina.termlog(f'Uploading directory {path} to: "{artifact_path}" ({type})')
        artifact.add_dir(path, skip_cache=skip_cache, policy=policy)
    elif os.path.isfile(path):
        lumina.termlog(f'Uploading file {path} to: "{artifact_path}" ({type})')
        artifact.add_file(path, skip_cache=skip_cache, policy=policy)
    elif '://' in path:
        lumina.termlog(f'Logging reference artifact from {path} to: "{artifact_path}" ({type})')
        artifact.add_reference(path)
    else:
        raise ClickException('Path argument must be a file or directory')
    with lumina.init(entity=entity, project=project, config={'path': path}, job_type='cli_put', id=run_id, resume=resume) as run:
        run.log_artifact(artifact, aliases=alias)
    artifact.wait()
    lumina.termlog('Artifact uploaded, use this artifact in a run by adding:\n', prefix=False)
    lumina.termlog(f'    artifact = run.use_artifact("{artifact.source_qualified_name}")\n', prefix=False)

@artifact.command(context_settings=CONTEXT)
@click.argument('path')
@click.option('--root', help='Directory to download the artifact to. Uses the default\n    artifact cache if not set.')
...
def get(path, root, type):
    ...
    try:
        ...
        if is_artifact_registry_project(project):
            ...
            entity = SDKInternalApi()._resolve_org_entity_name(entity=settings_entity, organization=organization)
        full_path = f'{entity}/{project}/{artifact_name}:{version}'
        lumina.termlog('Downloading {type} artifact {full_path}'.format(type=type or 'dataset', full_path=full_path))
        artifact = public_api.artifact(full_path, type=type)
        path = artifact.download(root=root)
        lumina.termlog(f'Artifact downloaded to {path}')
    except ValueError:
    ...
    for kind in types:
        for collection in kind.collections():
            ...
            if (latest := next(versions, None)) is not None:
                lumina.termlog(f'{kind.type:<15s}{latest.updated_at:<15s}{util.to_human_size(latest.size):>15s} {latest.name:<20s}')
            else:
                lumina.termlog(f"{kind.type:<15s}{'N/A':<15s}{'0 B':>15s} {collection.name:<20s} (no versions)")
...
@click.option('--remove-temp/--no-remove-temp', default=False, help='Also remove temporary files from the cache.')
@display_error
def cleanup(target_size, remove_temp):
    ...
    lumina.termlog(f'Reclaimed {util.to_human_size(reclaimed_bytes)} of space')

@cli.command(context_settings=CONTEXT)
@click.argument('run', envvar=env.RUN_ID)
@click.option('--project', '-p', envvar=env.PROJECT, help='The project containing the run to pull files from.')
@click.option('--entity', '-e', default='models', envvar=env.ENTITY, help='The entity that owns the project. Defaults to the value of the\n    WANDB_ENTITY environment variable or the default entity if not set.')
@display_error
...
@click.pass_context
@click.argument('run', envvar=env.RUN_ID)
@click.option('--no-git', is_flag=True, default=False, help='Skip git restoration. Only restore config and Docker state.')
@click.option('--branch/--no-branch', default=True, help='Create a wandb/run_id branch or check out the commit in detached HEAD mode.')
@click.option('--project', '-p', envvar=env.PROJECT, help='Specify the project to look up the run in.')
@click.option('--entity', '-e', envvar=env.ENTITY, help='Specify the entity to scope the run lookup to.')
@display_error
def restore(ctx, run, no_git, branch, project, entity):
    ...
    """
    from lumina.sdk.lib.gitlib import GitRepo
    api = _get_cling_api()
    if ':' in run:
        if '/' in run:
    ...
    project, run = api.parse_slug(run, project=project)
    commit, json_config, patch_content, metadata = api.run_config(project, run=run, entity=entity)
    ...
    if no_git:
        ...
        if repo:
            ...
            lumina.termlog('Original run has no git history.  Just restoring config and docker')
    if commit and git.enabled:
        lumina.termlog(f'Fetching origin and finding commit: {commit}')
        git.fetch_all()
        if not git.has_commit(commit):
            lumina.termlog(f"Couldn't find original commit: {commit}")
            commit = None
            ...
            for filename in files:
                if filename.startswith('upstream_diff_') and filename.endswith('.patch'):
                    ...
                    commit = None
            if commit:
                lumina.termlog(f'Falling back to upstream commit: {commit}')
        ...
        if branch and (not git.has_branch(branch_name)):
            ...
            lumina.termlog(f'Created branch {click.style(branch_name, bold=True)}')
        elif branch:
            lumina.termlog(f'Using existing branch, run `git branch -D {branch_name}` from master for a clean checkout')
            git.checkout(branch_name)
        else:
            lumina.termlog(f'Checking out {commit} in detached mode')
            git.checkout(commit)
        if patch_path:
            root = git.root_dir
            patch_rel_path = os.path.relpath(patch_path, start=root)
            exit_code = subprocess.call(['git', 'apply', '--reject', patch_rel_path], cwd=root)
            if exit_code == 0:
                lumina.termlog('Applied patch')
            else:
                lumina.termerror('Failed to apply patch, try un-staging any un-committed changes')
    ...
    for k, v in json_config.items():
        if k not in ('_wandb', 'wandb_version'):
            config[k] = v
    s = b'wandb_version: 1'
    s += b'\n\n' + yaml.dump(config._as_dict(), Dumper=yaml.SafeDumper, default_flow_style=False, allow_unicode=True, encoding='utf-8')
    s = s.decode('utf-8')
    with open(config_path, 'w') as f:
        f.write(s)
    lumina.termlog(f'Restored config variables to {config_path}')
    if image:
        if not metadata['program'].startswith('<') and metadata.get('args') is not None:
            ...
            lumina.termlog("Couldn't find original command, just restoring environment")
            cmd = None
        lumina.termlog('Docker image found, attempting to start')
...
def online():
    ...
    system_settings.clear('mode')
    system_settings.save()
    click.echo('W&B online. Running your script from this directory will now sync to the cloud.')
...
@cli.command()
@click.option('--settings/--no-settings', help='Display the current settings.', default=True)
def status(settings):
    ...
    if settings:
        ...
        click.echo(json.dumps(settings, sort_keys=True, indent=2, separators=(',', ': ')))

@cli.command()
@click.option('--service', is_flag=True, show_default=True, default=True, help='No effect. Accepted for backwards compatibility.')
...
def verify(host):
    ...
    if host is None:
        ...
        lumina.termlog(f'Default host selected: {host}')
    elif host != api.settings('base_url'):
        reinit = True
    tmp_dir = tempfile.mkdtemp()
    lumina.termlog('Find detailed logs for this test at: {}'.format(os.path.join(tmp_dir, 'wandb')))
    os.chdir(tmp_dir)
    os.environ['WANDB_BASE_URL'] = host
    lumina.login(host=host)
    if reinit:
        api = _get_cling_api(reset=True)
    ...
    wandb_verify.check_secure_requests(api.settings('base_url'), 'Checking requests to base url', 'Connections are not made over https. SSL required for secure communications.')
    if url:
        wandb_verify.check_secure_requests(url, 'Checking requests made over signed URLs', 'Signed URL requests not made over https. SSL is required for secure communications.')
    ...
    if not (check_artifacts_success and check_run_success and large_post_success and url_success and check_sweeps_success):
        sys.exit(1)

@cli.command('purge-cache', help='Purges cached logs, run history, and artifacts from the\n    local W&B cache.')
@click.option('--age', default='0d', help="Removes items older than the specified time\n    period (e.g., '10s', '5m', '8h', '7d', '6M', '1y').")
@click.option('--force', is_flag=True, default=False, help='Do not prompt for confirmation when deleting files.')
def purge_cache(age: str, force: bool):
    try:
    ...
    if not cache_dir.exists():
        lumina.termlog(f'Cache directory does not exist: {cache_dir}')
        return
    ...
    for file in files:
        if file.stat().st_mtime > cutoff_time or file.is_dir():
            continue
        if not force:
            confirm = click.confirm(f'Are you sure you want to delete cache file {file}?')
            if not confirm:
                lumina.termlog(f'Skipping cache file: {file}')
                continue
