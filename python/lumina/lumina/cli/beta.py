...
from lumina.errors import WandbCoreNotAvailableError
from lumina.util import get_core_path
...
def beta(ctx: click.Context) -> None:
    ...
    if ctx.invoked_subcommand == 'leet':
        click.secho('LEET is now generally available as `wandb leet`; `wandb beta leet` is kept as an alias.', fg='yellow', err=True)
beta.add_command(leet)

@beta.command()
@click.argument('paths', type=click.Path(exists=True), nargs=-1)
@click.option('--live', is_flag=True, default=False, help="Sync a run while it's still being logged.\n\n    This may hang if the process generating the run crashes uncleanly.\n    ")
@click.option('-e', '--entity', default='', help='An entity override to use for all runs being synced.')
@click.option('-p', '--project', default='', help='A project override to use for all runs being synced.')
@click.option('--id', 'run_id', default='', help='A run ID override to use for all runs being synced.\n\n    If setting this and syncing multiple files (with the same entity\n    and project), the files will be synced in order of start time.\n    This is intended to work with syncing multiple resumed fragments\n    of the same run.\n    ')
@click.option('--job-type', default='', help='A job type override for all runs being synced.')
@click.option('--replace-tags', default='', help="Rename tags using the format 'old1=new1,old2=new2'.")
@click.option('--skip-synced/--no-skip-synced', is_flag=True, default=True, help='Skip runs that have already been synced with this command.')
@click.option('--skip-online/--no-skip-online', is_flag=True, default=True, help='Skip online runs.')
@click.option('--dry-run', is_flag=True, default=False, help='Print what would happen without uploading anything.')
@click.option('--yes', 'skip_confirmation', is_flag=True, default=False, help='Skip confirmation.')
@click.option('-v', '--verbose', is_flag=True, default=False, help='Print more information.')
@click.option('-n', default=5, help='Max number of runs to sync at a time.\n\n    When syncing multiple files that are part of the same run,\n    the files are synced sequentially in order of start time\n    regardless of this setting. This happens for resumed runs\n    or when using the --id parameter.\n    ')
def sync(paths: tuple[str, ...], live: bool, entity: str, project: str, run_id: str, job_type: str, replace_tags: str, skip_synced: bool, skip_online: bool, dry_run: bool, skip_confirmation: bool, verbose: bool, n: int) -> None:
    ...
    beta_sync.sync([pathlib.Path(path) for path in paths], live=live, entity=entity, project=project, run_id=run_id, job_type=job_type, replace_tags=replace_tags, dry_run=dry_run, skip_confirmation=skip_confirmation, skip_synced=skip_synced, skip_online=skip_online, verbose=verbose, parallelism=n)
...
@core.command()
@click.option('--idle-timeout', default='10m', show_default=True, metavar='DURATION', help='Shut down wandb-core after this much idle time with no connected clients. Uses Go duration syntax, for example 30s, 10m, or 0 to disable idle shutdown.')
