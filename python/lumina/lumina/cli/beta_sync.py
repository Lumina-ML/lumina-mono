...
from lumina.proto.wandb_sync_pb2 import ServerSyncResponse
from lumina.sdk import wandb_setup
from lumina.sdk.lib import asyncio_compat, ratelimit, wbauth
from lumina.sdk.lib.printer import Printer, new_printer
from lumina.sdk.lib.progress import progress_printer
from lumina.sdk.lib.service.service_connection import ServiceConnection
from lumina.sdk.mailbox.mailbox_handle import MailboxHandle
_MAX_LIST_LINES = 20
_POLL_WAIT_SECONDS = 0.1

def sync(paths: list[pathlib.Path], *, live: bool, entity: str, project: str, run_id: str, job_type: str, replace_tags: str, dry_run: bool, skip_confirmation: bool, skip_synced: bool, skip_online: bool, verbose: bool, parallelism: int) -> None:
    ...
    wandb_files = _find_wandb_files(paths, skip_synced=skip_synced, skip_online=skip_online, verbose=verbose)
    if not wandb_files:
    ...
    if ask_for_confirmation and (not term.confirm('Sync the listed runs?')):
        return
    if not wbauth.authenticate_session(host=singleton.settings.base_url, source='wandb sync', no_offline=True):
        term.termlog('Not authenticated.')
        return
    service = singleton.ensure_service()
    printer = new_printer()
    singleton.asyncer.run(lambda: _do_sync(wandb_files, cwd=cwd, live=live, service=service, entity=entity, project=project, run_id=run_id, job_type=job_type, tag_replacements=tag_replacements, settings=singleton.settings, printer=printer, parallelism=parallelism))
...
def _parse_replace_tags(replace_tags: str) -> dict[str, str]:
    ...
    for pair in replace_tags.split(','):
        if '=' not in pair:
            raise ValueError(f"Invalid --replace-tags format: {pair}. Expected 'old=new'.")
        old_tag, new_tag = pair.split('=', 1)
        tag_replacements[old_tag.strip()] = new_tag.strip()
    return tag_replacements

async def _do_sync(wandb_files: set[pathlib.Path], *, cwd: pathlib.Path | None, live: bool, service: ServiceConnection, entity: str, project: str, run_id: str, job_type: str, tag_replacements: dict[str, str], settings: wandb.Settings, printer: Printer, parallelism: int) -> None:
    """Sync the specified files.

    This is factored out to make the progress animation testable.
    """
    init_handle = await service.init_sync(wandb_files, settings, cwd=cwd, live=live, entity=entity, project=project, run_id=run_id, job_type=job_type, tag_replacements=tag_replacements)
    init_result = await init_handle.wait_async(timeout=5)
    sync_handle = await service.sync(init_result.id, parallelism=parallelism)
    await _SyncStatusLoop(init_result.id, service, printer).wait_with_progress(sync_handle)

class _SyncStatusLoop:
    """Displays a sync operation's status until it completes."""

    def __init__(self, id: str, service: ServiceConnection, printer: Printer) -> None:
        self._id = id
    ...
    async def wait_with_progress(self, handle: MailboxHandle[ServerSyncResponse]) -> None:
    ...
    async def _wait_then_mark_done(self, handle: MailboxHandle[ServerSyncResponse]) -> None:
    ...
    async def _rate_limit_check_done(self) -> bool:
        ...
        await asyncio_compat.race(self._rate_limit.wait(), self._done.wait())
        return self._done.is_set()

def _find_wandb_files(paths: Iterable[pathlib.Path], *, skip_synced: bool, skip_online: bool, verbose: bool) -> set[pathlib.Path]:
    ...
    """
    unique_files = _to_unique_files([file for path in paths for file in _expand_wandb_files(path)], verbose=verbose)
    ...
    if skipped_synced:
        term.termlog(f'Skipped {skipped_synced} synced run(s).' + ' Include with --no-skip-synced.')
    if skipped_online:
        term.termlog(f'Skipped {skipped_online} online run(s).' + ' Include with --no-skip-online.')
    return filtered_files

def _to_unique_files(paths: list[pathlib.Path], *, verbose: bool) -> set[pathlib.Path]:
...
def _expand_wandb_files(path: pathlib.Path) -> Iterator[pathlib.Path]:
...
def _print_sorted_paths(paths: Iterable[pathlib.Path], verbose: bool, *, root: pathlib.Path | None) -> None:
