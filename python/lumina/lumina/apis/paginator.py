...
class Paginator(Iterator[_WandbT], ABC):
    ...
    def __init__(self, service_api: ServiceApi, variables: Mapping[str, Any], per_page: int=50, *, omit_variables: Iterable[str] | None=None, omit_fragments: Iterable[str] | None=None, omit_fields: Iterable[str] | None=None, rename_fields: Mapping[str, str] | None=None):
    ...
    def _execute_query(self, parse: Callable[[str], _T]=json.loads) -> _T:
        ...
        return self._service_api.execute_graphql(self.QUERY, variables=self.variables, parse=parse, omit_variables=self._omit_variables, omit_fragments=self._omit_fragments, omit_fields=self._omit_fields, rename_fields=self._rename_fields)
...
class SizedPaginator(Paginator[_WandbT], Sized, ABC):
    ...
    def length(self) -> int | None:
        lumina.termwarn('`.length` is deprecated and will be removed in a future version. Use `len(...)` instead.', repeat=False)
        return len(self)
...
class RelayPaginator(Paginator[_WandbT], Generic[_NodeT, _WandbT], ABC):
    ...
    def __init__(self, service_api: ServiceApi, variables: Mapping[str, Any], per_page: int=50, start: str | None=None, *, omit_variables: Iterable[str] | None=None, omit_fragments: Iterable[str] | None=None, omit_fields: Iterable[str] | None=None, rename_fields: Mapping[str, str] | None=None):
        super().__init__(service_api, variables, per_page, omit_variables=omit_variables, omit_fragments=omit_fragments, omit_fields=omit_fields, rename_fields=rename_fields)
        self._start = start
