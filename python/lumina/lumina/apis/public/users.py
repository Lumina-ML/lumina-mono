...
class User(Attrs):
    ...
    def __init__(self, service_api: ServiceApi, attrs: MutableMapping[str, Any], api_key: str | None=None):
    ...
    def user_api(self) -> Api | None:
        ...
        if self._user_api is None and self._api_key:
            self._user_api = lumina.Api(api_key=self._api_key)
    ...
    def create(cls, api: Api, email: str, admin: bool | None=False) -> Self:
        ...
        """
        from lumina.apis._generated import CREATE_USER_FROM_ADMIN_GQL, CreateUserFromAdmin
        data = api._service_api.execute_graphql(CREATE_USER_FROM_ADMIN_GQL, {'email': email, 'admin': admin}, parse=CreateUserFromAdmin.model_validate_json)
    ...
    def delete_api_key(self, api_key: str) -> bool:
        ...
        """
        from lumina.apis._generated import DELETE_API_KEY_GQL
        idx = self.api_keys.index(api_key)
        api_key_id = self._attrs['apiKeys']['edges'][idx]['node']['id']
        try:
            self._service_api.execute_graphql(DELETE_API_KEY_GQL, {'id': api_key_id})
    ...
    def generate_api_key(self, description: str | None=None) -> str | None:
        ...
        Returns:
            ...
            None on failure.
        """
        from lumina.apis._generated import GENERATE_API_KEY_GQL, GenerateApiKey
        try:
            data = self._service_api.execute_graphql(GENERATE_API_KEY_GQL, {'description': description}, parse=GenerateApiKey.model_validate_json)
