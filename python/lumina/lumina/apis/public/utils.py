...
def fetch_org_from_settings_or_entity(settings: dict, default_entity: str | None=None) -> str:
    ...
    if (organization := settings.get('organization')) is None:
        ...
        if entity is None:
            raise ValueError("No entity specified and can't fetch organization from the entity")
        entity_orgs = InternalApi()._fetch_orgs_and_org_entities_from_entity(entity)
        entity_org = one(entity_orgs, too_short=ValueError('No organizations found for entity. Please specify an organization in the settings.'), too_long=ValueError('Multiple organizations found for entity. Please specify an organization in the settings.'))
