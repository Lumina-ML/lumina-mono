...
from lumina.sdk.data_types._dtypes import InvalidType, Type, TypeRegistry
from lumina.sdk.launch.errors import LaunchError
from lumina.sdk.launch.utils import LAUNCH_DEFAULT_PROJECT, _fetch_git_repo, apply_patch, convert_jupyter_notebook_to_script
if TYPE_CHECKING:
    from lumina.apis.public import Api
    from lumina.apis.public.service_api import ServiceApi
    from lumina.sdk.launch._project_spec import LaunchProject
...
class Job:
    ...
    def __init__(self, api: Api, name, path: str | None=None, *, service_api: ServiceApi | None=None) -> None:
        try:
    ...
    def _get_code_artifact(self, artifact_string):
        from lumina.sdk.artifacts.artifact_state import ArtifactState
    ...
    def _configure_launch_project_repo(self, launch_project: LaunchProject) -> None:
        ...
        launch_project.set_job_source_info({'git_remote': git_info['remote'], 'git_commit': git_info['commit'], 'job_artifact': self._job_artifact.qualified_name})
    ...
    def _configure_launch_project_artifact(self, launch_project: LaunchProject) -> None:
        ...
        launch_project.set_job_source_info({'artifact_string': code_artifact.qualified_name, 'job_artifact': self._job_artifact.qualified_name})

    def _configure_launch_project_container(self, launch_project: LaunchProject) -> None:
    ...
    def call(self, config, project=None, entity=None, queue=None, resource='local-container', resource_args=None, template_variables=None, project_queue=None, priority=None):
        ...
        Args:
            ...
            priority (int, optional): The priority of the queued run.
                Defaults to None.
        """
        from lumina.sdk.launch import _launch_add
        run_config = {}
        for key, item in config.items():
            if util._is_artifact_object(item):
                if isinstance(item, lumina.Artifact) and item.is_draft():
        ...
        if self._partial:
            lumina.termwarn("Launching manually created job for the first time, can't verify types")
...
class QueuedRun:
    ...
    def __init__(self, service_api: ServiceApi, entity: str, project: str, queue_name: str, run_queue_item_id: str, project_queue: str=LAUNCH_DEFAULT_PROJECT, priority: int | None=None, api_key: str | None=None):
        self._service_api = service_api
        self._entity = entity
    ...
    def state(self) -> str:
        ...
        raise ValueError(f'Could not find QueuedRunItem associated with id: {self.id} on queue {self.queue_name} at itemId: {self.id}')
    ...
    def _get_run_queue_item_legacy(self) -> dict[str, Any] | None:
        ...
        variables = {'projectName': self.project_queue, 'entityName': self._entity, 'runQueue': self.queue_name}
    ...
    def _get_item(self) -> dict[str, Any] | None:
        ...
        variables = {'projectName': self.project_queue, 'entityName': self._entity, 'runQueue': self.queue_name, 'itemId': self.id}
        try:
            res = self._service_api.execute_graphql(query, variables)
    ...
    def delete(self, delete_artifacts: bool=False) -> None:
        ...
        res = self._service_api.execute_graphql(query, variables={'entityName': self.entity, 'projectName': self.project_queue, 'runQueueName': self.queue_name})
        ...
        self._service_api.execute_graphql(mutation, variables={'queueID': queue_id, 'runQueueItemId': self._run_queue_item_id})
    ...
    def wait_until_running(self) -> public.Run:
        ...
        while True:
            ...
            if item and item['associatedRunId'] is not None:
                try:
                    self._run = public.Run(self._service_api, self._entity, self.project, item['associatedRunId'], None, api_key=self._api_key)
                ...
                lumina.termlog('Waiting for run to start')
            time.sleep(3)

    def __repr__(self) -> str:
        return f'<QueuedRun {self.queue_name} ({self.id})'
RunQueueResourceType = Literal['local-container', 'local-process', 'kubernetes', 'sagemaker', 'gcp-vertex']
...
class RunQueue:
    ...
    def __init__(self, service_api: ServiceApi, name: str, entity: str, prioritization_mode: RunQueuePrioritizationMode | None=None, _access: RunQueueAccessType | None=None, _default_resource_config_id: int | None=None, _default_resource_config: dict[str, Any] | None=None, api_key: str | None=None) -> None:
        self._name: str = name
        self._service_api = service_api
        self._entity = entity
    ...
    def _get_metadata(self) -> None:
        ...
        variables = {'projectName': LAUNCH_DEFAULT_PROJECT, 'entityName': self._entity, 'runQueue': self._name}
        res = self._service_api.execute_graphql(query, variables)
    ...
    def _get_default_resource_config(self) -> None:
        ...
        variables = {'entityName': self._entity, 'id': self._default_resource_config_id}
        res = self._service_api.execute_graphql(query, variables)
    ...
    def _get_items(self) -> None:
        ...
        self._items = []
        for item in res['project']['runQueue']['runQueueItems']['edges']:
            self._items.append(QueuedRun(self._service_api, self._entity, LAUNCH_DEFAULT_PROJECT, self._name, item['node']['id'], api_key=self._api_key))

    @classmethod
    def create(cls, name: str, resource: RunQueueResourceType, entity: str | None=None, prioritization_mode: RunQueuePrioritizationMode | None=None, config: dict | None=None, template_variables: dict | None=None) -> RunQueue:
        """Create a RunQueue.
