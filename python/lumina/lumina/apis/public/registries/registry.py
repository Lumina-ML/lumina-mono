...
from lumina.apis.public.teams import Team
from lumina.apis.public.users import User
from lumina.proto import wandb_internal_pb2 as pb
from lumina.sdk.artifacts._models import RegistryData
from ._freezable_list import AddOnlyArtifactTypesList
from ._members import MemberId, MemberKind, MemberRole, TeamMember, UserMember, parse_member_ids
from ._utils import Visibility, fetch_org_entity_from_organization, prepare_artifact_types_input
...
if TYPE_CHECKING:
    ...
    from lumina.sdk.artifacts._generated import RegistryFragment
...
class Registry:
    ...
    def __init__(self, service_api: ServiceApi, organization: str, entity: str, name: str, attrs: RegistryFragment | None=None):
        self._service_api = service_api
        if attrs is None:
            draft = RegistryData.model_construct(organization=organization, entity=entity, name=name)
            self._saved = draft
    ...
    @tracked
    def collections(self, filter: dict[str, Any] | None=None, order: str | None=None, per_page: PositiveInt=100, start: str | None=None) -> Collections:
        ...
        """
        return Collections(service_api=self._service_api, organization=self.organization, registry_filter={'name': self.full_name}, collection_filter=filter, order=order, per_page=per_page, start=start)

    @tracked
    def versions(self, filter: dict[str, Any] | None=None, per_page: PositiveInt=100, start: str | None=None) -> Versions:
        ...
        Args:
            ...
            start: Pagination cursor for resuming a past query, captured
                from a previous paginator's `.cursor` attribute.
        """
        return Versions(service_api=self._service_api, organization=self.organization, registry_filter={'name': self.full_name}, collection_filter=None, artifact_filter=filter, per_page=per_page, start=start)

    @classmethod
    @tracked
    def create(cls, api: Api, organization: str, name: str, visibility: Literal['organization', 'restricted'], description: str | None=None, artifact_types: list[str] | None=None) -> Self:
        ...
        """
        from lumina.sdk.artifacts._generated import UPSERT_REGISTRY_GQL, UpsertModelInput, UpsertRegistry
        from lumina.sdk.artifacts._validators import REGISTRY_PREFIX, validate_project_name
        failed_msg = f'Failed to create registry {name!r} in organization {organization!r}.'
        org_entity = fetch_org_entity_from_organization(api._service_api, organization)
        gql_input = UpsertModelInput(description=description, entity_name=org_entity, name=validate_project_name(f'{REGISTRY_PREFIX}{name}'), access=Visibility.from_python(visibility).value, allow_all_artifact_types_in_registry=not artifact_types, artifact_types=prepare_artifact_types_input(artifact_types))
        try:
            data = api._service_api.execute_graphql(UPSERT_REGISTRY_GQL, {'input': gql_input.model_dump()}, parse=UpsertRegistry.model_validate_json)
        except Exception as e:
            raise ValueError(failed_msg) from e
        if not ((result := data.upsert_model) and result.inserted and (registry_project := result.project)):
            raise ValueError(failed_msg)
        return cls(api._service_api, organization=organization, entity=org_entity, name=name, attrs=registry_project)

    @tracked
    def delete(self) -> None:
        """Delete the registry. This is irreversible."""
        from lumina.sdk.artifacts._generated import DELETE_REGISTRY_GQL, DeleteRegistry
        failed_msg = f'Failed to delete registry {self.name!r} in organization {self.organization!r}'
        try:
            data = self._service_api.execute_graphql(DELETE_REGISTRY_GQL, variables={'id': self.id}, parse=DeleteRegistry.model_validate_json)
        except Exception as e:
            raise ValueError(failed_msg) from e
    ...
    def load(self) -> None:
        ...
        from lumina.sdk.artifacts._generated import FETCH_REGISTRY_GQL, FetchRegistry
        failed_msg = f'Failed to load registry {self.name!r} in organization {self.organization!r}.'
        try:
            result = self._service_api.execute_graphql(FETCH_REGISTRY_GQL, variables={'name': self.full_name, 'entity': self.entity}, parse=FetchRegistry.model_validate_json)
        except Exception as e:
            raise ValueError(failed_msg) from e
    ...
    def save(self) -> None:
        ...
        from lumina.sdk.artifacts._generated import RENAME_REGISTRY_GQL, UPSERT_REGISTRY_GQL, RenameProjectInput, RenameRegistry, UpsertModelInput, UpsertRegistry
        from lumina.sdk.artifacts._gqlutils import server_supports
        from lumina.sdk.artifacts._validators import validate_project_name
        if not server_supports(self._service_api, pb.INCLUDE_ARTIFACT_TYPES_IN_REGISTRY_CREATION):
            raise RuntimeError('Saving the registry is not enabled on this wandb server version. Please upgrade your server version or contact support at support@wandb.com.')
        if (new_artifact_types := self.artifact_types.draft) and self.allow_all_artifact_types:
            raise ValueError(f'Cannot update artifact types when `allows_all_artifact_types` is {True!r}. Set it to {False!r} first.')
        ...
        upsert_input = UpsertModelInput(description=self.description, entity_name=self.entity, name=old_project_name, access=self._current.visibility.value, allow_all_artifact_types_in_registry=self.allow_all_artifact_types, artifact_types=prepare_artifact_types_input(new_artifact_types))
        try:
            data = self._service_api.execute_graphql(UPSERT_REGISTRY_GQL, variables={'input': upsert_input.model_dump()}, parse=UpsertRegistry.model_validate_json)
        except Exception as e:
            raise ValueError(failed_msg) from e
        if (result := data.upsert_model) and result.inserted:
            lumina.termlog(f'Created registry {self.name!r} in organization {self.organization!r} on save')
        if not (result and (registry_project := result.project)):
            raise ValueError(failed_msg)
        self._update_attributes(registry_project)
        if old_project_name != new_project_name:
            rename_input = RenameProjectInput(entity_name=self.entity, old_project_name=old_project_name, new_project_name=new_project_name)
            data = self._service_api.execute_graphql(RENAME_REGISTRY_GQL, variables={'input': rename_input.model_dump()}, parse=RenameRegistry.model_validate_json)
            if not ((result := data.rename_project) and (registry_project := result.project)):
                raise ValueError(failed_msg)
            if result.inserted:
                lumina.termlog(f'Created new registry {self.name!r} on save')
            self._update_attributes(registry_project)
    ...
    def user_members(self) -> list[UserMember]:
        ...
        from lumina.sdk.artifacts._generated import REGISTRY_USER_MEMBERS_GQL, RegistryUserMembers
        result = self._service_api.execute_graphql(REGISTRY_USER_MEMBERS_GQL, variables={'project': self.full_name, 'entity': self.entity}, parse=RegistryUserMembers.model_validate_json)
        if not (project := result.project):
            raise ValueError(f'Failed to fetch user members for registry {self.name!r}')
        return [UserMember(user=User(self._service_api, attrs=m.model_dump(exclude_none=True, exclude={'role'})), role=m.role.name) for m in project.members]

    def team_members(self) -> list[TeamMember]:
        """Returns the current member teams of this registry."""
        from lumina.sdk.artifacts._generated import REGISTRY_TEAM_MEMBERS_GQL, RegistryTeamMembers
        result = self._service_api.execute_graphql(REGISTRY_TEAM_MEMBERS_GQL, variables={'project': self.full_name, 'entity': self.entity}, parse=RegistryTeamMembers.model_validate_json)
        if not (project := result.project):
            raise ValueError(f'Failed to fetch team members for registry {self.name!r}')
        return [TeamMember(team=Team(self._service_api, name=m.team.name, attrs=m.team.model_dump(exclude_none=True)), role=m.role.name) for m in project.team_members]

    def add_members(self, *members: User | UserMember | Team | TeamMember | str) -> Self:
        ...
        ```
        """
        from lumina.sdk.artifacts._generated import CREATE_REGISTRY_MEMBERS_GQL, CreateProjectMembersInput, CreateRegistryMembers
        if not members:
            raise TypeError(f'Must provide at least one member to {nameof(self.add_members)!r}.')
        user_ids, team_ids = parse_member_ids(members)
        gql_input = CreateProjectMembersInput(user_ids=user_ids, team_ids=team_ids, project_id=self.id)
        data = self._service_api.execute_graphql(CREATE_REGISTRY_MEMBERS_GQL, variables={'input': gql_input.model_dump()}, parse=CreateRegistryMembers.model_validate_json)
        if not ((result := data.result) and result.success):
            raise ValueError(f'Failed to add members to registry {self.name!r}')
        return self

    def remove_members(self, *members: User | UserMember | Team | TeamMember | str) -> Self:
        ...
        ```
        """
        from lumina.sdk.artifacts._generated import DELETE_REGISTRY_MEMBERS_GQL, DeleteProjectMembersInput, DeleteRegistryMembers
        if not members:
            raise TypeError(f'Must provide at least one member to {nameof(self.add_members)!r}.')
        user_ids, team_ids = parse_member_ids(members)
        gql_input = DeleteProjectMembersInput(user_ids=user_ids, team_ids=team_ids, project_id=self.id)
        data = self._service_api.execute_graphql(DELETE_REGISTRY_MEMBERS_GQL, variables={'input': gql_input.model_dump()}, parse=DeleteRegistryMembers.model_validate_json)
        if not ((result := data.result) and result.success):
            raise ValueError(f'Failed to remove members from registry {self.name!r}')
        return self

    def update_member(self, member: User | UserMember | Team | TeamMember | str, role: MemberRole | str) -> Self:
        ...
        ```
        """
        from lumina.sdk.artifacts._generated import UPDATE_TEAM_REGISTRY_ROLE_GQL, UPDATE_USER_REGISTRY_ROLE_GQL, UpdateProjectMemberInput, UpdateProjectTeamMemberInput, UpdateTeamRegistryRole, UpdateUserRegistryRole
        id_ = MemberId.from_obj(member)
        if id_.kind is MemberKind.USER:
            gql_op = UPDATE_USER_REGISTRY_ROLE_GQL
            gql_input = UpdateProjectMemberInput(user_id=id_.encode(), project_id=self.id, user_project_role=role)
            result_cls = UpdateUserRegistryRole
        elif id_.kind is MemberKind.ENTITY:
            gql_op = UPDATE_TEAM_REGISTRY_ROLE_GQL
            gql_input = UpdateProjectTeamMemberInput(team_id=id_.encode(), project_id=self.id, team_project_role=role)
            result_cls = UpdateTeamRegistryRole
        else:
            assert_never(id_.kind)
        data = self._service_api.execute_graphql(gql_op, variables={'input': gql_input.model_dump()}, parse=result_cls.model_validate_json)
        if not ((result := data.result) and result.success):
            raise ValueError(f'Failed to update member {member!r} role to {role!r} in registry {self.name!r}')
        return self
