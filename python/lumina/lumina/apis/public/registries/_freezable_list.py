...
class FreezableList(MutableSequence[T]):
    ...
    def insert(self, index: int, value: T) -> None:
        ...
        draft_index = index % size - len_frozen
        if draft_index < 0:
            raise IndexError(f'Cannot insert into the frozen list (index < {len_frozen})')
...
class AddOnlyArtifactTypesList(FreezableList[str]):
    ...
    def remove(self, value: str) -> None:
        try:
            ...
            raise ValueError(f'Cannot remove artifact type: {value!r} that has been saved to the registry')
