...
from lumina.sdk.lib.printer import new_printer
...
async def wait_for_download_with_progress(service_api: ServiceApi, request_id: int, contains_live_data: bool) -> DownloadHistoryResult:
    return await _DownloadStatusWatcher(service_api=service_api, request_id=request_id, contains_live_data=contains_live_data).wait_with_progress()

class _DownloadStatusWatcher:

    def __init__(self, service_api: ServiceApi, request_id: int, contains_live_data: bool):
    ...
    async def _wait_then_mark_done(self) -> None:
        api_request = apb.ApiRequest(read_run_history_request=apb.ReadRunHistoryRequest(download_run_history=apb.DownloadRunHistory(request_id=self.request_id)))
        handle = await self._service_api.send_api_request_async(api_request)
        response = await handle.wait_async(timeout=None)
        downloaded_files = [pathlib.Path(file_name) for file_name in response.read_run_history_response.download_run_history.downloaded_files]
        errors = {pathlib.Path(file_name): error_message for file_name, error_message in response.read_run_history_response.download_run_history.errors.items()}
        self.download_result = DownloadHistoryResult(paths=downloaded_files, contains_live_data=self.contains_live_data, errors=errors)
        self.done_event.set()
    ...
    async def _show_progress_until_done(self) -> None:
        ...
        with progress_printer(p, 'Downloading history...') as progress:
            while not await self._rate_limit_check_done():
                status_request = apb.ApiRequest(read_run_history_request=apb.ReadRunHistoryRequest(download_run_history_status=apb.DownloadRunHistoryStatus(request_id=self.request_id)))
                ...
                if last_response is not None:
                    progress.update(last_response.read_run_history_response.download_run_history_status.operation_stats)
    ...
    async def _rate_limit_check_done(self) -> bool:
        ...
        if last_time and (time_since_last := (now - last_time)) < _POLL_WAIT_SECONDS:
            await asyncio_compat.race(asyncio.sleep(_POLL_WAIT_SECONDS - time_since_last), self.done_event.wait())
