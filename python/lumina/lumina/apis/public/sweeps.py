...
from lumina.errors import Error, UnsupportedError
from lumina.proto import wandb_internal_pb2 as pb
from lumina.sdk.lib import ipython
_SWEEP_FILTERS_MIN_SERVER_VERSION = '0.81.4'
if TYPE_CHECKING:
    from lumina.apis._generated import GetSweeps
    from lumina.apis.public.api import Api
    from lumina.apis.public.runs import AgentRuns
...
class Sweeps(SizedPaginator['Sweep']):
    ...
    def __init__(self, service_api: ServiceApi, entity: str, project: str, per_page: int=50, filters: dict[str, Any] | None=None) -> None:
        ...
        if self.QUERY is None:
            from lumina.apis._generated import GET_SWEEPS_GQL
            type(self).QUERY = GET_SWEEPS_GQL
        self.entity = entity
        self.project = project
        self._service_api = service_api
        self._supports_filtering = service_api.feature_enabled(pb.SWEEPS_QUERY_FILTERING)
        if filters and (not self._supports_filtering):
            raise UnsupportedError(f'Filtering sweeps is not supported on this W&B server version. Please upgrade your server to release {_SWEEP_FILTERS_MIN_SERVER_VERSION} or later, or query sweeps on https://wandb.ai.')
        variables = {'project': self.project, 'entity': self.entity, 'filters': json.dumps(filters or {})}
    ...
    def _update_response(self) -> None:
        ...
        self.last_response = self._service_api.execute_graphql(self.QUERY, variables=self.variables, omit_variables=omit_variables, parse=GetSweeps.model_validate_json)
    ...
    def more(self) -> bool:
        ...
        <!-- lazydoc-ignore: internal -->
        """
        if self.last_response and self.last_response.project and self.last_response.project.sweeps and self.last_response.project.sweeps.page_info:
    ...
    def convert_objects(self) -> list[Sweep]:
        ...
        from lumina.apis._generated import SweepFragment
        ...
        if project.total_sweeps < 1:
            return []
        return [Sweep(self._service_api, self.entity, self.project, node.name) for node in Connection[SweepFragment].model_validate(project.sweeps).nodes()]

    def __repr__(self):
        return f'<Sweeps {self.entity}/{self.project}>'

def _get_sweep(service_api: ServiceApi, entity: str | None=None, project: str | None=None, sid: str | None=None, order: str | None=None, query: str | None=None, **kwargs: Any) -> Sweep | None:
    """Fetch a sweep using an already-owned service API."""
    from lumina.apis._generated import GET_SWEEP_GQL, GET_SWEEP_LEGACY_GQL
    if not order:
        order = '+created_at'
    ...
    try:
        ...
        data = service_api.execute_graphql(query, variables=variables)
    if not (data and (proj_dict := data.get('project')) and (sweep_dict := proj_dict.get('sweep'))):
        return None
    sweep = Sweep(service_api, entity, project, sid, attrs=sweep_dict)
    sweep.runs = public.Runs(service_api, entity, project, order=order, per_page=10, filters={'$and': [{'sweep': sweep.id}]})
    return sweep
...
class Sweep(Attrs):
    ...
    def __init__(self, service_api: ServiceApi, entity: str, project: str, sweep_id: str, attrs: Mapping[str, Any] | None=None):
    ...
    def username(self) -> str:
        ...
        lumina.termwarn('Sweep.username is deprecated. please use Sweep.entity instead.')
        return self._entity
    ...
    def load(self, force: bool=False):
        ...
        if force or not self._attrs:
            if not (sweep := _get_sweep(self._service_api, self.entity, self.project, self.id)):
    ...
    def order(self):
        ...
        if self._attrs.get('config') and self.config.get('metric'):
            ...
            return public.QueryGenerator.format_order_key(prefix + self.config['metric']['name'])
    ...
    def best_run(self, order=None):
        ...
        if order is None:
            lumina.termwarn("No order specified and couldn't find metric in sweep config, returning most recent run")
        else:
            lumina.termlog('Sorting runs by {}'.format(order))
        filters = {'$and': [{'sweep': self.id}]}
        try:
            return public.Runs(self._service_api, self.entity, self.project, order=order, filters=filters, per_page=1)[0]
        except IndexError:
            return None
    ...
    def path(self):
        ...
        return [urllib.parse.quote_plus(self.entity), urllib.parse.quote_plus(self.project), urllib.parse.quote_plus(self.id)]
    ...
    @classmethod
    def get(cls, api: Api, entity: str | None=None, project: str | None=None, sid: str | None=None, order: str | None=None, query: str | None=None, **kwargs):
        ...
        """
        return api._get_sweep(entity, project, sid, order=order, query=query, **kwargs)

    def _make_sweep_agent(self, attrs: Mapping[str, Any]) -> Agent:
        """Construct `Agent` from API payload."""
        try:
            return Agent(self._service_api, attrs=attrs, entity=self.entity, project=self.project, sweep_id=self.id)
        except ValueError as e:
            raise Error('Sweep agent data from the W&B API was incomplete or invalid.', context={'details': str(e)}) from e
    ...
    def agent(self, agent_id: str) -> Agent:
        ...
        """
        from lumina.apis._generated import GET_SWEEP_AGENT_GQL
        variables = {'agentID': agent_id, 'sweep': self.id, 'entity': self.entity, 'project': self.project}
        data = self._service_api.execute_graphql(GET_SWEEP_AGENT_GQL, variables=variables)
        return self._make_sweep_agent(data['project']['sweep']['agent'])

    def agents(self) -> list[Agent]:
        """Query the list of all agents for this sweep."""
        from lumina.apis._generated import GET_SWEEP_AGENTS_GQL, GetSweepAgents
        variables = {'sweep': self.id, 'entity': self.entity, 'project': self.project}
        parsed = self._service_api.execute_graphql(GET_SWEEP_AGENTS_GQL, variables=variables, parse=GetSweepAgents.model_validate_json)
        if not parsed.project or not parsed.project.sweep:
            return []
        return [self._make_sweep_agent(edge.node.model_dump(by_alias=True)) for edge in parsed.project.sweep.agents.edges]
...
class Agent(Attrs):
    ...
    def __init__(self, service_api: ServiceApi, attrs: Mapping[str, Any], entity: str, project: str, sweep_id: str) -> None:
        super().__init__(dict(attrs or {}))
        self._entity = entity
        ...
        if self._entity is None:
            raise ValueError('Agent requires entity. Use an Agent returned from sweep.agent(...) or sweep.agents().')
        if self._project is None:
            raise ValueError('Agent requires project. Use an Agent returned from sweep.agent(...) or sweep.agents().')
        if self._sweep_id is None:
            raise ValueError('Agent requires sweep_id. Use an Agent returned from sweep.agent(...) or sweep.agents().')
    ...
    def runs(self, per_page: int=50) -> AgentRuns:
        """Return a paginated collection of runs executed by this agent."""
        from lumina.apis.public.runs import AgentRuns
        total_runs = int(self._attrs.get('totalRuns') or 0)
        return AgentRuns(self._service_api, entity=self._entity, project=self._project, sweep_id=self._sweep_id, agent_key=self._agent_key, total_runs=total_runs, order='+created_at', per_page=per_page)
