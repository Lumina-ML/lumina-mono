...
from lumina.proto import wandb_api_pb2 as pb
if TYPE_CHECKING:
    from . import runs
...
class HistoryScan(Iterator[_RowDict]):
    ...
    def __init__(self, run: runs.Run, *, service_api: ServiceApi, min_step: int, max_step: int, keys: list[str] | None=None, page_size: int=1000, use_cache: bool=True):
        self.run = run
        ...
        scan_run_history_init = pb.ScanRunHistoryInit(entity=self.run.entity, project=self.run.project, run_id=self.run.id, keys=self.keys, use_cache=use_cache)
        scan_run_history_init_request = pb.ReadRunHistoryRequest(scan_run_history_init=scan_run_history_init)
        api_request = pb.ApiRequest(read_run_history_request=scan_run_history_init_request)
        response: pb.ApiResponse = self._service_api.send_api_request(api_request)
        self._scan_request_id = response.read_run_history_response.scan_run_history_init.request_id
        self.scan_offset = 0
        self.page_offset = self.min_step
        self.rows: list[_RowDict] = []
        self.keys = keys
        self._service_api.finalize(self, _scan_cleanup_request(self._scan_request_id))
    ...
    def _load_next(self) -> None:
        ...
        read_run_history_request = pb.ReadRunHistoryRequest(scan_run_history=pb.ScanRunHistory(min_step=self.page_offset, max_step=max_step, request_id=self._scan_request_id))
        api_request = pb.ApiRequest(read_run_history_request=read_run_history_request)
        response: pb.ApiResponse = self._service_api.send_api_request(api_request)
        run_history: pb.RunHistoryResponse = response.read_run_history_response.run_history
        self.rows = [self._convert_history_row_to_dict(row) for row in run_history.history_rows]
        self.page_offset += self.page_size
        self.scan_offset = 0

    @staticmethod
    def _convert_history_row_to_dict(history_row: pb.HistoryRow) -> _RowDict:
        return {item.key: json.loads(item.value_json) for item in history_row.history_items}
...
def _scan_cleanup_request(id: int) -> pb.ApiRequest:
    ...
    run_history_request = pb.ReadRunHistoryRequest(scan_run_history_cleanup=scan_cleanup_request)
