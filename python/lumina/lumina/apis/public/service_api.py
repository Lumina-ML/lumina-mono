...
from lumina.proto.wandb_api_pb2 import ApiRequest, ApiResponse, FeaturesRequest, GraphQLRequest
from lumina.sdk import wandb_settings, wandb_setup
from lumina.sdk.lib.service.service_connection import ServiceConnection, WandbApiFailedError
...
class ServiceApi:
    ...
    def __init__(self, settings: wandb_settings.Settings, timeout: float | None=None):
    ...
    def _get_api_session(self) -> _ServiceApiSession:
        ...
        session = _ServiceApiSession(connection=service_connection, api_id=response.api_id)
    ...
    def send_api_request(self, request: ApiRequest, timeout: float | None=None) -> ApiResponse:
    ...
    def finalize(self, obj: object, cleanup: ApiRequest) -> Callable[[], None]:
    ...
    def execute_graphql(self, query: str, variables: Mapping[str, Any] | None=None, timeout: float | None=None, *, parse: Callable[[str], _T]=json.loads, omit_variables: Iterable[str] | None=None, omit_fragments: Iterable[str] | None=None, omit_fields: Iterable[str] | None=None, rename_fields: Mapping[str, str] | None=None) -> _T:
        ...
        """
        req = ApiRequest(graphql_request=GraphQLRequest(query=query, variables_json=json.dumps(variables or {}), omit_variables=omit_variables, omit_fragments=omit_fragments, omit_fields=omit_fields, rename_fields=rename_fields))
        resp = self.send_api_request(req, timeout=self._timeout if timeout is None else timeout)
        return parse(resp.graphql_response.data_json)

    async def send_api_request_async(self, request: ApiRequest) -> MailboxHandle[ApiResponse]:
    ...
    def feature_enabled(self, feature: pb.ServerFeature | str, *, timeout: float=10) -> bool:
