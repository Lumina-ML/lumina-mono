...
if TYPE_CHECKING:
    ...
    from lumina.automations._generated import ProjectTriggersFields
...
class Automations(RelayPaginator['ProjectTriggersFields', 'Automation']):
    ...
    def __init__(self, service_api: ServiceApi, variables: Mapping[str, Any], per_page: int=50, *, start: str | None=None, _query: str, omit_variables: Iterable[str] | None=None, omit_fragments: Iterable[str] | None=None, omit_fields: Iterable[str] | None=None, rename_fields: Mapping[str, str] | None=None):
        self.QUERY = _query
        super().__init__(service_api, variables=variables, per_page=per_page, start=start, omit_variables=omit_variables, omit_fragments=omit_fragments, omit_fields=omit_fields, rename_fields=rename_fields)
