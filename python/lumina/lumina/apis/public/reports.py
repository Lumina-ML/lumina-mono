...
class Reports(SizedPaginator['BetaReport']):
    ...
    def __init__(self, service_api: ServiceApi, project: Project, name: str | None=None, entity: str | None=None, per_page: int=50):
    ...
    def convert_objects(self) -> list[BetaReport]:
        ...
        return [BetaReport(self._service_api, r['node'], entity=self.project.entity, project=self.project.name) for r in edges]
...
class BetaReport(Attrs):
    ...
    def __init__(self, service_api: ServiceApi, attrs: dict, entity: str | None=None, project: str | None=None):
        self._service_api = service_api
        self.project = project
        self.entity = entity
    ...
    def runs(self, section: dict[str, Any], per_page: int=50, only_selected: bool=True) -> public.Runs:
        ...
        return public.Runs(self._service_api, self.entity, self.project, filters=filters, order=order, per_page=per_page)
    ...
    def url(self) -> str | None:
        if not self._service_api or not self.entity or (not self.project) or (not self.display_name) or (not self.id):
            return None
        return self._service_api.app_url + '/'.join([self.entity, self.project, 'reports', '--'.join([urllib.parse.quote(re.sub('-+', '-', re.sub('\\W', '-', self.display_name)).strip('-')), self.id.replace('=', '')])])
...
class PythonMongoishQueryGenerator:
    ...
    def _replace_numeric_dots(self, s):
        ...
        for i, (left, mid, right) in enumerate(zip(s, s[1:], s[2:], strict=False), 1):
            if mid == '.' and (left.isdigit() and right.isdigit() or (left.isdigit() and right == ' ') or (left == ' ' and right.isdigit())):
    ...
    def back_to_front(self, name):
        ...
        lumina.termerror(f'Unknown token: {name}')
        return name
