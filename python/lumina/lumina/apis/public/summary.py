...
from lumina.sdk.data_types.utils import val_to_json
...
class Summary(SummarySubDict):
    ...
    def _root_del(self, path):
        ...
        if isinstance(val, dict) and val.get('_type') in H5_TYPES:
            if not h5py:
                lumina.termerror('Deleting tensors in summary requires h5py')
            else:
                self.open_h5()
    ...
    def write_h5(self, path, val):
        self.open_h5()
        if not self._h5:
            lumina.termerror('Storing tensors in summary requires h5py')
        else:
            try:
    ...
    def read_h5(self, path, val=None):
        self.open_h5()
        if not self._h5:
            lumina.termerror('Reading tensors from summary requires h5py')
        else:
    ...
    def _decode(self, path, json_value):
        ...
        if isinstance(json_value, dict):
            if json_value.get('_type') in H5_TYPES:
                ...
                lumina.termerror('This data frame was saved via the wandb data API. Contact support@wandb.com for help.')
                return None
            else:
    ...
    def _encode(self, value, path_from_root):
        ...
        if isinstance(value, dict):
            ...
            friendly_value, _ = util.json_friendly(val_to_json(self._run, path, value, namespace='summary'))
    ...
    def _write(self, commit=False):
        ...
        if commit:
            if self._h5:
                self._h5.close()
                self._h5 = None
            res = self._service_api.execute_graphql(mutation, variables={'id': self._run.storage_id, 'summaryMetrics': util.json_dumps_safer(self._json_dict)})
