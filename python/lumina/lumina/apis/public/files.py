...
from lumina.apis.public import utils
from lumina.apis.public.runs import Run
from lumina.proto.wandb_api_pb2 import ApiRequest, DownloadFileRequest
from lumina.util import POW_2_BYTES, to_human_size
if TYPE_CHECKING:
...
class Files(SizedPaginator['File']):
    ...
    def __init__(self, service_api: ServiceApi, run: Run, names: list[str] | None=None, per_page: int=50, upload: bool=False, pattern: str | None=None):
        ...
        if names and pattern:
            raise ValueError('Querying for files by both names and pattern is not supported. Please provide either a list of names or a pattern to match.')
        self.run = run
        variables = {'project': run.project, 'entity': run.entity, 'name': run.id, 'fileNames': names or [], 'upload': upload, 'pattern': pattern}
    ...
    def _update_response(self) -> None:
        ...
        self.last_response = self._service_api.execute_graphql(self._get_query(), variables=self.variables)
...
class File(Attrs):
    ...
    def __init__(self, service_api: ServiceApi, attrs: dict[str, Any], run: Run | None=None):
    ...
    def path_uri(self) -> str:
        ...
        if not (direct_url := self._attrs.get('directUrl')):
            lumina.termwarn('Unable to find direct_url of file')
            return ''
        ...
        try:
            ...
            lumina.termwarn('path_uri is only available for files stored in S3')
            return ''

    @normalize_exceptions
    def download(self, root: str='.', replace: bool=False, exist_ok: bool=False, api: Api | None=None) -> io.TextIOWrapper:
        ...
        service_api.send_api_request(ApiRequest(download_file_request=DownloadFileRequest(path=path, url=self.url, size=self.size)))
        return open(path)

    @normalize_exceptions
    def delete(self) -> None:
        """Delete the file from the W&B server."""
        variables = {'files': [self.id], 'projectId': self.run._project_internal_id}
        mutation = '\n            mutation deleteFiles($files: [ID!]!, $projectId: Int) {\n                deleteFiles(input: {\n                    files: $files\n                    projectId: $projectId\n                }) {\n                    success\n                }\n            }\n        '
        self._service_api.execute_graphql(mutation, variables=variables)
