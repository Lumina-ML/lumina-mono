...
from lumina._analytics import tracked
...
from lumina.sdk import wandb_login, wandb_setup
from lumina.sdk.artifacts._gqlutils import resolve_org_entity_name
from lumina.sdk.internal.internal_api import Api as InternalApi
from lumina.sdk.launch.utils import LAUNCH_DEFAULT_PROJECT
from lumina.sdk.lib import runid, wbauth
...
if TYPE_CHECKING:
    from lumina.automations import ActionType, Automation, EventType, Integration, NewAutomation, ScopeType, SlackIntegration, WebhookIntegration
    from lumina.automations._utils import WriteAutomationsKwargs
    from lumina.sdk.artifacts.artifact import Artifact
    from .artifacts import ArtifactCollection, ArtifactCollections, Artifacts, ArtifactType, ArtifactTypes
...
def _is_service_api_transport_error(error: BaseException | None) -> bool:
    ...
    return status in {None, HTTPStatus.REQUEST_TIMEOUT.value, HTTPStatus.GATEWAY_TIMEOUT.value}
...
class Api:
    ...
    def __init__(self, overrides: dict[str, Any] | None=None, timeout: int | None=None, api_key: str | None=None) -> None:
        """Initialize the API.
        ...
        if api_key:
            self._auth = wbauth.AuthApiKey(host=self.settings['base_url'], api_key=api_key)
        else:
        ...
        if isinstance(self._auth, wbauth.AuthApiKey):
            ...
            wandb_login._verify_login(key=self.api_key, base_url=base_url)
        else:
            self.api_key = None
        self._viewer = None
        self._projects = {}
        ...
        extra_headers['User-Agent'] = f'W&B Public Client {lumina.__version__}'
        ...
        self._service_api: ServiceApi = ServiceApi(settings=settings, timeout=self._timeout)
        self._sentry = lumina.analytics.sentry.Sentry(pid=os.getpid())
        self._configure_sentry()

    def _load_auth(self, base_url: str) -> wbauth.Auth:
        """Load or prompt for authentication credentials."""
        auth = wbauth.authenticate_session(host=base_url, source='wandb.Api()', no_offline=True, input_timeout=wandb_setup.singleton().settings.login_timeout)
        if not auth:
    ...
    def _configure_sentry(self) -> None:
        ...
        self._sentry.configure_scope(tags={'entity': entity, 'email': email})

    def _resolve_org_entity_name(self, *, non_org_entity: str | None, org_or_entity: str | None=None) -> str:
        return resolve_org_entity_name(self._service_api, non_org_entity=non_org_entity, org_or_entity=org_or_entity)
    ...
    def create_project(self, name: str, entity: str) -> None:
        ...
        """
        from lumina.apis._generated import CREATE_PROJECT_GQL, UpsertModelInput
        gql_input = UpsertModelInput(name=name, entity_name=entity)
        self._service_api.execute_graphql(CREATE_PROJECT_GQL, {'input': gql_input.model_dump()})

    def create_run(self, *, run_id: str | None=None, project: str | None=None, entity: str | None=None) -> public.Run:
        """Create a new run.
    ...
    def _create_run(self, *, run_id: str | None=None, project: str | None=None, entity: str | None=None, state: Literal['running', 'pending']='running') -> public.Run:
        ...
        res = self._service_api.execute_graphql(mutation, variables)
        res = res['upsertBucket']['bucket']
        return public.Run(self._service_api, res['project']['entity']['name'], res['project']['name'], res['name'], {'id': res['id'], 'config': '{}', 'systemMetrics': '{}', 'summaryMetrics': '{}', 'tags': [], 'description': None, 'notes': None, 'state': state}, lazy=False, api_key=self.api_key)

    def create_run_queue(self, name: str, type: public.RunQueueResourceType, entity: str | None=None, prioritization_mode: public.RunQueuePrioritizationMode | None=None, config: dict | None=None, template_variables: dict | None=None) -> public.RunQueue:
        ...
        if not create_config_result['success']:
            raise lumina.Error('failed to create default resource config')
        ...
        if not create_queue_result['success']:
            raise lumina.Error('failed to create run queue')
        return public.RunQueue(self._service_api, name=name, entity=entity, prioritization_mode=prioritization_mode, _access='PROJECT', _default_resource_config_id=config_id, _default_resource_config=config, api_key=self.api_key)

    def create_custom_chart(self, entity: str, name: str, display_name: str, spec_type: Literal['vega2'], access: Literal['private', 'public'], spec: str | dict) -> str:
        ...
        result = api.create_custom_chart(entity=entity, name=name, display_name=display_name, spec_type=spec_type, access=backend_access, spec=spec)
        if result is None or result.get('chart') is None:
            raise lumina.Error('failed to create custom chart')
        return result['chart']['id']

    def upsert_run_queue(self, name: str, resource_config: dict, resource_type: public.RunQueueResourceType, entity: str | None=None, template_variables: dict | None=None, external_links: dict | None=None, prioritization_mode: public.RunQueuePrioritizationMode | None=None):
        ...
        if prioritization_mode not in ['V0', 'DISABLED']:
            raise ValueError("prioritization_mode must be 'V0' or 'DISABLED' if present")
        if resource_type not in ['local-container', 'local-process', 'kubernetes', 'sagemaker', 'gcp-vertex']:
            raise ValueError("resource_type must be one of 'local-container', 'local-process', 'kubernetes', 'sagemaker', or 'gcp-vertex'")
        self.create_project(LAUNCH_DEFAULT_PROJECT, entity)
        api = InternalApi(default_settings={'entity': entity, 'project': self.project(LAUNCH_DEFAULT_PROJECT)}, retry_timedelta=RETRY_TIMEDELTA)
        external_links = external_links or {}
        external_links = {'links': [{'label': key, 'url': value} for key, value in external_links.items()]}
        upsert_run_queue_result = api.upsert_run_queue(name, entity, resource_type, {'resource_args': {resource_type: resource_config}}, template_variables=template_variables, external_links=external_links, prioritization_mode=prioritization_mode)
        if not upsert_run_queue_result['success']:
            raise lumina.Error('failed to create run queue')
        schema_errors = upsert_run_queue_result.get('configSchemaValidationErrors') or []
        for error in schema_errors:
            lumina.termwarn(f'resource config validation: {error}')
        return public.RunQueue(self._service_api, name=name, entity=entity, api_key=self.api_key)
    ...
    def sync_tensorboard(self, root_dir, run_id=None, project=None, entity=None):
        ...
        from lumina.sync import SyncManager
        run_id = run_id or runid.generate_id()
        project = project or self.settings.get('project') or 'uncategorized'
        entity = entity or self.default_entity
        sm = SyncManager(project=project, entity=entity, run_id=run_id, mark_synced=False, app_url=self._service_api.app_url, view=False, verbose=False, sync_tensorboard=True)
        sm.add(root_dir)
        sm.start()
    ...
    def user_agent(self) -> str:
        ...
        return 'W&B Public Client {}'.format(lumina.__version__)

    @property
    def default_entity(self) -> str | None:
        """Returns the default W&B entity."""
        from lumina.apis._generated import GET_DEFAULT_ENTITY_GQL, GetDefaultEntity
        if self._default_entity is None:
            result = self._service_api.execute_graphql(GET_DEFAULT_ENTITY_GQL, parse=GetDefaultEntity.model_validate_json)
    ...
    def viewer(self) -> User:
        ...
        """
        from lumina.apis._generated import GET_VIEWER_GQL, GetViewer
        from .users import User
        if self._viewer is None:
            result = self._service_api.execute_graphql(GET_VIEWER_GQL, parse=GetViewer.model_validate_json)
            if (viewer := result.viewer) is None:
                msg = 'Unable to fetch user data from W&B, please verify your API key is valid.'
                raise ValueError(msg)
            self._viewer = User(self._service_api, viewer.model_dump(), api_key=self.api_key)
    ...
    def from_path(self, path: str):
        ...
        if len(parts) == 1:
            ...
            if parts[2].startswith('run'):
                ...
                if '--' not in parts[-1]:
                    if '-' in parts[-1]:
                        raise lumina.Error('Invalid report path, should be team/project/reports/Name--XXXX')
                    else:
                        parts[-1] = '--' + parts[-1]
                name, id = parts[-1].split('--')
                return public.BetaReport(self._service_api, {'displayName': urllib.parse.unquote(name.replace('-', ' ')), 'id': id, 'spec': '{}'}, parts[0], parts[1])
        raise lumina.Error('Invalid path, should be TEAM/PROJECT/TYPE/ID where TYPE is runs, sweeps, or reports')
    ...
    def _parse_artifact_path(self, path: str | None) -> tuple[str | None, ...]:
        ...
        from lumina.sdk.artifacts._validators import ArtifactPath
        project = self.settings['project'] or 'uncategorized'
        entity = self.settings['entity'] or self.default_entity
        if path is None:
            return (entity, project)
    ...
    def projects(self, entity: str | None=None, per_page: int=200) -> public.Projects:
        ...
        if entity not in self._projects:
            self._projects[entity] = public.Projects(self._service_api, entity, per_page=per_page)
    ...
    def project(self, name: str, entity: str | None=None) -> public.Project:
        ...
        if is_artifact_registry_project(name):
            ...
            entity = self._resolve_org_entity_name(non_org_entity=settings_entity, org_or_entity=org)
        return public.Project(self._service_api, entity, name, {})

    def reports(self, path: str='', name: str | None=None, per_page: int=50) -> public.Reports:
        ...
        if key not in self._reports:
            self._reports[key] = public.Reports(self._service_api, public.Project(self._service_api, entity, project, {}), name=name, per_page=per_page)
    ...
    def organization(self, name: str | None=None) -> Organization:
        ...
        """
        from lumina.sdk.artifacts._generated import FETCH_ORGANIZATION_GQL, FetchOrganization
        from lumina.sdk.artifacts._gqlutils import resolve_org_name
        from .organizations import Organization
        if not (org_name := (name or self.settings.get('organization'))):
            org_name = resolve_org_name(self._service_api, non_org_entity=self.settings.get('entity') or self.default_entity)
        result = self._service_api.execute_graphql(FETCH_ORGANIZATION_GQL, variables={'org': org_name}, parse=FetchOrganization.model_validate_json)
    ...
    def user(self, username_or_email: str) -> User | None:
        ...
        """
        from lumina.apis._generated import SEARCH_USERS_GQL, SearchUsers
        from .users import User
        result = self._service_api.execute_graphql(SEARCH_USERS_GQL, variables={'query': username_or_email}, parse=SearchUsers.model_validate_json)
    ...
    def users(self, username_or_email: str) -> list[User]:
        ...
        if not ((conn := result.users) and (edges := conn.edges)):
            return []
        return [User(self._service_api, edge.node.model_dump(), api_key=self.api_key) for edge in edges]

    def runs(self, path: str | None=None, filters: dict[str, Any] | None=None, order: str='+created_at', per_page: int=50, include_sweeps: bool=False, lazy: bool=True):
        ...
        if self._runs.get(key):
            ...
            return cached_runs
        self._runs[key] = public.Runs(self._service_api, entity, project, filters=filters, order=order, per_page=per_page, include_sweeps=include_sweeps, lazy=lazy, api_key=self.api_key)
    ...
    def run(self, path=''):
        ...
        if not self._runs.get(path):
            self._runs[path] = public.Run(self._service_api, entity, project, run_id, lazy=False, api_key=self.api_key)
        return self._runs[path]

    def queued_run(self, entity: str, project: str, queue_name: str, run_queue_item_id: str, project_queue=None, priority=None):
        """Return a single queued run based on the path.

        Parses paths of the form `entity/project/queue_id/run_queue_item_id`.
        """
        return public.QueuedRun(self._service_api, entity, project, queue_name, run_queue_item_id, project_queue=project_queue, priority=priority, api_key=self.api_key)

    def run_queue(self, entity: str, name: str):
        """Return the named `RunQueue` for entity.

        See `Api.create_run_queue` for more information on how to create a run queue.
        """
        return public.RunQueue(self._service_api, name, entity, api_key=self.api_key)
    ...
    def sweep(self, path=''):
        ...
        if not self._sweeps.get(path):
            self._sweeps[path] = public.Sweep(self._service_api, entity, project, sweep_id)
        return self._sweeps[path]

    def _get_sweep(self, entity: str | None=None, project: str | None=None, sweep_id: str | None=None, order: str | None=None, query: Any | None=None, **kwargs: Any) -> public.Sweep | None:
        from .sweeps import _get_sweep
        return _get_sweep(self._service_api, entity, project, sweep_id, order=order, query=query, **kwargs)

    @normalize_exceptions
    def artifact_types(self, project: str | None=None, start: str | None=None) -> ArtifactTypes:
    ...
    @normalize_exceptions
    def artifact_collections(self, project_name: str, type_name: str, order: str | None=None, per_page: int=50, start: str | None=None) -> ArtifactCollections:
        ...
        if is_artifact_registry_project(project):
            ...
            settings_entity = self.settings['entity'] or self.default_entity
            entity = self._resolve_org_entity_name(non_org_entity=settings_entity, org_or_entity=org)
        return ArtifactCollections(self._service_api, entity, project, type_name, order=order, per_page=per_page, start=start)
    ...
    def artifact_collection(self, type_name: str, name: str) -> ArtifactCollection:
        ...
        if is_artifact_registry_project(project):
            ...
            entity = self._resolve_org_entity_name(non_org_entity=settings_entity, org_or_entity=org)
        if entity is None:
            raise ValueError('Could not determine entity. Please include the entity as part of the collection name path.')
        return ArtifactCollection(self._service_api, entity, project, collection_name, type_name)

    @normalize_exceptions
    def artifact_versions(self, type_name, name, per_page=50):
        """Deprecated. Use `Api.artifacts(type_name, name)` method instead."""
        warn_and_record_deprecation(feature=Deprecated(api__artifact_versions=True), message='Api.artifact_versions(type_name, name) is deprecated, use Api.artifacts(type_name, name) instead.')
        return self.artifacts(type_name, name, per_page=per_page)

    @normalize_exceptions
    def artifacts(self, type_name: str, name: str, order: str | None=None, per_page: int=50, tags: list[str] | None=None, start: str | None=None) -> Artifacts:
        ...
        if is_artifact_registry_project(project):
            ...
            settings_entity = self.settings['entity'] or self.default_entity
            entity = self._resolve_org_entity_name(non_org_entity=settings_entity, org_or_entity=org)
        return Artifacts(self._service_api, entity, project, collection_name, type_name, order=order, per_page=per_page, tags=tags, start=start)

    @normalize_exceptions
    def _artifact(self, name: str, type: str | None=None) -> Artifact:
        from lumina.sdk.artifacts._validators import FullArtifactPath, is_artifact_registry_project
        from lumina.sdk.artifacts.artifact import Artifact
        if name is None:
            raise ValueError('You must specify name= to fetch an artifact.')
        entity, project, artifact_name = self._parse_artifact_path(name)
        if is_artifact_registry_project(project):
            organization = name.split('/')[0] if name.count('/') == 2 else self.settings['organization']
            settings_entity = self.settings['entity'] or self.default_entity
            entity = self._resolve_org_entity_name(non_org_entity=settings_entity, org_or_entity=organization)
        if entity is None:
    ...
    def _expected_artifact_type(self, *, entity: str, project: str, name: str) -> str | None:
        from lumina.sdk.artifacts.artifact import Artifact
    ...
    def job(self, name: str | None, path: str | None=None) -> public.Job:
        ...
        if name is None:
            ...
            raise ValueError('Invalid job specification. A job must be of the form: <entity>/<project>/<job-name>:<alias-or-version>')
    ...
    def list_jobs(self, entity: str, project: str) -> list[dict[str, Any]]:
        ...
        try:
            artifact_query = self._service_api.execute_graphql(query, {'projectName': project, 'entityName': entity, 'artifactTypeName': 'job'})
            if not artifact_query or not artifact_query['project']:
                lumina.termerror(f"Project: '{project}' not found in entity: '{entity}' or access denied.")
                return []
            if artifact_query['project']['artifactType'] is None:
                return []
            artifacts = artifact_query['project']['artifactType']['artifactCollections']['edges']
    ...
    @tracked
    def registries(self, organization: str | None=None, filter: dict[str, Any] | None=None, order: str | None=None, per_page: int=100, start: str | None=None) -> Registries:
        ...
        if not self._service_api.feature_enabled(pb.ARTIFACT_REGISTRY_SEARCH):
            raise RuntimeError('Registry search API is not enabled on this wandb server version.' + ' Please upgrade your server version or contact support' + ' at support@wandb.com.')
        organization = organization or fetch_org_from_settings_or_entity(self.settings, self.default_entity)
        return Registries(self._service_api, organization=organization, filter=filter, order=order, per_page=per_page, start=start)
    ...
    def registry(self, name: str, organization: str | None=None) -> Registry:
        ...
        """
        if not self._service_api.feature_enabled(pb.ARTIFACT_REGISTRY_SEARCH):
            raise RuntimeError('api.registry() is not enabled on this wandb server version.' + ' Please upgrade your server version or contact support' + ' at support@wandb.com.')
        organization = organization or fetch_org_from_settings_or_entity(self.settings, self.default_entity)
        org_entity = fetch_org_entity_from_organization(self._service_api, organization)
        registry = Registry(self._service_api, organization, org_entity, name)
        registry.load()
        return registry

    @tracked
    def create_registry(self, name: str, visibility: Literal['organization', 'restricted'], organization: str | None=None, description: str | None=None, artifact_types: list[str] | None=None) -> Registry:
        ...
        ```
        """
        if not self._service_api.feature_enabled(pb.INCLUDE_ARTIFACT_TYPES_IN_REGISTRY_CREATION):
            raise RuntimeError('create_registry api is not enabled on this wandb server version.' + ' Please upgrade your server version or contact support' + ' at support@wandb.com.')
        organization = organization or fetch_org_from_settings_or_entity(self.settings, self.default_entity)
        try:
        ...
        if existing_registry:
            raise ValueError(f'Registry {name!r} already exists in organization {organization!r}, please use a different name.')
        return Registry.create(self, organization, name, visibility, description, artifact_types)

    @tracked
    def integrations(self, entity: str | None=None, *, per_page: int=50, start: str | None=None) -> Iterator[Integration]:
        ...
        """
        from lumina.apis.public.integrations import Integrations
        variables = {'entity': entity or self.default_entity}
        return Integrations(self._service_api, variables=variables, per_page=per_page, start=start)

    @tracked
    def webhook_integrations(self, entity: str | None=None, *, per_page: int=50, start: str | None=None) -> Iterator[WebhookIntegration]:
        ...
        ```
        """
        from lumina.apis.public.integrations import WebhookIntegrations
        variables = {'entity': entity or self.default_entity}
        return WebhookIntegrations(self._service_api, variables=variables, per_page=per_page, start=start)

    @tracked
    def slack_integrations(self, *, entity: str | None=None, per_page: int=50, start: str | None=None) -> Iterator[SlackIntegration]:
        ...
        ```
        """
        from lumina.apis.public.integrations import SlackIntegrations
        variables = {'entity': entity or self.default_entity}
        return SlackIntegrations(self._service_api, variables=variables, per_page=per_page, start=start)

    def _supports_automation(self, *, scope: ScopeType | None=None, event: EventType | None=None, action: ActionType | None=None) -> bool:
        """Returns whether the server recognizes the automation event and/or action."""
        from lumina.automations._utils import ALWAYS_SUPPORTED_ACTIONS, ALWAYS_SUPPORTED_EVENTS, ALWAYS_SUPPORTED_SCOPES
        supports_scope = scope is None or scope in ALWAYS_SUPPORTED_SCOPES or self._service_api.feature_enabled(f'AUTOMATION_SCOPE_{scope.value}')
        supports_event = event is None or event in ALWAYS_SUPPORTED_EVENTS or self._service_api.feature_enabled(f'AUTOMATION_EVENT_{event.value}')
        supports_action = action is None or action in ALWAYS_SUPPORTED_ACTIONS or self._service_api.feature_enabled(f'AUTOMATION_ACTION_{action.value}')
    ...
    def _omitted_automation_fragments(self) -> set[str]:
        ...
        removed from the body of the GraphQL request:
            ...
            - Fragment spread in selection set:
                ...
                ```
        """
        from lumina.automations import ActionType, ScopeType
        from lumina.automations._generated import EntityScopeFields, GenericWebhookActionFields, NoOpActionFields, NotificationActionFields, QueueJobActionFields
        scope_fragment_names: dict[ScopeType, str] = {ScopeType.ENTITY: nameof(EntityScopeFields)}
        action_fragment_names: dict[ActionType, str] = {ActionType.NO_OP: nameof(NoOpActionFields), ActionType.QUEUE_JOB: nameof(QueueJobActionFields), ActionType.NOTIFICATION: nameof(NotificationActionFields), ActionType.GENERIC_WEBHOOK: nameof(GenericWebhookActionFields)}
        omitted_scope_fragments = set((name for scope in ScopeType if not self._supports_automation(scope=scope) and (name := scope_fragment_names.get(scope))))
        omitted_action_fragments = set((name for action in ActionType if not self._supports_automation(action=action) and (name := action_fragment_names.get(action))))
        return omitted_scope_fragments | omitted_action_fragments

    @tracked
    def automation(self, name: str, *, entity: str | None=None) -> Automation:
        ...
        ```
        """
        return one(self.automations(entity=entity, name=name), too_short=ValueError('No automations found'), too_long=ValueError('Multiple automations found'))

    @tracked
    def automations(self, entity: str | None=None, *, name: str | None=None, per_page: int=50, start: str | None=None) -> Iterator[Automation]:
        ...
        from lumina.automations._generated import GET_AUTOMATIONS_LEGACY_GQL, GET_ENTITY_AUTOMATIONS_LEGACY_GQL
        variables = {'entity': entity}
        if entity is None:
            gql_str = GET_AUTOMATIONS_LEGACY_GQL
        else:
            gql_str = GET_ENTITY_AUTOMATIONS_LEGACY_GQL
        iterator = Automations(self._service_api, variables=variables, per_page=per_page, start=start, _query=gql_str, omit_fragments=self._omitted_automation_fragments())
        if name is not None:
    ...
    @normalize_exceptions
    @tracked
    def create_automation(self, obj: NewAutomation, *, fetch_existing: bool=False, **kwargs: Unpack[WriteAutomationsKwargs]) -> Automation:
        ...
        ```
        """
        from lumina.automations import Automation
        from lumina.automations._generated import CREATE_AUTOMATION_GQL, CreateAutomation
        from lumina.automations._utils import prepare_to_create
        gql_input = prepare_to_create(obj, **kwargs)
        if not self._supports_automation(event=(event := gql_input.triggering_event_type), action=(action := gql_input.triggered_action_type)):
            raise ValueError(f'Automation event or action ({event!r} -> {action!r}) is not supported on this wandb server version. Please upgrade your server version, or contact support at support@wandb.com.')
        omit_fragments = self._omitted_automation_fragments()
        variables = {'input': gql_input.model_dump()}
        name = gql_input.name
        try:
            data = self._service_api.execute_graphql(CREATE_AUTOMATION_GQL, variables=variables, omit_fragments=omit_fragments, parse=CreateAutomation.model_validate_json)
        except WandbApiFailedError as e:
            status = _api_error_status(e)
            if status == HTTPStatus.CONFLICT.value:
                if fetch_existing:
                    lumina.termlog(f'Automation {name!r} exists. Fetching it instead.')
                    return self.automation(name=name)
                raise ValueError(f'Automation {name!r} exists. Unable to create another with the same name.') from None
            raise
    ...
    @normalize_exceptions
    @tracked
    def update_automation(self, obj: Automation, *, create_missing: bool=False, **kwargs: Unpack[WriteAutomationsKwargs]) -> Automation:
        ...
        ```
        """
        from lumina.automations import ActionType, Automation
        from lumina.automations._generated import UPDATE_AUTOMATION_GQL, UpdateAutomation
        from lumina.automations._utils import prepare_to_update
        if not self._supports_automation(action=ActionType.NO_OP):
            raise RuntimeError('Updating existing automations is not enabled on this wandb server version. Please upgrade your server version, or contact support at support@wandb.com.')
        gql_input = prepare_to_update(obj, **kwargs)
        if not self._supports_automation(event=(event := gql_input.triggering_event_type), action=(action := gql_input.triggered_action_type)):
            raise ValueError(f'Automation event or action ({event.value} -> {action.value}) is not supported on this wandb server version. Please upgrade your server version, or contact support at support@wandb.com.')
        variables = {'input': gql_input.model_dump()}
        name = gql_input.name
        try:
            data = self._service_api.execute_graphql(UPDATE_AUTOMATION_GQL, variables=variables, omit_fragments=self._omitted_automation_fragments(), parse=UpdateAutomation.model_validate_json)
        except WandbApiFailedError as e:
            status = _api_error_status(e)
            if status == HTTPStatus.NOT_FOUND.value:
                if create_missing:
                    lumina.termlog(f'Automation {name!r} not found. Creating it.')
                    return self.create_automation(obj)
                raise ValueError(f'Automation {name!r} not found. Unable to edit it.') from e
            lumina.termerror(f'Got API error: {e}')
            raise
        except ValidationError as e:
    ...
    def delete_automation(self, obj: Automation | str) -> Literal[True]:
        ...
        """
        from lumina.automations._generated import DELETE_AUTOMATION_GQL, DeleteAutomation
        from lumina.automations._utils import extract_id
        id_ = extract_id(obj)
        try:
            data = self._service_api.execute_graphql(DELETE_AUTOMATION_GQL, variables={'id': id_}, parse=DeleteAutomation.model_validate_json)
        except ValidationError as e:
