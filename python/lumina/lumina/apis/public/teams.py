...
class Member(Attrs):
    ...
    def delete(self):
        ...
        """
        from lumina.apis._generated import DELETE_INVITE_GQL, DeleteInvite
        try:
            data = self._service_api.execute_graphql(DELETE_INVITE_GQL, {'id': self.id, 'entity': self.team}, parse=DeleteInvite.model_validate_json)
...
class Team(Attrs):
    ...
    def __init__(self, service_api: ServiceApi, name: str, attrs: Mapping[str, Any] | None=None):
    ...
    def create(cls, api: Api, team: str, admin_username: str | None=None) -> Team:
        ...
        Returns:
            A `Team` object
        """
        from lumina.apis._generated import CREATE_TEAM_GQL
        try:
            api._service_api.execute_graphql(CREATE_TEAM_GQL, {'teamName': team, 'teamAdminUserName': admin_username})
        except WandbApiFailedError:
            pass
    ...
    def invite(self, username_or_email: str, admin: bool=False) -> bool:
        ...
        """
        from lumina.apis._generated import CREATE_INVITE_GQL
        variables = {'entity': self.name, 'admin': admin, 'email' if '@' in username_or_email else 'username': username_or_email}
        try:
    ...
    def create_service_account(self, description: str) -> Member | None:
        ...
        """
        from lumina.apis._generated import CREATE_SERVICE_ACCOUNT_GQL
        try:
            self._service_api.execute_graphql(CREATE_SERVICE_ACCOUNT_GQL, {'entity': self.name, 'description': description})
            self.load(True)
    ...
    def load(self, force: bool=False) -> dict[str, Any]:
        ...
        """
        from lumina.apis._generated import GET_TEAM_ENTITY_GQL, GetTeamEntity
        if force or not self._attrs:
            result = self._service_api.execute_graphql(GET_TEAM_ENTITY_GQL, variables={'name': self.name}, parse=GetTeamEntity.model_validate_json)
            self._attrs = entity.model_dump() if (entity := result.entity) else {}
            self._attrs['members'] = [Member(self._service_api, self.name, member) for member in self._attrs['members']]
        return self._attrs
