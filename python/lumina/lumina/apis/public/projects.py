...
from lumina.apis.public.sweeps import Sweeps
...
if TYPE_CHECKING:
    ...
    from lumina.apis._generated import ProjectFragment
...
class Projects(RelayPaginator['ProjectFragment', 'Project']):
    ...
    def __init__(self, service_api: ServiceApi, entity: str, per_page: int=50) -> None:
        ...
        if self.QUERY is None:
            from lumina.apis._generated import GET_PROJECTS_GQL
    ...
    def _update_response(self) -> None:
        ...
        from lumina._pydantic import Connection
        from lumina.apis._generated import GetProjects, ProjectFragment
    ...
    def _convert(self, node: ProjectFragment) -> Project:
        return Project(self._service_api, self.entity, node.name, node.model_dump())
...
class Project(Attrs):
    ...
    def __init__(self, service_api: ServiceApi, entity: str, project: str, attrs: Mapping[str, Any]) -> None:
    ...
    def _load(self) -> None:
        from lumina.apis._generated import GET_PROJECT_GQL, GetProject
        gql_vars = {'name': self.name, 'entity': self.entity}
        try:
            data = self._service_api.execute_graphql(GET_PROJECT_GQL, variables=gql_vars, parse=GetProject.model_validate_json)
    ...
    @normalize_exceptions
    def collections(self, filters: Mapping[str, Any] | None=None, order: str | None=None, per_page: int=50) -> public.ProjectArtifactCollections:
        ...
        Args:
            ...
            per_page: The number of artifact collections to fetch per page.
                Default is 50.
        """
        return public.ProjectArtifactCollections(self._service_api, self.entity, self.name, filters=filters, order=order, per_page=per_page)

    @normalize_exceptions
    def sweeps(self, per_page: int=50, filters: dict[str, Any] | None=None, order: str | None=None) -> Sweeps:
        ...
        """
        return Sweeps(self._service_api, self.entity, self.name, per_page=per_page, filters=filters)
