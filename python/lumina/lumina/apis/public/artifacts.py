...
from typing import TYPE_CHECKING, Any, ClassVar, List, Literal, TypeVar
from typing_extensions import override
from lumina._iterutils import always_list
from lumina._pydantic import Connection, ConnectionWithTotal, Edge
from lumina._strutils import nameof
from lumina.apis.normalize import normalize_exceptions
from lumina.apis.paginator import RelayPaginator, SizedRelayPaginator
from lumina.errors.errors import UnsupportedError
...
from lumina.sdk.artifacts._gqlutils import server_supports
from lumina.sdk.artifacts._models import ArtifactCollectionData
...
if TYPE_CHECKING:
    ...
    from lumina.sdk.artifacts._generated import ArtifactAliasFragment, ArtifactCollectionFragment, ArtifactFragment, ArtifactTypeFragment, FileFragment
    from lumina.sdk.artifacts._models.pagination import ArtifactCollectionConnection, ArtifactFileConnection, ArtifactTypeConnection
...
def _run_artifacts_mode_to_gql() -> dict[Literal['logged', 'used'], str]:
    ...
    """
    from lumina.sdk.artifacts._generated import RUN_INPUT_ARTIFACTS_GQL, RUN_OUTPUT_ARTIFACTS_GQL
...
class _ArtifactCollectionAliases(RelayPaginator['ArtifactAliasFragment', str]):
    ...
    def __init__(self, service_api: ServiceApi, collection_id: str, per_page: int=1000, start: str | None=None):
        if self.QUERY is None:
            from lumina.sdk.artifacts._generated import ARTIFACT_COLLECTION_ALIASES_GQL
    ...
    def _update_response(self) -> None:
        from lumina.sdk.artifacts._generated import ArtifactAliasFragment, ArtifactCollectionAliases
        result = self._execute_query(parse=ArtifactCollectionAliases.model_validate_json)
...
class ArtifactTypes(RelayPaginator['ArtifactTypeFragment', 'ArtifactType']):
    ...
    def __init__(self, service_api: ServiceApi, entity: str, project: str, per_page: int=50, start: str | None=None):
        if self.QUERY is None:
            from lumina.sdk.artifacts._generated import PROJECT_ARTIFACT_TYPES_GQL
    ...
    def _update_response(self) -> None:
        ...
        from lumina.sdk.artifacts._generated import ProjectArtifactTypes
        from lumina.sdk.artifacts._models.pagination import ArtifactTypeConnection
    ...
    def _convert(self, node: ArtifactTypeFragment) -> ArtifactType:
        return ArtifactType(service_api=self._service_api, entity=self.entity, project=self.project, type_name=node.name, attrs=node)
...
class ArtifactType:
    ...
    def __init__(self, service_api: ServiceApi, entity: str, project: str, type_name: str, attrs: ArtifactTypeFragment | None=None):
        from lumina.sdk.artifacts._generated import ArtifactTypeFragment
        self._service_api = service_api
        self.entity = entity
        self.project = project
        self.type = type_name
    ...
    def load(self) -> ArtifactTypeFragment:
        ...
        """
        from lumina.sdk.artifacts._generated import PROJECT_ARTIFACT_TYPE_GQL, ArtifactTypeFragment, ProjectArtifactType
        data = self._service_api.execute_graphql(PROJECT_ARTIFACT_TYPE_GQL, {'entity': self.entity, 'project': self.project, 'type': self.type}, parse=ProjectArtifactType.model_validate_json)
    ...
    def collections(self, filters: Mapping[str, Any] | None=None, order: str | None=None, per_page: int=50, start: str | None=None) -> ArtifactCollections:
        ...
        """
        return ArtifactCollections(self._service_api, entity=self.entity, project=self.project, filters=filters, order=order, type_name=self.type, per_page=per_page, start=start)
    ...
    def collection(self, name: str) -> ArtifactCollection:
        ...
        """
        return ArtifactCollection(self._service_api, entity=self.entity, project=self.project, name=name, type=self.type)

    def __repr__(self) -> str:
        return f'<ArtifactType {self.type}>'

class ArtifactCollections(SizedRelayPaginator['ArtifactCollectionFragment', 'ArtifactCollection']):
    ...
    def __init__(self, service_api: ServiceApi, entity: str, project: str, type_name: str, filters: Mapping[str, Any] | None=None, order: str | None=None, per_page: int=50, start: str | None=None):
        if self.QUERY is None:
            from lumina.sdk.artifacts._generated import ARTIFACT_TYPE_ARTIFACT_COLLECTIONS_GQL
            type(self).QUERY = ARTIFACT_TYPE_ARTIFACT_COLLECTIONS_GQL
        if (order is not None or filters is not None) and (not server_supports(service_api, pb.ARTIFACT_COLLECTIONS_FILTERING_SORTING)):
            raise UnsupportedError('Filtering and ordering of artifact collections is not supported on this wandb server version. Please upgrade your server version or contact support at support@wandb.com.')
        self.entity = entity
        self.project = project
        ...
        self.order = order
        self._service_api = service_api
        variables = {'entity': entity, 'project': project, 'type': type_name, 'order': order, 'filters': json.dumps(f) if (f := filters) else None}
        super().__init__(service_api, variables=variables, per_page=per_page, start=start)

    @override
    def _update_response(self) -> None:
        """Fetch and validate the response data for the current page."""
        from lumina.sdk.artifacts._generated import ArtifactTypeArtifactCollections
        from lumina.sdk.artifacts._models.pagination import ArtifactCollectionConnection
        result = self._execute_query(parse=ArtifactTypeArtifactCollections.model_validate_json)
        if not ((proj := result.project) and (artifact_type := proj.artifact_type) and (conn := artifact_type.artifact_collections)):
            raise ValueError(f'Unable to parse {nameof(type(self))!r} response data')
    ...
    def _convert(self, node: ArtifactCollectionFragment) -> ArtifactCollection | None:
        if not node.project:
            return None
        return ArtifactCollection(service_api=self._service_api, entity=node.project.entity.name, project=node.project.name, name=node.name, type=node.type.name, attrs=node)

class ProjectArtifactCollections(SizedRelayPaginator['ArtifactCollectionFragment', 'ArtifactCollection']):
    ...
    def __init__(self, service_api: ServiceApi, entity: str, project: str, filters: Mapping[str, Any] | None=None, order: str | None=None, per_page: int=50, start: str | None=None):
        from lumina.sdk.artifacts._generated import PROJECT_ARTIFACT_COLLECTIONS_GQL
        supports_filtering = server_supports(service_api, pb.ARTIFACT_COLLECTIONS_FILTERING_SORTING)
        ...
        self.order = order
        self._service_api = service_api
        variables = {'entity': entity, 'project': project, 'order': order, 'filters': json.dumps(f) if (f := filters) else None}
        super().__init__(service_api, variables=variables, per_page=per_page, start=start, omit_variables=None if supports_filtering else {'filters'}, omit_fields=None if supports_filtering else {'totalCount'})

    @override
    def _update_response(self) -> None:
        """Fetch and validate the response data for the current page."""
        from lumina.sdk.artifacts._generated import ProjectArtifactCollections
        from lumina.sdk.artifacts._models.pagination import ProjectArtifactCollectionConnection
        result = self._execute_query(parse=ProjectArtifactCollections.model_validate_json)
...
class ArtifactCollection:
    ...
    def __init__(self, service_api: ServiceApi, entity: str, project: str, name: str, type: str, organization: str | None=None, attrs: ArtifactCollectionFragment | None=None):
        self._service_api = service_api
    ...
    def artifacts(self, per_page: int=50, start: str | None=None) -> Artifacts:
        ...
        Args:
            ...
            start: Pagination cursor for resuming a past query, captured
                from a previous paginator's `.cursor` attribute.
        """
        return Artifacts(service_api=self._service_api, entity=self.entity, project=self.project, collection_name=self._saved.name, type=self._saved.type, per_page=per_page, start=start)

    @property
    def aliases(self) -> list[str]:
        """The aliases for all artifact versions contained in this collection."""
        if (aliases := self._saved.aliases) is None:
            aliases = tuple(_ArtifactCollectionAliases(self._service_api, collection_id=self.id))
    ...
    def load(self, entity: str, project: str, type_: str, name: str) -> ArtifactCollectionFragment:
        """Fetch and return the validated artifact collection data from W&B.

        <!-- lazydoc-ignore: internal -->
        """
        from lumina.sdk.artifacts._generated import PROJECT_ARTIFACT_COLLECTION_GQL, ProjectArtifactCollection
        result = self._service_api.execute_graphql(PROJECT_ARTIFACT_COLLECTION_GQL, {'entity': entity, 'project': project, 'type': type_, 'name': name}, parse=ProjectArtifactCollection.model_validate_json)
        if not (result.project and (proj := result.project) and (artifact_type := proj.artifact_type) and (collection := artifact_type.artifact_collection)):
    ...
    def change_type(self, new_type: str) -> None:
        ...
        from lumina.sdk.artifacts._generated import UPDATE_ARTIFACT_SEQUENCE_TYPE_GQL, MoveArtifactSequenceInput
        from lumina.sdk.artifacts._validators import validate_artifact_type
        warn_and_record_deprecation(feature=Deprecated(artifact_collection__change_type=True), message='ArtifactCollection.change_type(type) is deprecated, use ArtifactCollection.save() instead.')
        if (old_type := self._saved.type) != new_type:
            try:
                validate_artifact_type(old_type, self.name)
            except ValueError as e:
                raise ValueError(f'The current type {old_type!r} is an internal type and cannot be changed.') from e
        ...
        gql_input = MoveArtifactSequenceInput(artifact_sequence_id=self.id, destination_artifact_type_name=new_type)
        self._service_api.execute_graphql(UPDATE_ARTIFACT_SEQUENCE_TYPE_GQL, variables={'input': gql_input.model_dump()})
    ...
    def delete(self) -> None:
        ...
        from lumina.sdk.artifacts._generated import DELETE_ARTIFACT_PORTFOLIO_GQL, DELETE_ARTIFACT_SEQUENCE_GQL
        gql_op = DELETE_ARTIFACT_SEQUENCE_GQL if self.is_sequence() else DELETE_ARTIFACT_PORTFOLIO_GQL
    ...
    def type(self, type: str) -> None:
        ...
        if not self.is_sequence():
            raise ValueError('Type can only be changed if the artifact collection is a sequence.')
        self._current.type = type

    def _update_collection(self) -> None:
        from lumina.sdk.artifacts._generated import UPDATE_ARTIFACT_PORTFOLIO_GQL, UPDATE_ARTIFACT_SEQUENCE_GQL, UpdateArtifactPortfolioInput, UpdateArtifactSequenceInput
        if self.is_sequence():
            gql_op = UPDATE_ARTIFACT_SEQUENCE_GQL
            gql_input = UpdateArtifactSequenceInput(artifact_sequence_id=self.id, name=self.name, description=self.description)
        else:
            gql_op = UPDATE_ARTIFACT_PORTFOLIO_GQL
            gql_input = UpdateArtifactPortfolioInput(artifact_portfolio_id=self.id, name=self.name, description=self.description)
    ...
    def _update_sequence_type(self) -> None:
        ...
        gql_input = MoveArtifactSequenceInput(artifact_sequence_id=self.id, destination_artifact_type_name=self.type)
        self._service_api.execute_graphql(gql_op, variables={'input': gql_input.model_dump()})
        self._saved.type = self._current.type

    def _add_tags(self, tag_names: Iterable[str]) -> None:
        from lumina.sdk.artifacts._generated import ADD_ARTIFACT_COLLECTION_TAGS_GQL, CreateArtifactCollectionTagAssignmentsInput
        gql_op = ADD_ARTIFACT_COLLECTION_TAGS_GQL
        gql_input = CreateArtifactCollectionTagAssignmentsInput(entity_name=self.entity, project_name=self.project, artifact_collection_name=self._saved.name, tags=[{'tagName': tag} for tag in tag_names])
        self._service_api.execute_graphql(gql_op, variables={'input': gql_input.model_dump()})

    def _delete_tags(self, tag_names: Iterable[str]) -> None:
        from lumina.sdk.artifacts._generated import DELETE_ARTIFACT_COLLECTION_TAGS_GQL, DeleteArtifactCollectionTagAssignmentsInput
        gql_op = DELETE_ARTIFACT_COLLECTION_TAGS_GQL
        gql_input = DeleteArtifactCollectionTagAssignmentsInput(entity_name=self.entity, project_name=self.project, artifact_collection_name=self._saved.name, tags=[{'tagName': tag} for tag in tag_names])
        self._service_api.execute_graphql(gql_op, variables={'input': gql_input.model_dump()})
    ...
    def save(self) -> None:
        ...
        if (old_type := self._saved.type) != (new_type := self.type):
            try:
                ...
                reason = str(e)
                raise ValueError(f'Failed to save artifact collection {self.name!r}: {reason}') from e
            try:
                validate_artifact_type(old_type, self.name)
            except ValueError as e:
...
class Artifacts(SizedRelayPaginator['ArtifactFragment', 'Artifact']):
    ...
    def __init__(self, service_api: ServiceApi, entity: str, project: str, collection_name: str, type: str, filters: Mapping[str, Any] | None=None, order: str | None=None, per_page: int=50, tags: str | list[str] | None=None, start: str | None=None):
        from lumina.sdk.artifacts._generated import PROJECT_ARTIFACTS_GQL
        ...
        self.order = order
        self._service_api = service_api
        variables = {'entity': self.entity, 'project': self.project, 'order': self.order, 'type': self.type, 'collection': self.collection_name, 'filters': json.dumps(self.filters)}
        super().__init__(service_api, variables=variables, per_page=per_page, start=start)

    @override
    def _update_response(self) -> None:
        from lumina.sdk.artifacts._generated import ArtifactFragment, ProjectArtifacts
        result = self._execute_query(parse=ProjectArtifacts.model_validate_json)
        if not ((proj := result.project) and (type_ := proj.artifact_type) and (collection := type_.artifact_collection) and (conn := collection.artifacts)):
            raise ValueError(f'Unable to parse {nameof(type(self))!r} response data')
        self.last_response = _ArtifactConnectionGeneric[ArtifactFragment].model_validate(conn)

    @override
    def _convert(self, edge: _ArtifactEdgeGeneric[ArtifactFragment]) -> Artifact:
        from lumina.sdk.artifacts._validators import FullArtifactPath
        from lumina.sdk.artifacts.artifact import Artifact
        return Artifact._from_attrs(path=FullArtifactPath(prefix=self.entity, project=self.project, name=f'{self.collection_name}:{edge.version}'), src_art=edge.node, service_api=self._service_api)
...
class RunArtifacts(SizedRelayPaginator['ArtifactFragment', 'Artifact']):
    ...
    def __init__(self, service_api: ServiceApi, run: Run, mode: Literal['logged', 'used']='logged', per_page: int=50, start: str | None=None):
        try:
    ...
    @override
    def _update_response(self) -> None:
        from lumina.sdk.artifacts._models.pagination import RunArtifactConnection
    ...
    def _convert(self, node: ArtifactFragment) -> Artifact | None:
        ...
        if node.artifact_sequence.project is None:
            return None
        return Artifact._from_attrs(path=FullArtifactPath(prefix=node.artifact_sequence.project.entity.name, project=node.artifact_sequence.project.name, name=f'{node.artifact_sequence.name}:v{node.version_index}'), src_art=node, service_api=self._service_api)
...
class ArtifactFiles(SizedRelayPaginator['FileFragment', 'File']):
    ...
    def __init__(self, service_api: ServiceApi, artifact: Artifact, names: Sequence[str] | None=None, per_page: int=50, start: str | None=None):
        from lumina.sdk.artifacts._gqlutils import server_supports
        if self.QUERY is None:
            from lumina.sdk.artifacts._generated import ARTIFACT_MEMBERSHIP_FILES_GQL
            type(self).QUERY = ARTIFACT_MEMBERSHIP_FILES_GQL
        self.artifact = artifact
        variables = {'entity': artifact.entity, 'project': artifact.project, 'collection': artifact.name.split(':')[0], 'alias': artifact.version, 'fileNames': names}
        super().__init__(service_api, variables=variables, per_page=per_page, start=start, omit_fields=None if server_supports(service_api, pb.TOTAL_COUNT_IN_FILE_CONNECTION) else {'totalCount'})

    @override
    def _update_response(self) -> None:
        from lumina.sdk.artifacts._generated import ArtifactMembershipFiles
        from lumina.sdk.artifacts._models.pagination import ArtifactFileConnection
