...
_P = ParamSpec('_P')
_T = TypeVar('_T')
SENTRY_DEFAULT_DSN = 'https://2592b1968ea94cca9b5ef5e348e094a7@o151352.ingest.sentry.io/4504800232407040'
SessionStatus = Literal['ok', 'exited', 'crashed', 'abnormal']

def _guard(method: Callable[Concatenate[Sentry, _P], _T]) -> Callable[Concatenate[Sentry, _P], _T | None]:
    ...
    def wrapper(self: Sentry, *args: _P.args, **kwargs: _P.kwargs) -> _T | None:
...
class Sentry:
    ...
    def __init__(self, *, pid: int) -> None:
        from lumina import env as _env
        self._pid: int = pid
    ...
    def _boot(self) -> bool:
        ...
        with self._boot_lock:
            ...
            try:
                ...
                client = self._sdk.Client(dsn=self.dsn, default_integrations=False, environment=self.environment, release=__version__)
    ...
    @_guard
    def message(self, message: str, repeat: bool=True, level: str='info') -> str | None:
        if not repeat and message in self._sent_messages:
            return None
        self._sent_messages.add(message)
        with self._sdk.scope.use_isolation_scope(self.scope):
            return self._sdk.capture_message(message, level=level)

    @_guard
    def exception(self, exc: str | BaseException | tuple[type[BaseException] | None, BaseException | None, TracebackType | None] | None, handled: bool=False, status: SessionStatus | None=None) -> str | None:
        if isinstance(exc, str):
            exc_info = self._sdk.utils.exc_info_from_error(Exception(exc))
        elif isinstance(exc, BaseException):
            exc_info = self._sdk.utils.exc_info_from_error(exc)
        else:
            exc_info = sys.exc_info()
        event, _ = self._sdk.utils.event_from_exception(exc_info, client_options=self.scope.get_client().options, mechanism={'type': 'generic', 'handled': handled})
        event_id = None
        with contextlib.suppress(Exception):
            with self._sdk.scope.use_isolation_scope(self.scope):
                event_id = self._sdk.capture_event(event)
    ...
    def configure_scope(self, tags: dict[str, Any] | None=None, process_context: str | None=None) -> None:
        ...
        if self.scope is None:
            return
        settings_tags = ('entity', 'project', 'run_id', 'run_url', 'sweep_url', 'sweep_id', 'deployment', 'launch', '_platform')
        if process_context:
        ...
        for obj in ('run', 'sweep'):
            ...
            if tags.get(obj_url, None):
                continue
            try:
                app_url = tags.get('app_url') or lumina.util.api_to_app_url(tags['base_url'])
                app_url = app_url.rstrip('/')
                entity, project = (quote(tags[k]) for k in ('entity', 'project'))
                self.scope.set_tag(obj_url, f'{app_url}/{entity}/{project}/{obj}s/{tags[obj_id]}')
            except Exception:
                pass
