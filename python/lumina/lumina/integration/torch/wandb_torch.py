...
from lumina.data_types import Node
torch = None
if TYPE_CHECKING:
...
class TorchHistory:
    ...
    def __init__(self):
        global torch
        torch = lumina.util.get_module('torch', 'Could not import torch')
        self._hook_handles = {}
        self._num_bins = 64
        self._is_cuda_histc_supported = None
        self.hook_torch = TorchGraph.hook_torch

    def add_log_parameters_hook(self, module: Module, name: str='', prefix: str='', log_freq: int=0) -> None:
        ...
        try:
            ...
            lumina.termwarn(f'Trying to register forward_hook failed ({e}) - skipping parameter tracking.')

    def add_log_gradients_hook(self, module: Module, name: str='', prefix: str='', log_freq: int=0) -> None:
    ...
    def log_tensor_stats(self, tensor, name):
        ...
        lumina.run._log({name: lumina.Histogram(np_histogram=(tensor.tolist(), bins.tolist()))}, commit=False)
...
class TorchGraph(lumina.data_types.Graph):
    ...
    def hook_torch(cls, model, criterion=None, graph_idx=0):
        lumina.termlog('logging graph, to disable use `wandb.watch(log_graph=False)`')
        graph = TorchGraph()
    ...
    def create_forward_hook(self, name, graph_idx):
        ...
        def after_forward_hook(module, input, output):
            ...
            if not self._graph_hooks:
                lumina.run.summary[f'graph_{graph_idx}'] = self
        ...
        for name, sub_module in module.named_children():
            ...
            if isinstance(sub_module, tuple(module_types)):
                ...
                try:
                    ...
                    lumina.run._torch._hook_handles['topology/' + str(id(graph_hook))] = graph_hook
                    ...
                    lumina.termwarn(f'Trying to register forward_hook failed ({e}) - skipping graph tracking.', repeat=False)
