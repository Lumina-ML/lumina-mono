...
openai = util.get_module(name='openai', required='This integration requires `openai`. To install, please run `pip install openai`', lazy=False)
if parse(openai.__version__) < parse('1.12.0'):
    raise lumina.Error(f'This integration requires openai version 1.12.0 and above. Your current version is {openai.__version__} To fix, please `pip install -U openai`')
from openai import OpenAI
from openai.types.fine_tuning import FineTuningJob
from openai.types.fine_tuning.fine_tuning_job import Error, Hyperparameters
np = util.get_module(name='numpy', required='`numpy` not installed >> This integration requires numpy!  To fix, please `pip install numpy`', lazy=False)
pd = util.get_module(name='pandas', required='`pandas` not installed >> This integration requires pandas!  To fix, please `pip install pandas`', lazy=False)

class WandbLogger:
    """Log OpenAI fine-tunes to [Weights & Biases](https://wandb.me/openai-docs)."""
    _wandb_api: lumina.Api | None = None
    _logged_in: bool = False
    openai_client: OpenAI | None = None
    _run: lumina.Run | None = None

    @classmethod
    def sync(cls, fine_tune_job_id: str | None=None, openai_client: OpenAI | None=None, num_fine_tunes: int | None=None, project: str='OpenAI-Fine-Tune', entity: str | None=None, overwrite: bool=False, wait_for_job_success: bool=True, log_datasets: bool=True, model_artifact_name: str='model-metadata', model_artifact_type: str='model', **kwargs_wandb_init: dict[str, Any]) -> str:
        ...
        if fine_tune_job_id:
            lumina.termlog('Retrieving fine-tune job...')
            ...
            if not fine_tunes or fine_tunes.data is None:
                lumina.termwarn('No fine-tune has been retrieved')
                return
            fine_tunes = fine_tunes.data[-num_fine_tunes if num_fine_tunes is not None else None:]
            ...
            if wandb_run:
                ...
                if show_individual_warnings:
                    if wandb_status == 'succeeded' and (not overwrite):
                        lumina.termwarn(f'Fine-tune {fine_tune_id} has already been logged successfully at {wandb_run.url}. Use `overwrite=True` if you want to overwrite previous run')
                    elif wandb_status != 'succeeded' or overwrite:
                        if wandb_status != 'succeeded':
                            lumina.termwarn(f"A run for fine-tune {fine_tune_id} was previously created but didn't end successfully")
                        lumina.termlog(f'A new wandb run will be created for fine-tune {fine_tune_id} and previous run will be overwritten')
                        overwrite = True
                if wandb_status == 'succeeded' and (not overwrite):
                    return
            if lumina.run is None:
                cls._run = lumina.init(job_type='fine-tune', project=project, entity=entity, name=fine_tune_id, id=fine_tune_id, **kwargs_wandb_init)
            else:
                cls._run = lumina.run
            if wait_for_job_success:
                fine_tune = cls._wait_for_job_success(fine_tune)
            cls._log_fine_tune(fine_tune, project, entity, overwrite, show_individual_warnings, log_datasets, model_artifact_name, model_artifact_type, **kwargs_wandb_init)
        if not show_individual_warnings and (not any(fine_tune_logged)):
            lumina.termwarn('No new successful fine-tunes were found')
        return '🎉 wandb sync completed successfully'

    @classmethod
    def _wait_for_job_success(cls, fine_tune: FineTuningJob) -> FineTuningJob:
        lumina.termlog('Waiting for the OpenAI fine-tuning job to finish training...')
        lumina.termlog('To avoid blocking, you can call `WandbLogger.sync` with `wait_for_job_success=False` after OpenAI training completes.')
        while True:
            if fine_tune.status == 'succeeded':
                lumina.termlog('Fine-tuning finished, logging metrics, model metadata, and run metadata to Weights & Biases')
                return fine_tune
            if fine_tune.status == 'failed':
                lumina.termwarn(f'Fine-tune {fine_tune.id} has failed and will not be logged')
                return fine_tune
            if fine_tune.status == 'cancelled':
                lumina.termwarn(f'Fine-tune {fine_tune.id} was cancelled and will not be logged')
                return fine_tune
            time.sleep(10)
            fine_tune = cls.openai_client.fine_tuning.jobs.retrieve(fine_tuning_job_id=fine_tune.id)

    @classmethod
    def _log_fine_tune(cls, fine_tune: FineTuningJob, project: str, entity: str | None, overwrite: bool, show_individual_warnings: bool, log_datasets: bool, model_artifact_name: str, model_artifact_type: str, **kwargs_wandb_init: dict[str, Any]):
        fine_tune_id = fine_tune.id
        ...
        if status != 'succeeded':
            if show_individual_warnings:
                lumina.termwarn(f'Fine-tune {fine_tune_id} has the status "{status}" and will not be logged')
            return
        try:
            ...
            if show_individual_warnings:
                lumina.termwarn(f'Fine-tune {fine_tune_id} has no results and will not be logged')
            return
        ...
        cls._log_artifacts(fine_tune, project, entity, log_datasets, overwrite, model_artifact_name, model_artifact_type)
    ...
    def _ensure_logged_in(cls):
        if not cls._logged_in:
            if lumina.login():
                ...
                raise Exception('It appears you are not currently logged in to Weights & Biases. Please run `wandb login` in your terminal or `wandb.login()` in a notebook. Create a new API key at https://wandb.ai/settings and store it securely.')
    ...
    def _get_config(cls, fine_tune: FineTuningJob) -> dict[str, Any]:
        ...
        if config.get('created_at'):
            config['created_at'] = datetime.datetime.fromtimestamp(config['created_at']).strftime('%Y-%m-%d %H:%M:%S')
        if config.get('finished_at'):
            config['finished_at'] = datetime.datetime.fromtimestamp(config['finished_at']).strftime('%Y-%m-%d %H:%M:%S')
    ...
    @classmethod
    def _log_artifacts(cls, fine_tune: FineTuningJob, project: str, entity: str | None, log_datasets: bool, overwrite: bool, model_artifact_name: str, model_artifact_type: str) -> None:
        if log_datasets:
            lumina.termlog('Logging training/validation files...')
            ...
            for file, prefix, artifact_type in ((training_file, 'train', 'training_files'), (validation_file, 'valid', 'validation_files')):
                if file is not None:
                    cls._log_artifact_inputs(file, prefix, artifact_type, project, entity, overwrite)
        fine_tune_id = fine_tune.id
        artifact = lumina.Artifact(model_artifact_name, type=model_artifact_type, metadata=dict(fine_tune))
    ...
    @classmethod
    def _log_artifact_inputs(cls, file_id: str | None, prefix: str, artifact_type: str, project: str, entity: str | None, overwrite: bool) -> None:
        ...
        if artifact is None or overwrite:
            try:
                ...
                lumina.termerror(f'File {file_id} could not be retrieved. Make sure you have OpenAI permissions to download training/validation files')
                return
            artifact = lumina.Artifact(artifact_name, type=artifact_type)
            ...
            try:
                ...
                lumina.termerror(f"Issue saving {file_id} as a Table to Artifacts, exception:\n  '{e}'")
        else:
    ...
    def _make_table(cls, file_content: str) -> tuple[Table, int]:
        table = lumina.Table(columns=['role: system', 'role: user', 'role: assistant'])
        df = pd.read_json(io.StringIO(file_content), orient='records', lines=True)
        for _idx, message in df.iterrows():
            messages = message.messages
            assert len(messages) == 3
            table.add_data(messages[0]['content'], messages[1]['content'], messages[2]['content'])
        return (table, len(df))
