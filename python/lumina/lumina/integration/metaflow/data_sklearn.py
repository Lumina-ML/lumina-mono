...
def use_estimator(name: str, run: wandb.Run | None, testing: bool=False) -> str | None:
    ...
    if testing:
        return 'models'
    assert run
    lumina.termlog(f'Using artifact: {name} (sklearn BaseEstimator)')
    run.use_artifact(f'{name}:latest')
    return None

def track_estimator(name: str, data: BaseEstimator, run: wandb.Run | None, testing: bool=False) -> str | None:
    ...
    with artifact.new_file(f'{name}.pkl', 'wb') as f:
        pickle.dump(data, f)
    lumina.termlog(f'Logging artifact: {name} (sklearn BaseEstimator)')
