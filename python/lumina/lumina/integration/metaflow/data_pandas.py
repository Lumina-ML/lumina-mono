...
def use_dataframe(name: str, run: wandb.Run | None, testing: bool=False) -> str | None:
    ...
    if testing:
        return 'datasets'
    assert run
    lumina.termlog(f'Using artifact: {name} (Pandas DataFrame)')
    run.use_artifact(f'{name}:latest')
    return None

def track_dataframe(name: str, data: pd.DataFrame, run: wandb.Run | None, testing: bool=False) -> str | None:
    ...
    lumina.termlog(f'Logging artifact: {name} (Pandas DataFrame)')
