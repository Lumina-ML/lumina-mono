...
try:
    ...
    raise Exception('Error: `metaflow` not installed >> This integration requires metaflow! To fix, please `pip install -Uqq metaflow`') from e
try:
...
def _track_scalar(name: str, data: dict | list | set | str | int | float | bool, run, testing: bool=False) -> str | None:
    if testing:
        return 'scalar'
    run.log({name: data})
    return None

def _track_path(name: str, data: Path, run, testing: bool=False) -> str | None:
    if testing:
        return 'Path'
    artifact = lumina.Artifact(name, type='dataset')
    if data.is_dir():
        artifact.add_dir(data)
    elif data.is_file():
        artifact.add_file(data)
    run.log_artifact(artifact)
    lumina.termlog(f'Logging artifact: {name} ({type(data)})')
    return None

def _track_generic(name: str, data, run, testing: bool=False) -> str | None:
    if testing:
        return 'generic'
    artifact = lumina.Artifact(name, type='other')
...
def wandb_track(name: str, data, datasets: bool=False, models: bool=False, others: bool=False, run: wandb.Run | None=None, testing: bool=False) -> str | None:
...
def wandb_use(name: str, data, datasets: bool=False, models: bool=False, others: bool=False, run=None, testing: bool=False) -> str | None:
    ...
    try:
        if data_pandas and data_pandas.is_dataframe(data) and datasets:
            ...
            return None
    except lumina.CommError:
        lumina.termwarn(f'This artifact ({name}, {type(data)}) does not exist in the wandb datastore! If you created an instance inline (e.g. sklearn.ensemble.RandomForestClassifier), then you can safely ignore this. Otherwise you may want to check your internet connection!')
        return None

def _use_path(name: str, data: Path, run, testing: bool=False) -> str | None:
    if testing:
        return 'datasets'
    run.use_artifact(f'{name}:latest')
    lumina.termlog(f'Using artifact: {name} ({type(data)})')
    return None

def _use_generic(name: str, data, run, testing: bool=False) -> str | None:
    if testing:
        return 'others'
    run.use_artifact(f'{name}:latest')
    lumina.termlog(f'Using artifact: {name} ({type(data)})')
...
def wandb_log(func=None, /, datasets: bool=False, models: bool=False, others: bool=False, settings: wandb.Settings | None=None):
    ...
    def decorator(func):
        ...
        def wrapper(self, *args, settings=settings, **kwargs):
            if not isinstance(settings, lumina.sdk.wandb_settings.Settings):
                settings = lumina.Settings()
            settings.update_from_dict({'run_group': coalesce(settings.run_group, f'{current.flow_name}/{current.run_id}'), 'run_job_type': coalesce(settings.run_job_type, current.step_name)})
            with lumina.init(settings=settings) as run:
