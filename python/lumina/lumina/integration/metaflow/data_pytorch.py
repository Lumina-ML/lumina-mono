...
try:
    ...
    warning = '`torch` (PyTorch) not installed >> @wandb_log(models=True) may not auto log your model!'
...
def use_nn_module(name: str, run: wandb.Run | None, testing: bool=False) -> str | None:
    ...
    if testing:
        return 'models'
    assert run
    lumina.termlog(f'Using artifact: {name} (PyTorch nn.Module)')
    run.use_artifact(f'{name}:latest')
    return None

def track_nn_module(name: str, data: nn.Module, run: wandb.Run | None, testing: bool=False) -> str | None:
    ...
    if testing:
        return 'nn.Module'
    assert run
    artifact = lumina.Artifact(name, type='model')
    with artifact.new_file(f'{name}.pkl', 'wb') as f:
        torch.save(data, f)
    lumina.termlog(f'Logging artifact: {name} (PyTorch nn.Module)')
