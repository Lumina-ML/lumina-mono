...
class WandbObserver(RunObserver):
        ...
        if result:
            ...
            for i, r in enumerate(result):
                if isinstance(r, (float, int)):
                    lumina.log({f'result_{i}': float(r)})
                elif isinstance(r, dict):
                    lumina.log(r)
                elif isinstance(r, object):
                    artifact = lumina.Artifact(f'result_{i}.pkl', type='result')
                    artifact.add_file(r)
                    self.run.log_artifact(artifact)
                elif isinstance(r, numpy.ndarray):
                    lumina.log({f'result_{i}': lumina.Image(r)})
                else:
    ...
    def artifact_event(self, name, filename, metadata=None, content_type=None):
        if content_type is None:
            content_type = 'file'
        artifact = lumina.Artifact(name, type=content_type)
    ...
    def log_metrics(self, metrics_by_name, info):
        for metric_name, metric_ptr in metrics_by_name.items():
            for _step, value in zip(metric_ptr['steps'], metric_ptr['values'], strict=False):
                if isinstance(value, numpy.ndarray):
                    lumina.log({metric_name: lumina.Image(value)})
                else:
                    lumina.log({metric_name: value})
