...
class WandbCallback:
    ...
    def after_iteration(self, info: SimpleNamespace) -> bool:
        if info.iteration % self.metric_period == 0:
            ...
            lumina.log({f'iteration@metric-period-{self.metric_period}': info.iteration})
        return True

def _checkpoint_artifact(model: CatBoostClassifier | CatBoostRegressor, aliases: list[str]) -> None:
    """Upload model checkpoint as W&B artifact."""
    if lumina.run is None:
        raise lumina.Error('You must call `wandb.init()` before `_checkpoint_artifact()`')
    model_name = f'model_{lumina.run.id}'
    model_path = Path(lumina.run.dir) / 'model'
    model.save_model(model_path)
    model_artifact = lumina.Artifact(name=model_name, type='model')
    model_artifact.add_file(str(model_path))
    lumina.log_artifact(model_artifact, aliases=aliases)

def _log_feature_importance(model: CatBoostClassifier | CatBoostRegressor) -> None:
    """Log feature importance with default settings."""
    if lumina.run is None:
        raise lumina.Error('You must call `wandb.init()` before `_checkpoint_artifact()`')
    feat_df = model.get_feature_importance(prettified=True)
    fi_data = [[feat, feat_imp] for feat, feat_imp in zip(feat_df['Feature Id'], feat_df['Importances'], strict=False)]
...
def log_summary(model: CatBoostClassifier | CatBoostRegressor, log_all_params: bool=True, save_model_checkpoint: bool=False, log_feature_importance: bool=True) -> None:
    ...
    """
    if lumina.run is None:
        raise lumina.Error('You must call `wandb.init()` before `log_summary()`')
    if not isinstance(model, (CatBoostClassifier, CatBoostRegressor)):
        raise lumina.Error('Model should be an instance of CatBoostClassifier or CatBoostRegressor')
    with wb_telemetry.context() as tel:
        tel.feature.catboost_log_summary = True
    params = model.get_all_params()
    if log_all_params:
        lumina.config.update(params)
    lumina.run.summary['best_iteration'] = model.get_best_iteration()
    lumina.run.summary['best_score'] = model.get_best_score()
