...
dspy = lumina.util.get_module(name='dspy', required='To use the W&B DSPy integration you need to have the `dspy` python package installed.  Install it with `uv pip install dspy`.', lazy=False)
if dspy is not None:
...
class WandbDSPyCallback(dspy.utils.BaseCallback):
    ...
    def __init__(self, log_results: bool=True, run: Run | None=None) -> None:
        ...
        if run is None:
            if lumina.run is None:
                raise lumina.Error('You must call `wandb.init()` before instantiating WandbDSPyCallback().')
            run = lumina.run
        ...
        self._program_table: lumina.Table | None = None
        self._row_idx: int = 0
    ...
    def _extract_program_info(self, program_obj: Any) -> dict[str, Any]:
        ...
        if program_obj is None:
            return info_dict
        try:
            sig = next((param.signature for _, param in program_obj.named_parameters() if isinstance(param, dspy.Predict)))
            ...
            logger.warning('Failed to extract program info from Evaluate instance: %s', e)
        return info_dict
    ...
    def on_evaluate_start(self, call_id: str, instance: Any, inputs: dict[str, Any]) -> None:
        ...
        if not self._did_log_config:
            ...
            serializable = {k: v for k, v in instance_vars.items() if not k.startswith('_')}
    ...
    def on_evaluate_end(self, call_id: str, outputs: Any | None, exception: Exception | None=None) -> None:
        ...
        if exception is None:
            if isinstance(outputs, dspy.evaluate.evaluate.EvaluationResult):
                score = outputs.score
                lumina.log({'score': float(score)}, step=self._row_idx)
                ...
                lumina.termwarn(f'on_evaluate_end received unexpected outputs type: {type(outputs)}. Expected dspy.evaluate.evaluate.EvaluationResult; skipping logging score and `log_results`.')
        else:
            lumina.termwarn(f'on_evaluate_end received exception: {exception}. Skipping logging score and `log_results`.')
        if self._program_table is None:
            columns = ['step', *self._program_info.keys()]
            if isinstance(score, float):
                columns.append('score')
            self._program_table = lumina.Table(columns=columns, log_mode='INCREMENTAL')
        if self._program_table is not None:
            values = list(self._program_info.values())
            if isinstance(score, float):
                values.append(score)
            self._program_table.add_data(self._row_idx, *values)
            self._run.log({'program_signature': self._program_table}, step=self._row_idx)
        self._row_idx += 1
    ...
    def _parse_results(self, results: list[tuple[dspy.Example, dspy.Prediction | dspy.Completions, bool]]) -> list[dict[str, Any]]:
        ...
        for example, prediction, is_correct in results:
            ...
            row: dict[str, Any] = {'example': example.toDict(), 'prediction': prediction_dict, 'is_correct': is_correct}
            _rows.append(row)
        return _rows
    ...
    def _log_predictions_table(self, rows: list[dict[str, Any]]) -> None:
        ...
        preds_table = lumina.Table(columns=columns, data=data, log_mode='IMMUTABLE')
        self._run.log({f'predictions_{self._row_idx}': preds_table}, step=self._row_idx)

    def log_best_model(self, model: dspy.Module, *, save_program: bool=True, save_dir: str | None=None, filetype: Literal['json', 'pkl']='json', aliases: Sequence[str]=('best', 'latest'), artifact_name: str='dspy-program') -> None:
        ...
        metadata = {'dspy_version': getattr(dspy, '__version__', 'unknown'), 'module_class': model.__class__.__name__, **info_dict}
        artifact = lumina.Artifact(name=f'{artifact_name}-{self._run.id}', type='model', metadata=metadata)
        if save_dir is None:
        ...
        try:
            ...
            lumina.termwarn(f"Could not create or access directory '{save_dir}': {exc}. Skipping artifact logging.")
            return
        if save_program:
