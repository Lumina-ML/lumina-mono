...
try:
    ...
    lumina.termwarn('matplotlib required if logging sample image predictions')
...
class WandbCallback(TrackerCallback):
    ...
    def __init__(self, learn: fastai.basic_train.Learner, log: Literal['gradients', 'parameters', 'all'] | None='gradients', save_model: bool=True, monitor: str | None=None, mode: Literal['auto', 'min', 'max']='auto', input_type: Literal['images'] | None=None, validation_data: list | None=None, predictions: int=36, seed: int=12345) -> None:
        ...
        self.model_path = Path(lumina.run.dir) / 'bestmodel.pth'
        self.log = log
        self.input_type = input_type
        self.best = None
        self.validation_data = validation_data
        if input_type and (not self.validation_data):
            wandb_random = random.Random(seed)
    ...
    def on_train_begin(self, **kwargs: Any) -> None:
        ...
        if not WandbCallback._watch_called:
            ...
            lumina.watch(self.learn.model, log=self.log)
    ...
    def on_epoch_end(self, epoch: int, smooth_loss: float, last_metrics: list, **kwargs: Any) -> None:
        ...
        if self.save_model:
            ...
            if current is not None and self.operator(current, self.best):
                lumina.termlog(f'Better model found at epoch {epoch} with {self.monitor} value: {current}.')
                self.best = current
        ...
        if self.validation_data:
            try:
                ...
                lumina.termwarn(e.message)
                self.validation_data = None
            except Exception as e:
                lumina.termwarn(f'Unable to log prediction samples.\n{e}')
                self.validation_data = None
        logs = {name: stat for name, stat in list(zip(self.learn.recorder.names, [epoch, smooth_loss] + last_metrics, strict=False))}
        lumina.log(logs)
    ...
    def on_train_end(self, **kwargs: Any) -> None:
        ...
        if self.save_model and self.model_path.is_file():
            with self.model_path.open('rb') as model_file:
                ...
                lumina.termlog(f'Loaded best saved model from {self.model_path}')
    ...
    def _wandb_log_predictions(self) -> None:
        ...
        for x, y in self.validation_data:
            ...
            if not pred[1].shape or pred[1].dim() == 1:
                pred_log.append(lumina.Image(x.data, caption=f'Ground Truth: {y}\nPrediction: {pred[0]}'))
            elif hasattr(x, 'show'):
                pred_log.append(lumina.Image(x.data, caption='Input data', grouping=3))
                for im, capt in ((pred[0], 'Prediction'), (y, 'Ground Truth')):
                    ...
                    fig.add_axes(ax)
                    x.show(ax=ax, y=im)
                    pred_log.append(lumina.Image(fig, caption=capt))
                    plt.close(fig)
            elif hasattr(y, 'shape') and (len(y.shape) == 2 or (len(y.shape) == 3 and y.shape[0] in [1, 3, 4])):
                pred_log.extend([lumina.Image(x.data, caption='Input data', grouping=3), lumina.Image(pred[0].data, caption='Prediction'), lumina.Image(y.data, caption='Ground Truth')])
            else:
                pred_log.append(lumina.Image(x.data, caption='Input data'))
            lumina.log({'Prediction Samples': pred_log}, commit=False)

class FastaiError(lumina.Error):
    pass
