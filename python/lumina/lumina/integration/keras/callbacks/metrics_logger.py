...
import lumina
from lumina.integration.keras.keras import patch_tf_keras
    ...
    * learning rate (both for a fixed value or a learning rate scheduler)
    ...
    def __init__(self, log_freq: LogStrategy | int='epoch', initial_global_step: int=0, *args: Any, **kwargs: Any) -> None:
        ...
        if lumina.run is None:
            raise lumina.Error('You must call `wandb.init()` before WandbMetricsLogger()')
        ...
        if self.logging_batch_wise:
            lumina.define_metric('batch/batch_step')
            lumina.define_metric('batch/*', step_metric='batch/batch_step')
        else:
            lumina.define_metric('epoch/epoch')
            lumina.define_metric('epoch/*', step_metric='epoch/epoch')

    def _get_lr(self) -> float | None:
        if isinstance(self.model.optimizer.learning_rate, (tf.Variable, tf.Tensor)) or (hasattr(self.model.optimizer.learning_rate, 'shape') and self.model.optimizer.learning_rate.shape == ()):
        ...
        try:
            ...
            lumina.termerror(f'Unable to log learning rate: {e}', repeat=False)
            return None
