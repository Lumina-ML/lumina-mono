...
class WandbModelCheckpoint(callbacks.ModelCheckpoint):
    ...
    def __init__(self, filepath: StrPath, monitor: str='val_loss', verbose: int=0, save_best_only: bool=False, save_weights_only: bool=False, mode: Mode='auto', save_freq: SaveStrategy | int='epoch', initial_value_threshold: float | None=None, **kwargs: Any) -> None:
        super().__init__(filepath=filepath, monitor=monitor, verbose=verbose, save_best_only=save_best_only, save_weights_only=save_weights_only, mode=mode, save_freq=save_freq, initial_value_threshold=initial_value_threshold, **kwargs)
        if lumina.run is None:
            raise lumina.Error('You must call `wandb.init()` before `WandbModelCheckpoint()`')
    ...
    def on_train_batch_end(self, batch: int, logs: dict[str, float] | None=None) -> None:
        if self._should_save_on_batch(batch):
            if self.is_old_tf_keras_version:
                ...
                filepath = self._get_file_path(epoch=self._current_epoch, batch=batch, logs=logs)
    ...
    def _log_ckpt_as_artifact(self, filepath: str, aliases: list[str] | None=None) -> None:
        """Log model checkpoint as  W&B Artifact."""
        try:
            assert lumina.run is not None
            model_checkpoint_artifact = lumina.Artifact(f'run_{lumina.run.id}_model', type='model')
            ...
            lumina.log_artifact(model_checkpoint_artifact, aliases=aliases or [])
        except ValueError:
            pass
    ...
    def _check_filepath(self) -> None:
        ...
        if len(placeholders) == 0:
            lumina.termwarn('When using `save_best_only`, ensure that the `filepath` argument contains formatting placeholders like `{epoch:02d}` or `{batch:02d}`. This ensures correct interpretation of the logged artifacts.', repeat=False)
