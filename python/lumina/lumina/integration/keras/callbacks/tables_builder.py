...
class WandbEvalCallback(Callback, abc.ABC):
    ...
    def __init__(self, data_table_columns: list[str], pred_table_columns: list[str], *args: Any, **kwargs: Any) -> None:
        ...
        if lumina.run is None:
            raise lumina.Error('You must call `wandb.init()` first before using this callback.')
    ...
    def init_data_table(self, column_names: list[str]) -> None:
        ...
        """
        self.data_table = lumina.Table(columns=column_names, allow_mixed_types=True)
    ...
    def init_pred_table(self, column_names: list[str]) -> None:
        ...
        Args:
            column_names: (list) Column names for W&B Tables.
        """
        self.pred_table = lumina.Table(columns=column_names)
    ...
    def log_data_table(self, name: str='val', type: str='dataset', table_name: str='val_data') -> None:
        ...
        Args:
            ...
            table_name: (str) The name of the table as will be displayed in the UI.
                (default is 'val_data').
        """
        data_artifact = lumina.Artifact(name, type=type)
        data_artifact.add(self.data_table, table_name)
        assert lumina.run is not None
        lumina.run.use_artifact(data_artifact)
        data_artifact.wait()
    ...
    def log_pred_table(self, type: str='evaluation', table_name: str='eval_data', aliases: list[str] | None=None) -> None:
        ...
        """
        assert lumina.run is not None
        pred_artifact = lumina.Artifact(f'run_{lumina.run.id}_pred', type=type)
        pred_artifact.add(self.pred_table, table_name)
        lumina.run.log_artifact(pred_artifact, aliases=aliases or ['latest'])
