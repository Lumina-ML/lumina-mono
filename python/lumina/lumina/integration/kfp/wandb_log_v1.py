def wandb_log(func=None, log_component_file=True):
    ...
    from kfp.components import InputArtifact, InputBinaryFile, InputPath, InputTextFile, OutputArtifact, OutputBinaryFile, OutputPath, OutputTextFile
    import lumina
    ...
    def log_input_scalar(name, data, run=None):
        ...
        lumina.termlog(f'Setting config: {name} to {data}')

    def log_input_artifact(name, data, type, run=None):
        artifact = lumina.Artifact(name, type=type)
        artifact.add_file(data)
        run.use_artifact(artifact)
        lumina.termlog(f'Using artifact: {name}')
    ...
    def log_output_artifact(name, data, type, run=None):
        ...
        lumina.termlog(f'Logging artifact: {name}')
    ...
    def _log_component_file(func, run=None):
        ...
        artifact = lumina.Artifact(name, type='kubeflow_component_file')
        artifact.add_file(output_component_file)
        run.log_artifact(artifact)
        lumina.termlog(f'Logging component file: {output_component_file}')
    sig = signature(func)
    no_default = []
    has_default = []
    ...
    new_params = tuple((*no_default, Parameter('mlpipeline_ui_metadata_path', annotation=OutputPath(), kind=Parameter.POSITIONAL_OR_KEYWORD), *has_default))
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            with lumina.init(job_type=func.__name__, group='{{workflow.annotations.pipelines.kubeflow.org/run_name}}') as run:
                warn_and_record_deprecation(feature=Deprecated(kfp_v1_wandb_log=True), message='KFP v1 (kfp<2.0.0) support for @wandb_log is deprecated and will be removed in a future release. Please upgrade to kfp>=2.0.0.', run=run)
                ...
                metadata = {'outputs': [{'type': 'markdown', 'storage': 'inline', 'source': iframe_html}]}
