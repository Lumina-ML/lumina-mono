...
if _KFP_V2:
    try:
        ...
        lumina.termerror('kfp>=2.0.0 detected but failed to import kfp internals. Please ensure kfp is installed correctly.')
    else:
        from . import wandb_log_v2 as _log_module
        _import_preamble = 'import os\nimport typing\nfrom typing import Any, NamedTuple\n\nimport wandb'
else:
    try:
        from kfp import __version__ as kfp_version
        ...
        if parse(kfp_version) < parse(MIN_KFP_VERSION):
            lumina.termwarn(f'Your version of kfp {kfp_version} may not work. This integration requires kfp>={MIN_KFP_VERSION}')
    except ImportError:
        lumina.termerror('kfp not found! Please `pip install kfp`')
    from . import wandb_log_v1 as _log_module
    _import_preamble = 'import typing\nfrom typing import NamedTuple\n\nimport collections\nfrom collections import namedtuple\n\nimport kfp\nfrom kfp import components\nfrom kfp.components import InputPath, OutputPath\n\nimport wandb'
if _log_module:
...
def _patch_kfp_v1() -> None:
    ...
    to_patch = [('kfp.components', _v1_create_component_from_func), ('kfp.components._python_op', _v1_create_component_from_func), ('kfp.components._python_op', _v1_get_function_source_definition), ('kfp.components._python_op', _v1_strip_type_hints)]
    successes = []
    for module_name, func in to_patch:
        success = patch(module_name, func)
        successes.append(success)
    if not all(successes):
        lumina.termerror('Failed to patch one or more kfp functions. Patching @wandb_log decorator to no-op.')
        patch('wandb.integration.kfp', _v1_wandb_log_noop)

def _v1_wandb_log_noop(func: Callable | None=None, log_component_file: bool=True) -> Callable:
...
def _v1_get_function_source_definition(func: Callable) -> str:
    ...
    func_code_lines = itertools.dropwhile(lambda x: not x.startswith(('def', '@wandb_log')), func_code_lines)
...
def _v1_create_component_from_func(func: Callable, output_component_file: str | None=None, base_image: str | None=None, packages_to_install: list[str] | None=None, annotations: Mapping[str, str] | None=None) -> Callable:
    ...
    if not packages_to_install:
        ...
        packages_to_install += core_packages
    component_spec = _func_to_component_spec(func=func, extra_code=_wandb_logging_extras, base_image=base_image, packages_to_install=packages_to_install)
    if annotations:
...
def _patch_kfp_v2() -> None:
    ...
    _kfp_v2_patch._orig_get_cmd = _component_factory._get_command_and_args_for_lightweight_component
    _kfp_v2_patch._wandb_logging_extras = _wandb_logging_extras
    to_patch = [('kfp.dsl.component_factory', _kfp_v2_patch.get_function_source_definition), ('kfp.dsl.component_factory', _kfp_v2_patch.create_component_from_func), ('kfp.dsl.component_factory', _kfp_v2_patch.get_command_and_args_for_lightweight_component)]
    successes = []
    for module_name, func in to_patch:
        success = patch(module_name, func)
        successes.append(success)
    if not all(successes):
        lumina.termerror('Failed to patch one or more kfp v2 functions. @wandb_log may not work correctly with @dsl.component.')
