...
import kfp.dsl
from kfp.dsl.types.type_annotations import InputPath, OutputPath, is_artifact_wrapped_in_Input, is_artifact_wrapped_in_Output
import lumina
...
def _log_artifact(run: wandb.Run, name: str, value: Any, *, use: bool=False) -> bool:
    ...
    if path is None:
        return False
    artifact = lumina.Artifact(name, type='kfp_artifact')
    ...
    def log_inputs(self, run: wandb.Run, bound_args: dict[str, Any]) -> None:
        ...
        for name in self._scalars_in:
            if name in bound_args:
                ...
                lumina.termlog(f'Setting config: {name} to {value}')
        for name in self._artifacts_in:
            if name in bound_args:
                try:
                    _log_artifact(run, name, bound_args[name], use=True)
                except Exception as e:
                    lumina.termwarn(f"Failed to log input artifact '{name}': {e}")

    def log_outputs(self, run: wandb.Run, func_name: str, result: Any, bound_args: dict[str, Any]) -> None:
        ...
        if result is not None and (not run._is_finished):
            if _is_namedtuple(result):
                run.log({f'{func_name}.{k}': v for k, v in zip(result._fields, result, strict=True)})
            else:
        ...
        for name in self._artifacts_out:
            if name in bound_args:
                try:
                    ...
                    lumina.termwarn(f"Failed to log output artifact '{name}': {e}")

def wandb_log(func: Callable | None=None) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            wandb_group = os.getenv('WANDB_RUN_GROUP') or os.getenv('KFP_RUN_NAME') or os.getenv('ARGO_WORKFLOW_NAME')
            with lumina.init(job_type=func.__name__, group=wandb_group) as run:
