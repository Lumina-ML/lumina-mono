...
def full_path_exists(full_func: str) -> bool:
    ...
    for i in range(1, len(components)):
        ...
        child = components[i]
        module = lumina.util.get_module(parent)
...
def patch(module_name: str, func: Callable) -> bool:
    ...
    """
    module = lumina.util.get_module(module_name)
    success = False
    full_func = f'{module_name}.{func.__name__}'
    if not full_path_exists(full_func):
        lumina.termerror(f'Failed to patch {module_name}.{func.__name__}! Please check if this package/module is installed!')
    else:
        lumina.patched.setdefault(module.__name__, [])
        if [module, func.__name__] not in lumina.patched[module.__name__]:
            setattr(module, f'orig_{func.__name__}', getattr(module, func.__name__))
            setattr(module, func.__name__, func)
            lumina.patched[module.__name__].append([module, func.__name__])
        success = True
    return success
...
def unpatch(module_name: str) -> None:
    ...
    """
    if module_name in lumina.patched:
        for module, func in lumina.patched[module_name]:
            setattr(module, func, getattr(module, f'orig_{func}'))
        lumina.patched[module_name] = []
