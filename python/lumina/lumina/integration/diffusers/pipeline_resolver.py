...
from .resolvers import SUPPORTED_MULTIMODAL_PIPELINES, DiffusersMultiModalPipelineResolver
...
class DiffusersPipelineResolver:
    ...
    def __call__(self, args: Sequence[Any], kwargs: dict[str, Any], response: Response, start_time: float, time_elapsed: float) -> Any:
        ...
        if pipeline_name in SUPPORTED_MULTIMODAL_PIPELINES:
            resolver = DiffusersMultiModalPipelineResolver(pipeline_name, self.pipeline_call_count)
            self.pipeline_call_count += 1
