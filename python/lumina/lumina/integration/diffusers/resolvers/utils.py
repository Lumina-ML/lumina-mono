...
def get_updated_kwargs(pipeline: Any, args: Sequence[Any], kwargs: dict[str, Any]) -> dict[str, Any]:
    ...
    if 'ip_adapter_image' in kwargs and kwargs['ip_adapter_image'] is not None:
        lumina.log({'IP-Adapter-Image': lumina.Image(kwargs['ip_adapter_image'])})
    return kwargs
...
def postprocess_np_arrays_for_video(images: list[np_array], normalize: bool | None=False) -> np_array:
    np = get_module('numpy', required='Please ensure NumPy is installed. You can run `pip install numpy` to install it.')
