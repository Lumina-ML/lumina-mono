...
import lumina
from lumina.env import strtobool
...
_AVAILABLE_WEAVE_INTEGRATIONS = ['anthropic', 'autogen', 'cohere', 'crewai', 'dspy', 'google.genai', 'groq', 'huggingface_hub.inference', 'instructor', 'langchain', 'litellm', 'llama_index', 'mcp', 'mistral', 'notdiamond', 'openai', 'agents', 'smolagents', 'verdict', 'verifiers', 'vertexai']
...
def init_weave_if_imported(entity: str | None, project: str | None) -> None:
    ...
    if _WEAVE_PACKAGE_NAME not in sys.modules:
        ...
        return
    lumina.termlog('Initializing weave.')
    try:
        _weave_init(project_path)
    except Exception as e:
        lumina.termwarn(f'Failed to automatically initialize weave: {e}')
...
def _maybe_suggest_weave_installation() -> None:
    ...
    if weave_spec is None:
        msg = 'Use W&B Weave for improved LLM call tracing. Install Weave with `pip install weave` then add `import weave` to the top of your script.'
    else:
        msg = 'Use W&B Weave for improved LLM call tracing. Weave is installed but not imported. Add `import weave` to the top of your script.'
    lumina.termlog(f"Detected [{', '.join(imported_libs)}] in use.", repeat=False)
    lumina.termlog(msg, repeat=False)
    lumina.termlog('For more information, check out the docs at: https://weave-docs.wandb.ai', repeat=False)

def ensure_version(min_version: str, error_msg: str | None=None) -> None:
...
def init_weave(entity: str | None, project: str | None) -> None:
    ...
    if has_get_client:
        ...
        if client is not None:
            ...
            if client_project_path != project_path:
                raise UsageError(f'Weave is already initialized for {client_project_path!r}; cannot initialize it for {project_path!r}.')
