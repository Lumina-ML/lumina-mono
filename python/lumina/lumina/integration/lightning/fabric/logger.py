...
try:
    ...
    if version.parse(lightning.__version__) > version.parse('2.1.3'):
        lumina.termwarn('This integration is tested and supported for lightning Fabric 2.1.3.\n            Please report any issues to https://github.com/wandb/wandb/issues with the tag `lightning-fabric`.', repeat=False)
    if TYPE_CHECKING:
...
class WandbLogger(Logger):
    """Log using `Weights and Biases <https://docs.wandb.ai/models/integrations/lightning>`_.
    ...
    **Downloading and Using Artifacts**
    ...
    def __init__(self, name: str | None=None, save_dir: _PATH='.', version: str | None=None, offline: bool=False, dir: _PATH | None=None, id: str | None=None, anonymous: bool | None=None, project: str | None=None, log_model: Literal['all'] | bool=False, experiment: lumina.Run | None=None, prefix: str='', checkpoint_name: str | None=None, log_checkpoint_on: Literal['success'] | Literal['all']='success', **kwargs: Any) -> None:
        ...
        self._wandb_init: dict[str, Any] = {'name': name, 'project': project, 'dir': save_dir or dir, 'id': version or id, 'resume': 'allow', 'anonymous': 'allow' if anonymous else None}
    ...
    def experiment(self) -> lumina.Run:
        """Actual wandb object.
        ...
        if self._experiment is None:
            ...
            if lumina.run is not None:
                rank_zero_warn('There is a wandb run already in progress and newly created instances of `WandbLogger` will reuse this run. If this is not desired, call `wandb.finish()` before instantiating `WandbLogger`.')
                self._experiment = lumina.run
            elif attach_id is not None and hasattr(lumina, '_attach'):
                self._experiment = lumina._attach(attach_id)
            else:
                self._experiment = lumina.init(**self._wandb_init)
                if isinstance(self._experiment, lumina.Run) and getattr(self._experiment, 'define_metric', None):
                    self._experiment.define_metric('trainer/global_step')
                    self._experiment.define_metric('*', step_metric='trainer/global_step', step_sync=True)
        self._experiment._label(repo='lightning_fabric_logger')
    ...
    @rank_zero_only
    def log_table(self, key: str, columns: list[str] | None=None, data: list[list[Any]] | None=None, dataframe: Any=None, step: int | None=None) -> None:
        """Log a Table containing any object type (text, image, audio, video, molecule, html, etc).

        Can be defined either with `columns` and `data` or with `dataframe`.

        """
        metrics = {key: lumina.Table(columns=columns, data=data, dataframe=dataframe)}
        self.log_metrics(metrics, step)

    @rank_zero_only
    def log_text(self, key: str, columns: list[str] | None=None, data: list[list[str]] | None=None, dataframe: Any=None, step: int | None=None) -> None:
        """Log text as a Table.
    ...
    @rank_zero_only
    def log_html(self, key: str, htmls: list[Any], step: int | None=None, **kwargs: Any) -> None:
        """Log html files.
        ...
        metrics = {key: [lumina.Html(html, **kwarg) for html, kwarg in zip(htmls, kwarg_list, strict=False)]}
        self.log_metrics(metrics, step)
    ...
    def log_image(self, key: str, images: list[Any], step: int | None=None, **kwargs: Any) -> None:
        ...
        for k, v in kwargs.items():
            if len(v) != n:
                raise ValueError(f'Expected {n} items but only found {len(v)} for {k}')
        kwarg_list = [{k: kwargs[k][i] for k in kwargs} for i in range(n)]
        metrics = {key: [lumina.Image(img, **kwarg) for img, kwarg in zip(images, kwarg_list, strict=False)]}
        self.log_metrics(metrics, step)
    ...
    def log_audio(self, key: str, audios: list[Any], step: int | None=None, **kwargs: Any) -> None:
        ...
        for k, v in kwargs.items():
            if len(v) != n:
                raise ValueError(f'Expected {n} items but only found {len(v)} for {k}')
        kwarg_list = [{k: kwargs[k][i] for k in kwargs} for i in range(n)]
        metrics = {key: [lumina.Audio(audio, **kwarg) for audio, kwarg in zip(audios, kwarg_list, strict=False)]}
        self.log_metrics(metrics, step)
    ...
    def log_video(self, key: str, videos: list[Any], step: int | None=None, **kwargs: Any) -> None:
        ...
        for k, v in kwargs.items():
            if len(v) != n:
                raise ValueError(f'Expected {n} items but only found {len(v)} for {k}')
        kwarg_list = [{k: kwargs[k][i] for k in kwargs} for i in range(n)]
        metrics = {key: [lumina.Video(video, **kwarg) for video, kwarg in zip(videos, kwarg_list, strict=False)]}
        self.log_metrics(metrics, step)
    ...
    @staticmethod
    @rank_zero_only
    def download_artifact(artifact: str, save_dir: _PATH | None=None, artifact_type: str | None=None, use_artifact: bool | None=True) -> str:
        ...
        """
        if lumina.run is not None and use_artifact:
            artifact = lumina.run.use_artifact(artifact)
        else:
            api = lumina.Api()
    ...
    def _scan_and_log_pytorch_checkpoints(self, checkpoint_callback: ModelCheckpoint) -> None:
        ...
        for t, p, s, _ in checkpoints:
            ...
            artifact = lumina.Artifact(name=self._checkpoint_name, type='model', metadata=metadata)
