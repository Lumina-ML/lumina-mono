...
class WandbCallback(BaseCallback):
    ...
    def __init__(self, verbose: int=0, model_save_path: str | None=None, model_save_freq: int=0, gradient_save_freq: int=0, log: Literal['gradients', 'parameters', 'all'] | None='all') -> None:
        ...
        if log not in ['gradients', 'parameters', 'all', None]:
            lumina.termwarn("`log` must be one of `None`, 'gradients', 'parameters', or 'all', falling back to 'all'")
            log = 'all'
        self.log = log
    ...
    def _init_callback(self) -> None:
        ...
        if self.gradient_save_freq > 0:
            lumina.watch(self.model.policy, log_freq=self.gradient_save_freq, log=self.log)
        lumina.config.setdefaults(d)

    def _on_step(self) -> bool:
        if self.model_save_freq > 0 and self.model_save_path is not None and (self.n_calls % self.model_save_freq == 0):
            self.save_model()
        return True
    ...
    def save_model(self) -> None:
        ...
        lumina.save(self.path, base_path=self.model_save_path)
        if self.verbose > 1:
