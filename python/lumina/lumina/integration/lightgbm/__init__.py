...
MINIMIZE_METRICS = ['l1', 'l2', 'rmse', 'mape', 'huber', 'fair', 'poisson', 'gamma', 'binary_logloss']
MAXIMIZE_METRICS = ['map', 'auc', 'average_precision']
if TYPE_CHECKING:
    from typing import Any, NamedTuple
    _EvalResultTuple = tuple[str, str, float, bool] | tuple[str, str, float, bool, float]
...
def _define_metric(data: str, metric_name: str) -> None:
    ...
    if 'loss' in str.lower(metric_name):
        lumina.define_metric(f'{data}_{metric_name}', summary='min')
    elif str.lower(metric_name) in MINIMIZE_METRICS:
        lumina.define_metric(f'{data}_{metric_name}', summary='min')
    elif str.lower(metric_name) in MAXIMIZE_METRICS:
        lumina.define_metric(f'{data}_{metric_name}', summary='max')

def _checkpoint_artifact(model: 'Booster', iteration: int, aliases: 'list[str]') -> None:
    """Upload model checkpoint as W&B artifact."""
    model_name = f'model_{lumina.run.id}'
    model_path = Path(lumina.run.dir) / f'model_ckpt_{iteration}.txt'
    model.save_model(model_path, num_iteration=iteration)
    model_artifact = lumina.Artifact(name=model_name, type='model')
    model_artifact.add_file(str(model_path))
    lumina.log_artifact(model_artifact, aliases=aliases)

def _log_feature_importance(model: 'Booster') -> None:
    """Log feature importance."""
    feat_imps = model.feature_importance()
    feats = model.feature_name()
    fi_data = [[feat, feat_imp] for feat, feat_imp in zip(feats, feat_imps, strict=True)]
    table = lumina.Table(data=fi_data, columns=['Feature', 'Importance'])
    lumina.log({'Feature Importance': lumina.plot.bar(table, 'Feature', 'Importance', title='Feature Importance')}, commit=False)
...
class _WandbCallback:
    ...
    def _init(self, env: 'CallbackEnv') -> None:
        ...
        if self.log_params:
            lumina.config.update(env.params)
    ...
    def __call__(self, env: 'CallbackEnv') -> None:
        ...
        for item in env.evaluation_result_list:
            if len(item) == 4:
                ...
                lumina.log({data_name + '_' + eval_name: result}, commit=False)
            else:
                data_name, eval_name = item[1].split()
                res_mean = item[2]
                res_stdv = item[4]
                lumina.log({data_name + '_' + eval_name + '-mean': res_mean, data_name + '_' + eval_name + '-stdv': res_stdv}, commit=False)
        lumina.log({'iteration': env.iteration}, commit=True)
...
def log_summary(model: Booster, feature_importance: bool=True, save_model_checkpoint: bool=False) -> None:
    ...
    if not isinstance(model, Booster):
        raise lumina.Error('Model should be an instance of lightgbm.basic.Booster')
    lumina.run.summary['best_iteration'] = model.best_iteration
    lumina.run.summary['best_score'] = model.best_score
