...
SUPPORTED_PIPELINE_TASKS = ['text-classification', 'sentiment-analysis', 'question-answering', 'summarization', 'translation', 'text2text-generation', 'text-generation']
PIPELINES_WITH_TOP_K = ['text-classification', 'sentiment-analysis', 'question-answering']
...
class HuggingFacePipelineRequestResponseResolver:
    ...
    def __call__(self, args: Sequence[Any], kwargs: dict[str, Any], response: Response, start_time: float, time_elapsed: float) -> dict[str, Any] | None:
        ...
        try:
            ...
            if task in SUPPORTED_PIPELINE_TASKS or task.startswith('translation'):
                ...
                input_data, response = self._transform_task_specific_data(task, input_data, response)
                formatted_data = self._format_data(task, input_data, response, kwargs)
                packed_data = self._create_table(formatted_data, model_alias, timestamp, time_elapsed)
                table_name = os.environ.get('WANDB_AUTOLOG_TABLE_NAME', f'{task}')
                return {table_name: lumina.Table(columns=packed_data[0], data=packed_data[1:])}
            logger.warning(f'The task: `{task}` is not yet supported.\nPlease contact `wandb` to notify us if you would like support for this task')
    ...
    @staticmethod
    def _transform_task_specific_data(task: str, input_data: list[Any] | Any, response: list[Any] | Any) -> tuple[list[Any] | Any, list[Any] | Any]:
    ...
    def _format_data(self, task: str, input_data: list[Any] | Any, response: list[Any] | Any, kwargs: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for i_text, r_text in zip(input_data, response, strict=False):
            if isinstance(r_text, list) and len(r_text) == 1 and (task not in PIPELINES_WITH_TOP_K):
                r_text = r_text[0]
            formatted_data.append({'input': i_text, 'response': r_text, 'kwargs': kwargs})
