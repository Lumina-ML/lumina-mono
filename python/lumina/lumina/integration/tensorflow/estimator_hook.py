...
class WandbHook(SessionRunHook):
    ...
    def begin(self):
        if lumina.run is None:
            raise lumina.Error('You must call `wandb.init()` before calling `WandbHook`')
    ...
    def after_run(self, run_context, run_values):
        ...
        if step % self._steps_per_log == 0:
            lumina.tensorboard._log(run_values.results['summary'], history=self._history, step=step)
