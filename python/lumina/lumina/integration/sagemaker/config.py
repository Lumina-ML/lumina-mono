...
def is_using_sagemaker() -> bool:
    ...
    return os.path.exists(sm_files.SM_PARAM_CONFIG) or 'SM_TRAINING_ENV' in os.environ
...
def parse_sm_config() -> dict[str, Any]:
    ...
    if (env := os.environ.get('SM_TRAINING_ENV')):
        try:
            ...
            warnings.warn('Failed to parse SM_TRAINING_ENV not valid JSON string', stacklevel=2)
    return conf
