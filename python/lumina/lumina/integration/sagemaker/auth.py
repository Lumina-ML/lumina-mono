...
def sagemaker_auth(overrides: dict[str, Any] | None=None, path: str='.', api_key: str | None=None) -> None:
    ...
    api_key = overrides.get(env.API_KEY, None) or api_key or wandb_setup.singleton().settings.api_key or wbauth.read_netrc_auth(host=wandb_setup.singleton().settings.base_url)
    if api_key is None:
        raise ValueError("Can't find W&B API key, set the WANDB_API_KEY env variable" + ' or run `wandb login`')
