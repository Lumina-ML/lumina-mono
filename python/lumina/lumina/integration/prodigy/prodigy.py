...
from lumina.plot.utils import test_missing
...
def standardize(item, structure, array_dict_types):
    ...
    for k, v in structure.items():
        if k not in item:
            ...
            condition = not (len(item[k]) > 0 and isinstance(item[k][0], list)) and (not (len(item[k]) > 0 and (not isinstance(item[k][0], (list, dict)))))
            if condition:
...
def create_table(data):
    ...
    main_table = lumina.Table(columns=columns)
            ...
            if 'image' in document and document['image'] is not None:
                ...
                if isurl:
                    try:
                        ...
                        document['image_visual'] = lumina.Image(im)
                    except urllib.error.URLError:
                        lumina.termwarn(f"Image URL {document['image']} is invalid.")
                        document['image_visual'] = None
                elif isbase64:
                    ...
                    try:
                        ...
                        lumina.termwarn(f"Base64 string {document['image']} is invalid.")
                        document['image_visual'] = None
                else:
                    document['image_visual'] = lumina.Image(document['image'])
...
def upload_dataset(dataset_name):
    ...
    with wb_telemetry.context(run=lumina.run) as tel:
    ...
    lumina.log({dataset_name: table})
    lumina.termlog(f'Prodigy dataset `{dataset_name}` uploaded.')
