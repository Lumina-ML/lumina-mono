...
_required_error_msg = "Couldn't import the gymnasium python package, install with `pip install gymnasium`"
...
def monitor():
    ...
    if lumina.util.get_module('gymnasium') is not None:
        gym_lib = 'gymnasium'
    elif lumina.util.get_module('gym') is not None:
        gym_lib = 'gym'
    if gym_lib is None:
        raise lumina.Error(_required_error_msg)
    ...
    if gym_lib == 'gymnasium' and (not _gymnasium_version_lt_1_0_0):
        ...
        wrappers = lumina.util.get_module(f'{gym_lib}.wrappers', required=_required_error_msg)
        recorder = getattr(wrappers, vcr_recorder_attribute)
    else:
        vcr = lumina.util.get_module(f'{gym_lib}.wrappers.monitoring.video_recorder', required=_required_error_msg)
    ...
    def close(self):
        ...
        if not self.enabled:
            return
        if lumina.run:
            m = re.match('.+(video\\.\\d+).+', getattr(self, path))
            key = m.group(1) if m else 'videos'
            lumina.log({key: lumina.Video(getattr(self, path))})
    ...
    lumina.patched['gym'].append([f'{gym_lib}.wrappers.{wrapper_name}', 'close'])
