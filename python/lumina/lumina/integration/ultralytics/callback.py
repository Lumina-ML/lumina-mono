...
try:
    ...
    if version.parse(ultralytics.__version__) > version.parse('8.0.238'):
        lumina.termwarn('This integration is tested and supported for ultralytics v8.0.238 and below.\n            Please report any issues to https://github.com/wandb/wandb/issues with the tag `yolov8`.', repeat=False)
    ...
    from lumina.integration.ultralytics.bbox_utils import plot_bbox_predictions, plot_detection_validation_results
    from lumina.integration.ultralytics.classification_utils import plot_classification_predictions, plot_classification_validation_results
    from lumina.integration.ultralytics.mask_utils import plot_mask_predictions, plot_sam_predictions, plot_segmentation_validation_results
    from lumina.integration.ultralytics.pose_utils import plot_pose_predictions, plot_pose_validation_results
...
PREDICTOR_TYPE = ClassificationPredictor | DetectionPredictor | SegmentationPredictor | PosePredictor | SAMPredictor
...
class WandBUltralyticsCallback:
    ...
    def __init__(self, model: YOLO, epoch_logging_interval: int=1, max_validation_batches: int=1, enable_model_checkpointing: bool=False, visualize_skeleton: bool=False) -> None:
        ...
        self.model_name = str(model.overrides['model']).split('.')[0] if 'model' in model.overrides else None
        self._make_tables()
    ...
    def _make_tables(self):
        if self.task in ['detect', 'segment']:
            ...
            self.train_validation_table = lumina.Table(columns=['Model-Name'] + train_columns)
            self.validation_table = lumina.Table(columns=['Model-Name'] + validation_columns)
            self.prediction_table = lumina.Table(columns=['Model-Name', 'Image', 'Num-Objects', 'Mean-Confidence', 'Speed'])
            ...
            self.train_validation_table = lumina.Table(columns=['Model-Name', 'Epoch'] + validation_columns)
            self.validation_table = lumina.Table(columns=['Model-Name'] + validation_columns)
            self.prediction_table = lumina.Table(columns=['Model-Name'] + classification_columns)
        elif self.task == 'pose':
            validation_columns = ['Data-Index', 'Batch-Index', 'Image-Ground-Truth', 'Image-Prediction', 'Num-Instances', 'Mean-Confidence', 'Speed']
            train_columns = ['Epoch'] + validation_columns
            self.train_validation_table = lumina.Table(columns=['Model-Name'] + train_columns)
            self.validation_table = lumina.Table(columns=['Model-Name'] + validation_columns)
            self.prediction_table = lumina.Table(columns=['Model-Name', 'Image-Prediction', 'Num-Instances', 'Mean-Confidence', 'Speed'])
    ...
    def _save_model(self, trainer: TRAINER_TYPE):
        model_checkpoint_artifact = lumina.Artifact(f'run_{lumina.run.id}_model', 'model')
        ...
        lumina.log_artifact(model_checkpoint_artifact, aliases=[f'epoch_{trainer.epoch}'])

    def on_train_start(self, trainer: TRAINER_TYPE):
        with telemetry.context(run=lumina.run) as tel:
            tel.feature.ultralytics_yolov8 = True
        lumina.config.train = vars(trainer.args)
        self.run_id = lumina.run.id
    ...
    def on_train_end(self, trainer: TRAINER_TYPE):
        if self.task in self.supported_tasks:
            lumina.log({'Train-Table': self.train_validation_table}, commit=False)

    def on_val_start(self, validator: VALIDATOR_TYPE):
        lumina.run or lumina.init(project=validator.args.project or 'YOLOv8', job_type='validation_' + validator.args.task)
    ...
    def on_val_end(self, trainer: VALIDATOR_TYPE):
        if self.task in self.supported_tasks:
            ...
            lumina.log({'Validation-Table': self.validation_table}, commit=False)

    def on_predict_start(self, predictor: PREDICTOR_TYPE):
        lumina.run or lumina.init(project=predictor.args.project or 'YOLOv8', config=vars(predictor.args), job_type='prediction_' + predictor.args.task)
        if isinstance(predictor, SAMPredictor):
            self.prompts = copy.deepcopy(predictor.prompts)
            self.prediction_table = lumina.Table(columns=['Image'])

    def on_predict_end(self, predictor: PREDICTOR_TYPE):
        lumina.config.prediction_configs = vars(predictor.args)
        if self.task in self.supported_tasks:
            ...
            lumina.log({'Prediction-Table': self.prediction_table}, commit=False)
...
def add_wandb_callback(model: YOLO, epoch_logging_interval: int=1, enable_model_checkpointing: bool=False, enable_train_validation_logging: bool=True, enable_validation_logging: bool=True, enable_prediction_logging: bool=True, max_validation_batches: int | None=1, visualize_skeleton: bool | None=True):
    ...
    if RANK in [-1, 0]:
        ...
        lumina.termerror('The RANK of the process to add the callbacks was neither 0 or -1. No Weights & Biases callbacks were added to this instance of the YOLO model.')
    return model
