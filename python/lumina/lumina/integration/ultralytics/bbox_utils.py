...
def get_ground_truth_bbox_annotations(img_idx: int, image_path: str, batch: dict, class_name_map: dict | None=None) -> list[dict[str, Any]]:
    ...
    if len(bboxes) == 0:
        lumina.termwarn(f'Image: {image_path} has no bounding boxes labels', repeat=False)
        return None
    if len(batch['cls'][indices]):
        cls_labels = batch['cls'][indices].squeeze(1).tolist()
    else:
...
def plot_bbox_predictions(result: Results, model_name: str, table: wandb.Table | None=None) -> lumina.Table | tuple[lumina.Image, dict, dict]:
    ...
    image = lumina.Image(result.orig_img[:, :, ::-1], boxes=boxes)
...
def plot_detection_validation_results(dataloader: Any, class_label_map: dict, model_name: str, predictor: DetectionPredictor, table: wandb.Table, max_validation_batches: int, epoch: int | None=None) -> lumina.Table:
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        for img_idx, prediction_result in progress_bar_result_iterable:
            ...
            try:
                ...
                wandb_image = lumina.Image(batch['im_file'][img_idx], boxes={'ground-truth': {'box_data': ground_truth_data, 'class_labels': class_label_map}, 'predictions': {'box_data': prediction_box_data['box_data'], 'class_labels': class_label_map}})
