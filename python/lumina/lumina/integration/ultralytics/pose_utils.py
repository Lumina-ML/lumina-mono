...
import lumina
from lumina.integration.ultralytics.bbox_utils import get_boxes, get_ground_truth_bbox_annotations
...
def plot_pose_predictions(result: Results, model_name: str, visualize_skeleton: bool, table: wandb.Table | None=None):
    ...
    prediction_image = lumina.Image(annotated_image, boxes=boxes)
...
def plot_pose_validation_results(dataloader, class_label_map, model_name: str, predictor: PosePredictor, visualize_skeleton: bool, table: wandb.Table, max_validation_batches: int, epoch: int | None=None) -> lumina.Table:
    data_idx = 0
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        for img_idx, prediction_result in progress_bar_result_iterable:
            ...
            ground_truth_image = lumina.Image(annotate_keypoint_batch(batch['im_file'][img_idx], batch['keypoints'][img_idx], visualize_skeleton), boxes={'ground-truth': {'box_data': get_ground_truth_bbox_annotations(img_idx, batch['im_file'][img_idx], batch, class_label_map), 'class_labels': class_label_map}})
