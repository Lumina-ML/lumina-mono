...
import lumina
from lumina.integration.ultralytics.bbox_utils import get_ground_truth_bbox_annotations, get_mean_confidence_map
...
def plot_mask_predictions(result: Results, model_name: str, table: wandb.Table | None=None) -> tuple[lumina.Image, dict, dict, dict]:
    result = result.to('cpu')
    boxes, masks, mean_confidence_map = get_boxes_and_masks(result)
    image = lumina.Image(result.orig_img[:, :, ::-1], boxes=boxes, masks=masks)
...
def plot_sam_predictions(result: Results, prompt: dict, table: wandb.Table) -> lumina.Table:
    result = result.to('cpu')
    image = result.orig_img[:, :, ::-1]
    image, wb_box_data = structure_prompts_and_image(image, prompt)
    image = lumina.Image(image, boxes=wb_box_data, masks={'predictions': {'mask_data': np.squeeze(result.masks.data.cpu().numpy().astype(int)), 'class_labels': {0: 'Background', 1: 'Prediction'}}})
    table.add_data(image)
    return table

def plot_segmentation_validation_results(dataloader, class_label_map, model_name: str, predictor: SegmentationPredictor, table: wandb.Table, max_validation_batches: int, epoch: int | None=None):
    data_idx = 0
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        for img_idx, prediction_result in progress_bar_result_iterable:
            ...
            try:
                ...
                wandb_image = lumina.Image(batch['im_file'][img_idx], boxes={'ground-truth': {'box_data': ground_truth_data, 'class_labels': class_label_map}, 'predictions': prediction_box_data}, masks=prediction_mask_data)
