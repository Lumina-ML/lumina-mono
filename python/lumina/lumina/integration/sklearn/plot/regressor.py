...
def regressor(model, X_train, X_test, y_train, y_test, model_name='Regressor'):
    ...
    lumina.termlog('Logged outlier candidates.')
    residuals(model, X_train, y_train)
    lumina.termlog('Logged residuals.')
...
def outlier_candidates(regressor=None, X=None, y=None):
    ...
    if is_missing and correct_types and is_fitted:
        ...
        lumina.log({'outlier_candidates': outliers_chart})
...
def residuals(regressor=None, X=None, y=None):
    ...
    if not_missing and correct_types and is_fitted:
        ...
        lumina.log({'residuals': residuals_chart})
