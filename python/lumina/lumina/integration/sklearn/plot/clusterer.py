...
def elbow_curve(clusterer=None, X=None, cluster_ranges=None, n_jobs=1, show_cluster_time=True):
    ...
    if not hasattr(clusterer, 'n_clusters'):
        lumina.termlog('n_clusters attribute not in classifier. Cannot plot elbow method.')
        return
    ...
    if not_missing and correct_types and is_fitted:
        ...
        lumina.log({'elbow_curve': elbow_curve_chart})
...
def silhouette(clusterer=None, X=None, cluster_labels=None, labels=None, metric='euclidean', kmeans=True):
    ...
    if not_missing and correct_types and is_fitted:
        ...
        lumina.log({'silhouette_plot': silhouette_chart})
