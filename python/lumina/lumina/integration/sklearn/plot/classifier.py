...
import lumina
import lumina.plot
from lumina.integration.sklearn import calculate, utils
from . import shared
simplefilter(action='ignore', category=FutureWarning)

def classifier(model, X_train, X_test, y_train, y_test, y_pred, y_probas, labels, is_binary=False, model_name='Classifier', feature_names=None, log_learning_curve=False):
...
def roc(y_true=None, y_probas=None, labels=None, plot_micro=True, plot_macro=True, classes_to_plot=None):
    ...
    ```
    """
    roc_chart = lumina.plot.roc_curve(y_true, y_probas, labels, classes_to_plot)
    lumina.log({'roc': roc_chart})

def confusion_matrix(y_true=None, y_pred=None, labels=None, true_labels=None, pred_labels=None, normalize=False):
    ...
    if not_missing and correct_types:
        confusion_matrix_chart = calculate.confusion_matrix(y_true, y_pred, labels, true_labels, pred_labels, normalize)
        lumina.log({'confusion_matrix': confusion_matrix_chart})
...
def precision_recall(y_true=None, y_probas=None, labels=None, plot_micro=True, classes_to_plot=None):
    ...
    ```
    """
    precision_recall_chart = lumina.plot.pr_curve(y_true, y_probas, labels, classes_to_plot)
    lumina.log({'precision_recall': precision_recall_chart})
...
def feature_importances(model=None, feature_names=None, title='Feature Importance', max_num_features=50):
    ...
    if not_missing and correct_types and model_fitted:
        ...
        lumina.log({'feature_importances': feature_importance_chart})
...
def class_proportions(y_train=None, y_test=None, labels=None):
    ...
    if not_missing and correct_types:
        ...
        lumina.log({'class_proportions': class_proportions_chart})
...
def calibration_curve(clf=None, X=None, y=None, clf_name='Classifier'):
    ...
    if not_missing and correct_types and is_fitted:
        ...
        if y.dtype.char == 'U' or not ((y == 0) | (y == 1)).all():
            lumina.termwarn('This function only supports binary classification at the moment and therefore expects labels to be binary. Skipping calibration curve.')
            return
        calibration_curve_chart = calculate.calibration_curves(clf, X, y, clf_name)
        lumina.log({'calibration_curve': calibration_curve_chart})
