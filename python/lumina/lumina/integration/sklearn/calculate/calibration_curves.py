...
def calibration_curves(clf, X, y, clf_name):
    ...
    chart = lumina.visualize('wandb/calibration/v1', table)
    return chart
...
def make_table(model_column, frac_positives_column, mean_pred_value_column, hist_column, edge_column):
    ...
    data = list(zip(model_column, frac_positives_column, mean_pred_value_column, hist_column, edge_column, strict=False))
    return lumina.Table(columns=columns, data=data)
