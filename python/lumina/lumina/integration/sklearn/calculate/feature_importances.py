...
def feature_importances(model, feature_names):
    ...
    if found_attribute is None:
        lumina.termwarn(f"could not find any of attributes {', '.join(attributes_to_check)} on classifier. Cannot plot feature importances.")
        return
    elif found_attribute == 'feature_importances_':
        importances = model.feature_importances_
    elif found_attribute == 'coef_':
    ...
    if len(importances.shape) > 1:
        ...
        if n_significant_dims > 1:
            ...
            lumina.termwarn(f'{nd}-dimensional feature importances array passed to plot_feature_importances. {nd}-dimensional and higher feature importances arrays are not currently supported. These importances will not be plotted.')
            return
        else:
    ...
    chart = lumina.visualize('wandb/feature_importances/v1', table)
    return chart

def make_table(feature_names, importances):
    table = lumina.Table(columns=['feature_names', 'importances'], data=[[feature_names[i], importances[i]] for i in range(len(feature_names))])
    return table
