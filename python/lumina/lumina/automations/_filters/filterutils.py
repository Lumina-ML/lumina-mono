...
from .operators import BaseVariadicLogicalOp, Eq, Exists, Gt, Gte, In, Lt, Lte, Ne, Nor, Not, NotIn, Op, Or
...
def _(op: Not) -> MongoLikeFilter:
    ...
    """
    if isinstance((expr := op.expr), (Not, Or, Nor, In, NotIn, Eq, Ne, Lt, Lte, Gt, Gte, Exists)):
        return simplify_expr(~expr)
    return Not(expr=simplify_expr(expr))

def flatten_inner(op: BaseVariadicLogicalOp, parent_cls: type[BaseVariadicLogicalOp]) -> Iterator[FilterExpr | Op]:
