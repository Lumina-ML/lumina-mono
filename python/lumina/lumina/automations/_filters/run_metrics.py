...
from pydantic import Field, PositiveFloat, PositiveInt, StrictFloat, StrictInt, field_validator
...
if TYPE_CHECKING:
    from lumina.automations.events import RunMetricFilter
MONGO2PY_OPS: Final[dict[str, str]] = {'$eq': '==', '$ne': '!=', '$gt': '>', '$lt': '<', '$gte': '>=', '$lte': '<='}
PY2MONGO_OPS: Final[dict[str, str]] = {v: k for k, v in MONGO2PY_OPS.items()}
PosNum: TypeAlias = PositiveInt | PositiveFloat

class Agg(LenientStrEnum):
...
class ChangeType(LenientStrEnum):
...
class ChangeDir(LenientStrEnum):
...
class MetricThresholdFilter(BaseMetricFilter):
    ...
    def __repr__(self) -> str:
        ...
        return repr(f'{metric} {op} {self.threshold}')

class MetricChangeFilter(BaseMetricFilter):
    ...
    prior_window: Annotated[PositiveInt, Field(alias='prior_window_size', default_factory=lambda data: data['window'])]
    ...
    def __repr__(self) -> str:
        metric = f'{self.agg.value}({self.name})' if self.agg else self.name
        verb = 'changes' if self.change_dir is ChangeDir.ANY else f'{self.change_dir.value.lower()}s'
        fmt_spec = '.2%' if self.change_type is ChangeType.REL else ''
        amt = f'{self.threshold:{fmt_spec}}'
        return repr(f'{metric} {verb} {amt}')
...
class MetricZScoreFilter(GQLBase, extra='forbid'):
    ...
    def __repr__(self) -> str:
        match self.change_dir:
            case ChangeDir.ANY:
                return repr(f'abs(zscore({self.name!r})) > {self.threshold}')
            case ChangeDir.DECREASE:
                return repr(f'zscore({self.name!r}) < -{self.threshold}')
...
class BaseMetricOperand(GQLBase, ABC, extra='forbid'):
    ...
    @overload
    def changes_by(self, *, diff: PosNum | None, frac: PosNum | None, _dir: ChangeDir) -> MetricChangeFilter:
        ...

    def changes_by(self, *, diff: PosNum | None=None, frac: PosNum | None=None, _dir: ChangeDir=ChangeDir.ANY) -> MetricChangeFilter:
        ...
        match (diff, frac):
            ...
            case [None, int() | float()] if frac <= 0:
                raise ValueError(f'Expected positive threshold, got: frac={frac!r}')
            case [int() | float(), None] if diff <= 0:
                raise ValueError(f'Expected positive threshold, got: diff={diff!r}')
            case [int() | float(), None]:
            ...
            case _:
                msg = f'Expected numeric `diff` or `frac`, got: diff={diff!r}, frac={frac!r}'
                raise TypeError(msg)
    ...
    def increases_by(self, *, diff: PosNum | None=None, frac: PosNum | None=None) -> MetricChangeFilter:
    ...
    def decreases_by(self, *, diff: PosNum | None=None, frac: PosNum | None=None) -> MetricChangeFilter:
...
class ZScoreMetricOperand(GQLBase, extra='forbid'):
    ...
    def lt(self, value: int | float, /) -> MetricZScoreFilter:
        ...
        return MetricZScoreFilter(name=self.name, window=self.window, change_dir=ChangeDir.DECREASE, threshold=abs(value))
    ...
    def gt(self, value: int | float, /) -> MetricZScoreFilter:
        ...
        """
        if value <= 0:
            raise ValueError(f'Expected positive threshold, got: value={value!r}')
        return MetricZScoreFilter(name=self.name, window=self.window, change_dir=ChangeDir.ANY if self.is_absolute else ChangeDir.INCREASE, threshold=abs(value))
