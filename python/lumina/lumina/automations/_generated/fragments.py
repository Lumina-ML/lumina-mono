...
from lumina._pydantic import GQLId, GQLResult, Typename
...
class FilterEventFields(GQLResult):
    typename__: Typename[Literal['FilterEventTriggeringCondition']] = 'FilterEventTriggeringCondition'
    event_type: EventTriggeringConditionType = Field(alias='eventType')
    filter: str

class WebhookIntegrationFields(GQLResult):
    typename__: Typename[Literal['GenericWebhookIntegration']] = 'GenericWebhookIntegration'
    id: GQLId
    name: str
    url_endpoint: str = Field(alias='urlEndpoint')

class GenericWebhookActionFields(GQLResult):
    typename__: Typename[Literal['GenericWebhookTriggeredAction']] = 'GenericWebhookTriggeredAction'
    integration: GenericWebhookActionFieldsIntegrationIntegration | WebhookIntegrationFields = Field(discriminator='typename__')
    request_payload: str | None = Field(alias='requestPayload')

class GenericWebhookActionFieldsIntegrationIntegration(GQLResult):
    typename__: Typename[Literal['GitHubOAuthIntegration', 'Integration', 'SlackIntegration']]
...
class NotificationActionFields(GQLResult):
    typename__: Typename[Literal['NotificationTriggeredAction']] = 'NotificationTriggeredAction'
    integration: NotificationActionFieldsIntegrationIntegration | SlackIntegrationFields = Field(discriminator='typename__')
    title: str | None
    message: str | None
    severity: AlertSeverity | None

class NotificationActionFieldsIntegrationIntegration(GQLResult):
    typename__: Typename[Literal['GenericWebhookIntegration', 'GitHubOAuthIntegration', 'Integration']]
...
class TriggerFields(GQLResult):
    ...
    enabled: bool
    scope: ArtifactPortfolioScopeFields | ArtifactSequenceScopeFields | EntityScopeFields | ProjectScopeFields = Field(discriminator='typename__')
    event: FilterEventFields
    action: GenericWebhookActionFields | NoOpActionFields | NotificationActionFields | TriggerFieldsActionPushNotificationTriggeredAction | QueueJobActionFields = Field(discriminator='typename__')
