...
from lumina._pydantic import GQLId, GQLInput, computed_field, model_validator, to_json
from ._filters import MongoLikeFilter
from ._generated import CreateFilterTriggerInput, QueueJobActionInput, TriggeredActionConfig, UpdateFilterTriggerInput
from ._validators import parse_input_action
from .actions import ActionType, DoNothing, InputAction, SavedAction, SendNotification, SendWebhook
from .automations import Automation, NewAutomation
from .events import EventType, InputEvent, RunMetricFilter, RunStateFilter, SavedEvent, _WrappedSavedEventFilter
...
INVALID_INPUT_ACTIONS: Final[Collection[ActionType]] = (ActionType.QUEUE_JOB, ActionType.PUSH_NOTIFICATION)
...
ALWAYS_SUPPORTED_SCOPES: Final[Collection[ScopeType]] = frozenset({ScopeType.ARTIFACT_COLLECTION, ScopeType.PROJECT})
'Scope types that should be supported by all current, non-EOL server versions.'
ALWAYS_SUPPORTED_EVENTS: Final[Collection[EventType]] = frozenset({EventType.CREATE_ARTIFACT, EventType.LINK_ARTIFACT, EventType.ADD_ARTIFACT_ALIAS})
'Event types that should be supported by all current, non-EOL server versions.'
ALWAYS_SUPPORTED_ACTIONS: Final[Collection[ActionType]] = frozenset({ActionType.NOTIFICATION, ActionType.GENERIC_WEBHOOK})
...
def prepare_event_filter_input(obj: _WrappedSavedEventFilter | MongoLikeFilter | RunMetricFilter | RunStateFilter) -> str:
...
def prepare_to_create(obj: NewAutomation | None=None, /, **kwargs: Unpack[WriteAutomationsKwargs]) -> CreateFilterTriggerInput:
...
def prepare_to_update(obj: Automation | None=None, /, **kwargs: Unpack[WriteAutomationsKwargs]) -> UpdateFilterTriggerInput:
