...
from lumina._pydantic import to_json
...
def parse_scope(v: Any) -> Any:
    ...
    from lumina.apis.public import ArtifactCollection, Organization, Project, Team
    from .scopes import OrgScope, ProjectScope, TeamScope, _ArtifactPortfolioScope, _ArtifactSequenceScope
    match v:
        case Project():
...
def parse_saved_action(v: Any) -> Any:
    ...
    from .actions import DoNothing, SavedNoOpAction, SavedNotificationAction, SavedWebhookAction, SendNotification, SendWebhook
    match v:
...
def wrap_mutation_event_filter(f: MongoLikeFilter) -> MongoLikeFilter:
    ...
    """
    return Or.wrap(And.wrap(simplify_expr(f)))
