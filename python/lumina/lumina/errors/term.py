...
def termlog(string: str='', newline: bool=True, repeat: bool=True, prefix: bool=True) -> None:
    ...
    """
    _log(string, newline=newline, repeat=repeat, prefix=prefix, silent=not _show_info)

def termwarn(string: str, newline: bool=True, repeat: bool=True, prefix: bool=True) -> None:
    """Log a warning to stderr.

    The arguments are the same as for `termlog()`.
    """
    string = '\n'.join([f'{WARN_STRING} {s}' for s in string.split('\n')])
    _log(string, newline=newline, repeat=repeat, prefix=prefix, silent=not _show_warnings, level=logging.WARNING)

def termerror(string: str, newline: bool=True, repeat: bool=True, prefix: bool=True) -> None:
    """Log an error to stderr.

    The arguments are the same as for `termlog()`.
    """
    string = '\n'.join([f'{ERROR_STRING} {s}' for s in string.split('\n')])
    _log(string, newline=newline, repeat=repeat, prefix=prefix, silent=not _show_errors, level=logging.ERROR)
...
def terminput(prompt: str, *, timeout: float | None=None, hide: bool=False) -> str:
...
def _terminput(prefixed_prompt: str, *, timeout: float | None=None, hide: bool=False) -> str:
    ...
    if timeout is not None:
        from lumina.sdk.lib.timed_input import timed_input
        try:
            return timed_input(prefixed_prompt, timeout=timeout, err=True, jupyter=_in_jupyter())
        except KeyboardInterrupt:
            sys.stderr.write('\n')
            raise
    try:
        return click.prompt(prefixed_prompt, prompt_suffix='', hide_input=hide, err=True)
    except click.Abort:
        sys.stderr.write('\n')
...
class DynamicBlock:
    ...
    def set_text(self, text: str, prefix: bool=True) -> None:
        ...
        with _dynamic_text_lock:
            ...
            if prefix:
                self._lines_to_print = [f'{LOG_STRING}: {line}' for line in self._lines_to_print]
            _l_rerender_dynamic_blocks()
    ...
    def _l_clear(self) -> None:
        ...
        click.echo(move_up_and_delete_line * self._num_lines_printed, file=sys.stderr, nl=False)
        self._num_lines_printed = 0
    ...
    def _l_print(self) -> None:
        ...
        if self._lines_to_print:
            ...
            click.echo('\n'.join((_ansi_shorten(line, term_width) for line in self._lines_to_print)), file=sys.stderr)
...
def _ansi_shorten(text: str, width: int) -> str:
    ...
    return text[:first_ansi.end()] + _ansi_shorten(text[first_ansi.end():], width - first_ansi.start())
...
def _log(string: str='', newline: bool=True, repeat: bool=True, prefix: bool=True, silent: bool=False, level: int=logging.INFO) -> None:
