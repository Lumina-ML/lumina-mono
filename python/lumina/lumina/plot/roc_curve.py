...
def roc_curve(
    ...
    for i in indices_to_plot:
        ...
        fpr[class_label], tpr[class_label], _ = sklearn_metrics.roc_curve(y_true, y_probas[..., i], pos_label=classes[i])
    df = pd.DataFrame({'class': np.hstack([[k] * len(v) for k, v in fpr.items()]), 'fpr': np.hstack(list(fpr.values())), 'tpr': np.hstack(list(tpr.values()))}).round(3)
    if len(df) > lumina.Table.MAX_ROWS:
        lumina.termwarn(f'wandb uses only {lumina.Table.MAX_ROWS} data points to create the plots.')
        df = sklearn_utils.resample(df, replace=False, n_samples=lumina.Table.MAX_ROWS, random_state=42, stratify=df['class']).sort_values(['fpr', 'tpr', 'class'])
    return plot_table(data_table=lumina.Table(dataframe=df), vega_spec_name='wandb/area-under-curve/v0', fields={'x': 'fpr', 'y': 'tpr', 'class': 'class'}, string_fields={'title': title, 'x-axis-title': 'False positive rate', 'y-axis-title': 'True positive rate'}, split_table=split_table)
