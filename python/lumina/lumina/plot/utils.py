...
def test_missing(**kwargs):
    ...
    for k, v in kwargs.items():
        ...
        if k == 'X' or k == 'X_test':
            ...
            if missing > 0:
                lumina.termwarn('%s contains %d missing values. ' % (k, missing))
                test_passed = False
            non_nums = 0
            if v.ndim == 1:
...
def test_types(**kwargs):
    ...
    test_passed = True
    for k, v in kwargs.items():
        if (k == 'X' or k == 'X_test' or k == 'y' or (k == 'y_test') or (k == 'y_true') or (k == 'y_probas') or (k == 'x_labels') or (k == 'y_labels') or (k == 'matrix_values')) and (not isinstance(v, (Sequence, Iterable, np.ndarray, np.generic, pd.DataFrame, pd.Series, list))):
