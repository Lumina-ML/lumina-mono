...
_entity_override: ContextVar[object | str] = ContextVar('wandb_sandbox_entity_override', default=_OVERRIDE_UNSET)
...
@contextmanager
def override_sandbox_entity(entity: str | None=None) -> Generator[None]:
...
def _client_version_headers() -> list[tuple[str, str]]:
    ...
    """
    return [('x-wandb-sdk-version', lumina.__version__), ('x-cwsandbox-client-version', cwsandbox.__version__)]
...
def _resolve_wandb_sdk_auth() -> AuthHeaders:
    ...
    if auth is None:
        raise CWSandboxAuthenticationError('wandb.sandbox could not resolve W&B credentials for sandbox auth.')
    ...
    return AuthHeaders(headers={key: value for key, value in metadata}, strategy='wandb_api_key')
