...
def _placement_override_error(fields: Sequence[str]) -> UsageError:
    ...
    return UsageError(f'W&B Serverless automatically selects the runner and profile. Remove ({fields_display}).')

def _reject_placement_override_kwargs(kwargs: Mapping[str, Any]) -> None:
    blocked = [field for field in _PLACEMENT_OVERRIDE_FIELDS if field in kwargs and kwargs[field] is not None]
    if blocked:
        raise _placement_override_error(blocked)

def _resources_include_gpu(resources: ResourceOptions | Mapping[str, Any] | None) -> bool:
...
def _reject_gpu_resources(resources: ResourceOptions | Mapping[str, Any] | None) -> None:
    if _resources_include_gpu(resources):
        raise UsageError('W&B Serverless currently supports only CPU and memory resources. Remove resources.gpu.')

def _reject_unsupported_egress_mode(network: NetworkOptions | Mapping[str, Any] | None) -> None:
    if network is None:
        return
    ...
    if egress_mode is not None and egress_mode not in _SUPPORTED_EGRESS_MODES:
        ...
        raise UsageError(f'wandb.sandbox supports only egress modes ({modes_display}). Got {egress_mode!r}.')
...
def _with_serverless_max_lifetime_default(defaults: SandboxDefaults | Mapping[str, Any] | None) -> SandboxDefaults:
    if defaults is None:
        return SandboxDefaults(max_lifetime_seconds=_SERVERLESS_DEFAULT_MAX_LIFETIME_SECONDS)
    if isinstance(defaults, Mapping):
        coerced = SandboxDefaults.from_dict(defaults)
        if coerced.max_lifetime_seconds is not None:
            return coerced
        return coerced.with_overrides(max_lifetime_seconds=_SERVERLESS_DEFAULT_MAX_LIFETIME_SECONDS)
    if defaults.max_lifetime_seconds is not None:
        return defaults
    return defaults.with_overrides(max_lifetime_seconds=_SERVERLESS_DEFAULT_MAX_LIFETIME_SECONDS)
...
def _reject_invalid_defaults(defaults: SandboxDefaults | Mapping[str, Any] | None) -> None:
    if defaults is None:
        return
    if isinstance(defaults, Mapping):
        blocked = [field for field in _PLACEMENT_OVERRIDE_FIELDS if field in defaults and defaults[field] is not None]
    else:
        blocked = [field for field in _PLACEMENT_OVERRIDE_FIELDS if getattr(defaults, field, None) is not None]
    if blocked:
        raise _placement_override_error(blocked)
    resources = defaults.get('resources') if isinstance(defaults, Mapping) else getattr(defaults, 'resources', None)
    _reject_gpu_resources(resources)
    network = defaults.get('network') if isinstance(defaults, Mapping) else getattr(defaults, 'network', None)
...
class Session(_BaseSession):
    ...
    if TYPE_CHECKING:
        ...
        def sandbox(self, **kwargs: Any) -> _BaseSandbox:
            _reject_invalid_kwargs(kwargs)
            return super().sandbox(**kwargs)

        def function(self, **kwargs: Any) -> Callable[[Callable[P, R]], RemoteFunction[P, R]]:
            _reject_invalid_kwargs(kwargs)
