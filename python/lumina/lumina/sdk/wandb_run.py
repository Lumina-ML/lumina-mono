...
from lumina.plot import CustomChart, Visualize
from lumina.proto.wandb_internal_pb2 import MetricRecord, PollExitResponse, Result, RunRecord
from lumina.proto.wandb_telemetry_pb2 import Deprecated
from lumina.sdk.lib import asyncio_manager, logger_capture, run_messages, run_stopping, wb_logging
from lumina.sdk.lib.filesystem import FilesDict, GlobStr, LinkStats, PolicyName, link_or_copy_with_policy, unlink_path, validate_glob_path
from lumina.sdk.lib.import_hooks import register_post_import_hook, unregister_post_import_hook
from lumina.sdk.lib.paths import FilePathStr, StrPath
from lumina.util import _is_artifact_object, _is_artifact_string, _is_artifact_version_weave_dict, _is_py_requirements_or_dockerfile, _resolve_aliases, add_import_hook, parse_artifact_string
from . import wandb_config, wandb_metric, wandb_summary
from .data_types._dtypes import TypeRegistry
from .interface.interface import InterfaceBase
from .interface.summary_record import SummaryRecord
from .lib import config_util, deprecation, filenames, filesystem, interrupt, ipython, module, printer, progress, proto_util, redirect, telemetry
from .lib.exit_hooks import ExitHooks
from .mailbox import HandleAbandonedError, MailboxClosedError, MailboxHandle, wait_with_progress
from .wandb_alerts import AlertLevel
from .wandb_setup import _WandbSetup
if TYPE_CHECKING:
    from typing import TypedDict
    import torch
    from lumina.apis.public import Api as PublicApi
    from lumina.proto.wandb_internal_pb2 import GetSummaryResponse, SampledHistoryResponse
...
class RunStatusChecker:
    ...
    def __init__(self, run_id: str, asyncer: asyncio_manager.AsyncioManager, interface: InterfaceBase, settings: Settings, retry_polling_interval: int=5, internal_messages_polling_interval: int=10) -> None:
        self._run_id = run_id
        ...
        self._stop_checker = run_stopping.RunStopChecker(asyncer, interface, settings.stop_fn or interrupt.interrupt_main)
    ...
    @staticmethod
    def _abandon_status_check(lock: threading.Lock, handle: MailboxHandle[Result] | None):
        with lock:
            if handle:
                handle.cancel()
    ...
    def check_network_status(self) -> None:
        ...
        def _process_network_status(result: Result) -> None:
            ...
            for hr in network_status.network_responses:
                if hr.http_status_code == 200 or hr.http_status_code == 0:
                    lumina.termlog(f'{hr.http_response_text}')
                else:
                    lumina.termlog(f'{hr.http_status_code} encountered ({hr.http_response_text.rstrip()}), retrying request')
        with wb_logging.log_to_run(self._run_id):
            try:
                ...
                self._abandon_status_check(self._network_status_lock, self._network_status_handle)
    ...
    def join(self) -> None:
        ...
        self._internal_messages.stop(timeout=5)
_P = ParamSpec('_P')
_T = TypeVar('_T')

def _log_to_run(func: Callable[Concatenate[Run, _P], _T]) -> Callable[Concatenate[Run, _P], _T]:
...
def _attach(func: Callable[Concatenate[Run, _P], _T]) -> Callable[Concatenate[Run, _P], _T]:
    ...
    def wrapper(self: Run, *args: _P.args, **kwargs: _P.kwargs) -> _T:
        ...
        if is_using_service and attach_pid != os.getpid():
            if _is_attaching:
                raise RuntimeError(f'Trying to attach `{func.__name__}`' + f' while in the middle of attaching `{_is_attaching}`')
            _is_attaching = func.__name__
            try:
                lumina._attach(run=self)
            finally:
                _is_attaching = ''
        return func(self, *args, **kwargs)
    return wrapper

def _raise_if_finished(func: Callable[Concatenate[Run, _P], _T]) -> Callable[Concatenate[Run, _P], _T]:
    ...
    def wrapper_fn(self: Run, *args: _P.args, **kwargs: _P.kwargs) -> _T:
        if not getattr(self, '_is_finished', False):
            return func(self, *args, **kwargs)
        message = f'Run ({self.id}) is finished. The call to `{func.__name__}` will be ignored. Please make sure that you are using an active run.'
...
class Run:
    ...
    def __init__(self, settings: Settings, config: dict[str, Any] | None=None, sweep_config: dict[str, Any] | None=None, launch_config: dict[str, Any] | None=None) -> None:
        ...
        if settings._noop:
            self._settings = settings
            return
        self._init(settings=settings, config=config, sweep_config=sweep_config, launch_config=launch_config)

    def _init(self, settings: Settings, config: dict[str, Any] | None=None, sweep_config: dict[str, Any] | None=None, launch_config: dict[str, Any] | None=None) -> None:
        self._settings = settings
        ...
        self.summary = wandb_summary.Summary(self._summary_get_current_summary_callback)
        ...
        self._torch_history: wandb_torch.TorchHistory | None = None
        ...
        get_sentry().configure_scope(tags=dict(self._settings), process_context='user')
        self._launch_artifact_mapping: dict[str, Any] = {}
        self._unique_launch_artifact_sequence_names: dict[str, Any] = {}
        config = config or dict()
        self._config._update(config, allow_val_change=True, ignore_locked=True)
        if sweep_config:
            self._config.merge_locked(sweep_config, user='sweep', _allow_val_change=True)
        if launch_config:
            self._config.merge_locked(launch_config, user='launch', _allow_val_change=True)
        launch_queue_name = lumina.env.get_launch_queue_name()
        if launch_queue_name:
            self._config[wandb_key]['launch_queue_name'] = launch_queue_name
        launch_queue_entity = lumina.env.get_launch_queue_entity()
        if launch_queue_entity:
            self._config[wandb_key]['launch_queue_entity'] = launch_queue_entity
        launch_trace_id = lumina.env.get_launch_trace_id()
        if launch_trace_id:
    ...
    def _handle_launch_artifact_overrides(self) -> None:
        if self._settings.launch and os.environ.get('WANDB_ARTIFACTS') is not None:
            try:
                artifacts: dict[str, Any] = json.loads(os.environ.get('WANDB_ARTIFACTS', '{}'))
            except (ValueError, SyntaxError):
                lumina.termwarn('Malformed WANDB_ARTIFACTS, using original artifacts')
            else:
                self._initialize_launch_artifact_maps(artifacts)
        elif self._settings.launch and self._settings.launch_config_path and os.path.exists(self._settings.launch_config_path):
    ...
    def __getstate__(self) -> Any:
        ...
        if not _attach_id:
            return
        return dict(_attach_id=_attach_id, _init_pid=self._init_pid, _is_finished=self._is_finished)
    ...
    @property
    def _torch(self) -> wandb_torch.TorchHistory:
        if self._torch_history is None:
            self._torch_history = wandb_torch.TorchHistory()
    ...
    def tags(self, tags: Sequence) -> None:
        ...
        try:
            ...
            lumina.termwarn(f'Invalid tag detected: {error_msg} Tags not updated.')
            return
        if self._interface:
            self._interface.publish_run(self)
    ...
    def _get_path(self) -> str:
        return '/'.join((e for e in [self._settings.entity, self._settings.project, self._settings.run_id] if e is not None))
    ...
    def project_name(self) -> str:
        ...
        <!-- lazydoc-ignore: internal -->
        """
        deprecation.warn_and_record_deprecation(feature=Deprecated(run__project_name=True), message='The project_name method is deprecated and will be removed in a future release. Please use `run.project` instead.')
        return self.project
    ...
    def get_project_url(self) -> str | None:
        ...
        <!-- lazydoc-ignore: internal -->
        """
        deprecation.warn_and_record_deprecation(feature=Deprecated(run__get_project_url=True), message='The get_project_url method is deprecated and will be removed in a future release. Please use `run.project_url` instead.')
    ...
    def project_url(self) -> str | None:
        ...
        if self._settings._offline:
            lumina.termwarn('URL not available in offline run')
            return None
    ...
    @_log_to_run
    @_attach
    def log_code(self, root: str | None='.', name: str | None=None, include_fn: Callable[[str, str], bool] | Callable[[str], bool]=_is_py_requirements_or_dockerfile, exclude_fn: Callable[[str, str], bool] | Callable[[str], bool]=filenames.exclude_wandb_fn) -> Artifact | None:
        ...
        if name is None:
            if self.settings._jupyter:
                ...
                name_string = f'{self._settings.project}-{self._settings.program_relpath}'
            name = lumina.util.make_artifact_name_safe(f'source-{name_string}')
        ...
        for dirpath, _, files in os.walk(self._settings._tmp_code_dir):
            for fname in files:
                ...
                art.add_file(file_path, name=save_name)
        if not files_added:
            lumina.termwarn('No relevant files were detected in the specified directory. No code will be logged to your run.')
            return None
        artifact = self._log_artifact(art)
        self._config.update({'_wandb': {'code_path': artifact.name}}, allow_val_change=True)
        return artifact
    ...
    def get_sweep_url(self) -> str | None:
        ...
        <!-- lazydoc-ignore: internal -->
        """
        deprecation.warn_and_record_deprecation(feature=Deprecated(run__get_sweep_url=True), message='The get_sweep_url method is deprecated and will be removed in a future release. Please use `run.sweep_url` instead.')
    ...
    def get_url(self) -> str | None:
        ...
        <!-- lazydoc-ignore: internal -->
        """
        deprecation.warn_and_record_deprecation(feature=Deprecated(run__get_url=True), message='The get_url method is deprecated and will be removed in a future release. Please use `run.url` instead.')
        return self.url
    ...
    def _label_internal(self, code: str | None=None, repo: str | None=None, code_version: str | None=None) -> None:
        with telemetry.context(run=self) as tel:
    ...
    def _label(self, code: str | None=None, repo: str | None=None, code_version: str | None=None, **kwargs: str) -> None:
        if self._settings.label_disable:
            return
        for k, v in (('code', code), ('repo', repo), ('code_version', code_version)):
            if v and (not RE_LABEL.match(v)):
                lumina.termwarn(f"Label added for '{k}' with invalid identifier '{v}' (ignored).", repeat=False)
        for v in kwargs:
            lumina.termwarn(f'Label added for unsupported key {v!r} (ignored).', repeat=False)
    ...
    def _repr_mimebundle_(self, include: Any | None=None, exclude: Any | None=None) -> dict[str, str]:
        return {'text/html': self.to_html(hidden=True)}

    @_log_to_run
    @_raise_if_finished
    def _config_callback(self, key: tuple[str, ...] | str | None=None, val: Any | None=None, data: dict[str, object] | None=None) -> None:
        logger.info(f'config_cb {key} {val} {data}')
        if self._interface:
            self._interface.publish_config(key=key, val=val, data=data)

    @_log_to_run
    def _config_artifact_callback(self, key: str, val: str | Artifact | dict) -> Artifact:
        from lumina.apis import public
    ...
    def _summary_get_current_summary_callback(self) -> dict[str, Any]:
        if self._is_finished:
            lumina.termwarn('Summary data not available in finished run')
            return {}
    ...
    def _pop_all_charts(self, data: dict[str, Any], key_prefix: str | None=None) -> dict[str, Any]:
    ...
    def _serialize_custom_charts(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        for k, v in charts.items():
            v.set_key(k)
            self._config_callback(val=v.spec.config_value, key=v.spec.config_key)
    ...
    @_log_to_run
    def _partial_history_callback(self, data: dict[str, Any], step: int | None=None, commit: bool | None=None) -> None:
        if not self._interface:
            return
        data = data.copy()
        data = self._serialize_custom_charts(data)
        not_using_tensorboard = len(lumina.patched['tensorboard']) == 0
        self._interface.publish_partial_history(self, data, user_step=self._local_step, step=step, flush=commit, publish_step=not_using_tensorboard)
    ...
    @_log_to_run
    def _tensorboard_callback(self, logdir: str, save: bool=True, root_logdir: str='') -> None:
    ...
    def _set_run_obj(self, run_obj: RunRecord) -> None:
        ...
        if run_obj.start_time:
            self._start_time = run_obj.start_time.ToMicroseconds() / 1000000.0
        if run_obj.runtime:
        ...
        get_sentry().configure_scope(process_context='user', tags=dict(self._settings))

    def _populate_git_info(self) -> None:
        from .lib.gitlib import GitRepo
        try:
            repo = GitRepo(root=self._settings.git_root, remote=self._settings.git_remote, remote_url=self._settings.git_remote_url, commit=self._settings.git_commit, lazy=False)
            self._settings.git_remote_url = repo.remote_url
            self._settings.git_commit = repo.last_commit
        except Exception:
            lumina.termwarn('Cannot find valid git repo associated with this directory.')

    def _add_singleton(self, data_type: str, key: str, value: dict[int | str, str]) -> None:
    ...
    def _log(self, data: dict[str, Any], step: int | None=None, commit: bool | None=None) -> None:
        ...
        if step is not None:
            if os.getpid() != self._init_pid or self._is_attached:
                lumina.termwarn('Note that setting step in multiprocessing can result in data loss. Please use `run.define_metric(...)` to define a custom metric to log your step values.', repeat=False)
            if len(lumina.patched['tensorboard']) > 0:
                lumina.termwarn('Step cannot be set when using tensorboard syncing. Please use `run.define_metric(...)` to define a custom metric to log your step values.', repeat=False)
    ...
    def log(self, data: dict[str, Any], step: int | None=None, commit: bool | None=None) -> None:
        ...
        if self._settings._shared and step is not None:
            lumina.termwarn(f"In shared mode, the use of `wandb.log` with the step argument is not supported and will be ignored. Please refer to {url_registry.url('define-metric')} on how to customize your x-axis.", repeat=False)
    ...
    @_raise_if_finished
    @_attach
    def save(self, glob_str: str | os.PathLike, base_path: str | os.PathLike | None=None, policy: PolicyName='live', glob: bool=True) -> bool | list[str]:
        ...
        if isinstance(glob_str, str) and glob_str.startswith(('gs://', 's3://')):
            lumina.termlog(f"{glob_str} is a cloud storage url, can't save file to W&B.")
            return []
        ...
        if base_path is not None:
            ...
            lumina.termwarn('Saving files without folders. If you want to preserve subdirectories pass base_path to wandb.save, i.e. wandb.save("/mnt/folder/file.h5", base_path="/mnt")', repeat=False)
            base_path = resolved_glob_path.parent.parent
        if policy not in ('live', 'end', 'now'):
            raise ValueError('Only "live", "end" and "now" policies are currently supported.')
        resolved_base_path = pathlib.PurePath(os.path.abspath(base_path))
        return self._save(resolved_glob_path, resolved_base_path, policy, glob)

    def _save(self, glob_path: pathlib.PurePath, base_path: pathlib.PurePath, policy: PolicyName, expand_glob: bool=True) -> list[str]:
        ...
        if expand_glob:
            src_paths, preexisting = self._expand_glob(base_path, relative_glob, files_root)
        else:
            src_paths, preexisting = self._find_literal(base_path, relative_glob, files_root)
        publish_entries, stats = self._materialize_saved_files(src_paths, preexisting, base_path, files_root, policy)
        stats.emit_warnings()
    ...
    @staticmethod
    def _expand_glob(base_path: pathlib.PurePath, relative_glob: pathlib.PurePath, files_root: pathlib.Path) -> tuple[list[pathlib.Path], set[pathlib.Path]]:
        ...
        src_paths = [pathlib.Path(p).absolute() for p in sorted(glob.glob(str(base_path / relative_glob)))]
        if not src_paths and os.path.lexists(base_path / relative_glob):
            lumina.termwarn(f'No files found at glob pattern {str(relative_glob)!r}, but a file exists at that literal path. If you meant to match it literally rather than as a glob pattern, call save() with glob=False.', repeat=False)
        return (src_paths, preexisting)

    @staticmethod
    def _find_literal(base_path: pathlib.PurePath, relative_path: pathlib.PurePath, files_root: pathlib.Path) -> tuple[list[pathlib.Path], set[pathlib.Path]]:
        ...
        if os.path.lexists(literal_src):
            ...
            if glob.escape(str(relative_path)) != str(relative_path):
                lumina.termwarn(f"No file found at literal path {str(literal_src)!r}, and it looks like it contains glob metacharacters ('*', '?', or '[]'). If you meant to use a glob pattern, call save() with glob=True (the default).", repeat=False)
        return (src_paths, preexisting)

    def _materialize_saved_files(self, src_paths: list[pathlib.Path], preexisting: set[pathlib.Path], base_path: pathlib.PurePath, files_root: pathlib.Path, policy: PolicyName) -> tuple[list[tuple[GlobStr, PolicyName]], LinkStats]:
        ...
        for src in src_paths:
            ...
            with contextlib.suppress(OSError):
                if dst.exists() and src.samefile(dst):
                    publish_entries.append((GlobStr(str(dst.relative_to(files_root))), policy))
                    continue
            dst.parent.mkdir(parents=True, exist_ok=True)
            unlink_path(dst)
            effective_policy = link_or_copy_with_policy(self._settings, src, dst, policy, stats)
            publish_entries.append((GlobStr(str(dst.relative_to(files_root))), effective_policy))
        for p in sorted(preexisting):
            if p not in created_targets:
                publish_entries.append((GlobStr(str(p.relative_to(files_root))), policy))
    ...
    def restore(self, name: str, run_path: str | None=None, replace: bool=False, root: str | None=None) -> None | TextIO:
        return restore(name, run_path or self._get_path(), replace, root or self._settings.files_dir)
    ...
    def finish(self, exit_code: int | None=None, quiet: bool | None=None) -> None:
        ...
        if quiet is not None:
            deprecation.warn_and_record_deprecation(feature=Deprecated(run__finish_quiet=True), message='The `quiet` argument to `wandb.run.finish()` is deprecated, use `wandb.Settings(quiet=...)` to set this instead.', run=self)
        self._finish(exit_code)

    @_log_to_run
    def _finish(self, exit_code: int | None=None) -> None:
    ...
    def status(self) -> RunStatus:
        ...
        if sync_data.sync_time.seconds:
            sync_time = datetime.fromtimestamp(sync_data.sync_time.seconds + sync_data.sync_time.nanos / 1000000000.0)
        return RunStatus(sync_items_total=sync_data.sync_items_total, sync_items_pending=sync_data.sync_items_pending, sync_time=sync_time)

    def _add_panel(self, visualize_key: str, panel_type: str, panel_config: dict) -> None:
        config = {'panel_type': panel_type, 'panel_config': panel_config}
        self._config_callback(val=config, key=('_wandb', 'visualize', visualize_key))

    def _redirect(self, stdout_slave_fd: int | None, stderr_slave_fd: int | None, console: str | None=None) -> None:
        if console is None:
        ...
        if console == 'redirect':
            ...
            out_redir = redirect.Redirect(src='stdout', cbs=[lambda data: self._console_callback('stdout', data), self._output_writer.write], flush_periodically=self._settings.mode == 'online')
            err_redir = redirect.Redirect(src='stderr', cbs=[lambda data: self._console_callback('stderr', data), self._output_writer.write], flush_periodically=self._settings.mode == 'online')
            if os.name == 'nt':
            ...
            out_redir = redirect.StreamWrapper(src='stdout', cbs=[lambda data: self._console_callback('stdout', data), self._output_writer.write], flush_periodically=self._settings.mode == 'online')
            err_redir = redirect.StreamWrapper(src='stderr', cbs=[lambda data: self._console_callback('stderr', data), self._output_writer.write], flush_periodically=self._settings.mode == 'online')
        ...
        try:
            ...
            lumina.termwarn(f'Failed to redirect: {e}')
    ...
    def _atexit_cleanup(self, exit_code: int | None=None) -> None:
        ...
        try:
            ...
            if not lumina.wandb_agent._is_running():
                lumina.termerror('Control-C detected -- Run data was not synced')
            raise
        except Exception:
            self._console_stop()
            logger.exception('Problem finishing run')
            lumina.termerror('Problem finishing run')
            raise
        Run._footer(sampled_history=self._sampled_history, final_summary=self._final_summary, poll_exit_response=self._poll_exit_response, settings=self._settings, printer=self._printer)
    ...
    def _begin_capturing_loggers(self) -> None:
        ...
        for name, level in capture_loggers.items():
            ...
            def unregister(logger: logging.Logger=logger, handler: logging.Handler=handler) -> None:
                logger.removeHandler(handler)
            self._teardown_hooks.append(TeardownHook(call=unregister, stage=TeardownStage.EARLY))
    ...
    def _on_start(self) -> None:
        ...
        if self._settings.x_save_requirements and self._interface:
            from lumina.util import working_set
        ...
        if self._interface and (not self._settings._offline):
            ...
            self._run_status_checker = RunStatusChecker(self._settings.run_id, asyncer=self._wl.asyncer, interface=self._interface, settings=self._settings)
    ...
    def _register_telemetry_import_hooks(self) -> None:
        ...
        for module_name in import_telemetry_set:
            register_post_import_hook(import_hook_fn, self._settings.run_id, module_name)
    ...
    def _detect_and_apply_job_inputs(self) -> None:
        ...
        from lumina.sdk.launch.inputs.internal import StagedLaunchInputs
    ...
    def _construct_job_artifact(self, name: str, source_dict: JobSourceDict, installed_packages_list: list[str], patch_path: os.PathLike | None=None) -> Artifact:
        from lumina.sdk.artifacts._internal_artifact import InternalArtifact
        from lumina.sdk.internal import job_builder
    ...
    def _create_image_job(self, input_types: dict[str, Any], output_types: dict[str, Any], installed_packages_list: list[str], docker_image_name: str | None=None, args: list[str] | None=None) -> Artifact | None:
        docker_image_name = docker_image_name or os.getenv('WANDB_DOCKER')
        if not docker_image_name:
            return None
        name = lumina.util.make_artifact_name_safe(f'job-{docker_image_name}')
    ...
    def _log_job_artifact_with_image(self, docker_image_name: str, args: list[str] | None=None) -> Artifact:
        ...
        if not artifact:
            raise lumina.Error(f'Job Artifact log unsuccessful: {artifact}')
        else:
            return artifact
    ...
    def _on_finish(self) -> None:
        ...
        with progress.progress_printer(self._printer, default_text='Finishing up...') as progress_printer:
            result = wait_with_progress(exit_handle, timeout=None, display_progress=functools.partial(progress.loop_printing_operation_stats, progress_printer, self._interface))
    ...
    @_raise_if_finished
    @_attach
    def define_metric(self, name: str, step_metric: str | wandb_metric.Metric | None=None, step_sync: bool | None=None, hidden: bool | None=None, summary: str | None=None, goal: str | None=None, overwrite: bool | None=None) -> wandb_metric.Metric:
        ...
        if summary and 'copy' in summary:
            deprecation.warn_and_record_deprecation(feature=Deprecated(run__define_metric_copy=True), message="define_metric(summary='copy') is deprecated and will be removed.", run=self)
        if summary and 'best' in summary or goal is not None:
            deprecation.warn_and_record_deprecation(feature=Deprecated(run__define_metric_best_goal=True), message="define_metric(summary='best', goal=...) is deprecated and will be removed. Use define_metric(summary='min') or define_metric(summary='max') instead.", run=self)
        return self._define_metric(name, step_metric, step_sync, hidden, summary, goal, overwrite)

    def _define_metric(self, name: str, step_metric: str | wandb_metric.Metric | None=None, step_sync: bool | None=None, hidden: bool | None=None, summary: str | None=None, goal: str | None=None, overwrite: bool | None=None) -> wandb_metric.Metric:
        if not name:
            raise lumina.Error('define_metric() requires non-empty name argument')
        if isinstance(step_metric, wandb_metric.Metric):
            step_metric = step_metric.name
        for arg_name, arg_val, exp_type in (('name', name, str), ('step_metric', step_metric, str), ('step_sync', step_sync, bool), ('hidden', hidden, bool), ('summary', summary, str), ('goal', goal, str), ('overwrite', overwrite, bool)):
            if arg_val is not None and (not isinstance(arg_val, exp_type)):
                arg_type = type(arg_val).__name__
                raise lumina.Error(f'Unhandled define_metric() arg: {arg_name} type: {arg_type}')
        stripped = name[:-1] if name.endswith('*') else name
        if '*' in stripped:
            raise lumina.Error(f'Unhandled define_metric() arg: name (glob suffixes only): {name}')
        ...
        if summary:
            ...
            for i in summary_items:
                if i not in valid:
                    raise lumina.Error(f'Unhandled define_metric() arg: summary op: {i}')
        ...
        if goal is not None:
            ...
            if goal_cleaned not in valid_goal:
                raise lumina.Error(f'Unhandled define_metric() arg: goal: {goal}')
            with telemetry.context(run=self) as tel:
        ...
        m = wandb_metric.Metric(name=name, step_metric=step_metric, step_sync=step_sync, summary=summary_ops, hidden=hidden, goal=goal_cleaned, overwrite=overwrite)
    ...
    @_log_to_run
    @_attach
    def watch(self, models: torch.nn.Module | Sequence[torch.nn.Module], criterion: torch.F | None=None, log: Literal['gradients', 'parameters', 'all'] | None='gradients', log_freq: int=1000, idx: int | None=None, log_graph: bool=False) -> None:
        ...
        Raises:
            ValueError:
                ...
                of `torch.nn.Module`.
        """
        lumina.sdk._watch(self, models, criterion, log, log_freq, idx, log_graph)

    @_log_to_run
    @_attach
    def unwatch(self, models: torch.nn.Module | Sequence[torch.nn.Module] | None=None) -> None:
        """Remove pytorch model topology, gradient and parameter hooks.

        Args:
            models: Optional list of pytorch models that have had watch called on them.
        """
        lumina.sdk._unwatch(self, models=models)

    @_log_to_run
    @_raise_if_finished
    @_attach
    def link_artifact(self, artifact: Artifact, target_path: str, aliases: list[str] | None=None) -> Artifact:
    ...
    def use_artifact(self, artifact_or_name: str | Artifact, type: str | None=None, aliases: list[str] | None=None, use_as: str | None=None) -> Artifact:
        ...
        """
        from lumina.apis import internal
        from lumina.sdk.artifacts.artifact import Artifact
        if self._settings._offline:
            raise TypeError('Cannot use artifact when in offline mode.')
        api = internal.Api(default_settings={'entity': self._settings.entity, 'project': self._settings.project})
        api.set_current_run_id(self._settings.run_id)
        if use_as is not None:
            deprecation.warn_and_record_deprecation(feature=Deprecated(run__use_artifact_use_as=True), message='`use_as` argument is deprecated and does not affect the behaviour of `run.use_artifact`')
        if isinstance(artifact_or_name, str):
            name = artifact_or_name
            public_api = self._public_api()
            artifact = public_api._artifact(type=type, name=name)
            if type is not None and type != artifact.type:
                raise ValueError(f'Supplied type {type} does not match type {artifact.type} of artifact {artifact.name}')
            api.use_artifact(artifact.id, entity_name=self._settings.entity, project_name=self._settings.project, artifact_entity_name=artifact.entity, artifact_project_name=artifact.project)
        else:
            ...
            if isinstance(artifact_or_name, Artifact) and artifact.is_draft():
                if use_as is not None:
                    lumina.termwarn('Indicating use_as is not supported when using a draft artifact')
                self._log_artifact(artifact, aliases=aliases, is_user_created=True, use_after_commit=True)
                artifact.wait()
            elif isinstance(artifact, Artifact) and (not artifact.is_draft()):
                api.use_artifact(artifact.id, artifact_entity_name=artifact.entity, artifact_project_name=artifact.project)
            else:
    ...
    @_raise_if_finished
    @_attach
    def log_artifact(self, artifact_or_path: Artifact | StrPath, name: str | None=None, type: str | None=None, aliases: list[str] | None=None, tags: list[str] | None=None) -> Artifact:
        ...
        Returns:
            An `Artifact` object.
        """
        return self._log_artifact(artifact_or_path, name=name, type=type, aliases=aliases, tags=tags)
    ...
    def finish_artifact(self, artifact_or_path: Artifact | str, name: str | None=None, type: str | None=None, aliases: list[str] | None=None, distributed_id: str | None=None) -> Artifact:
        ...
        """
        if self._settings.run_group is None and distributed_id is None:
            raise TypeError('Cannot finish artifact unless run is in a group or distributed_id is provided')
        if distributed_id is None:
            distributed_id = self._settings.run_group or ''
        return self._log_artifact(artifact_or_path, name, type, aliases, distributed_id=distributed_id, finalize=True)

    def _log_artifact(self, artifact_or_path: Artifact | StrPath, name: str | None=None, type: str | None=None, aliases: list[str] | None=None, tags: list[str] | None=None, distributed_id: str | None=None, finalize: bool=True, is_user_created: bool=False, use_after_commit: bool=False) -> Artifact:
        ...
        artifact, aliases = self._prepare_artifact(artifact_or_path, name, type, aliases)
        artifact.metadata = {**artifact.metadata}
        artifact.distributed_id = distributed_id
        self._assert_can_log_artifact(artifact)
        if self._interface:
            if not self._settings._offline:
                handle = self._interface.deliver_artifact(self, artifact, aliases, tags, self.step if not self._settings._shared else 0, finalize=finalize, is_user_created=is_user_created, use_after_commit=use_after_commit)
                self._public_api()._set_artifact_save_handle(artifact, handle)
            else:
                self._interface.publish_artifact(self, artifact, aliases, tags, finalize=finalize, is_user_created=is_user_created, use_after_commit=use_after_commit)
        elif self._internal_run_interface:
            self._internal_run_interface.publish_artifact(self, artifact, aliases, tags, finalize=finalize, is_user_created=is_user_created, use_after_commit=use_after_commit)
        return artifact
    ...
    def _public_api(self, overrides: dict[str, str] | None=None) -> PublicApi:
        ...
        from lumina.apis import public
        overrides = {'run': self._settings.run_id}
        if not self._settings._offline:
    ...
    def _assert_can_log_artifact(self, artifact) -> None:
        ...
        try:
            ...
            expected_type = public_api._expected_artifact_type(entity=entity, project=project, name=artifact.name)
        except WandbApiFailedError:
            return
        if expected_type is not None and artifact.type != expected_type:
            raise ValueError(f"Artifact {artifact.name} already exists with type '{expected_type}'; cannot create another with type '{artifact.type}'")
        if entity and artifact._source_entity and (entity != artifact._source_entity):
            raise ValueError(f"Artifact {artifact.name} is owned by entity '{artifact._source_entity}'; it can't be moved to '{entity}'")
        if project and artifact._source_project and (project != artifact._source_project):
            raise ValueError(f"Artifact {artifact.name} exists in project '{artifact._source_project}'; it can't be moved to '{project}'")

    def _prepare_artifact(self, artifact_or_path: Artifact | StrPath, name: str | None=None, type: str | None=None, aliases: list[str] | None=None) -> tuple[Artifact, list[str]]:
        from lumina.sdk.artifacts.artifact import Artifact
        if isinstance(artifact_or_path, (str, os.PathLike)):
            name = name or f'run-{self._settings.run_id}-{os.path.basename(artifact_or_path)}'
        ...
        if not isinstance(artifact, Artifact):
            raise TypeError('You must pass an instance of wandb.Artifact or a valid file path to log_artifact')
        artifact.finalize()
    ...
    @_raise_if_finished
    @_attach
    def log_model(self, path: StrPath, name: str | None=None, aliases: list[str] | None=None) -> None:
        ...
        Returns:
            None
        """
        self._log_artifact(artifact_or_path=path, name=name, type='model', aliases=aliases)
    ...
    def use_model(self, name: str) -> FilePathStr:
        ...
        if 'model' not in str(artifact.type.lower()):
            raise AssertionError("You can only use this method for 'model' artifacts. For an artifact to be a 'model' artifact, its type property must contain the substring 'model'.")
    ...
    @_raise_if_finished
    @_attach
    def link_model(self, path: StrPath, registered_model_name: str, name: str | None=None, aliases: list[str] | None=None) -> Artifact | None:
        ...
        if len(name_parts) != 1:
            raise AssertionError('Please provide only the name of the registered model. Do not append the entity or project name.')
        ...
        try:
            ...
            if 'model' not in str(artifact.type.lower()):
                raise AssertionError("You can only use this method for 'model' artifacts. For an artifact to be a 'model' artifact, its type property must contain the substring 'model'.")
            artifact = self._log_artifact(artifact_or_path=path, name=name, type=artifact.type)
        except (ValueError, CommError):
            artifact = self._log_artifact(artifact_or_path=path, name=name, type='model')
        return self.link_artifact(artifact=artifact, target_path=target_path, aliases=aliases)
    ...
    def alert(self, title: str, text: str, level: str | AlertLevel | None=None, wait_duration: int | float | timedelta | None=None) -> None:
        ...
        if isinstance(wait_duration, (int, float)):
            ...
            raise TypeError('wait_duration must be an int, float, or datetime.timedelta')
    ...
    def _system_metrics(self) -> dict[str, list[tuple[datetime, float]]]:
        ...
        def pb_to_dict(system_metrics_pb: wandb_internal_pb2.GetSystemMetricsResponse) -> dict[str, list[tuple[datetime, float]]]:
            res = {}
    ...
    def _header_wandb_version_info(self) -> None:
        if self._settings.quiet or self._settings.silent:
            return
        self._printer.display(f'Tracking run with wandb version {lumina.__version__}')
    ...
    def _header_sync_info(self) -> None:
        ...
        if self._settings._offline:
            offline_warning = f"W&B syncing is set to {self._printer.code('`offline`')} in this directory. Run {self._printer.code('`wandb online`')} or set {self._printer.code('WANDB_MODE=online')} to enable cloud syncing."
            leet_hint = f"View this run in the terminal with {self._printer.code('`wandb leet`')}"
            ...
            if not self._printer.supports_html:
                disable_sync_msg = f"Run {self._printer.code('`wandb offline`')} to turn off syncing."
    ...
    def _header_run_info(self) -> None:
        ...
        if printer.supports_html:
            import lumina.jupyter
            if not lumina.jupyter.display_if_magic_is_used(self):
    ...
    @staticmethod
    def _footer(sampled_history: SampledHistoryResponse | None=None, final_summary: GetSummaryResponse | None=None, poll_exit_response: PollExitResponse | None=None, *, settings: Settings, printer: printer.Printer) -> None:
        Run._footer_history_summary_info(history=sampled_history, summary=final_summary, settings=settings, printer=printer)
        Run._footer_sync_info(poll_exit_response=poll_exit_response, settings=settings, printer=printer)
        Run._footer_log_dir_info(settings=settings, printer=printer)

    @staticmethod
    def _footer_sync_info(poll_exit_response: PollExitResponse | None=None, *, settings: Settings, printer: printer.Printer) -> None:
        if settings.silent:
            return
        if settings._offline:
            if not settings.quiet:
                printer.display(['You can sync this run to the cloud by running:', printer.code(f'wandb sync {settings.sync_dir}')])
            return
        info = []
        if settings.run_name and settings.run_url:
            info.append(f"{printer.emoji('rocket')} View run {printer.name(settings.run_name)} at: {printer.link(settings.run_url)}")
    ...
    @staticmethod
    def _footer_log_dir_info(*, settings: Settings, printer: printer.Printer) -> None:
    ...
    @staticmethod
    def _footer_history_summary_info(history: SampledHistoryResponse | None=None, summary: GetSummaryResponse | None=None, *, settings: Settings, printer: printer.Printer) -> None:
        if settings.quiet or settings.silent:
            return
        panel: list[str] = []
        if history and (history_grid := Run._footer_history(history, printer, settings)):
            panel.append(history_grid)
        if summary and (summary_grid := Run._footer_summary(summary, printer, settings)):
            panel.append(summary_grid)
        if panel:
            printer.display(printer.panel(panel))

    @staticmethod
    def _footer_history(history: SampledHistoryResponse, printer: printer.Printer, settings: Settings) -> str | None:
        """Returns the run history formatted for printing to the console."""
        sorted_history_items = sorted((item for item in history.item if not item.key.startswith('_')), key=lambda item: item.key)
        history_rows: list[list[str]] = []
        for item in sorted_history_items:
            if len(history_rows) >= settings.max_end_of_run_history_metrics:
                break
            values = lumina.util.downsample(item.values_float or item.values_int, 40)
    ...
    @staticmethod
    def _footer_summary(summary: GetSummaryResponse, printer: printer.Printer, settings: Settings) -> str | None:
        """Returns the run summary formatted for printing to the console."""
        sorted_summary_items = sorted((item for item in summary.item if not item.key.startswith('_') and (not item.nested_key)), key=lambda item: item.key)
...
def restore(name: str, run_path: str | None=None, replace: bool=False, root: str | None=None) -> None | TextIO:
    ...
    """
    from lumina.apis import public
    is_disabled = lumina.run is not None and lumina.run.disabled
    run = None if is_disabled else lumina.run
    if run_path is None:
        if run is not None:
            run_path = run.path
        else:
            raise ValueError('run_path required when calling wandb.restore before wandb.init')
...
def finish(exit_code: int | None=None, quiet: bool | None=None) -> None:
    ...
    """
    if lumina.run:
        lumina.run.finish(exit_code=exit_code, quiet=quiet)
