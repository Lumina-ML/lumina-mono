...
class MailboxResponseHandle(MailboxHandle[spb.ServerResponse]):
    ...
    def __init__(self, address: str, *, asyncer: asyncio_manager.AsyncioManager, cancel: Callable[[str], Awaitable[None]]) -> None:
    ...
    async def deliver(self, response: spb.ServerResponse) -> None:
        if self._abandoned:
            return
        if self._response:
            raise ValueError(f'A response has already been delivered to {self._address}.')
    ...
    def cancel(self) -> None:
        ...
        try:
            ...
            _logger.exception('Failed to abandon and cancel request %r', self._address)
    ...
    async def wait_async(self, *, timeout: float | None) -> spb.ServerResponse:
        ...
        try:
            ...
            if (response := self._response_or_error()):
                ...
                self.cancel()
                raise TimeoutError(f'Timed out waiting for response on {self._address}') from e
        except:
            self.cancel()
            raise
        else:
