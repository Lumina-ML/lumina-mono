...
class Mailbox:
    ...
    def __init__(self, asyncer: asyncio_manager.AsyncioManager, cancel: Callable[[str], Awaitable[None]]) -> None:
        """Create a mailbox.
    ...
    def require_response(self, request: spb.ServerRequest | pb.Record) -> MailboxHandle[spb.ServerResponse]:
        ...
        if isinstance(request, spb.ServerRequest):
            if (address := request.request_id) or (address := request.record_publish.control.mailbox_slot):
        ...
        with self._handles_lock:
            ...
            handle = MailboxResponseHandle(address, asyncer=self._asyncer, cancel=self._cancel)
    ...
    def close(self) -> None:
        ...
        with self._handles_lock:
            self._closed = True
            _logger.info(f'Closing mailbox, abandoning {len(self._handles)} handles.')
