...
def wait_with_progress(handle: MailboxHandle[_T], *, timeout: float | None, display_progress: Callable[[], Coroutine[Any, Any, None]]) -> _T:
    ...
    """
    return wait_all_with_progress([handle], timeout=timeout, display_progress=display_progress)[0]

def wait_all_with_progress(handle_list: list[MailboxHandle[_T]], *, timeout: float | None, display_progress: Callable[[], Coroutine[Any, Any, None]]) -> list[_T]:
    ...
    async def progress_loop_with_timeout() -> list[_T]:
        async with asyncio_compat.cancel_on_exit(display_progress()):
            ...
            return await _wait_handles_async(handle_list, timeout=remaining_timeout)
    return asyncer.run(progress_loop_with_timeout)

async def _wait_handles_async(handle_list: list[MailboxHandle[_T]], *, timeout: float | None) -> list[_T]:
