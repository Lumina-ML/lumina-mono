...
def _watch(run: lumina.Run, models: torch.nn.Module | Sequence[torch.nn.Module], criterion: torch.F | None=None, log: Literal['gradients', 'parameters', 'all'] | None='gradients', log_freq: int=1000, idx: int | None=None, log_graph: bool=False):
    ...
    if not isinstance(models, (tuple, list)):
        models = (models,)
    torch = lumina.util.get_module('torch', required="wandb.watch only works with pytorch, couldn't import torch.")
    for model in models:
        if not isinstance(model, torch.nn.Module):
            raise TypeError(f'Expected a pytorch model (torch.nn.Module). Received {type(model)}')
    graphs = []
    prefix = ''
    if idx is None:
    ...
    for local_idx, model in enumerate(models):
        ...
        if log_parameters:
            run._torch.add_log_parameters_hook(model, prefix=prefix, log_freq=log_freq)
        if log_gradients:
            run._torch.add_log_gradients_hook(model, prefix=prefix, log_freq=log_freq)
        if log_graph:
    ...
    if models:
        ...
        for model in models:
            if not hasattr(model, '_wandb_hook_names'):
                lumina.termwarn(f'{model} model has not been watched')
            else:
