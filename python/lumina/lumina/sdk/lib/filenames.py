...
def is_wandb_file(name: str) -> bool:
    return name in (METADATA_FNAME, CONFIG_FNAME, REQUIREMENTS_FNAME, OUTPUT_FNAME, DIFF_FNAME, CONDA_ENVIRONMENTS_FNAME) or name.startswith('wandb')

def filtered_dir(root: str, include_fn: Callable[[str, str], bool] | Callable[[str], bool], exclude_fn: Callable[[str, str], bool] | Callable[[str], bool]) -> Generator[str, None, None]:
