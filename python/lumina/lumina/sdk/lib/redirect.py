...
import lumina
from lumina.sdk.lib import console_capture
...
ANSI_BG_DEFAULT = '49'
ANSI_RESET = '0'
ANSI_STYLES = {'1': 'bold', '2': '/bold', '3': 'italics', '4': 'underscore', '5': 'blink', '7': 'reverse', '9': 'strikethrough', '22': '/bold', '23': '/italics', '24': '/underscore', '25': '/blink', '27': '/reverse', '29': '/strikethrough'}
...
class TerminalEmulator:
    ...
    def display(self):
        return [[self.buffer[i][j].data for j in range(self._get_line_len(i))] for i in range(self.num_lines)]
    ...
    def read(self):
        ...
        return ret
_MIN_CALLBACK_INTERVAL = 2

class RedirectBase:

    def __init__(self, src: Literal['stdout', 'stderr'], cbs: Iterable[Callable[[str], None]]=()) -> None:
        """# Arguments.
...
class StreamWrapper(RedirectBase):
    ...
    def __init__(self, src: Literal['stdout', 'stderr'], cbs: Iterable[Callable[[str], None]]=(), *, flush_periodically: bool) -> None:
    ...
    def _emulator_write(self) -> None:
        while True:
            ...
            if self._stopped.is_set() and sum(map(len, data)) > 100000:
                lumina.termlog('Terminal output too large. Logging without processing.')
                self.flush()
                for line in data:
                    self.flush(line)
                return
    ...
    def install(self) -> None:
        ...
        try:
            ...
            lumina.termwarn(f'Failed to wrap {self.src}. Console logs will not be captured.')
            return
    ...
    def uninstall(self) -> None:
        ...
        if self._emulator_write_thread.is_alive():
            lumina.termlog(f'Processing terminal output ({self.src})...')
