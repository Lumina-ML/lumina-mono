...
from lumina.sdk.lib import urls
...
class HostUrl:
    ...
    def __init__(self, url: str, *, app_url: str | None=None) -> None:
        ...
        if re.match('.*wandb\\.ai[^\\.]*$', url):
            if 'api.' not in url:
                raise ValueError(f'{url!r} is not a valid server address,' + ' did you mean https://api.wandb.ai?')
            elif not url.startswith('https'):
                raise ValueError('http is not secure, please use https://api.wandb.ai')
        self._url = url.rstrip('/')
        self._app_url = (app_url or env.get_app_url() or util.api_to_app_url(self._url)).rstrip('/')
