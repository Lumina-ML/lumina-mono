...
_LOGIN_CHOICES = [_LOGIN_CHOICE_NEW, _LOGIN_CHOICE_EXISTS, _LOGIN_CHOICE_OFFLINE]

def prompt_and_save_api_key(*, host: str | HostUrl, no_offline: bool=False, no_create: bool=False, referrer: str='', input_timeout: float | None=None) -> str | None:
    ...
    if not isinstance(host, HostUrl):
        host = HostUrl(host)
    api_key = _prompt_api_key(host=host, no_offline=no_offline, no_create=no_create, referrer=referrer, input_timeout=input_timeout)
    if not api_key:
        return None
    wbnetrc.write_netrc_auth(host=host.url, api_key=api_key)
    return api_key

def _prompt_api_key(*, host: HostUrl, no_offline: bool=False, no_create: bool=False, referrer: str='', input_timeout: float | None=None) -> str | None:
...
def _use_existing_account(host: HostUrl, referrer: str) -> str:
    ...
    if saas.is_wandb_domain(host.url):
        ...
        term.termlog(f'Logging into {host}. ' + f'(Learn how to deploy a W&B server locally: {help_url})')
    ...
    return term.terminput('Paste your API key and hit enter: ', hide=True)
