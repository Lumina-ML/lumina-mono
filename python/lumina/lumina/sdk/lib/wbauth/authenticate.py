...
from lumina.errors import AuthenticationError, UsageError, term
...
def _locked_set_session_auth(auth: Auth | None, *, update_settings: bool=True) -> None:
...
def authenticate_session(*, host: str | HostUrl, source: str, no_offline: bool=False, no_create: bool=False, input_timeout: float | None=None, referrer: str='models', relogin: bool=False) -> Auth | None:
    ...
    if not relogin and (auth := _use_system_auth(host=host, source=source)):
        return auth
    try:
        return _use_prompted_auth(host=host, no_offline=no_offline, no_create=no_create, referrer=referrer, input_timeout=input_timeout)
    except term.NotATerminalError:
        raise UsageError('No API key configured. Use `wandb login` to log in.') from None
...
def use_explicit_auth(auth: Auth, *, source: str) -> None:
    ...
    with _session_auth_lock:
        if _session_auth == auth:
            return
        if _session_auth:
            term.termwarn(f'[{source}] Changing session credentials to explicit value' + f' for {auth.host}.')
        else:
            term.termlog(f'[{source}] Using explicit session credentials for {auth.host}.')
...
def _use_system_auth(*, host: HostUrl, source: str) -> Auth | None:
    ...
    """
    auth = _try_env_auth(host=host) or wbnetrc.read_netrc_auth_with_source(host=host)
    with _session_auth_lock:
        if auth:
            term.termlog(f'[{source}] Loaded credentials for {auth.auth.host}' + f' from {auth.source}.')
...
def _try_env_auth(*, host: HostUrl) -> AuthWithSource | None:
    ...
    if api_key and identity_token_file:
        raise AuthenticationError(f'Both {env.API_KEY} and {env.IDENTITY_TOKEN_FILE} are set,' + ' which is not allowed.')
    if api_key:
        try:
            return AuthWithSource(auth=AuthApiKey(host=host, api_key=api_key), source=env.API_KEY)
        except AuthenticationError as e:
            raise AuthenticationError(f'{env.API_KEY} invalid: {e}') from None
    elif identity_token_file:
        return AuthWithSource(auth=AuthIdentityTokenFile(host=host, path=identity_token_file, credentials_file=wandb_setup.singleton().settings.credentials_file), source=env.IDENTITY_TOKEN_FILE)
    return None

def _use_prompted_auth(*, host: HostUrl, no_offline: bool, no_create: bool, referrer: str, input_timeout: float | None=None) -> Auth | None:
    ...
    """
    api_key = prompt.prompt_and_save_api_key(host=host, no_offline=no_offline, no_create=no_create, referrer=referrer, input_timeout=input_timeout)
    with _session_auth_lock:
        if api_key:
