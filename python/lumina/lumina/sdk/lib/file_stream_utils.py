...
def split_files(files: dict[str, Any], max_bytes: int=10 * 1024 * 1024) -> Iterable[dict[str, dict]]:
    ...
    for k, v in files.items():
        if isinstance(v, list):
            for item in v:
                files_stack.append({'name': k, 'offset': item['offset'], 'content': item['content']})
        else:
            files_stack.append({'name': k, 'offset': v['offset'], 'content': v['content']})
    while files_stack:
        f = files_stack.pop()
        ...
        if fsize <= rem:
            current_volume[f['name']] = {'offset': f['offset'], 'content': f['content']}
            current_size += fsize
        else:
            ...
            if num_lines:
                ...
                current_volume[f1['name']] = {'offset': f1['offset'], 'content': f1['content']}
