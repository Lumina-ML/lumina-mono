...
from lumina.sdk.lib import asyncio_compat
from . import printer as p
_INDENT = '  '
_MAX_LINES_TO_PRINT = 6
_MAX_OPS_TO_PRINT = 5

async def loop_printing_operation_stats(progress: ProgressPrinter, interface: interface.InterfaceBase) -> None:
    ...
    async def loop_poll_stats() -> NoReturn:
        ...
        while True:
            ...
            handle = await interface.deliver_async(pb.Record(request=pb.Request(operations=pb.OperationStatsRequest())))
...
def progress_printer(printer: p.Printer, default_text: str) -> Generator[ProgressPrinter]:
    ...
    with printer.dynamic_text() as text_area:
        try:
            yield ProgressPrinter(printer, text_area, default_text=default_text)
        finally:
...
class ProgressPrinter:
    ...
    def __init__(self, printer: p.Printer, progress_text_area: p.DynamicText | None, default_text: str) -> None:
    ...
    def update(self, stats_or_list: pb.OperationStats | list[pb.OperationStats]) -> None:
    ...
    def _to_dynamic_text(self, stats_or_list: pb.OperationStats | list[pb.OperationStats]) -> list[str]:
        """Returns text to show in a dynamic text area."""
        loading_symbol = self._printer.loading_symbol(self._tick)
        if isinstance(stats_or_list, list):
            return _GroupedOperationStatsPrinter(self._printer, _MAX_LINES_TO_PRINT, loading_symbol).render(stats_or_list)
        else:
            return _OperationStatsPrinter(self._printer, _MAX_LINES_TO_PRINT, loading_symbol).render(stats_or_list)

    def _to_static_text(self, stats_or_list: pb.OperationStats | list[pb.OperationStats]) -> str:
        """Returns a single line of text to print out."""
        if isinstance(stats_or_list, list):
            sorted_prefixed_stats = list(((f'[{stats.label}] ' if stats.label else '', stats) for stats in sorted(stats_or_list, key=lambda s: s.label)))
        else:
...
class _GroupedOperationStatsPrinter:
    ...
    def __init__(self, printer: p.Printer, max_lines: int, loading_symbol: str) -> None:
        self._printer = printer
    ...
    def render(self, stats_list: list[pb.OperationStats]) -> list[str]:
        ...
        for stats in sorted(stats_list, key=lambda s: s.label):
            ...
            stats_lines = _OperationStatsPrinter(printer=self._printer, max_lines=remaining_non_header_lines, loading_symbol=self._loading_symbol).render(stats)
...
class _OperationStatsPrinter:
    ...
    def _add_operation(self, op: pb.Operation, is_subtask: bool, indent: str) -> None:
        ...
        if op.error_status:
            ...
            self._lines.append(f'{indent}{status_indent}{error_word} {error_desc}')
        if op.subtasks:
            subtask_indent = indent + _INDENT
            for task in op.subtasks:
                self._add_operation(task, is_subtask=True, indent=subtask_indent)
