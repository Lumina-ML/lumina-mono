...
from lumina.util import load_yaml
...
def dict_from_config_file(filename: str, must_exist: bool=False) -> dict[str, Any] | None:
    import yaml
    ...
    if loaded is None:
        lumina.termwarn('Found an empty default config file (config-defaults.yaml). Proceeding with no defaults.')
        return None
