...
from lumina.util import CheckRetryFnType
logger = logging.getLogger(__name__)
NOW_FN = datetime.datetime.now
SLEEP_FN = time.sleep
SLEEP_ASYNC_FN = asyncio.sleep

class RetryCancelledError(lumina.errors.Error):
...
class TransientError(Exception):
    ...
    def __init__(self, msg: str | None=None, exc: BaseException | None=None) -> None:
        super().__init__(msg)
        self.message = msg
        self.exception = exc
_R = TypeVar('_R')
...
class Retry(Generic[_R]):
    ...
    def _sleep_check_cancelled(self, wait_seconds: float, cancel_event: threading.Event | None) -> bool:
        if not cancel_event:
    ...
    def __call__(self, *args: Any, num_retries: int | None=None, retry_timedelta: datetime.timedelta | None=None, retry_sleep_base: float | None=None, retry_cancel_event: threading.Event | None=None, check_retry_fn: CheckRetryFnType | None=None, **kwargs: Any) -> _R:
        ...
        if retry_sleep_base is not None:
            ...
            initial_sleep = 1
        retry_loop = _RetryLoop(max_retries=max_retries, timeout=timeout, initial_sleep=initial_sleep, max_sleep=self.MAX_SLEEP_SECONDS, cancel_event=retry_cancel_event or self._retry_cancel_event, retry_check=check_retry_fn or self._check_retry_fn)
        start_time = NOW_FN()
        self._num_iter = 0
        while True:
            try:
    ...
    def _print_entered_retry_loop(self, exception: Exception) -> None:
        ...
        if isinstance(exception, HTTPError) and exception.response is not None and (self.retry_callback is not None):
            self.retry_callback(exception.response.status_code, exception.response.text)
        else:
            lumina.termlog(f'{self._error_prefix}' + f' ({exception.__class__.__name__}), entering retry loop.')
    ...
    def _print_recovered(self, start_time: datetime.datetime) -> None:
        ...
        self.retry_callback(200, f'{self._error_prefix} resolved after {time_to_recover}, resuming normal operation.')

class _RetryLoop:
    """An invocation of a Retry instance."""

    def __init__(self, *, max_retries: int, timeout: datetime.timedelta, initial_sleep: float, max_sleep: float, cancel_event: threading.Event | None, retry_check: CheckRetryFnType) -> None:
...
class ExponentialBackoff(Backoff):
    ...
    def __init__(self, initial_sleep: datetime.timedelta, max_sleep: datetime.timedelta, max_retries: int | None=None, timeout_at: datetime.datetime | None=None) -> None:
...
async def retry_async(backoff: Backoff, fn: Callable[..., Awaitable[_R]], *args: Any, on_exc: Callable[[Exception], None] | None=None, **kwargs: Any) -> _R:
