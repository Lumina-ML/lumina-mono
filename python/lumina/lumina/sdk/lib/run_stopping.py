...
class RunStopChecker:
    ...
    def __init__(self, asyncer: asyncio_manager.AsyncioManager, interface: InterfaceBase, stop_fn: Callable[[], None], *, poll_interval: float=_POLL_INTERVAL) -> None:
        self._asyncer = asyncer

        async def async_init() -> _RunStopCheckerImpl:
            return _RunStopCheckerImpl(interface, stop_fn, poll_interval=poll_interval)
        self._impl = asyncer.run(async_init)

    def start(self) -> None:
        """Start checking for whether the run is stopped."""
        self._asyncer.run_soon(self._impl.loop, daemon=True, name='RunStopChecker.loop')
...
class _RunStopCheckerImpl:
    ...
    def __init__(self, interface: InterfaceBase, stop_fn: Callable[[], None], *, poll_interval: float) -> None:
    ...
    async def loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if remaining > 0:
                await asyncio_compat.race(self._stop_event.wait(), _SLEEP(remaining))

    async def _check_should_stop(self) -> bool:
        """Check whether the run should stop."""
        if pyagent.is_running():
            return False
        request = pb.Record(request=pb.Request(stop_status=pb.StopStatusRequest()))
