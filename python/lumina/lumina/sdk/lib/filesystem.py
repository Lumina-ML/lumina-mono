...
class LinkStats:
    ...
    def record(self, mode: Literal['symlink', 'hardlink', 'copy'], downgraded: bool=False) -> None:
    ...
    def emit_warnings(self) -> None:
        ...
        if self.n_symlink:
            termwarn(f"Symlinked {self.n_symlink} file{('s' if self.n_symlink > 1 else '')} into the W&B run directory; call wandb.save again to sync new files.")
        if self.n_hardlink:
            termwarn(f"Linked {self.n_hardlink} file{('s' if self.n_hardlink > 1 else '')} into the W&B run directory (hardlinks); call wandb.save again to sync new files.")
        if self.n_copy:
            if self.n_copy_downgraded:
                termwarn(f"Copied {self.n_copy} file{('s' if self.n_copy > 1 else '')} (cross-volume or links unavailable). Downgrading policy to 'now' for those files because live updates won't propagate from the originals. Re-run wandb.save to resync, or place your run directory on the same drive to enable hardlinks.")
            else:
                termwarn(f"Copied {self.n_copy} file{('s' if self.n_copy > 1 else '')} into the W&B run directory; call wandb.save again to sync new files.")
...
def link_or_copy(settings: Settings, src: Path, dst: Path) -> Literal['symlink', 'hardlink', 'copy']:
...
def link_or_copy_with_policy(settings: Settings, src: Path, dst: Path, requested_policy: PolicyName, stats: LinkStats) -> PolicyName:
    """Link or copy a file using the best available method.
