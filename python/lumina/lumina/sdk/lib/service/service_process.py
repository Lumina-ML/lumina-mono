...
from lumina.env import core_debug, dcgm_profiling_enabled, error_reporting_enabled
from lumina.errors import WandbCoreNotAvailableError
from lumina.sdk.lib.service import ipc_support
...
def start(settings: Settings) -> ServiceProcess:
    ...
    """
    return _start(settings, detached=False, idle_timeout=None)

def start_detached(settings: Settings, *, idle_timeout: str=DEFAULT_DETACHED_IDLE_TIMEOUT) -> ServiceProcess:
    ...
    Returns:
        A handle to the process.
    """
    return _start(settings, detached=True, idle_timeout=idle_timeout)

def _start(settings: Settings, *, detached: bool, idle_timeout: str | None) -> ServiceProcess:
    get_sentry().configure_scope(tags=dict(settings), process_context='service')
    try:
        return _launch_server(settings, detached=detached, idle_timeout=idle_timeout)
...
class ServiceProcess:
    ...
    def __init__(self, *, connection_token: service_token.ServiceToken, process: subprocess.Popen) -> None:
...
def _launch_server(settings: Settings, *, detached: bool, idle_timeout: str | None) -> ServiceProcess:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        proc = subprocess.Popen(service_args, env=os.environ, close_fds=True, creationflags=creationflags, start_new_session=start_new_session, stdin=subprocess.DEVNULL if detached else None, stdout=subprocess.DEVNULL if detached else None, stderr=subprocess.DEVNULL if detached else None)
        token = service_port_file.poll_for_token(port_file, proc, timeout=settings.x_service_wait)
