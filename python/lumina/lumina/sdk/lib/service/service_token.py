...
_CURRENT_VERSION = '3'
_UNIX_TOKEN_RE = re.compile(f'{_CURRENT_VERSION}-(\\d+)-unix-(.+)')
_TCP_TOKEN_RE = re.compile(f'{_CURRENT_VERSION}-(\\d+)-tcp-localhost-(\\d+)')
...
class ServiceToken(abc.ABC):
    ...
    @abc.abstractmethod
    def connect(self, *, asyncer: asyncio_manager.AsyncioManager) -> ServiceClient:
...
class UnixServiceToken(ServiceToken):
    ...
    def connect(self, *, asyncer: asyncio_manager.AsyncioManager) -> ServiceClient:
        ...
        try:
            reader, writer = asyncer.run(lambda: asyncio.open_unix_connection(self._path))
        except Exception as e:
            raise WandbServiceConnectionError(f'Failed to connect to service on socket {self._path}') from e
        return ServiceClient(asyncer, reader, writer)

    @override
    def _as_env_string(self) -> str:
        return '-'.join((_CURRENT_VERSION, str(self._parent_pid), 'unix', str(self._path)))
...
class TCPServiceToken(ServiceToken):
    ...
    def connect(self, *, asyncer: asyncio_manager.AsyncioManager) -> ServiceClient:
        try:
            reader, writer = asyncer.run(lambda: asyncio.open_connection('localhost', self._port))
        except Exception as e:
            raise WandbServiceConnectionError(f'Failed to connect to service on port {self._port}') from e
        return ServiceClient(asyncer, reader, writer)

    @override
    def _as_env_string(self) -> str:
        return '-'.join((_CURRENT_VERSION, str(self._parent_pid), 'tcp', 'localhost', str(self._port)))
