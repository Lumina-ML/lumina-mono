...
from lumina.sdk.mailbox.mailbox import Mailbox
...
class ServiceClient:
    ...
    def __init__(self, asyncer: asyncio_manager.AsyncioManager, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        asyncer.run_soon(self._forward_responses, daemon=True, name='ServiceClient._forward_responses')

    async def publish(self, request: spb.ServerRequest) -> None:
        """Send a request without waiting for a response."""
        await self._send_server_request(request)

    async def deliver(self, request: spb.ServerRequest) -> MailboxHandle[spb.ServerResponse]:
    ...
    async def _cancel_request(self, id: str, /) -> None:
        ...
        """
        await self.publish(spb.ServerRequest(cancel=spb.ServerCancelRequest(request_id=id)))
