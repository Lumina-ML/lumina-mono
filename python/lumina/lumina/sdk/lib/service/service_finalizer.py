...
from lumina.sdk.lib.service.service_client import ServiceClient
...
class ServiceFinalizer:
    ...
    def __init__(self, asyncer: asyncio_manager.AsyncioManager, client: ServiceClient) -> None:
        self._asyncer = asyncer
        self._client = client
        self._queue = queue.SimpleQueue[spb.ServerRequest | None]()
        self._thread = threading.Thread(name='wandb-ServiceFinalizer', target=self._publish_queue, daemon=True)
        self._thread.start()
