...
from lumina.proto import wandb_api_pb2, wandb_settings_pb2, wandb_sync_pb2
from lumina.proto import wandb_server_pb2 as spb
from lumina.sdk import wandb_settings
from lumina.sdk.interface.interface import InterfaceBase
from lumina.sdk.interface.interface_sock import InterfaceSock
from lumina.sdk.lib import asyncio_manager
from lumina.sdk.lib.exit_hooks import ExitHooks
from lumina.sdk.mailbox import HandleAbandonedError, MailboxClosedError
from lumina.sdk.mailbox.mailbox_handle import MailboxHandle
...
class WandbApiFailedError(Exception):
    ...
    def __init__(self, message: str, response: wandb_api_pb2.ApiErrorResponse | None=None):
        super().__init__(message)
        self.response = response

def connect_to_service(asyncer: asyncio_manager.AsyncioManager, settings: wandb_settings.Settings) -> ServiceConnection:
...
def _start_and_connect_service(asyncer: asyncio_manager.AsyncioManager, settings: wandb_settings.Settings) -> ServiceConnection:
    ...
    conn = ServiceConnection(asyncer=asyncer, client=client, proc=proc, cleanup=lambda: atexit.unregister(teardown_atexit))
...
class ServiceConnection:
    ...
    def __init__(self, asyncer: asyncio_manager.AsyncioManager, client: ServiceClient, proc: service_process.ServiceProcess | None, cleanup: Callable[[], None] | None=None):
    ...
    async def init_sync(self, paths: set[pathlib.Path], settings: wandb_settings.Settings, *, cwd: pathlib.Path | None, live: bool, entity: str, project: str, run_id: str, job_type: str, tag_replacements: dict[str, str]) -> MailboxHandle[wandb_sync_pb2.ServerInitSyncResponse]:
        """Send a ServerInitSyncRequest."""
        init_sync = wandb_sync_pb2.ServerInitSyncRequest(path=(str(path) for path in paths), cwd=str(cwd) if cwd else '', live=live, settings=settings.to_proto(), new_entity=entity, new_project=project, new_run_id=run_id, new_job_type=job_type, tag_replacements=tag_replacements)
    ...
    async def sync(self, id: str, *, parallelism: int) -> MailboxHandle[wandb_sync_pb2.ServerSyncResponse]:
    ...
    def api_init_request(self, settings: wandb_settings_pb2.Settings) -> wandb_api_pb2.ServerApiInitResponse:
        ...
        try:
            ...
            raise WandbApiFailedError('Failed to initialize API resources:' + ' the service process is not running.') from None
        except TimeoutError:
            raise WandbApiFailedError('Failed to initialize API resources:' + ' the service process is busy and did not respond in time.') from None
    ...
    def finalize(self, obj: object, cleanup: spb.ServerRequest) -> Callable[[], None]:
    ...
    async def sync_status(self, id: str) -> MailboxHandle[wandb_sync_pb2.ServerSyncStatusResponse]:
    ...
    async def api_request_async(self, api_request: wandb_api_pb2.ApiRequest) -> MailboxHandle[wandb_api_pb2.ApiResponse]:
    ...
    def api_request(self, api_request: wandb_api_pb2.ApiRequest, timeout: float | None=None) -> wandb_api_pb2.ApiResponse:
        ...
        try:
            ...
            raise WandbApiFailedError('Failed to execute API request:' + ' the service process is not running.') from None
        except TimeoutError:
            raise WandbApiFailedError('Failed to execute API request:' + ' the service process is busy and did not respond in time.') from None
        if response.HasField('api_error_response'):
            raise WandbApiFailedError(response.api_error_response.message, response.api_error_response)
        return response
    ...
    def inform_init(self, settings: wandb_settings_pb2.Settings, run_id: str) -> InterfaceBase:
        ...
        return InterfaceSock(self._asyncer, self._client, stream_id=run_id)

    def inform_finish(self, run_id: str) -> None:
        """Send an finish request to the service."""
        request = spb.ServerInformFinishRequest()
        request._info.stream_id = run_id
        self._asyncer.run(lambda: self._client.publish(spb.ServerRequest(inform_finish=request)))

    def inform_attach(self, attach_id: str) -> tuple[wandb_settings_pb2.Settings, InterfaceBase]:
        ...
        try:
            ...
            raise WandbAttachFailedError('Failed to attach: the service process is not running.') from None
        except TimeoutError:
            raise WandbAttachFailedError('Failed to attach because the run does not belong to' + ' the current service process, or because the service' + ' process is busy (unlikely).') from None
        settings = response.inform_attach_response.settings
        interface = InterfaceSock(self._asyncer, self._client, stream_id=attach_id)
    ...
    def teardown(self, exit_code: int) -> int | None:
        ...
        async def publish_teardown_and_close() -> None:
            await self._client.publish(spb.ServerRequest(inform_teardown=spb.ServerInformTeardownRequest(exit_code=exit_code)))
