...
class ServicePollForTokenError(lumina.Error):
    """Failed to discover how to connect to the service."""

def poll_for_token(file: pathlib.Path, proc: subprocess.Popen, *, timeout: float) -> service_token.ServiceToken:
    ...
    while _MONOTONIC() < end_time:
        if (code := proc.poll()) is not None:
            raise ServicePollForTokenError(f'wandb-core exited with code {code}', context={'command': proc.args, 'proc_out': proc.stdout.read() if proc.stdout else '', 'proc_err': proc.stderr.read() if proc.stderr else ''})
        if (token := _poll_once(file)):
            return token
        _SLEEP(max(0, min(0.2, end_time - _MONOTONIC())))
    raise ServicePollForTokenError(f'Failed to read port info after {timeout} seconds' + f' (wandb-core PID={proc.pid}): {file}')
_UNIX_NAME_RE = re.compile('unix=(.+)')
_TCP_PORT_RE = re.compile('sock=(\\d+)')
...
def _poll_once(file: pathlib.Path) -> service_token.ServiceToken | None:
    ...
    for line in lines:
        if ipc_support.SUPPORTS_UNIX and (match := _UNIX_NAME_RE.fullmatch(line)):
            return service_token.UnixServiceToken(parent_pid=os.getpid(), path=match.group(1))
        elif (match := _TCP_PORT_RE.fullmatch(line)):
            return service_token.TCPServiceToken(parent_pid=os.getpid(), port=int(match.group(1)))
    raise ServicePollForTokenError(f'No known connection method in {file}:\n{text}')
