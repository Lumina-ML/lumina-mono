...
def run_git(*args: str, cwd: str | None=None, executable: str | None=None) -> str:
    ...
    try:
        proc = subprocess.run([git, *args], cwd=cwd, env=env, check=True, capture_output=True)
    except subprocess.CalledProcessError as e:
        stderr = e.stderr.decode(errors='replace')
        raise GitCommandError(f"git {' '.join(args)} exited with {e.returncode}: {stderr}") from e
    except OSError as e:
...
class GitRepo:
    ...
    def __init__(self, root: str | None=None, remote: str | None='origin', lazy: bool=True, remote_url: str | None=None, commit: str | None=None, _git_executable: str | None=None) -> None:
        self.remote_name = remote if remote_url is None else None
        self._root = root
        self._remote_url = remote_url
        self._commit = commit
        self._git_executable = _git_executable or os.environ.get(GIT_EXECUTABLE_ENV) or 'git'
    ...
    def _find_root_dir(self) -> str | None:
        ...
        try:
            ...
            lumina.termwarn('current working directory has been invalidated')
        ...
        try:
            ...
            if shutil.which(self._git_executable) is None:
                ...
                lumina.termwarn(f'git root {self._root} does not exist')
    ...
    def untracked_files(self, pathspec: str) -> list[str]:
        output = self.run_git('ls-files', '--others', '--exclude-standard', '--', pathspec)
        return [path for path in output.splitlines() if path]

    def has_tracked_changes(self) -> bool:
        return bool(self.run_git('status', '--porcelain', '--untracked-files=no').strip())
    ...
    def git_version(self) -> GitVersion | None:
        ...
        if match is None:
            return None
        return GitVersion(major=int(match.group(1)), minor=int(match.group(2)), patch=int(match.group(3)))
    ...
    def tracking_branches(self) -> list[str]:
        output = self.run_git('for-each-ref', '--format=%(upstream:short)', 'refs/heads/')
        ...
        try:
            ...
            for possible_relative in possible_relatives:
                try:
                    ...
                    continue
                if most_recent_ancestor is None or self.is_ancestor(most_recent_ancestor, ancestor):
