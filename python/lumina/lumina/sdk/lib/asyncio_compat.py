...
class CancellableRunner:
    ...
    async def _run_or_cancel(self, fn: Callable[[], Coroutine[Any, Any, _T]]) -> _T:
        with self._lock:
        ...
        try:
            await asyncio.wait([cancellation_task, fn_task], return_when=asyncio.FIRST_COMPLETED)
            if fn_task.done():
...
class TaskGroup:
    ...
    async def _wait_all(self, *, race: bool, timeout: float | None) -> None:
        ...
        done, pending = await asyncio.wait(self._tasks, timeout=timeout, return_when=return_when)
        if not done:
...
async def open_task_group(*, exit_timeout: float | None=None, race: bool=False) -> AsyncGenerator[TaskGroup]:
    """Create a task group.
...
@contextlib.asynccontextmanager
async def cancel_on_exit(coro: Coroutine[Any, Any, Any]) -> AsyncGenerator[None]:
