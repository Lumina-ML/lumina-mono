...
class _WriteCallback(Protocol):
    ...
    def __call__(self, data: bytes | str, written: int, /) -> None:
...
_is_writing = False
_is_caused_by_callback = contextvars.ContextVar('_is_caused_by_callback', default=False)
...
def capture_stdout(callback: _WriteCallback) -> Callable[[], None]:
    ...
    with _module_rlock:
        return _insert_disposably(_stdout_callbacks, callback)
...
def capture_stderr(callback: _WriteCallback) -> Callable[[], None]:
    ...
    _maybe_raise_patch_exception()
    with _module_rlock:
        return _insert_disposably(_stderr_callbacks, callback)

def _insert_disposably(callback_dict: dict[int, _WriteCallback], callback: _WriteCallback) -> Callable[[], None]:
...
def _patch(stdout_or_stderr: IO[AnyStr], callbacks: dict[int, _WriteCallback]) -> None:
    ...
    stdout_or_stderr.write = write_with_callbacks

@contextlib.contextmanager
def _enter_callbacks(callbacks: dict[int, _WriteCallback]) -> Generator[list[_WriteCallback]]:
