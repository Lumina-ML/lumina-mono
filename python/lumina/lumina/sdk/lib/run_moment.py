...
class RunMoment:
    ...
    def __post_init__(self):
        if self.metric != '_step':
            raise ValueError(f"Only the metric '_step' is supported, got '{self.metric}'.")
        if not isinstance(self.value, (int, float)):
            raise TypeError(f"Only int or float values are supported, got '{self.value}'.")
    ...
    def from_uri(cls, uri: str) -> RunMoment:
        ...
        parse_err = ValueError(f"Could not parse passed run moment string '{uri}', expected format '<run>?<metric>=<numeric_value>'. Currently, only the metric '_step' is supported. Example: 'ans3bsax?_step=123'.")
        try:
