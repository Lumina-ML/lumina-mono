...
class PreInitObject:
    ...
    def __getitem__(self, key: str) -> None:
        raise lumina.Error(f'You must call wandb.init() before {self._name}[{key!r}]')
    ...
    def __setattr__(self, key: str, value: Any) -> Any:
        if not key.startswith('_'):
            raise lumina.Error(f'You must call wandb.init() before {self._name}.{key}')
        else:
...
def PreInitCallable(name: str, destination: Any | None=None) -> Callable:

    def preinit_wrapper(*args: Any, **kwargs: Any) -> Any:
        raise lumina.Error(f'You must call wandb.init() before {name}()')
