...
DEBUG = 10
NOTSET = 0
_level_to_name = {CRITICAL: 'CRITICAL', ERROR: 'ERROR', WARNING: 'WARNING', INFO: 'INFO', DEBUG: 'DEBUG', NOTSET: 'NOTSET'}
_name_to_level = {'CRITICAL': CRITICAL, 'FATAL': FATAL, 'ERROR': ERROR, 'WARN': WARNING, 'WARNING': WARNING, 'INFO': INFO, 'DEBUG': DEBUG, 'NOTSET': NOTSET}
...
class Printer(abc.ABC):
    ...
    @abc.abstractmethod
    def display(self, text: str | list[str] | tuple[str], *, level: str | int | None=None) -> None:
...
class _PrinterTerm(Printer):
    ...
    def progress_update(self, text: str, percent_done: float | None=None) -> None:
        if self._settings and self._settings.silent:
            return
        lumina.termlog(f'{next(self._progress)} {text}', newline=False)
    ...
    @override
    def supports_unicode(self) -> bool:
        return lumina.util.is_unicode_safe(sys.stderr)
    ...
    def emoji(self, name: str) -> str:
        emojis = dict()
        if platform.system() != 'Windows' and lumina.util.is_unicode_safe(sys.stdout):
    ...
    def loading_symbol(self, tick: int) -> str:
        ...
        return click.style(_PROGRESS_SYMBOL_ANIMATION[idx], fg=_PROGRESS_SYMBOL_COLOR)
...
class _PrinterJupyter(Printer):
    ...
    def dynamic_text(self) -> Generator[DynamicText | None]:
        if self._settings and self._settings.silent:
            yield None
            return
        handle = self._ipython_display.display(self._ipython_display.HTML(''), display_id=True)
        if not handle:
            yield None
            return
        try:
