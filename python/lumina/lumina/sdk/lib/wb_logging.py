...
_run_id: contextvars.ContextVar[str | _NotRunSpecific | None] = contextvars.ContextVar('_run_id', default=None)
...
def add_file_handler(run_id: str, filepath: pathlib.Path) -> logging.Handler:
    ...
    handler.setFormatter(logging.Formatter('%(asctime)s %(levelname)-7s %(threadName)-10s:%(process)d [%(filename)s:%(funcName)s():%(lineno)s]%(run_id_tag)s %(message)s'))
