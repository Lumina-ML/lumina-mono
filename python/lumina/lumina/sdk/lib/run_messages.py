...
from lumina.sdk.lib import asyncio_compat, asyncio_manager, ratelimit
...
class RunMessages:
    ...
    def __init__(self, asyncer: asyncio_manager.AsyncioManager, interface: InterfaceBase) -> None:
    ...
    def start(self) -> None:
        ...
        self._asyncer.run_soon(self._impl.loop, daemon=True, name='RunMessages.loop')
...
class _RunMessagesImpl:
    ...
    async def loop(self) -> None:
        ...
        try:
            await asyncio_compat.race(self._print_all(), self._force_stop_event.wait())
        finally:
    ...
    async def _print_all(self) -> None:
        ...
        while True:
            ...
            request = pb.Record(request=pb.Request(internal_messages=pb.InternalMessagesRequest(wait=True)))
