...
def _jupyter_timed_input(prompt: str, timeout: float, err: bool) -> str:
    ...
    try:
        ...
        clear = False
        lumina.termwarn("Unable to clear output, can't import clear_output from ipython.core")
    _echo(prompt, err=err)
    user_inp = None
...
def timed_input(prompt: str, timeout: float, show_timeout: bool=True, jupyter: bool=False, err: bool=False) -> str:
