...
class AsyncioManager:
    ...
    def __init__(self) -> None:
        ...
        self._thread = threading.Thread(target=self._main, name='wandb-AsyncioManager-main', daemon=True)
        self._pid = os.getpid()
    ...
    def run_soon(self, fn: Callable[[], Awaitable[None]], *, daemon: bool=False, name: str | None=None) -> None:
    ...
    def _schedule(self, fn: Callable[[], Awaitable[_T]], daemon: bool, name: str | None=None) -> concurrent.futures.Future[_T]:
        self._ensure_not_in_fork()
        self._ready_event.wait()
        with self._lock:
            if self._joined:
                raise AlreadyJoinedError('Cannot schedule tasks after join().' + ' Did you call wandb.teardown()?')
            if not daemon:
                self._remaining_tasks += 1
        return asyncio.run_coroutine_threadsafe(self._wrap(fn, daemon=daemon, name=name), self._loop)

    async def _wrap(self, fn: Callable[[], Awaitable[_T]], daemon: bool, name: str | None) -> _T:
    ...
    def _ensure_not_in_fork(self) -> None:
        ...
        if (pid := os.getpid()) != self._pid:
            raise ForkedError('This operation is not valid in a forked process.' + f' Original PID={self._pid}, current PID={pid}.' + ' Avoid using os.fork() and make sure your multiprocessing' + " start method is 'forkserver' or 'spawn'.")
    ...
    async def _main_async(self) -> None:
        ...
        async with self._task_finished_cond:
            await self._task_finished_cond.wait_for(lambda: self._remaining_tasks <= 0)
