...
import lumina
import lumina.env
from lumina import env, trigger
from lumina.analytics import get_sentry
from lumina.errors import CommError, Error, UsageError
from lumina.errors.links import url_registry
from lumina.errors.util import ProtobufErrorHandler
from lumina.integration import sagemaker, weave
from lumina.proto.wandb_telemetry_pb2 import Deprecated
from lumina.sdk.lib import ipython as wb_ipython
from lumina.sdk.lib import noop_run, progress, runid, wb_logging
from lumina.sdk.lib.paths import StrPath
from lumina.util import _is_artifact_representation
...
def _huggingface_version() -> str | None:
    if 'transformers' in sys.modules:
        trans = lumina.util.get_module('transformers')
...
def _handle_launch_config(settings: Settings) -> dict[str, Any]:
    ...
    if os.environ.get('WANDB_CONFIG') is not None:
        try:
            ...
            lumina.termwarn('Malformed WANDB_CONFIG, using original config')
...
def _concat_printer_callbacks(cbs: Iterable[_PrinterCallback]) -> _PrinterCallback:
...
class _WandbInit:
    ...
    def __init__(self, wl: wandb_setup._WandbSetup, telemetry: telemetry.TelemetryRecord) -> None:
        self._wl = wl
        ...
        self.notebook: lumina.jupyter.Notebook | None = None
    ...
    def maybe_login(self, init_settings: Settings) -> None:
        ...
        wandb_login._login(host=run_settings.base_url, force=run_settings.force, _silent=run_settings.quiet or run_settings.silent, key=explicit_key, update_api_key=explicit_key is None)
    ...
    def warn_env_vars_change_after_setup(self) -> _PrinterCallback:
        ...
        def print_warning(run_printer: printer.Printer) -> None:
            line = f"Changes to your `wandb` environment variables will be ignored because your `wandb` session has already started. For more information on how to modify your settings with `wandb.init()` arguments, please refer to {run_printer.link(url_registry.url('wandb-init'), 'the W&B docs')}."
            run_printer.display(line, level='warn')
        return print_warning

    def clear_run_path_if_sweep_or_launch(self, init_settings: Settings) -> _PrinterCallback:
    ...
    def make_run_settings(self, init_settings: Settings) -> tuple[Settings, _PrinterCallback]:
        ...
        Args:
            init_settings: Settings passed to `wandb.init()` or set via
                keyword arguments.
        """
        warning_callbacks: list[_PrinterCallback] = [self.warn_env_vars_change_after_setup(), self.clear_run_path_if_sweep_or_launch(init_settings)]
        settings = self._wl.settings.model_copy()
        settings.update_from_settings(init_settings)
        if not settings.sagemaker_disable and sagemaker.is_using_sagemaker() and sagemaker.set_run_id(settings):
        ...
        if settings.project is None:
            settings.project = lumina.util.auto_project_name(settings.program)
    ...
    def _load_autoresume_run_id(self, resume_file: pathlib.Path) -> str | None:
        ...
        with resume_file.open() as f:
            try:
                ...
                self._logger.exception(f'could not decode {resume_file}, ignoring', exc_info=e)
                return None
            except KeyError:
                self._logger.exception(f'resume file at {resume_file} did not store a run_id')
                return None

    def _save_autoresume_run_id(self, *, resume_file: pathlib.Path, run_id: str) -> None:
    ...
    def set_run_id(self, settings: Settings) -> None:
        ...
        if resume_path:
            ...
            if not previous_id:
                ...
                lumina.termwarn(f"Ignoring ID {previous_id} loaded due to resume='auto' because the run ID is set to {settings.run_id}.")
        ...
        if resume_path:
            self._save_autoresume_run_id(resume_file=resume_path, run_id=settings.run_id)
    ...
    def make_run_config(self, settings: Settings, config: dict | str | None=None, config_exclude_keys: list[str] | None=None, config_include_keys: list[str] | None=None) -> _ConfigParts:
        ...
        if config_exclude_keys:
            self.deprecated_features_used.append((Deprecated(init__config_exclude_keys=True), "config_exclude_keys is deprecated. Use `config=wandb.helper.parse_config(config_object, exclude=('key',))` instead."))
        if config_include_keys:
            self.deprecated_features_used.append((Deprecated(init__config_include_keys=True), "config_include_keys is deprecated. Use `config=wandb.helper.parse_config(config_object, include=('key',))` instead."))
        config = parse_config(config or dict(), include=config_include_keys, exclude=config_exclude_keys)
        result = _ConfigParts(base_no_artifacts=dict(), sweep_no_artifacts=dict(), launch_no_artifacts=dict(), artifacts=dict())
        if not settings.sagemaker_disable and sagemaker.is_using_sagemaker():
            sagemaker_config = sagemaker.parse_sm_config()
            self._split_artifacts_from_config(sagemaker_config, config_target=result.base_no_artifacts, artifacts=result.artifacts)
            self._telemetry.feature.sagemaker = True
        if self._wl.config:
            self._split_artifacts_from_config(self._wl.config, config_target=result.base_no_artifacts, artifacts=result.artifacts)
        if config and isinstance(config, dict):
            self._split_artifacts_from_config(config, config_target=result.base_no_artifacts, artifacts=result.artifacts)
        if self._wl._sweep_config:
            self._split_artifacts_from_config(self._wl._sweep_config, config_target=result.sweep_no_artifacts, artifacts=result.artifacts)
        if (launch_config := _handle_launch_config(settings)):
            self._split_artifacts_from_config(launch_config, config_target=result.launch_no_artifacts, artifacts=result.artifacts)
        wandb_internal = result.base_no_artifacts.setdefault('_wandb', dict())
        if settings.save_code and settings.program_relpath:
            wandb_internal['code_path'] = paths.LogicalPath(os.path.join('code', settings.program_relpath))
        if settings.fork_from is not None:
            wandb_internal['branch_point'] = {'run_id': settings.fork_from.run, 'step': settings.fork_from.value}
        if settings.resume_from is not None:
            wandb_internal['branch_point'] = {'run_id': settings.resume_from.run, 'step': settings.resume_from.value}
        return result
    ...
    def _split_artifacts_from_config(self, config_source: dict, config_target: dict, artifacts: dict) -> None:
    ...
    def monkeypatch_ipython(self, settings: Settings) -> None:
        ...
        self.notebook = lumina.jupyter.Notebook(settings)
        ipython = self.notebook.shell
    ...
    def setup_run_log_directory(self, settings: Settings) -> Generator[None]:
        ...
        handler = wb_logging.add_file_handler(settings.run_id, pathlib.Path(settings.log_user))
        if env.is_debug():
        ...
        def dispose_handler() -> None:
            ...
            if not disposed:
                ...
                handler.close()
        try:
            self._teardown_hooks.append(TeardownHook(call=dispose_handler, stage=TeardownStage.LATE))
    ...
    def make_disabled_run(self, config: _ConfigParts) -> Run:
        ...
        return noop_run.init_noop_run(settings=Settings(mode='disabled', root_dir=tempfile.gettempdir(), run_id=run_id, run_tags=tuple(), run_notes=None, run_group=None, run_name=f'dummy-{run_id}', project='dummy', entity='dummy'), config={**config.base_no_artifacts, **config.sweep_no_artifacts})

    def init(self, settings: Settings, config: _ConfigParts, run_printer: printer.Printer) -> Run:
        self._logger.info('calling init triggers')
        trigger.call('on_init')
        assert self._wl is not None
        self._logger.info(f'wandb.init called with sweep_config: {config.sweep_no_artifacts}\nconfig: {config.base_no_artifacts}')
        if (previous_run := self._wl.most_recent_active_run):
            if settings.reinit in (True, 'finish_previous') or (settings.reinit == 'default' and wb_ipython.in_notebook()):
                run_printer.display(f'Finishing previous runs because reinit is set to {settings.reinit!r}.')
                self._wl.finish_all_active_runs()
            elif settings.reinit == 'create_new':
                self._logger.info("wandb.init() called while a run is active, and reinit is set to 'create_new', so continuing")
            elif settings.resume == 'must':
                raise lumina.Error("Cannot resume a run while another run is active. You must either finish it using run.finish(), or use reinit='create_new' when calling wandb.init().")
            else:
                run_printer.display(f'wandb.init() called while a run is active and reinit is set to {settings.reinit!r}, so returning the previous run.')
        ...
        if not service.owns_service:
            self._logger.info('Connected to an existing wandb-core service via WANDB_SERVICE')
            if not _shared_service_notice_shown:
                run_printer.display('Using an existing wandb-core service via WANDB_SERVICE.')
                _shared_service_notice_shown = True
        self._logger.info('sending inform_init request')
        interface = service.inform_init(settings=settings.to_proto(), run_id=settings.run_id)
        self._logger.info('backend started and connected')
        run = Run(config=config.base_no_artifacts, settings=settings, sweep_config=config.sweep_no_artifacts, launch_config=config.launch_no_artifacts)
        with telemetry.context(run=run, obj=self._telemetry) as tel:
            tel.cli_version = lumina.__version__
            ...
            if settings._shared:
                lumina.termwarn('The `shared` mode feature is experimental and may change. Please contact support@wandb.com for guidance and to report any issues.')
                tel.feature.shared_mode = True
            if settings.x_label:
                tel.feature.user_provided_label = True
            if lumina.env.dcgm_profiling_enabled():
        ...
        for deprecated_feature, msg in self.deprecated_features_used:
            warn_and_record_deprecation(feature=deprecated_feature, message=msg, run=run)
        ...
        if settings._offline and settings.resume:
            lumina.termwarn(f'`resume` will be ignored since W&B syncing is set to `offline`. Starting a new run with run id {run.id}.')
        error: lumina.Error | None = None
        timeout = settings.init_timeout
        self._logger.info(f'communicating run to backend with {timeout} second timeout')
        run_init_handle = interface.deliver_run(run)
        try:
            with progress.progress_printer(run_printer, default_text='Waiting for wandb.init()...') as progress_printer:
                result = wait_with_progress(run_init_handle, timeout=timeout, display_progress=functools.partial(progress.loop_printing_operation_stats, progress_printer, interface))
        except TimeoutError:
            raise CommError(f'Run initialization has timed out after {timeout} sec.' + ' Please try increasing the timeout with the `init_timeout`' + ' setting: `wandb.init(settings=wandb.Settings(init_timeout=120))`.') from None
        ...
        if settings.launch and settings.launch_config_path and os.path.exists(settings.launch_config_path):
            run.save(settings.launch_config_path)
        for k, v in config.artifacts.items():
            run.config.update({k: v}, allow_val_change=True)
        job_artifact = run._launch_artifact_mapping.get(lumina.util.LAUNCH_JOB_ARTIFACT_SLOT_NAME)
        if job_artifact:
...
def _attach(attach_id: str | None=None, run_id: str | None=None, *, run: Run | None=None) -> Run | None:
    ...
    if attach_id is None:
        raise UsageError('Either `attach_id` or `run_id` must be specified or `run` must have `_attach_id`')
    ...
    settings.update_from_dict({'run_id': attach_id, 'x_start_time': attach_settings.x_start_time.value, 'mode': attach_settings.mode.value})
...
def _monkeypatch_tensorboard() -> None:
    ...
    if len(lumina.patched['tensorboard']) > 0:
        return
    from lumina.integration import tensorboard as tb_module
    tb_module.patch()
...
def try_create_root_dir(settings: Settings) -> None:
    ...
    try:
        ...
        lumina.termwarn(f'Unable to create root directory {settings.root_dir}', repeat=False)
        fallback_to_temp_dir = True
    else:
        if not os.access(settings.root_dir, os.W_OK | os.R_OK):
            lumina.termwarn(f"Path {settings.root_dir} wasn't read/writable", repeat=False)
            fallback_to_temp_dir = True
    if not fallback_to_temp_dir:
        return
    tmp_dir = tempfile.gettempdir()
    if not os.access(tmp_dir, os.W_OK | os.R_OK):
        raise ValueError(f'System temp directory ({tmp_dir}) is not writable/readable, please set the `dir` argument in `wandb.init()` to a writable/readable directory.')
    settings.root_dir = tmp_dir
    lumina.termwarn(f'Falling back to temporary directory {tmp_dir}.', repeat=False)
    os.makedirs(settings.root_dir, exist_ok=True)

def init(entity: str | None=None, project: str | None=None, dir: StrPath | None=None, id: str | None=None, name: str | None=None, notes: str | None=None, tags: Sequence[str] | None=None, config: dict[str, Any] | str | None=None, config_exclude_keys: list[str] | None=None, config_include_keys: list[str] | None=None, allow_val_change: bool | None=None, group: str | None=None, job_type: str | None=None, mode: Literal['online', 'offline', 'disabled', 'shared'] | None=None, force: bool | None=None, reinit: bool | Literal[None, 'default', 'return_previous', 'finish_previous', 'create_new']=None, resume: bool | Literal['allow', 'never', 'must', 'auto'] | None=None, resume_from: str | None=None, fork_from: str | None=None, save_code: bool | None=None, tensorboard: bool | None=None, sync_tensorboard: bool | None=None, monitor_gym: bool | None=None, settings: Settings | dict[str, Any] | None=None, anonymous: DoNotSet=UNSET) -> Run:
    ...
    if mode is not None:
        init_settings.mode = mode
    if resume is not None:
        init_settings.resume = resume
    ...
    if fork_from is not None:
        init_settings.fork_from = fork_from
    if resume_from is not None:
        init_settings.resume_from = resume_from
    ...
    try:
        ...
        if isinstance(run_settings.reinit, bool):
            wi.deprecated_features_used.append((Deprecated(run__reinit_bool=True), "Using a boolean value for 'reinit' is deprecated. Use 'return_previous' or 'finish_previous' instead."))
        ...
        with contextlib.ExitStack() as exit_stack:
            ...
            run_config = wi.make_run_config(settings=run_settings, config=config, config_exclude_keys=config_exclude_keys, config_include_keys=config_include_keys)
