...
import lumina
import lumina.util
from lumina.filesync import stats, step_checksum, step_upload
from lumina.sdk.lib.paths import LogicalPath
if TYPE_CHECKING:
    from lumina.sdk.artifacts.artifact_manifest import ArtifactManifest
    from lumina.sdk.artifacts.artifact_saver import SaveFn
...
class FilePusher:
    ...
    def __init__(self, api: internal_api.Api, file_stream: file_stream.FileStreamApi, settings: SettingsStatic | None=None) -> None:
        ...
        self._step_checksum = step_checksum.StepChecksum(self._api, self._tempdir, self._incoming_queue, self._event_queue, self._stats)
        self._step_checksum.start()
        self._step_upload = step_upload.StepUpload(self._api, self._stats, self._event_queue, self.MAX_UPLOAD_JOBS, file_stream=file_stream, settings=settings)
        self._step_upload.start()
        self._stats_thread_stop = threading.Event()
        if os.environ.get('WANDB_DEBUG'):
            self._stats_thread = threading.Thread(target=self._file_pusher_stats, daemon=True, name='FPStatsThread')
    ...
    def print_status(self, prefix: bool=True) -> None:
        ...
        while True:
            ...
            step += 1
            lumina.termlog(line, newline=False, prefix=prefix)
            if stop:
                break
            time.sleep(0.25)
        dedupe_fraction = summary.deduped_bytes / float(summary.total_bytes) if summary.total_bytes > 0 else 0
        if dedupe_fraction > 0.01:
            lumina.termlog('W&B sync reduced upload amount by %.1f%%             ' % (dedupe_fraction * 100), prefix=prefix)
        lumina.termlog(' ' * 79, prefix=prefix)
    ...
    def store_manifest_files(self, manifest: ArtifactManifest, artifact_id: str, save_fn: SaveFn) -> None:
    ...
    def commit_artifact(self, artifact_id: str, *, finalize: bool=True, before_commit: step_upload.PreCommitFn, result_future: concurrent.futures.Future[None]):
        event = step_checksum.RequestCommitArtifact(artifact_id, finalize, before_commit, result_future)
        self._incoming_queue.put(event)
