...
from lumina.proto.wandb_internal_pb2 import HistoryRecord, InternalMessages, MetricRecord, Record, Result, RunRecord, SummaryItem, SummaryRecord, SummaryRecordRequest
from ..interface.interface_queue import InterfaceQueue
from ..lib import handler_util, proto_util
from . import tb_watcher
from .settings_static import SettingsStatic
if TYPE_CHECKING:
    from lumina.proto.wandb_internal_pb2 import MetricSummary
...
class HandleManager:
    ...
    def __init__(self, settings: SettingsStatic, record_q: Queue[Record], result_q: Queue[Result], stopped: Event, writer_q: Queue[Record], interface: InterfaceQueue) -> None:
    ...
    def _update_summary_metrics(self, s: MetricSummary, kl: list[str], v: numbers.Real, float_v: float, goal_max: bool | None) -> bool:
        updated = False
    ...
    def _update_summary_leaf(self, kl: list[str], v: Any, d: MetricRecord | None=None) -> bool:
        ...
        return bool(self._update_summary_metrics(d.summary, kl=kl, v=v, float_v=float_v, goal_max=goal_max))

    def _update_summary_list(self, kl: list[str], v: Any, d: MetricRecord | None=None) -> bool:
    ...
    def _update_summary_media_objects(self, v: dict[str, Any]) -> dict[str, Any]:
        for nk, nv in v.items():
            if REPLACE_SUMMARY_ART_PATH_WITH_LATEST and (isinstance(nv, dict) and handler_util.metric_is_wandb_dict(nv) and ('_latest_artifact_path' in nv) and ('artifact_path' in nv)):
    ...
    def _history_assign_step(self, history: HistoryRecord, history_dict: dict[str, Any]) -> None:
    ...
    def _history_update_list(self, kl: list[str], v: Any, history_dict: dict[str, Any], update_history: dict[str, Any]) -> None:
        if isinstance(v, dict):
            for nk, nv in v.items():
                self._history_update_list(kl=kl[:] + [nk], v=nv, history_dict=history_dict, update_history=update_history)
            return
        self._history_update_leaf(kl=kl, v=v, history_dict=history_dict, update_history=update_history)
    ...
    def _flush_partial_history(self, step: int | None=None) -> None:
        ...
        if step is not None:
            if step < self._step:
                if not self._dropped_history:
                    message = f"Step only supports monotonically increasing values, use define_metric to set a custom x axis. For details see: {url_registry.url('define-metric')}"
    ...
    def handle_request_run_start(self, record: Record) -> None:
        ...
        self._run_start_time = run_start.run.start_time.ToMicroseconds() / 1000000.0
        self._track_time = time.time()
        if run_start.run.resumed and run_start.run.runtime:
            self._accumulate_time = run_start.run.runtime
        else:
            self._accumulate_time = 0
        self._tb_watcher = tb_watcher.TBWatcher(self._settings, interface=self._interface, run_proto=run_start.run)
