...
if TYPE_CHECKING:
    ...
    from lumina.proto.wandb_internal_pb2 import RunRecord
    from lumina.sdk.lib.filesystem import FilesDict
...
def _link_and_save_file(path: str, base_path: str, interface: InterfaceQueue, settings: SettingsStatic) -> None:
    ...
    if os.path.islink(wandb_path) and abs_path != os.readlink(wandb_path):
        ...
        os.symlink(abs_path, wandb_path)
    interface.publish_files(dict(files=[(filesystem.GlobStr(glob.escape(file_name)), 'live')]))
...
class TBWatcher:
    ...
    def __init__(self, settings: SettingsStatic, run_proto: RunRecord, interface: InterfaceQueue, force: bool=False) -> None:
        ...
        lumina.tensorboard.reset_state()
...
class TBDirWatcher:
    ...
    def _is_our_tfevents_file(self, path: str) -> bool:
        ...
        if self._force:
            ...
            return is_tfevents_file_created_by(path, self._hostname, self._tbwatcher._settings.x_start_time)

    def _loader(self, save: bool=True, namespace: str | None=None) -> EventFileLoader:
...
class TBEventConsumer:
    ...
    def _handle_event(self, event: ProtoEvent, history: TBHistory | None=None) -> None:
        lumina.tensorboard._log(event.event, step=event.event.step, namespace=event.namespace, history=history)
    ...
    def _save_row(self, row: HistoryDict) -> None:
        ...
        for k, v in row.items():
            if isinstance(v, CustomChart):
                chart_keys.add(k)
                v.set_key(k)
                self._tbwatcher._interface.publish_config(key=v.spec.config_key, val=v.spec.config_value)
        for k in chart_keys:
            chart = row.pop(k)
            if isinstance(chart, CustomChart):
                row[chart.spec.table_key] = chart.table
        self._tbwatcher._interface.publish_history(self._internal_run, row, publish_step=False)
        ...
        if self._step_size > util.MAX_LINE_BYTES:
            ...
            for k, v in metrics:
                if self._step_size - bad < util.MAX_LINE_BYTES - 100000:
                    ...
                    del self._data[k]
            lumina.termwarn(f'Step {self._step} exceeds max data limit, dropping {len(dropped_keys)} of the largest keys:')
            print('\t' + '\n\t'.join(dropped_keys))
