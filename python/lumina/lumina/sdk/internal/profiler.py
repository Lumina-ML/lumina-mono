...
import os
import lumina
from lumina.errors import Error, UsageError
...
def torch_trace_handler():
    ...
    torch = lumina.util.get_module(PYTORCH_MODULE, required=True)
    torch_profiler = lumina.util.get_module(PYTORCH_PROFILER_MODULE, required=True)
    if parse(torch.__version__) < parse('1.9.0'):
        raise Error(f'torch version must be at least 1.9 in order to use the PyTorch Profiler API.            \nVersion of torch currently installed: {torch.__version__}')
    try:
        logdir = os.path.join(lumina.run.dir, 'pytorch_traces')
        os.mkdir(logdir)
    except AttributeError:
        raise UsageError('Please call `wandb.init()` before `wandb.profiler.torch_trace_handler()`') from None
