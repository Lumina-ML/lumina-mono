...
from lumina.sdk.lib import proto_util, telemetry
...
class ConfigState:
    ...
    def merge_resumed_config(self, old_config_tree: dict[str, Any]) -> None:
        ...
        for key in ['viz', 'visualize', 'mask/class_labels']:
            self._add_unset_keys_from_subtree(old_config_tree, [_WANDB_INTERNAL_KEY, key])
    ...
    def to_backend_dict(self, telemetry_record: telemetry.TelemetryRecord, framework: str | None, start_time_millis: int, metric_pbdicts: Sequence[dict[int, Any]], environment_record: wandb_internal_pb2.EnvironmentRecord) -> BackendConfigDict:
...
def _subtree(tree: dict[str, Any], key_path: Sequence[str], *, create: bool=False) -> dict[str, Any] | None:
