...
from lumina.sdk.internal import internal_api
...
class DefaultFilePolicy:
    ...
    def _debug_log(self, data: Any):
        ...
        lumina.termerror(f"Step: {loaded['_step']} | {key_msg}", repeat=False)
    ...
    def process_chunks(self, chunks: list[Chunk]) -> ProcessedChunk:
        ...
        for chunk in chunks:
            if len(chunk.data) > util.MAX_LINE_BYTES:
                ...
                lumina.termerror(msg, repeat=False)
...
class FileStreamApi:
    ...
    def __init__(self, api: internal_api.Api, run_id: str, start_time: float, timeout: float=0, settings: dict | None=None) -> None:
        ...
        self._exc_info: tuple[type[BaseException], BaseException, TracebackType] | tuple[None, None, None] | None = None
    ...
    def _handle_response(self, response: Exception | requests.Response) -> None:
        ...
        if isinstance(response, Exception):
            lumina.termerror('Dropped streaming file chunk (see wandb/debug-internal.log)')
    ...
    def _send(self, chunks: list[Chunk], uploaded: set[str] | None=None) -> bool:
        ...
        for filename, file_chunks in itertools.groupby(chunks, lambda c: c.filename):
            file_chunks_list = list(file_chunks)
            self.set_default_file_policy(filename, DefaultFilePolicy())
            files[filename] = self._file_policies[filename].process_chunks(file_chunks_list)
            if not files[filename]:
                del files[filename]
        for fs in file_stream_utils.split_files(files, max_bytes=util.MAX_LINE_BYTES):
            self._handle_response(request_with_retry(self._client.post, self._endpoint, json={'files': fs, 'dropped': self._dropped_chunks}, retry_callback=self._api.retry_callback))
        return not (uploaded_list and isinstance(request_with_retry(self._client.post, self._endpoint, json={'complete': False, 'failed': False, 'dropped': self._dropped_chunks, 'uploaded': uploaded_list}), Exception))
...
def request_with_retry(func: Callable, *args: Any, **kwargs: Any) -> requests.Response | requests.RequestException:
    ...
    while True:
        try:
            ...
            if isinstance(e, requests.exceptions.HTTPError) and (e.response is not None and e.response.status_code in {400, 403, 404, 409}):
                return e
