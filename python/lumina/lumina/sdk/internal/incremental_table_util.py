...
def _get_artifact_name(run: LocalRun, key: str) -> str:
    from lumina.sdk.artifacts._internal_artifact import sanitize_artifact_name
...
def init_artifact(run: LocalRun, sanitized_key: str) -> Artifact:
    ...
    artifact = InternalArtifact(_get_artifact_name(run, sanitized_key), ART_TYPE, incremental=True)
    return artifact
