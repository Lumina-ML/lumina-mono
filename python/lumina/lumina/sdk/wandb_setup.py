...
import lumina
import lumina.integration.sagemaker as sagemaker
from lumina.env import CONFIG_DIR
from lumina.errors import UsageError
from lumina.sdk.lib import asyncio_manager, import_hooks, wb_logging
from .lib import config_util, server
if TYPE_CHECKING:
    from lumina.sdk import wandb_run
    from lumina.sdk.lib.service.service_connection import ServiceConnection
    from lumina.sdk.wandb_settings import Settings
...
class _WandbSetup:
    ...
    def did_environment_change(self) -> bool:
        ...
        singleton_env = {k: v for k, v in self._settings_environ.items() if k.startswith('WANDB_') and k not in exclude_env_vars}
        os_env = {k: v for k, v in os.environ.items() if k.startswith('WANDB_') and k not in exclude_env_vars}
        return set(singleton_env.keys()) != set(os_env.keys()) or set(singleton_env.values()) != set(os_env.values())

    def _load_settings(self, *, system_settings_path: str | None, disable_sagemaker: bool, overrides: Settings | None=None) -> None:
        ...
        pid = os.getpid()
        self._logger.info(f'Current SDK version is {lumina.__version__}')
        ...
        if system_settings_path:
            ...
            self._settings.settings_system = str(pathlib.Path('~', '.config', 'wandb', 'settings').expanduser())
        ...
        if not self._settings.sagemaker_disable and (not disable_sagemaker) and sagemaker.is_using_sagemaker():
        ...
        lumina.termsetup(self._settings, None)
    ...
    def _update(self, settings: Settings | None) -> None:
        ...
        if not self._settings:
            ...
            self._load_settings(system_settings_path=system_settings_path, disable_sagemaker=disable_sagemaker, overrides=settings)
        elif settings:
    ...
    def settings(self) -> Settings:
        ...
        """
        if not self._settings:
            self._load_settings(system_settings_path=None, disable_sagemaker=False)
    ...
    def ensure_service(self) -> ServiceConnection:
        ...
        with self._connection_lock:
            if self._connection:
                return self._connection
            from lumina.sdk.lib.service import service_connection
            self._connection = service_connection.connect_to_service(self._asyncer, self.settings)
            return self._connection
...
def _setup(settings: Settings | None=None, start_service: bool=True, load_settings: bool=True) -> _WandbSetup:
...
def _maybe_update_credentials(settings: Settings) -> None:
    ...
    if settings.api_key and settings.identity_token_file:
        raise UsageError('The api_key and identity_token_file settings cannot be used together.')
    from lumina.sdk.lib import wbauth
    if settings.api_key:
        wbauth.use_explicit_auth(wbauth.AuthApiKey(host=wbauth.HostUrl(settings.base_url, app_url=settings.app_url), api_key=settings.api_key), source='wandb.setup()')
    elif settings.identity_token_file:
        wbauth.use_explicit_auth(wbauth.AuthIdentityTokenFile(host=wbauth.HostUrl(settings.base_url, app_url=settings.app_url), path=settings.identity_token_file, credentials_file=settings.credentials_file), source='wandb.setup()')
