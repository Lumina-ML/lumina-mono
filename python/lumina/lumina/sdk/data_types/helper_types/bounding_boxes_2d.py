...
from lumina.util import has_num
...
def _convert_pytorch_tensor_to_list(box_data):
    for box in box_data:
        if 'position' in box and 'middle' in box['position'] and util.is_pytorch_tensor_typename(util.get_full_typename(box['position']['middle'])):
...
class BoundingBoxes2D(JSONMetadata):
    ...
    def __init__(self, val: dict, key: str) -> None:
        ...
        if 'class_labels' not in val:
            np = util.get_module('numpy', required='Bounding box support requires numpy')
            classes = np.unique(list((box['class_id'] for box in val['box_data']))).astype(np.int32).tolist()
    ...
    def bind_to_run(self, run: LocalRun, key: int | str, step: int | str, id_: int | str | None=None, ignore_copy_err: bool | None=None) -> None:
        ...
        run._add_singleton('bounding_box/class_labels', str(key) + '_wandb_delimeter_' + self._key, self._class_labels)
    ...
    def validate(self, val: dict) -> bool:
        ...
        for box in boxes:
            ...
            if 'position' not in box:
                ...
                valid = False
                if 'middle' in box['position'] and len(box['position']['middle']) == 2 and has_num(box['position'], 'width') and has_num(box['position'], 'height'):
                    valid = True
                elif has_num(box['position'], 'minX') and has_num(box['position'], 'maxX') and has_num(box['position'], 'minY') and has_num(box['position'], 'maxY'):
                    valid = True
                if not valid:
                    raise TypeError(error_str)
    ...
    @classmethod
    def from_json(cls: type[BoundingBoxes2D], json_obj: dict, source_artifact: Artifact) -> BoundingBoxes2D:
