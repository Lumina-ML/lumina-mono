...
class ImageMask(Media):
    ...
    def __init__(self, val: dict, key: str) -> None:
        ...
        if 'path' in val:
            ...
            if util.is_pytorch_tensor_typename(util.get_full_typename(val['mask_data'])):
    ...
    @classmethod
    def from_json(cls: type[ImageMask], json_obj: dict, source_artifact: Artifact) -> ImageMask:
        return cls({'path': source_artifact.get_entry(json_obj['path']).download()}, key='')
    ...
    def validate(self, val: dict) -> bool:
        ...
        if 'mask_data' not in val:
            raise TypeError('Missing key "mask_data": An image mask requires mask data: a 2D array representing the predictions')
        else:
            error_str = 'mask_data must be a 2D array'
            shape = val['mask_data'].shape
            if len(shape) != 2:
                raise TypeError(error_str)
            if not ((val['mask_data'] >= 0).all() and (val['mask_data'] <= 255).all()) and issubclass(val['mask_data'].dtype.type, np.integer):
        ...
        if 'class_labels' in val:
            for k, v in list(val['class_labels'].items()):
                if not isinstance(k, numbers.Number) or not isinstance(v, str):
                    raise TypeError('Class labels must be a dictionary of numbers to strings')
        return True
