...
class WBValue:
    ...
    def _set_artifact_source(self, artifact: Artifact, name: str | None=None) -> None:
        assert self._artifact_source is None, f'Cannot update artifact_source. Existing source: {self._artifact_source.artifact}/{self._artifact_source.name}'
    ...
    def _set_artifact_target(self, artifact: Artifact, name: str | None=None) -> None:
        assert self._artifact_target is None, f'Cannot update artifact_target. Existing target: {self._artifact_target.artifact}/{self._artifact_target.name}'
    ...
    def _get_artifact_entry_ref_url(self) -> str | None:
        ...
        if (target := self._artifact_target) and target.name and ((client_id := target.artifact._client_id) is not None) and target.artifact._final and _server_accepts_client_ids():
            return f'wandb-client-artifact://{client_id}/{self.with_suffix(target.name)}'
        if (target := self._artifact_target) and target.name and target.artifact._is_draft_save_started() and (not _is_maybe_offline()) and (not _server_accepts_client_ids()):
    ...
    def _get_artifact_entry_latest_ref_url(self) -> str | None:
        if (target := self._artifact_target) and target.name and ((sequence_client_id := target.artifact._sequence_client_id) is not None) and target.artifact._final and _server_accepts_client_ids():
