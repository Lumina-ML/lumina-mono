...
from lumina.sdk.data_types._private import MEDIA_TMP
from lumina.sdk.data_types.base_types.media import Media, _numpy_arrays_to_lists
...
class Graph(Media):
    ...
    def _to_graph_json(self, run=None):
        return {'format': self.format, 'nodes': [node.to_json() for node in self.nodes], 'edges': [edge.to_json() for edge in self.edges]}
    ...
    def _is_sequential(cls, model):
        ...
        if model.__class__.__name__ != 'Sequential' and hasattr(model, '_is_graph_network') and model._is_graph_network:
