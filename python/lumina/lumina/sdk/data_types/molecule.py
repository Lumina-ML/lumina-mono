...
class Molecule(BatchableMedia):
    ...
    SUPPORTED_TYPES = {'pdb', 'pqr', 'mmcif', 'mcif', 'cif', 'sdf', 'sd', 'gro', 'mol2', 'mmtf'}
    SUPPORTED_RDKIT_TYPES = {'mol', 'sdf'}
    _log_type = 'molecule-file'

    def __init__(self, data_or_path: str | pathlib.Path | TextIO, caption: str | None=None, **kwargs: str) -> None:
        ...
        if hasattr(data_or_path, 'read'):
            ...
            if extension is None:
                raise ValueError('Must pass file_type keyword argument when using io objects.')
            ...
            if extension not in Molecule.SUPPORTED_TYPES:
                raise ValueError('Molecule only supports files of the type: ' + ', '.join(Molecule.SUPPORTED_TYPES))
    ...
    @classmethod
    def seq_to_json(cls: type[Molecule], seq: Sequence[BatchableMedia], run: LocalRun, key: str, step: int | str) -> dict:
        ...
        return {'_type': 'molecule', 'filenames': [obj['path'] for obj in jsons], 'count': len(jsons), 'captions': Media.captions(seq)}
