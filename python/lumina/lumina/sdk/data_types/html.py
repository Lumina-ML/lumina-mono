...
class Html(BatchableMedia):
    ...
    def __init__(self, data: str | pathlib.Path | TextIO, inject: bool=True, data_is_not_path: bool=False) -> None:
        ...
        """
        super().__init__()
        data_is_path = (isinstance(data, (str, pathlib.Path)) and os.path.isfile(data) and (os.path.splitext(data)[1] == '.html')) and (not data_is_not_path)
        data_path = ''
        if data_is_path:
            data_path = str(data)
    ...
    @classmethod
    def seq_to_json(cls: type[Html], seq: Sequence[BatchableMedia], run: LocalRun, key: str, step: int | str) -> dict:
        ...
        meta = {'_type': 'html', 'count': len(seq), 'html': [h.to_json(run) for h in seq]}
        return meta
