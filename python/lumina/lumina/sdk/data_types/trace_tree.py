...
import lumina
from lumina.sdk.data_types import _dtypes
from lumina.sdk.data_types.base_types.media import Media
from lumina.sdk.data_types.utils import _json_helper
if TYPE_CHECKING:
...
class WBTraceTree(Media):
    ...
    def __init__(self, root_span: Span, model_dict: dict | None=None):
        super().__init__()
...
def _safe_serialize(obj: dict) -> str:
    try:
        return json.dumps(_json_helper(obj, None), skipkeys=True, default=_fallback_serialize)
    except Exception:
        return '{}'
...
class Trace:
    ...
    def __init__(self, name: str, kind: str | None=None, status_code: str | None=None, status_message: str | None=None, metadata: dict | None=None, start_time_ms: int | None=None, end_time_ms: int | None=None, inputs: dict | None=None, outputs: dict | None=None, model_dict: dict | None=None):
        self._span = self._assert_and_create_span(name=name, kind=kind, status_code=status_code, status_message=status_message, metadata=metadata, start_time_ms=start_time_ms, end_time_ms=end_time_ms, inputs=inputs, outputs=outputs)
        if model_dict is not None:
            assert isinstance(model_dict, dict), 'Model dict must be a dictionary'
        self._model_dict = model_dict

    def _assert_and_create_span(self, name: str, kind: str | None=None, status_code: str | None=None, status_message: str | None=None, metadata: dict | None=None, start_time_ms: int | None=None, end_time_ms: int | None=None, inputs: dict | None=None, outputs: dict | None=None) -> Span:
    ...
    def log(self, name: str) -> None:
        ...
        assert lumina.run is not None, 'You must call wandb.init() before logging a trace'
        assert len(name.strip()) > 0, 'You must provide a valid name to log the trace'
        lumina.run.log({name: trace_tree})
