...
from .table_decorators import allow_incremental_logging_after_append, allow_relogging_after_mutation, ensure_not_incremental
from .utils import _json_helper
if TYPE_CHECKING:
    import numpy as np
    import pandas as pd
    from lumina.sdk.artifacts import artifact
...
class _TableIndex(int, _TableLinkMixin):
    ...
    def get_row(self):
        row = {}
        if self._table:
            row = {c: self._table.data[self][i] for i, c in enumerate(self._table.columns)}
        return row
...
class _ForeignIndexType(_dtypes.Type):
    ...
    def assign_type(self, wb_type=None):
        if isinstance(wb_type, _dtypes.NumberType):
            return self
        elif isinstance(wb_type, _ForeignIndexType) and id(self.params['table']) == id(wb_type.params['table']):
            return self
        return _dtypes.InvalidType()
    ...
    def to_json(self, artifact=None):
        ...
        if artifact is not None:
            ...
            raise AssertionError('_ForeignIndexType does not support serialization without an artifact')
        return res
    ...
    def from_json(cls, json_dict, artifact):
        table = None
        if artifact is None:
            raise AssertionError('_ForeignIndexType does not support deserialization without an artifact')
        else:
            table = artifact.get(json_dict['params']['table'])
        if table is None:
            raise AssertionError('Unable to deserialize referenced table')
...
class Table(Media):
    ...
    def __init__(self, columns: list[ColumnKey] | None=None, data: list[InputRow] | np.ndarray | pd.DataFrame | None=None, rows: list[InputRow] | None=None, dataframe: pd.DataFrame | None=None, dtype: Any=None, optional: bool | list[bool]=True, allow_mixed_types: bool=False, log_mode: LogMode | None='IMMUTABLE') -> None:
    ...
    def _validate_log_mode(self, log_mode: LogMode | None) -> None:
        assert log_mode in _SUPPORTED_LOGGING_MODES, f'Invalid log_mode: {log_mode}. Must be one of {_SUPPORTED_LOGGING_MODES}'
    ...
    def _assert_valid_columns(columns: list[ColumnKey]) -> None:
        ...
        assert len(columns) == 0 or all([type(col) in valid_col_types for col in columns]), 'columns argument expects list of strings or ints'

    def _init_from_list(self, data: list[InputRow], columns: list[ColumnKey], optional: bool | list[bool]=True, dtype: Any=None) -> None:
    ...
    def _init_from_ndarray(self, ndarray: np.ndarray, columns: list[ColumnKey], optional: bool | list[bool]=True, dtype: Any=None) -> None:
        assert util.is_numpy_array(ndarray), 'ndarray argument expects a `numpy.ndarray` object'
        self.data = []
        self._assert_valid_columns(columns)
        self.columns = columns
        self._make_column_types(dtype, optional)
    ...
    def _init_from_dataframe(self, dataframe: pd.DataFrame, columns: list[ColumnKey], optional: bool | list[bool]=True, dtype: Any=None) -> None:
        assert util.is_pandas_data_frame(dataframe), 'dataframe argument expects a `pandas.core.frame.DataFrame` object'
        self.data = []
    ...
    def _make_column_types(self, dtype: Any=None, optional: bool | list[bool]=True) -> None:
        ...
        for col_name, dt, opt in zip(self.columns, dtype_list, optional_list, strict=False):
    ...
    def _load_incremental_table_state_from_resumed_run(self, run: LocalRun, key: str):
        ...
        if summary_from_key is None or not isinstance(summary_from_key, dict) or summary_from_key.get('_type') != 'incremental-table-file':
            return
    ...
    def cast(self, col_name: ColumnKey, dtype: Any, optional: bool=False) -> _dtypes.Type:
        ...
        for row in self.data:
            ...
            if isinstance(result_type, _dtypes.InvalidType):
                raise TypeError(f'Existing data {row[col_ndx]}, of type {_dtypes.TypeRegistry.type_of(row[col_ndx])} cannot be cast to {wbtype}')
            wbtype = result_type
        ...
        if is_pk or is_fk or is_fi:
            assert not optional, 'Primary keys, foreign keys, and foreign indexes cannot be optional.'
        ...
        if is_pk:
            assert self._pk_col is None, f'Cannot have multiple primary keys - {self._pk_col} is already set as the primary key.'
    ...
    def _eq_debug(self, other, should_assert=False):
        ...
        assert not should_assert or eq, f'Found type {other.__class__}, expected {Table}'
        eq = eq and len(self.data) == len(other.data)
        assert not should_assert or eq, f'Found {len(other.data)} rows, expected {len(self.data)}'
        eq = eq and self.columns == other.columns
        assert not should_assert or eq, f'Found columns {other.columns}, expected {self.columns}'
        eq = eq and self._column_types == other._column_types
        assert not should_assert or eq, f'Found column type {other._column_types}, expected column type {self._column_types}'
        if eq:
            for row_ndx in range(len(self.data)):
                for col_ndx in range(len(self.data[row_ndx])):
                    ...
                    if util.is_numpy_array(_eq):
                        _eq = (_eq * -1 + 1).sum() == 0
                    eq = eq and _eq
                    assert not should_assert or eq, f'Unequal data at row_ndx {row_ndx} col_ndx {col_ndx}: found {other.data[row_ndx][col_ndx]}, expected {self.data[row_ndx][col_ndx]}'
                    if not eq:
                        return eq
        return eq
    ...
    def add_data(self, *data: Any) -> None:
        ...
        if len(data) != len(self.columns):
            raise ValueError(f'This table expects {len(self.columns)} columns: {self.columns}, found {len(data)}')
        for ndx, item in enumerate(data):
            if isinstance(item, _TableLinkMixin):
                self.cast(self.columns[ndx], _dtypes.TypeRegistry.type_of(item), optional=False)
    ...
    def _get_updated_result_type(self, row: Sequence[Any]) -> _dtypes.Type:
        ...
        """
        incoming_row_dict = {col_key: row[ndx] for ndx, col_key in enumerate(self.columns)}
        current_type = self._column_types
        result_type = current_type.assign(incoming_row_dict)
        if isinstance(result_type, _dtypes.InvalidType):
            raise TypeError(f'Data row contained incompatible types:\n{current_type.explain(incoming_row_dict)}')
        return result_type

    def _to_table_json(self, max_rows: int | None=None, warn: bool=True) -> dict[str, Any]:
        if max_rows is None:
            max_rows = Table.MAX_ROWS
        n_rows = len(self.data)
        if n_rows > max_rows and warn:
            if lumina.run and (lumina.run.settings.table_raise_on_max_row_limit_exceeded or lumina.run.settings.strict):
    ...
    @classmethod
    def from_json(cls, json_obj: dict[str, Any], source_artifact: artifact.Artifact) -> Table:
        ...
        if json_obj.get('column_types') is not None:
            column_types = _dtypes.TypeRegistry.type_from_dict(json_obj['column_types'], source_artifact)
            for col_name in column_types.params['type_map']:
                ...
                if isinstance(col_type, _dtypes.NDArrayType):
                    ...
                    timestamp_column_indices.add(json_obj['columns'].index(col_name))
                if ndarray_type is not None and ndarray_type._get_serialization_path() is not None:
                    serialization_path = ndarray_type._get_serialization_path()
                    if serialization_path is None:
                        continue
                    np = util.get_module('numpy', required='Deserializing NumPy columns requires NumPy to be installed.')
                    deserialized = np.load(source_artifact.get_entry(serialization_path['path']).download())
                    np_deserialized_columns[json_obj['columns'].index(col_name)] = deserialized[serialization_path['key']]
                    ndarray_type._clear_serialization_path()
        if log_mode == 'INCREMENTAL':
            unprocessed_table_data = _get_data_from_increments(json_obj, source_artifact)
        else:
            unprocessed_table_data = json_obj['data']
        for r_ndx, row in enumerate(unprocessed_table_data):
            data.append(_process_table_row(row, timestamp_column_indices, np_deserialized_columns, source_artifact, r_ndx))
        dtypes = None
        if column_types is not None:
            dtypes = [column_types.params['type_map'][str(col)] for col in json_obj['columns']]
    ...
    def to_json(self, run_or_artifact: Any) -> dict[str, Any]:
        ...
        if self.log_mode == 'INCREMENTAL':
            ...
            json_dict.update({'increment_num': self._increment_num, 'previous_increments_paths': self._previous_increments_paths})
        if isinstance(run_or_artifact, lumina.Run):
            if self.log_mode == 'INCREMENTAL':
                wbvalue_type = 'incremental-table-file'
            else:
                wbvalue_type = 'table-file'
            json_dict.update({'_type': wbvalue_type, 'ncols': len(self.columns), 'nrows': len(self.data), 'log_mode': self.log_mode})
        elif isinstance(run_or_artifact, lumina.Artifact):
            ...
            for col_ndx, col_name in enumerate(self.columns):
                ...
                if isinstance(col_type, _dtypes.NDArrayType):
                    ...
                    for t in col_type.params['allowed_types']:
                        if isinstance(t, _dtypes.NDArrayType):
                            ndarray_type = t
                is_1d_array = ndarray_type is not None and 'shape' in ndarray_type._params and isinstance(ndarray_type._params['shape'], list) and (len(ndarray_type._params['shape']) == 1) and (ndarray_type._params['shape'][0] <= self._MAX_EMBEDDING_DIMENSIONS)
                if is_1d_array:
                    assert ndarray_type is not None
                    self._column_types.params['type_map'][col_name] = _dtypes.ListType(_dtypes.NumberType, ndarray_type._params['shape'][0])
                elif ndarray_type is not None:
                    np = util.get_module('numpy', required='Serializing NumPy requires NumPy to be installed.')
                    file_name = f'{str(col_name)}_{runid.generate_id()}.npz'
                    npz_file_name = os.path.join(MEDIA_TMP.name, file_name)
                    np.savez_compressed(npz_file_name, **{str(col_name): self.get_column(col_name, convert_to='numpy')})
                    entry = artifact.add_file(npz_file_name, 'media/serialized_data/' + file_name, is_tmp=True)
            ...
            json_dict.update({'_type': Table._log_type, 'columns': self.columns, 'data': mapped_data, 'ncols': len(self.columns), 'nrows': len(mapped_data), 'column_types': self._column_types.to_json(artifact), 'log_mode': self.log_mode})
        else:
    ...
    def _update_keys(self, force_last: bool=False) -> None:
        ...
        if has_update:
            ...
            if len(self._fk_cols - _fk_cols) > 0:
                raise AssertionError(f'Cannot unset foreign key. Attempted to unset ({self._fk_cols - _fk_cols})')
    ...
    @allow_relogging_after_mutation
    def add_column(self, name: str, data: list[Any] | np.ndarray, optional: bool=False) -> None:
        ...
        assert is_first_col or len(data) == len(self.data), f'Expected length {len(self.data)}, found {len(data)}'
    ...
    @overload
    def get_column(self, name: ColumnKey, convert_to: None=None) -> list[Any]:
        ...

    @overload
    def get_column(self, name: ColumnKey, convert_to: Literal['numpy']) -> np.ndarray:
        ...

    def get_column(self, name: ColumnKey, convert_to: Literal['numpy'] | None=None) -> list[Any] | np.ndarray:
        ...
        if convert_to == 'numpy':
            np_module = util.get_module('numpy', required='Converting to NumPy requires installing NumPy')
        col: list[Any] = []
    ...
    def get_dataframe(self) -> pd.DataFrame:
        ...
        pd = util.get_module('pandas', required='Converting to pandas.DataFrame requires installing pandas')
    ...
    @ensure_not_incremental
    @allow_relogging_after_mutation
    def add_computed_columns(self, fn: Callable[[int, dict[ColumnKey, Any]], dict[str, Any]]) -> None:
        ...
        for ndx, row in self.iterrows():
            row_dict: dict[ColumnKey, Any] = {self.columns[i]: row[i] for i in range(len(self.columns))}
...
class PartitionedTable(Media):
    ...
    def to_json(self, artifact_or_run):
        ...
        if isinstance(artifact_or_run, lumina.Run):
    ...
    def iterrows(self):
        ...
        for entry_path in self._loaded_part_entries:
            ...
            if columns is None:
                ...
                raise ValueError(f'Table parts have non-matching columns. {columns} != {part.columns}')
...
class JoinedTable(Media):
    ...
    def __init__(self, table1, table2, join_key):
        super().__init__()
        if not isinstance(join_key, str) and (not isinstance(join_key, list) or len(join_key) != 2):
            raise ValueError('JoinedTable join_key should be a string or a list of two strings')
        if not self._validate_table_input(table1):
            raise ValueError('JoinedTable table1 should be an artifact path to a table or wandb.Table object')
        if not self._validate_table_input(table2):
            raise ValueError('JoinedTable table2 should be an artifact path to a table or wandb.Table object')
        self._table1 = table1
        self._table2 = table2
    ...
    def _validate_table_input(table):
        ...
        return isinstance(table, str) and table.endswith('.table.json') or isinstance(table, (Table, PartitionedTable)) or (hasattr(table, 'ref_url') and table.ref_url().endswith('.table.json'))
    ...
    def to_json(self, artifact_or_run):
        json_obj = {'_type': JoinedTable._log_type}
        if isinstance(artifact_or_run, lumina.Run):
            artifact_entry_url = self._get_artifact_entry_ref_url()
            if artifact_entry_url is None:
                raise ValueError('JoinedTables must first be added to an Artifact before logging to a Run')
            json_obj['artifact_path'] = artifact_entry_url
        else:
            ...
            json_obj.update({'table1': table1, 'table2': table2, 'join_key': self._join_key})
        return json_obj
    ...
    def _eq_debug(self, other, should_assert=False):
        ...
        assert not should_assert or eq, f'Found type {other.__class__}, expected {JoinedTable}'
        eq = eq and self._join_key == other._join_key
        assert not should_assert or eq, f'Found {other._join_key} join key, expected {self._join_key}'
...
class _TableType(_dtypes.Type):
    ...
    def __init__(self, column_types=None):
        ...
        elif not isinstance(column_types, (_dtypes.TypedDictType, _dtypes.UnknownType)):
...
def _get_data_from_increments(json_obj: dict[str, Any], source_artifact: artifact.Artifact) -> list[Any]:
    ...
    if 'latest' not in source_artifact.aliases:
        lumina.termwarn('It is recommended to use the latest version of the incremental table artifact for ordering guarantees.', repeat=False)
    data: list[Any] = []
    ...
    def get_sort_key(key: str) -> tuple[int, int]:
        try:
            ...
            lumina.termwarn(f'Could not parse artifact entry for increment {key}. The entry name does not follow the naming convention <increment_number>-<timestamp>.<key>.table.json The data in the table will be out of order.', repeat=False)
            return (0, 0)
    ...
    for entry_key in sorted_increment_keys:
        try:
            ...
            raise lumina.Error(f'Invalid table file {entry_key}') from e
    return data

def _process_table_row(row: list[Any], timestamp_column_indices: set[_dtypes.TimestampType], np_deserialized_columns: dict[int, Any], source_artifact: artifact.Artifact, row_idx: int) -> list[Any]:
    ...
    for c_ndx, item in enumerate(row):
        ...
        if c_ndx in timestamp_column_indices and isinstance(item, (int, float)):
            cell = datetime.datetime.fromtimestamp(item / 1000, tz=datetime.timezone.utc)
        elif c_ndx in np_deserialized_columns:
            cell = np_deserialized_columns[c_ndx][row_idx]
        elif isinstance(item, dict) and '_type' in item and (obj := WBValue.init_from_json(item, source_artifact)):
            cell = obj
        else:
            cell = item
