...
import math
import typing as t
from lumina.util import _is_artifact_string, _is_artifact_version_weave_dict, get_module, is_numpy_array
np = get_module('numpy')
if t.TYPE_CHECKING:
...
class TypeRegistry:
    ...
    def add(wb_type: type[Type]) -> None:
        ...
        TypeRegistry.types_by_class().update({_type: wb_type for _type in wb_type.types})

    @staticmethod
    def type_of(py_obj: t.Any | None) -> Type:
        if py_obj.__class__ is float and math.isnan(py_obj):
            return NoneType()
    ...
    @staticmethod
    def type_from_dict(json_dict: dict[str, t.Any], artifact: Artifact | None=None) -> Type:
...
def _params_obj_to_json_obj(params_obj: t.Any, artifact: Artifact | None=None) -> t.Any:
    """Helper method."""
...
def _json_obj_to_params_obj(json_obj: t.Any, artifact: Artifact | None=None) -> t.Any:
    """Helper method."""
...
class Type:
    ...
    def to_json(self, artifact: Artifact | None=None) -> dict[str, t.Any]:
        ...
        """
        res = {'wb_type': self.name, 'params': _params_obj_to_json_obj(self.params, artifact)}
        if res['params'] is None or res['params'] == {}:
            del res['params']
        return res

    @classmethod
    def from_json(cls, json_dict: dict[str, t.Any], artifact: Artifact | None=None) -> Type:
    ...
    def __eq__(self, other):
        return self is other or (isinstance(self, Type) and isinstance(other, Type) and (self.name == other.name) and (self.params.keys() == other.params.keys()) and all([self.params[k] == other.params[k] for k in self.params]))
...
class AnyType(Type):
    ...
    def assign_type(self, wb_type: Type) -> AnyType | InvalidType:
        return self if not isinstance(wb_type, (NoneType, InvalidType)) else InvalidType()
...
def _union_assigner(allowed_types: list[Type], obj_or_type: Type | t.Any | None, type_mode=False) -> list[Type] | InvalidType:
    resolved_types = []
    valid = False
    unknown_count = 0
...
class UnionType(Type):
    ...
    def __init__(self, allowed_types: t.Sequence[ConvertibleToType] | None=None):
...
def OptionalType(dtype: ConvertibleToType) -> UnionType:
...
class ListType(Type):
    ...
    def __init__(self, element_type: ConvertibleToType | None=None, length: int | None=None):
    ...
    def assign_type(self, wb_type: Type) -> ListType | InvalidType:
        if isinstance(wb_type, ListType):
            assigned_type = self.params['element_type'].assign_type(wb_type.params['element_type'])
            if not isinstance(assigned_type, InvalidType):
                return ListType(assigned_type, None if self.params['length'] != wb_type.params['length'] else self.params['length'])
        return InvalidType()
...
class NDArrayType(Type):
    ...
    name = 'ndarray'
    types: t.ClassVar[list[type]] = []
    _serialization_path: dict[str, str] | None

    def __init__(self, shape: t.Sequence[int], serialization_path: dict[str, str] | None=None):
    ...
    def from_obj(cls, py_obj: t.Any | None=None) -> NDArrayType:
        if is_numpy_array(py_obj):
            ...
            raise TypeError(f'NDArrayType.from_obj expects py_obj to be ndarray or list, found {py_obj.__class__}')
    ...
    def _get_serialization_path(self) -> dict[str, str] | None:
...
class TypedDictType(Type):
    ...
    def __init__(self, type_map: dict[str, ConvertibleToType] | None=None):
        if type_map is None:
            type_map = {}
