...
class Plotly(Media):
    ...
    @classmethod
    def make_plot_media(cls: type[Plotly], val: plotly.Figure | matplotlib.artist.Artist) -> Image | Plotly:
    ...
    def __init__(self, val: plotly.Figure | matplotlib.artist.Artist):
        ...
        if not util.is_plotly_figure_typename(util.get_full_typename(val)):
            if util.is_matplotlib_typename(util.get_full_typename(val)):
                if util.matplotlib_contains_images(val):
                    raise ValueError('Plotly does not currently support converting matplotlib figures containing images.                             You can convert the plot to a static image with `wandb.Image(plt)` ')
                val = util.matplotlib_to_plotly(val)
            else:
