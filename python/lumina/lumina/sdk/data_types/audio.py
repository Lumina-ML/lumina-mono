...
class Audio(BatchableMedia):
    ...
    def __init__(self, data_or_path: str | pathlib.Path | list | np.ndarray, sample_rate: int | None=None, caption: str | None=None):
        ...
        if isinstance(data_or_path, (str, pathlib.Path)):
            ...
            soundfile = util.get_module('soundfile', required='Raw audio requires the soundfile package. To get it, run "pip install soundfile"')
    ...
    def from_json(cls, json_obj, source_artifact):
        ...
        <!-- lazydoc-ignore-classmethod: internal -->
        """
        return cls(source_artifact.get_entry(json_obj['path']).download(), caption=json_obj['caption'])

    def bind_to_run(self, run, key, step, id_=None, ignore_copy_err: bool | None=None):
    ...
    def to_json(self, run):
        ...
        json_dict.update({'_type': self._log_type})
        return json_dict
    ...
    def seq_to_json(cls, seq, run, key, step):
        ...
        """
        audio_list = list(seq)
        util.get_module('soundfile', required='wandb.Audio requires the soundfile package. To get it, run: pip install soundfile')
        base_path = os.path.join(run.dir, 'media', 'audio')
        filesystem.mkdir_exists_ok(base_path)
        meta = {'_type': 'audio', 'count': len(audio_list), 'audio': [a.to_json(run) for a in audio_list]}
    ...
    def __eq__(self, other):
        if self.path_is_reference(self._path) or self.path_is_reference(other._path):
            return self.resolve_ref() == other.resolve_ref() and self._caption == other._caption
