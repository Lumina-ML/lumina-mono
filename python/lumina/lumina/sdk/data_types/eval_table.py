...
import lumina
import lumina.integration.weave as weave_integration
import lumina.integration.weave.media_adapters as media_adapters
from lumina.errors import UsageError
from lumina.sdk.data_types.base_types.media import _numpy_arrays_to_lists
from lumina.sdk.data_types.table import ColumnKey, InputRow, LogMode, Table
if TYPE_CHECKING:
    import numpy as np
    import pandas as pd
    from lumina.sdk.wandb_run import Run as LocalRun
...
def _normalize_numpy_datetime64(val: Any) -> datetime.datetime | None:
    ...
    if isinstance(py_val, datetime.date):
        return datetime.datetime(py_val.year, py_val.month, py_val.day, tzinfo=datetime.timezone.utc)
...
def _normalize_non_media_value(val: Any) -> Any:
    ...
    val, _ = lumina.util.json_friendly(val)
...
def _normalize_value(val: Any, col: str | int, *, unsupported_media_mode: media_adapters.UnsupportedMediaMode) -> Any:
    ...
    """
    val = media_adapters.unwrap_value(val, col, unsupported_media_mode=unsupported_media_mode)
    val = media_adapters.handle_nested_wandb_values(val, col, unsupported_media_mode)
...
class EvalTable(Table):
    ...
    def __init__(self, columns: list[ColumnKey] | None=None, data: list[InputRow] | np.ndarray | pd.DataFrame | None=None, rows: list[InputRow] | None=None, dataframe: pd.DataFrame | None=None, dtype: Any=None, optional: bool | list[bool]=True, allow_mixed_types: bool=False, log_mode: LogMode='IMMUTABLE', *, input_columns: list[str] | None=None, output_columns: list[str] | None=None, score_columns: list[str] | None=None, unsupported_media_mode: media_adapters.UnsupportedMediaMode='stub') -> None:
        ...
        weave_integration.ensure_version(_MIN_WEAVE_VERSION, 'EvalTable dependency error. Fix with: `pip install wandb["eval-table"]`.')
        ...
        if columns is None and dataframe is None and (self._input_columns or self._output_columns or self._score_columns):
            table_columns = [*self._input_columns, *self._output_columns, *self._score_columns]
        super().__init__(columns=table_columns, data=data, rows=rows, dataframe=dataframe, dtype=dtype, optional=optional, allow_mixed_types=allow_mixed_types, log_mode=log_mode)
    ...
    def bind_to_run(self, run: LocalRun, key: int | str, step: int | str, id_: int | str | None=None, ignore_copy_err: bool | None=None) -> None:
        ...
        """
        if run.offline:
            raise UsageError("EvalTable does not support offline mode yet. Use wandb.init(mode='online') or unset WANDB_MODE.")
    ...
    def to_json(self, run_or_artifact: Any) -> dict[str, Any]:
        ...
        if not isinstance(run_or_artifact, lumina.Run):
        ...
        if self._run is not run:
            raise UsageError('EvalTable cannot be serialized for a different run than it was bound to.')
        ...
        json_dict = {'_type': 'eval-table', 'ncols': len(self.columns), 'nrows': len(self.data), 'log_mode': self.log_mode, 'evaluate_call_id': evaluate_call_id}
    ...
    def _validate_cell_value(self, val: Any, col: ColumnKey) -> None:
        media_adapters.validate_supported_value(val, col, unsupported_media_mode=self._unsupported_media_mode)
    ...
    def add_column(self, name: str, data: list[Any] | np.ndarray, optional: bool=False) -> None:
        if isinstance(data, list) or lumina.util.is_numpy_array(data):
            for val in data:
                self._validate_cell_value(val, name)
        super().add_column(name, data, optional=optional)

    def _validate_column_mappings(self, input_cols: list[str], output_cols: list[str], score_cols: list[str]) -> None:
        ...
        if unknown:
            raise ValueError(f'Column(s) {sorted(unknown)} listed in input/output/score_columns do not exist in the table.')
        seen: set[str] = set()
        for col in input_cols + output_cols + score_cols:
            if col in seen:
                lumina.termwarn(f'Column {col!r} appears in more than one role list; it will be included in all matching dicts.', repeat=False)
            seen.add(col)
    ...
    def _string_columns(self) -> list[str]:
        ...
        if duplicates:
            raise ValueError(f'EvalTable column names must be unique after converting to strings for Weave logging. Duplicate column name(s): {duplicates}.')
        return columns

    def _iterrows_for_weave(self, start: int=0) -> Iterator[dict[str, Any]]:
        str_columns = self._string_columns()
        for row in self.data[start:]:
            yield {str_col: _normalize_value(val, col, unsupported_media_mode=self._unsupported_media_mode) for col, str_col, val in zip(self.columns, str_columns, row, strict=True)}

    def _create_weave_eval_logger(self, eval_name: str) -> Any:
        from weave.evaluation.eval_imperative import EvaluationLogger
        self._validate_column_mappings(self._input_columns, self._output_columns, self._score_columns)
        return EvaluationLogger._create_with_meta(EVAL_TABLE_MARKER, name=eval_name)

    def _warn_immutable_already_logged(self) -> None:
        lumina.termwarn("EvalTable with log_mode='IMMUTABLE' has already been logged. Subsequent run.log() calls have no effect.", repeat=False)

    def _log_to_weave(self, eval_name: str) -> str:
        if self.log_mode == 'IMMUTABLE' and self._immutable_evaluate_call_id is not None:
            self._warn_immutable_already_logged()
            return self._immutable_evaluate_call_id
        ev = self._create_weave_eval_logger(eval_name)
        str_columns = self._string_columns()
        assigned = set(self._input_columns) | set(self._output_columns) | set(self._score_columns)
        output_cols = self._output_columns + [col for col in str_columns if col not in assigned]
        inject_row_index = not self._input_columns
        start_idx = 0
        for offset, row in enumerate(self._iterrows_for_weave(start=start_idx)):
            row_idx = start_idx + offset + 1
            if inject_row_index:
