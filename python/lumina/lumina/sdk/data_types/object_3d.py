...
from lumina.sdk.lib import json_util, runid
...
if TYPE_CHECKING:
    ...
    numeric: TypeAlias = int | float | np.integer | np.float64
    FileFormat3D = Literal['obj', 'gltf', 'glb', 'babylon', 'stl', 'pts.json']
    ...
    Point: TypeAlias = Point3D | Point3DWithCategory | Point3DWithColors
...
def _install_numpy_error() -> lumina.Error:
    return lumina.Error("wandb.Object3D requires NumPy. To get it, run 'pip install numpy'.")
...
def _quaternion_to_rotation(quaternion: np.ndarray) -> np.ndarray:
    ...
    return np.array(((1 - 2 * (qjj + qkk), 2 * (qij + qkr), 2 * (qik - qjr)), (2 * (qij - qkr), 1 - 2 * (qii + qkk), 2 * (qjk + qir)), (2 * (qik + qjr), 2 * (qjk - qir), 1 - 2 * (qii + qjj))), dtype=np.float64)

def box3d(*, center: npt.ArrayLike, size: npt.ArrayLike, orientation: npt.ArrayLike, color: RGBColor, label: str | None=None, score: numeric | None=None) -> Box3D:
    ...
    unit_corners = np.array(list(itertools.product((-1, 1), (-1, 1), (-1, 1))), dtype=np.float64)
    corners = center + 0.5 * size * unit_corners @ rot
    return {'corners': cast('tuple[Point3D, Point3D, Point3D, Point3D, Point3D, Point3D, Point3D, Point3D]', tuple((tuple(pt) for pt in corners))), 'color': color, 'label': label, 'score': score}

class Object3D(BatchableMedia):
    """W&B class for 3D point clouds."""
    SUPPORTED_TYPES: ClassVar[set[str]] = {'obj', 'gltf', 'glb', 'babylon', 'stl', 'pts.json'}
    SUPPORTED_POINT_CLOUD_TYPES: ClassVar[set[str]] = {'lidar/beta'}
    _log_type: ClassVar[str] = 'object3D-file'

    def __init__(self, data_or_path: np.ndarray | str | pathlib.Path | TextIO | dict, caption: str | None=None, **kwargs: str | FileFormat3D | None) -> None:
        ...
        if hasattr(data_or_path, 'read'):
            ...
            if extension is None:
                raise ValueError('Must pass file type keyword argument when using io objects.')
            if extension not in Object3D.SUPPORTED_TYPES:
                raise ValueError('Object 3D only supports numpy arrays or files of the type: ' + ', '.join(Object3D.SUPPORTED_TYPES))
            extension = '.' + extension
            ...
            for supported_type in Object3D.SUPPORTED_TYPES:
                if path.endswith(supported_type):
                    ...
                    break
            if not extension:
                raise ValueError("File '" + path + "' is not compatible with Object3D: supported types are: " + ', '.join(Object3D.SUPPORTED_TYPES))
            ...
            if data_or_path['type'] == 'lidar/beta':
                data = {'type': data_or_path['type'], 'vectors': data_or_path['vectors'].tolist() if 'vectors' in data_or_path else [], 'points': data_or_path['points'].tolist() if 'points' in data_or_path else [], 'boxes': data_or_path['boxes'].tolist() if 'boxes' in data_or_path else []}
            else:
    ...
    def from_point_cloud(cls, points: Sequence[Point], boxes: Sequence[Box3D], vectors: Sequence[Vector3D] | None=None, point_cloud_type: PointCloudType='lidar/beta') -> Object3D:
        ...
        numpy = lumina.util.get_module('numpy', required='wandb.Object3D.from_point_cloud requires numpy. Install with `pip install numpy`')
    ...
    def to_json(self, run_or_artifact: LocalRun | Artifact) -> dict:
        ...
        if isinstance(run_or_artifact, lumina.Artifact) and (self._path is None or not self._path.endswith('.pts.json')):
            raise ValueError('Non-point cloud 3D objects are not yet supported with Artifacts')
        return json_dict

    @classmethod
    def seq_to_json(cls: type[Object3D], seq: Sequence[BatchableMedia], run: LocalRun, key: str, step: int | str) -> dict:
        ...
        for obj in jsons:
            ...
            if not obj['path'].startswith(expected):
                raise ValueError("Files in an array of Object3D's must be in the {} directory, not {}".format(expected, obj['path']))
        return {'_type': 'object3D', 'filenames': [os.path.relpath(j['path'], cls.get_media_subdir()) for j in jsons], 'count': len(jsons), 'objects': jsons}
