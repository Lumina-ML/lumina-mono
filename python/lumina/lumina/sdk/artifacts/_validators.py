...
from lumina._iterutils import always_list, unique_list
from lumina._pydantic import from_json
from lumina._strutils import nameof
from lumina.sdk.lib.paths import FilePathStr, LogicalPath, StrPath
from lumina.util import json_friendly_val
...
def validate_artifact_path(path: StrPath) -> LogicalPath:
    ...
    if logical_path == '.' or posix_path.anchor or '..' in posix_path.parts or windows_path.anchor or ('..' in windows_path.parts):
...
def validate_artifact_name(name: str) -> str:
    ...
    if len(name) > NAME_MAXLEN:
        ...
        raise ValueError(f'Artifact name is longer than {NAME_MAXLEN!r} characters: {trunc_name!r}')
    if INVALID_ARTIFACT_NAME_CHARS.intersection(name):
        raise ValueError(f"Artifact names must not contain any of the following characters: {', '.join(sorted(INVALID_ARTIFACT_NAME_CHARS))}.  Got: {name!r}")
    return name
...
def validate_project_name(name: str) -> str:
    ...
    if (invalid_chars := set(INVALID_URL_CHARS).intersection(name)):
        ...
        raise ValueError(f'Invalid project/registry name {error_name!r}, cannot contain characters: {invalid_chars_repr!s}')
    return name
...
def validate_aliases(aliases: Iterable[str] | str) -> list[str]:
    ...
    if any((ARTIFACT_SEP_CHARS.intersection(name) for name in aliases_list)):
        ...
        raise ValueError(f'Aliases must not contain any of the following characters: {invalid_chars}')
    return aliases_list
...
def validate_artifact_types(types: Iterable[str] | str) -> list[str]:
    ...
    if any((ARTIFACT_SEP_CHARS.intersection(name) for name in types_list)):
        invalid_chars = ', '.join(sorted(map(repr, ARTIFACT_SEP_CHARS)))
        raise ValueError(f'Artifact types must not contain any of the following characters: {invalid_chars}')
    if any((len(name) > NAME_MAXLEN for name in types_list)):
        raise ValueError(f'Artifact types must be less than or equal to {NAME_MAXLEN!r} characters')
    return types_list
TAG_REGEX: re.Pattern[str] = re.compile('^[-\\w]+( +[-\\w]+)*$')
...
def validate_tags(tags: Iterable[str] | str) -> list[str]:
    ...
    if any((not TAG_REGEX.match(tag) for tag in tags_list)):
        raise ValueError('Invalid tag(s).  Tags must only contain alphanumeric characters separated by hyphens, underscores, and/or spaces.')
    return tags_list
RESERVED_ARTIFACT_TYPE_PREFIX: Final[str] = 'wandb-'
'Internal, reserved artifact type prefix.'
RESERVED_ARTIFACT_NAME_PREFIX_BY_TYPE: Final[dict[str, str]] = {'job': '', 'run_table': 'run-', 'code': 'source-'}
...
def validate_artifact_type(typ: str, name: str) -> str:
    ...
    if (bad_prefix := RESERVED_ARTIFACT_NAME_PREFIX_BY_TYPE.get(typ)) is not None and name.startswith(bad_prefix) or typ.startswith(RESERVED_ARTIFACT_TYPE_PREFIX):
        raise ValueError(f'Artifact type {typ!r} is reserved for internal use. Please use a different type.')
    return typ
...
def _(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(metadata) > MAX_ARTIFACT_METADATA_KEYS:
        raise ValueError(f'Artifact must not have more than {MAX_ARTIFACT_METADATA_KEYS!r} metadata keys.')
    return metadata
...
def remove_registry_prefix(project: str) -> str:
    if not is_artifact_registry_project(project):
        raise ValueError(f'Project {project!r} is not a registry project. Must start with: {REGISTRY_PREFIX!r}')
...
class ArtifactPath:
    ...
    def with_defaults(self, *, prefix: str | None=None, project: str | None=None) -> Self:
        """Returns a copy of this path with missing values set to the given defaults."""
        return replace(self, prefix=self.prefix or prefix, project=self.project or project)
