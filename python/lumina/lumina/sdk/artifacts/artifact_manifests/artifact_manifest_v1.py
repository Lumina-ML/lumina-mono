...
from lumina.sdk.lib.hashutil import HexMD5, _md5
...
class ArtifactManifestV1(ArtifactManifest):
    ...
    storage_policy: StoragePolicy = Field(default_factory=make_storage_policy, exclude=True, repr=False)
    ...
    def from_manifest_json(cls, manifest_json: dict[str, Any]) -> ArtifactManifestV1:
        ...
        return cls(manifest_version=data.version, entries=data.contents, storage_policy=policy)
    ...
    def to_manifest_json(self) -> dict:
        ...
        return {'version': self.manifest_version, 'storagePolicy': self.storage_policy.name(), 'storagePolicyConfig': self.storage_policy.config(), 'contents': {path: entry.model_dump(exclude=omit_entry_fields, exclude_defaults=True) for path, entry in self.entries.items()}}
