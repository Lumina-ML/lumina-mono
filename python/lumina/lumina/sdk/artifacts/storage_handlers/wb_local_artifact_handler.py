...
import lumina
from lumina.sdk.artifacts.artifact_instance_cache import artifact_instance_cache_by_client_id
...
class WBLocalArtifactHandler(StorageHandler):
    ...
    def load_path(self, manifest_entry: ArtifactManifestEntry, local: bool=False) -> URIStr | FilePathStr:
        raise NotImplementedError('Should not be loading a path for an artifact entry with unresolved client id.')
    ...
    def store_path(self, artifact: Artifact, path: URIStr | FilePathStr, name: StrPath | None=None, checksum: bool=True, max_objects: int | None=None) -> list[ArtifactManifestEntry]:
        ...
        if not isinstance(target_artifact, lumina.Artifact):
            raise TypeError('Artifact passed to store_path() must be a wandb.Artifact.')
        target_entry = target_artifact.manifest.entries[target_path]
        if target_entry is None:
            raise RuntimeError('Local entry not found - invalid reference')
        return [ArtifactManifestEntry(path=name or os.path.basename(path), ref=path, size=0, digest=target_entry.digest)]
