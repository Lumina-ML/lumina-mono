...
class TrackingHandler(StorageHandler):
    ...
    def store_path(self, artifact: Artifact, path: URIStr | FilePathStr, name: StrPath | None=None, checksum: bool=True, max_objects: int | None=None) -> list[ArtifactManifestEntry]:
        url = urlparse(path)
        if name is None:
            raise ValueError(f'You must pass name="<entry_name>" when tracking references with unknown schemes. ref: {path}')
        termwarn(f'Artifact references with unsupported schemes cannot be checksummed: {path}')
