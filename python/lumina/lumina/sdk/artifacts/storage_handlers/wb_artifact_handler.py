...
from lumina.apis import PublicApi
...
class WBArtifactHandler(StorageHandler):
    ...
    def store_path(self, artifact: Artifact, path: URIStr | FilePathStr, name: StrPath | None=None, checksum: bool=True, max_objects: int | None=None) -> list[ArtifactManifestEntry]:
        ...
        path = f'{self._scheme}://{b64_to_hex_id(target_artifact.id)}/{artifact_file_path}'
        assert entry is not None
        return [ArtifactManifestEntry(path=name or os.path.basename(path), ref=path, size=0, digest=entry.digest)]
