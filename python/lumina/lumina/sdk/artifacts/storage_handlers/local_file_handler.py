...
from lumina.sdk.lib.hashutil import B64MD5, md5_file_b64, md5_string
from lumina.sdk.lib.paths import FilePathStr, StrPath, URIStr
from lumina.util import local_file_uri_to_path
...
class LocalFileHandler(StorageHandler):
    ...
    def load_path(self, manifest_entry: ArtifactManifestEntry, local: bool=False) -> URIStr | FilePathStr:
        ...
        if not os.path.exists((local_path := local_file_uri_to_path(ref_uri))):
            raise ValueError(f'Local file reference: Failed to find file at path {local_path!r}')
        expected_digest = manifest_entry.digest
        path, hit, cache_open = self._cache.check_md5_obj_path(b64_md5=expected_digest, size=manifest_entry.size or 0)
        if hit:
            return path
        if (digest := md5_file_b64(local_path)) != expected_digest:
            raise ValueError(f'Local file reference: Digest mismatch for path {local_path!r}: expected {expected_digest!r} but found {digest!r}')
    ...
    def store_path(self, artifact: Artifact, path: URIStr | FilePathStr, name: StrPath | None=None, checksum: bool=True, max_objects: int | None=None) -> list[ArtifactManifestEntry]:
        ...
        if os.path.isdir(local_path):
            ...
            with TimedIf(checksum):
                if checksum:
                    termlog(f'Generating checksum for up to {max_objects!r} files in {local_path!r}... ', newline=False)
                physical_paths = (os.path.join(root, subpath) for root, _, files in os.walk(local_path) for subpath in files)
                for i, physical_path in enumerate(physical_paths):
                    ...
                    entry = ArtifactManifestEntry(path=artifact_path, ref=os.path.join(path, file_path), size=os.path.getsize(physical_path), digest=check_md5(physical_path))
                    entries.append(entry)
            return list(entries)
        if os.path.isfile(local_path):
            return [ArtifactManifestEntry(path=name or os.path.basename(local_path), ref=path, size=os.path.getsize(local_path), digest=check_md5(local_path))]
        else:
