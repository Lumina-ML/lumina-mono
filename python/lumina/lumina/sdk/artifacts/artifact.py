...
from typing import IO, TYPE_CHECKING, Any, Final, Literal, Type
from urllib.parse import quote, urljoin, urlparse
from pydantic import NonNegativeInt
import lumina
from lumina import data_types, env
from lumina._iterutils import one, unique_list
from lumina._pydantic import from_json
from lumina._strutils import nameof
from lumina.apis.normalize import normalize_exceptions
from lumina.apis.public import ArtifactCollection, ArtifactFiles, Run
from lumina.data_types import WBValue
from lumina.errors import CommError
from lumina.errors.term import termerror, termlog, termwarn
...
from lumina.sdk.data_types._dtypes import Type as WBType
from lumina.sdk.data_types._dtypes import TypeRegistry
from lumina.sdk.lib import retry, telemetry
from lumina.sdk.lib.deprecation import warn_and_record_deprecation
from lumina.sdk.lib.filesystem import check_exists, system_preferred_path
from lumina.sdk.lib.hashutil import B64MD5, b64_to_hex_id, md5_file_b64
from lumina.sdk.lib.paths import FilePathStr, LogicalPath, StrPath, URIStr
from lumina.sdk.lib.runid import generate_fast_id, generate_id
from lumina.sdk.mailbox import MailboxHandle
from lumina.util import alias_is_version_index, artifact_to_json, fsync_open, json_dumps_safer, uri_from_path
...
from ._validators import ensure_logged, ensure_not_finalized, validate_artifact_path, validate_fspath
from .artifact_download_logger import ArtifactDownloadLogger
from .artifact_instance_cache import artifact_instance_cache, artifact_instance_cache_by_client_id
...
from .exceptions import ArtifactNotLoggedError, TooFewItemsError, TooManyItemsError, WaitTimeoutError
...
class Artifact:
    ...
    def __init__(self, name: str, type: str, description: str | None=None, metadata: dict[str, Any] | None=None, incremental: bool=False, use_as: str | None=None, storage_region: str | None=None) -> None:
        from lumina.sdk.artifacts._internal_artifact import InternalArtifact
        from ._validators import validate_artifact_name, validate_artifact_type, validate_metadata
        if not re.match('^[a-zA-Z0-9_\\-.]+$', name):
            raise ValueError(f'Artifact name may only contain alphanumeric characters, dashes, underscores, and dots. Invalid name: {name!r}')
        ...
        self._name: str = validate_artifact_name(name)
        ...
        if use_as is not None:
            warn_and_record_deprecation(feature=Deprecated(artifact__init_use_as=True), message='`use_as` argument is deprecated and does not affect the behaviour of `wandb.Artifact()`')
        self._use_as: str | None = None
        self._state: ArtifactState = ArtifactState.PENDING
        self._size: NonNegativeInt | None = None
        self._digest: str | None = None
        self._manifest: ArtifactManifest | None = ArtifactManifestV1(storage_policy=make_storage_policy(region=storage_region))
    ...
    def _from_id(cls, artifact_id: str, service_api: ServiceApi) -> Artifact | None:
        ...
        result = service_api.execute_graphql(ARTIFACT_BY_ID_GQL, variables={'id': artifact_id}, parse=ArtifactByID.model_validate_json)
    ...
    def _from_name(cls, *, path: FullArtifactPath, service_api: ServiceApi) -> Artifact:
        from ._generated import ARTIFACT_MEMBERSHIP_BY_NAME_GQL, ArtifactMembershipByName
        result = service_api.execute_graphql(ARTIFACT_MEMBERSHIP_BY_NAME_GQL, {'entity': path.prefix, 'project': path.project, 'name': path.name}, parse=ArtifactMembershipByName.model_validate_json)
    ...
    @classmethod
    def _from_membership(cls, membership: ArtifactMembershipFragment, target: FullArtifactPath, service_api: ServiceApi) -> Artifact:
        from ._validators import is_artifact_registry_project
        if not ((collection := membership.artifact_collection) and (name := collection.name) and (proj := collection.project)):
            raise ValueError('Missing artifact collection project in GraphQL response')
        if is_artifact_registry_project(proj.name) and target.project == 'model-registry':
            lumina.termwarn(f"This model registry has been migrated and will be discontinued. Your request was redirected to the corresponding artifact {name!r} in the new registry. Please update your paths to point to the migrated registry directly, '{proj.name}/{name}'.")
        ...
        return cls._from_attrs(new_target, artifact, service_api, membership=membership)

    @classmethod
    def _from_attrs(cls, path: FullArtifactPath, src_art: ArtifactFragment, service_api: ServiceApi, *, membership: ArtifactMembershipFragment | None=None) -> Artifact:
    ...
    def _assign_attrs(self, src_art: ArtifactFragment, *, membership: ArtifactMembershipFragment | None=None, is_link: bool | None=None) -> None:
        ...
        if is_link is None:
            self._is_link = self._entity != self._source_entity or self._project != self._source_project or self._name.split(':')[0] != self._source_name.split(':')[0]
        else:
        ...
        if membership:
            ...
            aliases = [a.alias for a in src_art.aliases if (alias_coll := a.artifact_collection) and (alias_proj := alias_coll.project) and (alias_proj.entity.name == entity) and (alias_proj.name == project) and (alias_coll.name == collection)]
        else:
            aliases = []
        version_aliases = list(filter(alias_is_version_index, aliases))
        other_aliases = list(filterfalse(alias_is_version_index, aliases))
        try:
            version = one(version_aliases, too_short=TooFewItemsError, too_long=TooManyItemsError)
        ...
        self._ttl_duration_seconds = validate_ttl_duration_seconds(src_art.ttl_duration_seconds)
    ...
    def new_draft(self) -> Artifact:
        ...
        artifact._manifest = ArtifactManifest.from_manifest_json(self.manifest.to_manifest_json())
        return artifact
    ...
    def collection(self) -> ArtifactCollection:
        ...
        return ArtifactCollection(service_api, self.entity, self.project, base_name, self.type)
    ...
    def source_collection(self) -> ArtifactCollection:
        ...
        return ArtifactCollection(client, self.source_entity, self.source_project, base_name, self.type)
    ...
    def source_artifact(self) -> Artifact:
        ...
        if self._source_artifact is None:
            try:
                path = FullArtifactPath(prefix=self.source_entity, project=self.source_project, name=self.source_name)
                self._source_artifact = self._from_name(path=path, service_api=self._get_service_api())
            except Exception as e:
                raise ValueError(f'Unable to fetch source artifact for linked artifact {self.name}') from e
    ...
    def url(self) -> str:
        ...
        from ._validators import is_artifact_registry_project
        try:
            base_url = self._service_api.app_url
    ...
    def _construct_standard_url(self, base_url: str) -> str:
        if not all([base_url, self.entity, self.project, self._type, self.collection.name, self._version]):
            return ''
        return urljoin(base_url, f'{self.entity}/{self.project}/artifacts/{quote(self._type)}/{quote(self.collection.name)}/{self._version}')

    def _construct_registry_url(self, base_url: str) -> str:
        from ._validators import remove_registry_prefix
        if not all([base_url, self.entity, self.project, self.collection.name, self._version]):
            return ''
        try:
            org_name = org_info_from_entity(self._service_api, self.entity).organization.name
        except (AttributeError, ValueError):
            return ''
        selection_path = quote(f'{self.entity}/{self.project}/{self.collection.name}', safe='')
        return urljoin(base_url, f'orgs/{org_name}/registry/{remove_registry_prefix(self.project)}?selectionPath={selection_path}&view=membership&version={self.version}')

    def _construct_model_registry_url(self, base_url: str) -> str:
        if not all([base_url, self.entity, self.project, self.collection.name, self._version]):
            return ''
        selection_path = quote(f'{self.entity}/{self.project}/{self.collection.name}', safe='')
        return urljoin(base_url, f'{self.entity}/registry/model?selectionPath={selection_path}&view=membership&version={self._version}')
    ...
    def description(self, description: str | None) -> None:
        ...
        """
        if self.is_link:
            lumina.termwarn("Editing the description of this linked artifact will edit the description for the source artifact and it's linked artifacts as well.")
    ...
    def metadata(self, metadata: dict) -> None:
        ...
        if self.is_link:
            lumina.termwarn("Editing the metadata of this linked artifact will edit the metadata for the source artifact and it's linked artifacts as well.")
    ...
    def ttl(self, ttl: timedelta | ArtifactTTL | None) -> None:
        ...
        if self.is_link:
            raise ValueError('Cannot set TTL for link artifact. Unlink the artifact first then set the TTL for the source artifact')
        ...
        if isinstance(ttl, ArtifactTTL):
            ...
            if ttl is None:
                ...
                if ttl.total_seconds() <= 0:
                    raise ValueError(f'Artifact TTL Duration has to be positive. ttl: {ttl.total_seconds()}')
    ...
    def tags(self, tags: list[str]) -> None:
        ...
        if self.is_link:
            lumina.termwarn('Editing tags will apply the changes to the source artifact and all linked artifacts associated with it.')
    ...
    def use_as(self) -> str | None:
        """Deprecated."""
        warn_and_record_deprecation(feature=Deprecated(artifact__use_as=True), message='The use_as property of Artifact is deprecated.')
        return self._use_as
    ...
    def _fetch_manifest(self) -> ArtifactManifest:
        ...
        result = client.execute_graphql(FETCH_ARTIFACT_MANIFEST_GQL, variables={'id': self.id}, parse=FetchArtifactManifest.model_validate_json)
    ...
    def digest(self) -> str:
        ...
        """
        return self._digest if self._manifest is None and self._digest is not None else self.manifest.digest()
    ...
    def size(self) -> int:
        ...
        """
        return self._size if self._manifest is None and self._size is not None else self.manifest.size()
    ...
    def save(self, project: str | None=None, settings: wandb.Settings | None=None) -> None:
        ...
        if (run := wandb_setup.singleton().most_recent_active_run):
            ...
            if settings is None:
                settings = lumina.Settings(silent='true')
            with lumina.init(entity=self._source_entity, project=project or self._source_project, job_type='auto', settings=settings) as run:
                if self._incremental:
                    with telemetry.context(run=run) as tel:
                        tel.feature.artifact_incremental = True
                run.log_artifact(self)

    def _set_save_handle(self, save_handle: MailboxHandle[pb.Result], service_api: ServiceApi) -> None:
    ...
    def wait(self, timeout: int | None=None) -> Artifact:
        ...
        if self.is_draft():
            ...
            try:
                ...
                raise WaitTimeoutError('Artifact upload wait timed out, failed to fetch Artifact response') from e
    ...
    def _populate_after_save(self, artifact_id: str) -> None:
        ...
        if (client := self._service_api) is None:
            raise RuntimeError('Client not initialized for artifact queries')
        result = client.execute_graphql(ARTIFACT_BY_ID_GQL, variables={'id': artifact_id}, parse=ArtifactByID.model_validate_json)
    ...
    def _update(self) -> None:
        ...
        if not skip_update_artifact:
            ...
            gql_input = UpdateArtifactInput(artifact_id=self.id, description=self.description, metadata=json_dumps_safer(self.metadata), ttl_duration_seconds=self._ttl_duration_seconds_to_gql(), tags_to_add=[{'tagName': t} for t in validate_tags(new_tags - old_tags)], tags_to_delete=[{'tagName': t} for t in validate_tags(old_tags - new_tags)])
            data = client.execute_graphql(UPDATE_ARTIFACT_GQL, variables={'input': gql_input.model_dump()}, parse=UpdateArtifact.model_validate_json)
            ...
            lumina.termwarn('Skipping non-alias updates due to insufficient permissions on the source artifact.')
    ...
    def _is_source_project_read_only(self) -> bool:
        ...
        if (client := self._service_api) is None:
            raise RuntimeError('Client not initialized for artifact mutations')
        read_only = is_project_read_only(client, self.source_entity, self.source_project)
    ...
    def _add_aliases(self, alias_names: set[str], target: FullArtifactPath) -> None:
        ...
        if (client := self._service_api) is None:
            raise RuntimeError('Client not initialized for artifact mutations')
        if alias_names:
            target_props = {'entityName': target.prefix, 'projectName': target.project, 'artifactCollectionName': target.name}
            ...
            try:
                ...
                msg = f"You do not have permission to add {('at least one of the following aliases' if len(alias_names) > 1 else 'the following alias')} to this artifact: {alias_names!r}"
    ...
    def _delete_aliases(self, alias_names: set[str], target: FullArtifactPath) -> None:
        ...
        if alias_names:
            ...
            try:
                ...
                msg = f"You do not have permission to delete {('at least one of the following aliases' if len(alias_names) > 1 else 'the following alias')} from this artifact: {alias_names!r}"
                raise CommError(msg) from e
    ...
    def new_file(self, name: str, mode: str='x', encoding: str | None=None) -> Generator[IO]:
        ...
        try:
            ...
            termerror(f'Failed to open the provided file ({nameof(type(e))}: {e}). Please provide the proper encoding.')
            raise
        self.add_file(path, name=name, policy='immutable', skip_cache=True, overwrite=overwrite)

    @ensure_not_finalized
    def add_file(self, local_path: str, name: str | None=None, is_tmp: bool | None=False, skip_cache: bool | None=False, policy: Literal['mutable', 'immutable'] | None='mutable', overwrite: bool=False) -> ArtifactManifestEntry:
        ...
        return self._add_local_file(name, local_path, digest=digest, skip_cache=skip_cache, policy=policy, overwrite=overwrite)

    @ensure_not_finalized
    def add_dir(self, local_path: str, name: str | None=None, skip_cache: bool | None=False, policy: Literal['mutable', 'immutable'] | None='mutable', merge: bool=False) -> None:
        ...
        termlog(f"Adding directory to artifact ({Path('.', local_path)})... ", newline=False)
        start_time = time.monotonic()
        paths: deque[tuple[str, str]] = deque()
        logical_root = name or ''
        ...
        def add_manifest_file(logical_pth: str, physical_pth: str) -> None:
            self._add_local_file(name=logical_pth, path=physical_pth, skip_cache=skip_cache, policy=policy, overwrite=merge)
        num_threads = 8
    ...
    def add_reference(self, uri: ArtifactManifestEntry | str, name: StrPath | None=None, checksum: bool=True, max_objects: int | None=None) -> Sequence[ArtifactManifestEntry]:
        ...
        manifest_entries = self.manifest.storage_policy.store_reference(self, URIStr(uri_str), name=name, checksum=checksum, max_objects=max_objects)
        for entry in manifest_entries:
            self.manifest.add_entry(entry)
        return manifest_entries

    @ensure_not_finalized
    def add(self, obj: WBValue, name: StrPath, overwrite: bool=False) -> ArtifactManifestEntry:
        ...
        allowed_types = (data_types.Bokeh, data_types.JoinedTable, data_types.PartitionedTable, data_types.Table, data_types.Classes, data_types.ImageMask, data_types.BoundingBoxes2D, data_types.Audio, data_types.Image, data_types.Video, data_types.Html, data_types.Object3D, data_types.Molecule, data_types._SavedModel)
        if not isinstance(obj, allowed_types):
            raise TypeError(f'Found object of type {obj.__class__}, expected one of: {allowed_types}')
        obj_id = id(obj)
    ...
    def _add_local_file(self, name: StrPath, path: StrPath, digest: B64MD5 | None=None, skip_cache: bool | None=False, policy: Literal['mutable', 'immutable'] | None='mutable', overwrite: bool=False) -> ArtifactManifestEntry:
        policy = policy or 'mutable'
        if policy not in ['mutable', 'immutable']:
            raise ValueError(f'Invalid policy {policy!r}. Policy may only be `mutable` or `immutable`.')
        upload_path = path
        if policy == 'mutable':
        ...
        entry = ArtifactManifestEntry(path=name, digest=digest or md5_file_b64(upload_path), size=os.path.getsize(upload_path), local_path=upload_path, skip_cache=skip_cache)
    ...
    def get_path(self, name: StrPath) -> ArtifactManifestEntry:
        ...
        warn_and_record_deprecation(feature=Deprecated(artifact__get_path=True), message='Artifact.get_path(name) is deprecated, use Artifact.get_entry(name) instead.')
    ...
    def _get_obj_entry(self, name: str) -> tuple[ArtifactManifestEntry, Type[WBValue]] | tuple[None, None]:
    ...
    @ensure_logged
    def download(self, root: StrPath | None=None, allow_missing_references: bool=False, skip_cache: bool | None=None, path_prefix: StrPath | None=None, multipart: bool | None=None) -> FilePathStr:
        ...
        return self._download(root=root, allow_missing_references=allow_missing_references, skip_cache=skip_cache, path_prefix=path_prefix, multipart=multipart)

    def _download_using_core(self, root: str, allow_missing_references: bool=False, skip_cache: bool=False, path_prefix: StrPath | None=None) -> FilePathStr:
        import pathlib
        if lumina.run is None:
            ...
            assert lumina.run._interface
            interface = lumina.run._interface
        handle = interface.deliver_download_artifact(self.id, root, allow_missing_references, skip_cache, path_prefix)
        self._download(root=root, allow_missing_references=allow_missing_references, skip_cache=skip_cache, path_prefix=path_prefix)
    ...
    def _download(self, root: str, allow_missing_references: bool=False, skip_cache: bool | None=None, path_prefix: StrPath | None=None, multipart: bool | None=None) -> FilePathStr:
        nfiles = len(self.manifest.entries)
        size_mb = self.size / _MB
        if (log := (nfiles > 5000 or size_mb > 50)):
            termlog(f'Downloading large artifact {self.name!r}, {size_mb:.2f}MB. {nfiles!r} files...')
            start_time = time.monotonic()
        download_logger = ArtifactDownloadLogger(nfiles=nfiles)

        def _download_entry(entry: ArtifactManifestEntry, mp_executor: Executor) -> None:
            multipart_executor = mp_executor if should_multipart_download(entry.size, override=multipart) else None
            try:
                ...
                lumina.termwarn(f'Unable to download file {entry.path!r} as there is a directory with the same path, skipping.')
                return
            except NotADirectoryError:
                lumina.termwarn(f'Unable to download file {entry.path!r} as there is a file with the same path as a directory this file is expected to be in, skipping.')
                return
            download_logger.notify_downloaded()
        with ThreadPoolExecutor(max_workers=_FILE_EXECUTOR_WORKERS) as file_executor, ThreadPoolExecutor(max_workers=_MP_EXECUTOR_WORKERS) as mp_executor:
            ...
            while has_more:
                ...
                for node in file_nodes:
                    ...
                    if not path_prefix or entry.path.startswith(str(path_prefix)):
                        active_futures.add(file_executor.submit(_download_entry, entry, mp_executor=mp_executor))
        ...
        if log:
            ...
            termlog(f'Done. {int(hrs):02d}:{int(mins):02d}:{secs:04.1f} ({size_mb / dt_secs:.1f}MB/s)', prefix=False)
        return FilePathStr(root)

    def _fetch_file_urls(self, cursor: str | None, per_page: int=5000) -> FileWithUrlConnection:
        import requests

        @retry.retriable(retry_timedelta=timedelta(minutes=3), retryable_exceptions=requests.RequestException)
        def _fetch() -> FileWithUrlConnection:
            from ._generated import GET_ARTIFACT_MEMBERSHIP_FILE_URLS_GQL, GetArtifactMembershipFileUrls
            from ._models.pagination import FileWithUrlConnection
            if self._service_api is None:
                raise RuntimeError('Client not initialized')
            query = GET_ARTIFACT_MEMBERSHIP_FILE_URLS_GQL
            gql_vars = {'entity': self.entity, 'project': self.project, 'collection': self.name.split(':')[0], 'alias': self.version, 'cursor': cursor, 'perPage': per_page}
            membership_result = self._service_api.execute_graphql(query, variables=gql_vars, timeout=60, parse=GetArtifactMembershipFileUrls.model_validate_json)
            if not ((project := membership_result.project) and (collection := project.artifact_collection) and (membership := collection.artifact_membership) and (membership_files := membership.files)):
        ...
        for dirpath, _, files in os.walk(root):
            for file in files:
                ...
                try:
                    ...
                    raise ValueError(f'Found file {full_path} which is not a member of artifact {self.name}')
        ref_count = 0
    ...
    def file(self, root: str | None=None) -> StrPath:
        ...
        if len(self.manifest.entries) > 1:
            raise ValueError('This artifact contains more than one file, call `.download()` to get all files or call .get_entry("filename").download()')
        return self.get_entry(list(self.manifest.entries)[0]).download(root)

    @ensure_logged
    def files(self, names: list[str] | None=None, per_page: int=50, start: str | None=None) -> ArtifactFiles:
    ...
    def delete(self, delete_aliases: bool=False) -> None:
        ...
        """
        if self.is_link:
            lumina.termwarn('Deleting a link artifact will only unlink the artifact from the source artifact and not delete the source artifact and the data of the source artifact.')
            self._unlink()
        else:
    ...
    def _delete(self, delete_aliases: bool=False) -> None:
        ...
        gql_input = DeleteArtifactInput(artifact_id=self.id, delete_aliases=delete_aliases)
        self._service_api.execute_graphql(gql_op, variables={'input': gql_input.model_dump()})
    ...
    def link(self, target_path: str, aliases: Iterable[str] | None=None) -> Artifact:
        ...
        if self.is_link:
            lumina.termwarn('Linking to a link artifact will result in directly linking to the source artifact of that link artifact.')
        if self.is_draft():
        ...
        target = ArtifactPath.from_str(target_path).with_defaults(project=settings.get('project') or 'uncategorized')
        if target.is_registry_path():
            org = target.prefix or settings.get('organization') or None
            target.prefix = resolve_org_entity_name(service_api, self.source_entity, org)
        else:
            target = target.with_defaults(prefix=self.source_entity)
        target = FullArtifactPath(**asdict(target))
        alias_inputs = [{'artifactCollectionName': target.name, 'alias': a} for a in validate_aliases(aliases or [])]
        gql_input = LinkArtifactInput(artifact_id=self.id, artifact_portfolio_name=target.name, entity_name=target.prefix, project_name=target.project, aliases=alias_inputs)
        gql_vars = {'input': gql_input.model_dump()}
        omit_variables = omit_fields = None
        if not server_supports(service_api, pb.ARTIFACT_MEMBERSHIP_IN_LINK_ARTIFACT_RESPONSE):
            omit_variables = {'includeAliases'}
            omit_fields = {'artifactMembership'}
        data = service_api.execute_graphql(LINK_ARTIFACT_GQL, variables=gql_vars, omit_variables=omit_variables, omit_fields=omit_fields, parse=LinkArtifact.model_validate_json)
        if (result := data.result) and (membership := result.artifact_membership):
            return self._from_membership(membership, target=target, service_api=self._get_service_api())
    ...
    def unlink(self) -> None:
        ...
        """
        if not self.is_link:
            raise ValueError(f'Artifact {self.qualified_name!r} is not a linked artifact and cannot be unlinked.  To delete it, use {nameof(self.delete)!r} instead.')
        self._unlink()
    ...
    def _unlink(self) -> None:
        ...
        gql_input = UnlinkArtifactInput(artifact_id=self.id, artifact_portfolio_id=self.collection.id)
        gql_vars = {'input': gql_input.model_dump()}
        try:
            self._service_api.execute_graphql(mutation, variables=gql_vars)
        except CommError as e:
            raise CommError(f'You do not have permission to unlink the artifact {self.qualified_name!r}') from e
    ...
    def used_by(self) -> list[Run]:
        ...
        if (service_api := self._service_api) is None:
            raise RuntimeError('Client not initialized for artifact queries')
        result = service_api.execute_graphql(ARTIFACT_USED_BY_GQL, variables={'id': self.id}, parse=ArtifactUsedBy.model_validate_json)
        if (artifact := result.artifact) and (used_by := artifact.used_by) and (edges := used_by.edges):
            run_nodes = (e.node for e in edges)
            service_api = self._get_service_api()
            return [Run(service_api, proj.entity.name, proj.name, run.name) for run in run_nodes if (proj := run.project)]
        return []
    ...
    def logged_by(self) -> Run | None:
        ...
        Raises:
            ArtifactNotLoggedError: If the artifact is not logged.
        """
        from ._generated import ARTIFACT_CREATED_BY_GQL, ArtifactCreatedBy, RunInfoFragment
        if (service_api := self._service_api) is None:
            raise RuntimeError('Client not initialized for artifact queries')
        result = service_api.execute_graphql(ARTIFACT_CREATED_BY_GQL, variables={'id': self.id}, parse=ArtifactCreatedBy.model_validate_json)
        if (artifact := result.artifact) and (creator := artifact.created_by) and isinstance(creator, RunInfoFragment) and (name := creator.name) and (project := creator.project):
            return Run(self._get_service_api(), project.entity.name, project.name, name)
        return None
    ...
    @staticmethod
    def _expected_type(entity_name: str, project_name: str, name: str, service_api: ServiceApi) -> str | None:
        """Returns the expected type for a given artifact name and project."""
        from ._generated import ARTIFACT_TYPE_GQL, ArtifactType
        name = name if ':' in name else f'{name}:latest'
        result = service_api.execute_graphql(ARTIFACT_TYPE_GQL, {'entity': entity_name, 'project': project_name, 'name': name}, parse=ArtifactType.model_validate_json)
    ...
    def _fetch_linked_artifacts(self) -> list[Artifact]:
        ...
        from lumina._pydantic import gql_typename
        from ._generated import FETCH_LINKED_ARTIFACTS_GQL, ArtifactPortfolioTypeFields, FetchLinkedArtifacts
        from ._validators import LinkArtifactFields
        if self.id is None:
            raise ValueError('Unable to find any artifact memberships for artifact without an ID')
        if (service_api := self._service_api) is None:
            raise ValueError('Client is not initialized')
        result = service_api.execute_graphql(FETCH_LINKED_ARTIFACTS_GQL, variables={'artifactID': self.id}, parse=FetchLinkedArtifacts.model_validate_json)
        if not ((artifact := result.artifact) and (memberships := artifact.artifact_memberships) and (membership_edges := memberships.edges)):
            raise ValueError('Unable to find any artifact memberships for artifact')
        linked_artifacts: deque[Artifact] = deque()
        linked_nodes = (node for edge in membership_edges if (node := edge.node) and (col := node.artifact_collection) and (col.typename__ == gql_typename(ArtifactPortfolioTypeFields)))
        for node in linked_nodes:
            alias_names = unique_list((a.alias for a in node.aliases))
            version = f'v{node.version_index}'
            aliases = [*alias_names, version] if version not in alias_names else [*alias_names]
            if not (node and (col := node.artifact_collection) and (proj := col.project) and (proj.entity.name and proj.name)):
                raise ValueError('Unable to fetch fields for linked artifact')
            link_fields = LinkArtifactFields(entity_name=proj.entity.name, project_name=proj.name, name=f'{col.name}:{version}', version=version, aliases=aliases)
    ...
    def _create_linked_artifact_using_source_artifact(self, link_fields: LinkArtifactFields) -> Artifact:
