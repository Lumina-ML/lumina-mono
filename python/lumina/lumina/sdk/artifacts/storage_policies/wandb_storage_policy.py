...
from lumina.errors.term import termwarn
from lumina.proto import wandb_internal_pb2 as pb
from lumina.sdk.artifacts._models.storage import StoragePolicyConfig
from lumina.sdk.artifacts.artifact_file_cache import ArtifactFileCache, get_artifact_file_cache
from lumina.sdk.artifacts.staging import get_staging_dir
from lumina.sdk.artifacts.storage_handlers.multi_handler import MultiHandler
from lumina.sdk.artifacts.storage_handlers.tracking_handler import TrackingHandler
from lumina.sdk.artifacts.storage_layout import StorageLayout
from lumina.sdk.artifacts.storage_policies._multipart import MAX_MULTI_UPLOAD_SIZE, MIN_MULTI_UPLOAD_SIZE, KiB, calc_part_size, multipart_download, scan_chunks
from lumina.sdk.artifacts.storage_policies.register import WANDB_STORAGE_POLICY
from lumina.sdk.artifacts.storage_policy import StoragePolicy
from lumina.sdk.internal.internal_api import Api as InternalApi
from lumina.sdk.lib.hashutil import b64_to_hex_id, hex_to_b64_id
...
if TYPE_CHECKING:
    ...
    from lumina.sdk.internal import progress
...
class WandbStoragePolicy(StoragePolicy):
    ...
    def __init__(self, config: StoragePolicyConfig | None=None, cache: ArtifactFileCache | None=None, api: InternalApi | None=None) -> None:
    ...
    def _handler(self) -> MultiHandler:
        if self._maybe_handler is None:
            self._maybe_handler = MultiHandler(handlers=make_storage_handlers(self._session), default_handler=TrackingHandler())
        return self._maybe_handler

    def config(self) -> dict[str, Any]:
        return self._config.model_dump(exclude_none=True)

    def load_file(self, artifact: Artifact, manifest_entry: ArtifactManifestEntry, dest_path: str | None=None, executor: concurrent.futures.Executor | None=None) -> FilePathStr:
        ...
        path, hit, cache_open = self._cache.check_md5_obj_path(manifest_entry.digest, size=manifest_entry.size or 0)
        if hit:
            return path
        if (url := manifest_entry._download_url):
            if executor and (size := manifest_entry.size):
                ...
                def fetch_fresh_url() -> str:
                    files = artifact.files(names=[str(manifest_entry.path)], per_page=1)
                    try:
                        file = next(iter(files))
                    except StopIteration:
                        raise ValueError(f'Failed to fetch URL for file: {manifest_entry.path}')
                    else:
                        return file.direct_url
                multipart_download(executor, self._session, size, cache_open, initial_url=url, fetch_fn=fetch_fresh_url)
                return path
            try:
    ...
    def store_reference(self, artifact: Artifact, path: URIStr | FilePathStr, name: str | None=None, checksum: bool=True, max_objects: int | None=None) -> list[ArtifactManifestEntry]:
        return self._handler.store_path(artifact, path, name=name, checksum=checksum, max_objects=max_objects)

    def load_reference(self, manifest_entry: ArtifactManifestEntry, local: bool=False, dest_path: str | None=None) -> FilePathStr | URIStr:
    ...
    def _file_url(self, artifact: Artifact, entry: ArtifactManifestEntry) -> str:
        ...
        if layout is StorageLayout.V2:
            ...
            if api._server_supports(pb.ARTIFACT_V2_DOWNLOAD_HANDLER_SUPPORTS_ARTIFACT_ID):
    ...
    def s3_multipart_file_upload(self, file_path: str, chunk_size: int, hex_digests: dict[int, str], multipart_urls: dict[int, str], extra_headers: dict[str, str]) -> list[dict[str, Any]]:
        ...
        for num, data in enumerate(file_chunks, start=1):
            rsp = self._api.upload_multipart_file_chunk_retry(multipart_urls[num], data, extra_headers={'content-md5': hex_to_b64_id(hex_digests[num]), 'content-length': str(len(data)), 'content-type': extra_headers.get('Content-Type') or ''})
    ...
    def default_file_upload(self, upload_url: str, file_path: str, extra_headers: dict[str, Any], progress_callback: progress.ProgressFn | None=None) -> None:
        ...
        with open(file_path, 'rb') as file:
            self._api.upload_file_retry(upload_url, file, progress_callback, extra_headers=extra_headers)

    def store_file(self, artifact_id: str, artifact_manifest_id: str, entry: ArtifactManifestEntry, preparer: StepPrepare, progress_callback: progress.ProgressFn | None=None) -> bool:
        ...
        if MIN_MULTI_UPLOAD_SIZE <= file_size <= MAX_MULTI_UPLOAD_SIZE:
            file_chunks = scan_chunks(file_path, chunk_size)
            upload_parts = [{'partNumber': num, 'hexMD5': hashlib.md5(data).hexdigest()} for num, data in enumerate(file_chunks, start=1)]
            hex_digests = dict(map(itemgetter('partNumber', 'hexMD5'), upload_parts))
        else:
            upload_parts = []
            hex_digests = {}
        resp = preparer.prepare({'artifactID': artifact_id, 'artifactManifestID': artifact_manifest_id, 'name': entry.path, 'md5': entry.digest, 'uploadPartsInput': upload_parts}).get()
    ...
    def _write_cache(self, entry: ArtifactManifestEntry) -> None:
        if entry.local_path is None:
            return
        _, hit, cache_open = self._cache.check_md5_obj_path(entry.digest, size=entry.size or 0)
