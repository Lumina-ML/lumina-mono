...
def make_http_session() -> Session:
    ...
    HTTP_RETRY_STRATEGY: Final[Retry] = Retry(backoff_factor=1, total=16, status_forcelist=(308, 408, 409, 429, 500, 502, 503, 504))
    session = Session()
    settings = wandb_setup.singleton().settings
    headers = settings.x_extra_http_headers or {}
    session.headers.update(headers)
    adapter = HTTPAdapter(max_retries=HTTP_RETRY_STRATEGY, pool_connections=HTTP_POOL_CONNECTIONS, pool_maxsize=HTTP_POOL_MAXSIZE)
...
def make_storage_handlers(session: Session) -> list[StorageHandler]:
    ...
    return [S3Handler(), GCSHandler(), AzureHandler(), HTTPHandler(session, scheme='http'), HTTPHandler(session, scheme='https'), WBArtifactHandler(), WBLocalArtifactHandler(), LocalFileHandler()]
