...
from lumina.sdk.lib.filesystem import files_in
from lumina.sdk.lib.hashutil import B64MD5, ETag, b64_to_hex_id
...
class ArtifactFileCache:
    ...
    def check_etag_obj_path(self, url: URIStr, etag: ETag, size: int) -> tuple[FilePathStr, bool, Opener]:
        if self._override_cache_path is not None:
            ...
            skip_cache = False
            hexhash = hashlib.sha256(hashlib.sha256(url.encode('utf-8')).digest() + hashlib.sha256(etag.encode('utf-8')).digest()).hexdigest()
            path = self._obj_dir / 'etag' / hexhash[:2] / hexhash[2:]
        return self._check_or_create(path, size, skip_cache=skip_cache)

    def _check_or_create(self, path: Path, size: int, skip_cache: bool=False) -> tuple[FilePathStr, bool, Opener]:
    ...
    def cleanup(self, target_size: int | None=None, remove_temp: bool=False, target_fraction: float | None=None) -> int:
        ...
        for entry in files_in(self._temp_dir):
            ...
            if remove_temp:
                ...
                temp_size += size
        if temp_size:
            lumina.termwarn(f'Cache contains {util.to_human_size(temp_size)} of temporary files. Run `wandb artifact cache cleanup --remove-temp` to remove them.')
        entries = []
        ...
        if total_size > target_size:
            lumina.termerror(f'Failed to reclaim enough space in {self._cache_dir}. Try running `wandb artifact cache cleanup --remove-temp` to remove temporary files.')
        return bytes_reclaimed
    ...
    def _reserve_space(self, size: int) -> None:
        ...
        if size <= self._free_space():
            return
        lumina.termwarn('Cache size exceeded. Attempting to reclaim space...')
    ...
    def _ensure_write_permissions(self) -> None:
        ...
        try:
            ...
            raise PermissionError(f'Unable to write to {self._cache_dir}. Ensure that the current user has write permissions.') from e
