...
from lumina._pydantic import GQLId
from lumina._strutils import nameof
from lumina.apis.public.registries._freezable_list import AddOnlyArtifactTypesList
from lumina.apis.public.registries._utils import Visibility
...
class RegistryData(ArtifactsBase):
    ...
    artifact_types: AddOnlyArtifactTypesList = Field(default_factory=AddOnlyArtifactTypesList)
    ...
    def _validate_artifact_types(cls, v: Any) -> AddOnlyArtifactTypesList:
        ...
        from lumina.sdk.artifacts._generated.fragments import RegistryFragmentArtifactTypes
    ...
    def from_fragment(cls, obj: RegistryFragment) -> Self:
        ...
        if not obj.entity.organization:
            raise ValueError(f'Unable to parse registry organization from {nameof(type(obj))!r} object')
        return cls(id=obj.id, created_at=obj.created_at, updated_at=obj.updated_at, organization=obj.entity.organization.name, entity=obj.entity.name, name=remove_registry_prefix(obj.name), description=obj.description, allow_all_artifact_types=obj.allow_all_artifact_types, artifact_types=obj.artifact_types, visibility=obj.access)
