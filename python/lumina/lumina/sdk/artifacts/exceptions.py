...
class ArtifactStatusError(AttributeError):
    ...
    def __init__(self, msg: str='Artifact is in an invalid state for the requested operation.', name: str | None=None, obj: ArtifactT | None=None):
        try:
...
class ArtifactNotLoggedError(ArtifactStatusError):
    ...
    def __init__(self, fullname: str, obj: ArtifactT):
        *_, name = fullname.split('.')
        msg = f'{fullname!r} used prior to logging artifact or while in offline mode. Call {nameof(obj.wait)}() before accessing logged artifact properties.'
