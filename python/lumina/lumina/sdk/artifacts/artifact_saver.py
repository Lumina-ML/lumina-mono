...
import lumina
import lumina.filesync.step_prepare
...
from lumina.sdk.lib.paths import URIStr
if TYPE_CHECKING:
    ...
    from lumina.sdk.internal.progress import ProgressFn
...
class ArtifactSaver:
    ...
    def save(self, entity: str, project: str, type: str, name: str, client_id: str, sequence_client_id: str, distributed_id: str | None=None, finalize: bool=True, metadata: dict | None=None, ttl_duration_seconds: int | None=None, description: str | None=None, aliases: Sequence[str] | None=None, tags: Sequence[str] | None=None, use_after_commit: bool=False, incremental: bool=False, history_step: int | None=None, base_id: str | None=None) -> dict | None:
        return self._save_internal(entity, project, type, name, client_id, sequence_client_id, distributed_id, finalize, metadata, ttl_duration_seconds, description, aliases, tags, use_after_commit, incremental, history_step, base_id)

    def _save_internal(self, entity: str, project: str, type: str, name: str, client_id: str, sequence_client_id: str, distributed_id: str | None=None, finalize: bool=True, metadata: dict | None=None, ttl_duration_seconds: int | None=None, description: str | None=None, aliases: Sequence[str] | None=None, tags: Sequence[str] | None=None, use_after_commit: bool=False, incremental: bool=False, history_step: int | None=None, base_id: str | None=None) -> dict | None:
        alias_specs = []
        ...
        self._server_artifact, latest = self._api.create_artifact(type, name, self._digest, metadata=metadata, ttl_duration_seconds=ttl_duration_seconds, aliases=alias_specs, tags=tag_specs, description=description, is_user_created=self._is_user_created, distributed_id=distributed_id, client_id=client_id, sequence_client_id=sequence_client_id, history_step=history_step)
        ...
        if self._server_artifact['state'] == 'COMMITTED':
            if use_after_commit:
                self._api.use_artifact(artifact_id, artifact_entity_name=entity, artifact_project_name=project)
        ...
        step_prepare = lumina.filesync.step_prepare.StepPrepare(self._api, 0.1, 0.01, 1000)
        step_prepare.start()
        self._file_pusher.store_manifest_files(self._manifest, artifact_id, lambda entry, progress_callback: self._manifest.storage_policy.store_file(artifact_id, artifact_manifest_id, entry, step_prepare, progress_callback=progress_callback))
    ...
    def _resolve_client_id_manifest_references(self) -> None:
        for entry_path in self._manifest.entries:
            ...
            if entry.ref is not None and entry.ref.startswith('wandb-client-artifact:'):
                ...
                entry.ref = URIStr(f'wandb-artifact://{b64_to_hex_id(B64MD5(artifact_id))}/{artifact_file_path}')
