...
from lumina import termwarn
from lumina.proto import wandb_internal_pb2 as pb
from lumina.proto import wandb_telemetry_pb2 as tpb
from lumina.sdk.lib import json_util as json
from lumina.sdk.lib.filesystem import FilesDict, PolicyName
from lumina.sdk.mailbox import HandleAbandonedError, MailboxHandle
from lumina.util import WandBJSONEncoderOld, get_h5_typename, json_dumps_safer, json_dumps_safer_history, json_friendly, json_friendly_val, maybe_compress_summary
from ..data_types.utils import history_dict_to_json, val_to_json
from . import summary_record as sr
MANIFEST_FILE_SIZE_THRESHOLD = 100000
if TYPE_CHECKING:
    from lumina.sdk.artifacts.artifact import Artifact
    from lumina.sdk.artifacts.artifact_manifest import ArtifactManifest
...
class InterfaceBase(abc.ABC):
    ...
    @abc.abstractmethod
    def _deliver_status(self, status: pb.StatusRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError

    def _make_config(self, data: dict | None=None, key: tuple[str, ...] | str | None=None, val: Any | None=None, obj: pb.ConfigRecord | None=None) -> pb.ConfigRecord:
    ...
    def _make_run(self, run: Run) -> pb.RunRecord:
        ...
        if run._start_time is not None:
            proto_run.start_time.FromMicroseconds(int(run._start_time * 1000000.0))
    ...
    def _summary_encode(self, value: Any, path_from_root: str, run: Run) -> dict:
        ...
        if isinstance(value, dict):
            ...
            for key, value in value.items():
                json_value[key] = self._summary_encode(value, path_from_root + '.' + key, run=run)
            return json_value
        else:
            friendly_value, converted = json_friendly(val_to_json(run, path_from_root, value, namespace='summary'))
            json_value, compressed = maybe_compress_summary(friendly_value, get_h5_typename(value))
            if compressed:
                pass
            return json_value

    def _make_summary(self, summary_record: sr.SummaryRecord, run: Run) -> pb.SummaryRecord:
        ...
        for item in summary_record.update:
            ...
            json_value = self._summary_encode(item.value, path_from_root, run=run)
    ...
    def publish_summary(self, run: Run, summary_record: sr.SummaryRecord) -> None:
    ...
    @abc.abstractmethod
    def _publish_python_packages(self, python_packages: pb.PythonPackagesRequest) -> None:
        raise NotImplementedError
    ...
    def _make_artifact_manifest(self, artifact_manifest: ArtifactManifest, obj: pb.ArtifactManifest | None=None) -> pb.ArtifactManifest:
    ...
    def _write_artifact_manifest_file(self, manifest: ArtifactManifest) -> str:
        from lumina.sdk.artifacts.staging import get_staging_dir
    ...
    def deliver_link_artifact(self, artifact: Artifact, portfolio_name: str, aliases: Iterable[str], entity: str | None=None, project: str | None=None, organization: str | None=None) -> MailboxHandle[pb.Result]:
    ...
    @abc.abstractmethod
    def _deliver_link_artifact(self, link_artifact: pb.LinkArtifactRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError
    ...
    def publish_use_artifact(self, artifact: Artifact) -> None:
        ...
        if '_partial' in artifact.metadata:
            ...
            try:
                ...
                termwarn(f'Failed to download partial job info from artifact {artifact}, : {e}')
                return
            try:
    ...
    def deliver_artifact(self, run: Run, artifact: Artifact, aliases: Iterable[str], tags: Iterable[str] | None=None, history_step: int | None=None, is_user_created: bool=False, use_after_commit: bool=False, finalize: bool=True) -> MailboxHandle[pb.Result]:
        from lumina.sdk.artifacts.staging import get_staging_dir
    ...
    @abc.abstractmethod
    def _deliver_artifact(self, log_artifact: pb.LogArtifactRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError

    def deliver_download_artifact(self, artifact_id: str, download_root: str, allow_missing_references: bool, skip_cache: bool, path_prefix: str | None) -> MailboxHandle[pb.Result]:
    ...
    @abc.abstractmethod
    def _deliver_download_artifact(self, download_artifact: pb.DownloadArtifactRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError

    def publish_artifact(self, run: Run, artifact: Artifact, aliases: Iterable[str], tags: Iterable[str] | None=None, is_user_created: bool=False, use_after_commit: bool=False, finalize: bool=True) -> None:
        proto_run = self._make_run(run)
        proto_artifact = self._make_artifact(artifact)
        proto_artifact.run_id = proto_run.run_id
        proto_artifact.project = proto_run.project
    ...
    def publish_partial_history(self, run: Run, data: dict, user_step: int, step: int | None=None, flush: bool | None=None, publish_step: bool=True) -> None:
    ...
    def publish_history(self, run: Run, data: dict, step: int | None=None, publish_step: bool=True) -> None:
    ...
    def publish_output(self, name: str, data: str, *, nowait: bool=False) -> None:
        if name == 'stdout':
    ...
    def publish_output_raw(self, name: str, data: str, *, nowait: bool=False) -> None:
        if name == 'stdout':
    ...
    @abc.abstractmethod
    def _publish_output_raw(self, outdata: pb.OutputRawRecord, *, nowait: bool) -> None:
        raise NotImplementedError

    def publish_output_logger(self, line: str, *, nowait: bool=False) -> None:
        o = pb.OutputLoggerRecord(line=line)
        self._publish_output_logger(o, nowait=nowait)

    @abc.abstractmethod
    def _publish_output_logger(self, outdata: pb.OutputLoggerRecord, *, nowait: bool) -> None:
        raise NotImplementedError
    ...
    def publish_job_input(self, include_paths: list[list[str]], exclude_paths: list[list[str]], input_schema: dict | None, run_config: bool=False, file_path: str=''):
        ...
        if run_config and file_path:
            raise ValueError('run_config and file_path are mutually exclusive arguments.')
        ...
        source = pb.JobInputSource(run_config=pb.JobInputSource.RunConfigSource())
        if run_config:
            source.run_config.CopyFrom(pb.JobInputSource.RunConfigSource())
        else:
            source.file.CopyFrom(pb.JobInputSource.ConfigFileSource(path=file_path))
    ...
    @abc.abstractmethod
    def _publish_job_input(self, request: pb.JobInputRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError
    ...
    @abc.abstractmethod
    def _publish_probe_system_info(self, probe_system_info: pb.ProbeSystemInfoRequest) -> None:
        raise NotImplementedError
    ...
    def deliver_finish_sync(self) -> MailboxHandle[pb.Result]:
        sync = pb.SyncFinishRequest()
        return self._deliver_finish_sync(sync)

    @abc.abstractmethod
    def _deliver_finish_sync(self, sync: pb.SyncFinishRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError
    ...
    @abc.abstractmethod
    def _deliver_run_start(self, run_start: pb.RunStartRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError
    ...
    @abc.abstractmethod
    def _deliver_attach(self, status: pb.AttachRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError
    ...
    @abc.abstractmethod
    def _deliver_network_status(self, status: pb.NetworkStatusRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError
    ...
    @abc.abstractmethod
    def _deliver_get_summary(self, get_summary: pb.GetSummaryRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError
    ...
    @abc.abstractmethod
    def _deliver_get_system_metrics(self, get_summary: pb.GetSystemMetricsRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError
    ...
    @abc.abstractmethod
    def _deliver_exit(self, exit_data: pb.RunExitRecord) -> MailboxHandle[pb.Result]:
        raise NotImplementedError
    ...
    @abc.abstractmethod
    def _deliver_poll_exit(self, poll_exit: pb.PollExitRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError
    ...
    @abc.abstractmethod
    def _deliver_request_sampled_history(self, sampled_history: pb.SampledHistoryRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError
    ...
    @abc.abstractmethod
    def _deliver_request_run_status(self, run_status: pb.RunStatusRequest) -> MailboxHandle[pb.Result]:
        raise NotImplementedError
