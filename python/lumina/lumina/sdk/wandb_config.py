...
import logging
import lumina
from lumina.util import _is_artifact_representation, check_dict_contains_nested_artifact, json_friendly_val
...
class Config:
    ...
    def _check_locked(self, key, ignore_locked=False) -> bool:
        ...
        if locked is not None:
            ...
            if not ignore_locked:
                lumina.termwarn(f"Config item '{key}' was locked by '{locked_user}' (ignored update).")
            return True
        return False
    ...
    def __setitem__(self, key, val):
        if self._check_locked(key):
            return
        with lumina.sdk.lib.telemetry.context() as tel:
    ...
    def __getattr__(self, key):
        try:
            ...
            raise AttributeError(f'{self.__class__!r} object has no attribute {key!r}') from ke
    ...
    def _update(self, d, allow_val_change=None, ignore_locked=None):
        ...
        for key in list(parsed_dict):
            if self._check_locked(key, ignore_locked=ignore_locked):
                locked_keys.add(key)
        sanitized = self._sanitize_dict(parsed_dict, allow_val_change, ignore_keys=locked_keys)
    ...
    def _sanitize_dict(self, config_dict, allow_val_change=None, ignore_keys: set | None=None):
        sanitized = {}
    ...
    def _sanitize(self, key, val, allow_val_change=None):
        if isinstance(val, lumina.sdk.data_types.base_types.wb_value.WBValue):
        ...
        if not isinstance(val, lumina.Artifact):
            val = json_friendly_val(val)
        if not allow_val_change and key in self._items and (val != self._items[key]):
            raise config_util.ConfigError(f'Attempted to change value of key "{key}" from {self._items[key]} to {val}\nIf you really want to do this, pass allow_val_change=True to config.update()')
        return (key, val)
    ...
    def _raise_value_error_on_nested_artifact(self, v, nested=False):
        if isinstance(v, dict) and check_dict_contains_nested_artifact(v, nested):
            raise ValueError("Instances of wandb.Artifact can only be top level keys in a run's config")
