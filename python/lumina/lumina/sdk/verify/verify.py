...
def check_logged_in(api: Api, host: str) -> bool:
    ...
    if api.api_key is None:
        fail_string = 'Not logged in. Please log in using `wandb login`. See the docs: {}'.format(click.style(login_doc_url, underline=True, fg='blue'))
    else:
        res = api.api.viewer()
        if not res:
            fail_string = f'Could not get viewer with default API key. Please relogin using `WANDB_BASE_URL={host} wandb login --relogin` and try again'
...
def check_run(api: Api) -> bool:
    ...
    f.write('test')
    f.close()
    with lumina.init(id=nice_id('check_run'), reinit=True, config=config, project=PROJECT_NAME) as run:
        run_id = run.id
        entity = run.entity
        logged = True
        try:
        ...
        try:
            run.log({'HT%3ML ': lumina.Html('<a href="https://mysite">Link</a>')})
        except Exception:
            failed_test_strings.append('Failed to log to media. Contact W&B for support.')
        run.save(filepath)
    public_api = lumina.Api()
...
def artifact_with_path_or_paths(name: str, verify_dir: str | None=None, singular: bool=False) -> Artifact:
    art = lumina.Artifact(type='artsy', name=name)
...
def log_use_download_artifact(artifact: Artifact, alias: str, name: str, download_dir: str, failed_test_strings: list[str], add_extra_file: bool) -> tuple[bool, Artifact | None, list[str]]:
    with lumina.init(id=nice_id('log_artifact'), reinit=True, project=PROJECT_NAME, config={'test': 'artifact log'}) as log_art_run:
        if add_extra_file:
    ...
    with lumina.init(id=nice_id('use_artifact'), project=PROJECT_NAME, config={'test': 'artifact use'}) as use_art_run:
        try:
...
def check_artifacts() -> bool:
    ...
    computed = lumina.Artifact('computed', type='dataset')
...
def check_graphql_put(api: Api, host: str) -> tuple[bool, str | None]:
    ...
    f.write('test2')
    f.close()
    with lumina.init(id=nice_id('graphql_put'), reinit=True, project=PROJECT_NAME, config={'test': 'put to graphql'}) as run:
        run.save(gql_fp)
    public_api = lumina.Api()
...
def check_large_post() -> bool:
    ...
    public_api = lumina.Api()
    try:
        public_api._service_api.execute_graphql(query, variables={'entity': username, 'name': PROJECT_NAME, 'runName': '', 'desc': descy}, timeout=60)
    except Exception as e:
        if isinstance(e, WandbApiFailedError) and e.response is not None and (e.response.http_status == HTTPStatus.REQUEST_ENTITY_TOO_LARGE.value):
...
def check_wandb_version(api: Api) -> None:
    ...
    if parse(lumina.__version__) < parse(min_cli_version):
        fail_string = f'wandb version out of date, please run pip install --upgrade wandb=={max_cli_version}'
    elif parse(lumina.__version__) > parse(max_cli_version):
...
def check_sweeps(api: Api) -> bool:
    print('Checking sweep creation and agent execution'.ljust(72, '.'), end='')
    failed_test_strings: list[str] = []
    sweep_config = {'method': 'random', 'metric': {'goal': 'minimize', 'name': 'score'}, 'parameters': {'x': {'values': [0.01, 0.05, 0.1]}, 'y': {'values': [1, 2, 3]}}, 'name': 'verify_sweep'}
    try:
        with contextlib.redirect_stdout(io.StringIO()):
            sweep_id = lumina.sweep(sweep=sweep_config, project=PROJECT_NAME, entity=api.default_entity)
    except Exception as e:
    ...
    try:
        ...
        def main():
            with lumina.init(project=PROJECT_NAME) as run:
                score = objective(run.config)
                run.log({'score': score})
        lumina.agent(sweep_id, function=main, count=10)
    except Exception as e:
