...
from lumina.sdk.launch.inputs.internal import _validate_schema
from lumina.sdk.launch.utils import _is_git_uri, get_current_python_version, get_entrypoint_file
...
def create_job(path: str, job_type: str, entity: str | None=None, project: str | None=None, name: str | None=None, description: str | None=None, aliases: list[str] | None=None, runtime: str | None=None, entrypoint: str | None=None, git_hash: str | None=None, build_context: str | None=None, dockerfile: str | None=None) -> Artifact | None:
    ...
    """
    api = Api()
    artifact_job, _action, _aliases = _create_job(api, job_type, path, entity, project, name, description, aliases, runtime, entrypoint, git_hash, build_context, dockerfile)
    return artifact_job

def _create_job(api: Api, job_type: str, path: str, entity: str | None=None, project: str | None=None, name: str | None=None, description: str | None=None, aliases: list[str] | None=None, runtime: str | None=None, entrypoint: str | None=None, git_hash: str | None=None, build_context: str | None=None, dockerfile: str | None=None, base_image: str | None=None, services: dict[str, str] | None=None, schema: dict[str, Any] | None=None) -> tuple[Artifact | None, str, list[str]]:
    lumina.termlog(f'Creating launch job of type: {job_type}...')
    if name and name != make_artifact_name_safe(name):
        lumina.termerror(f'Artifact names may only contain alphanumeric characters, dashes, underscores, and dots. Did you mean: {make_artifact_name_safe(name)}')
        return (None, '', [])
    if runtime is not None and (not re.match('^3\\.\\d+$', runtime)):
        lumina.termerror(f'Runtime (-r, --runtime) must be a minor version of Python 3, e.g. 3.10 or 3.11, received {runtime}')
        return (None, '', [])
    aliases = aliases or []
    ...
    try:
        ...
        lumina.termerror(f'Error creating job: {e}')
        return (None, '', [])
    _dump_metadata_and_requirements(metadata=metadata, tmp_path=tempdir.name, requirements=requirements)
    try:
        run = lumina.init(dir=tempdir.name, settings={'silent': True, 'disable_job_creation': True}, entity=entity, project=project, job_type='cli_create_job')
    except Exception:
        return (None, '', [])
    ...
    if job_type == 'code':
        ...
        if not job_name:
            return (None, '', [])
        name = job_name
    artifact = job_builder.build(api.api, dockerfile=dockerfile, build_context=build_context, base_image=base_image)
    if not artifact:
        lumina.termerror('JobBuilder failed to build a job')
    ...
    if schema:
        ...
        metadata = {'input_schemas': {'@wandb.config': schema}}
    ...
    artifact.wait()
    run.finish()
    _run = lumina.Api().run(f'{entity}/{project}/{run.id}')
    _run.delete()
    return (artifact, action, aliases)

def _make_metadata_for_partial_job(job_type: str, tempdir: tempfile.TemporaryDirectory, git_hash: str | None, runtime: str | None, path: str, entrypoint: str | None) -> tuple[dict[str, Any] | None, list[str] | None]:
    ...
    if job_type == 'image':
        if runtime:
            lumina.termwarn('Setting runtime is not supported for image jobs, ignoring runtime')
        if entrypoint:
            lumina.termwarn('Setting an entrypoint is not currently supported for image jobs, ignoring entrypoint argument')
        metadata.update({'python': runtime or '', 'docker': path})
        return (metadata, None)
    lumina.termerror(f'Invalid job type: {job_type}')
    return (None, None)
...
def _maybe_warn_python_no_executable(entrypoint: str):
    ...
    if len(entrypoint_list) == 1 and entrypoint_list[0].endswith('.py'):
        lumina.termwarn(f'Entrypoint {entrypoint} is a python file without an executable, you may want to use `python {entrypoint}` as the entrypoint instead.')

def _create_repo_metadata(path: str, tempdir: str, entrypoint: str, git_hash: str | None=None, runtime: str | None=None) -> dict[str, Any] | None:
    if entrypoint and '..' in entrypoint:
        lumina.termerror('Entrypoint cannot contain backward path traversal')
        return None
    _maybe_warn_python_no_executable(entrypoint)
    if not _is_git_uri(path):
        lumina.termerror('Path must be a git URI')
        return None
    ref = GitReference(path, git_hash)
    if not ref:
        lumina.termerror('Could not parse git URI')
        return None
    ref.fetch(tempdir)
    commit = ref.commit_hash
    if not commit:
        if not ref.commit_hash:
            lumina.termerror('Could not find git commit hash')
            return None
        commit = ref.commit_hash
    ...
    metadata = {'git': {'commit': commit, 'remote': ref.url}, 'entrypoint': entrypoint.split(' '), 'python': python_version, 'notebook': False}
    return metadata

def _create_artifact_metadata(path: str, entrypoint: str, runtime: str | None=None) -> tuple[dict[str, Any] | None, list[str] | None]:
    if not os.path.isdir(path):
        lumina.termerror('Path must be a valid file or directory')
        return ({}, [])
    _maybe_warn_python_no_executable(entrypoint)
    entrypoint_list = entrypoint.split(' ')
    ...
    if not any(['wandb' in r for r in requirements]):
        lumina.termwarn('wandb is not present in requirements.txt.')
    if runtime:
...
def _configure_job_builder_for_partial(tmpdir: str, job_source: str) -> JobBuilder:
    ...
    if job_source == 'code':
        job_source = 'artifact'
    settings = lumina.Settings(job_source=job_source)
    job_builder = JobBuilder(settings=settings, verbose=True, files_dir=tmpdir)
...
def _make_code_artifact(api: Api, job_builder: JobBuilder, run: wandb.Run, path: str, entrypoint: str, entity: str | None, project: str | None, name: str | None) -> str | None:
    ...
    code_artifact = lumina.Artifact(name=artifact_name, type='code', description='Code artifact for job')
    try:
        code_artifact.add_dir(path)
    except Exception as e:
        if os.path.islink(path):
            lumina.termerror('Symlinks are not supported for code artifact jobs, please copy the code into a directory and try again')
        lumina.termerror(f'Error adding to code artifact: {e}')
        return None
...
def _dump_metadata_and_requirements(tmp_path: str, metadata: dict[str, Any], requirements: list[str] | None) -> None:
