...
import click
import lumina
import lumina.docker as docker
from lumina import util
from lumina.apis.internal import Api
from lumina.sdk.launch.errors import LaunchError
from lumina.sdk.launch.git_reference import GitReference
from lumina.sdk.launch.wandb_reference import WandbReference
from lumina.sdk.wandb_config import Config
from .builder.templates._wandb_bootstrap import FAILED_PACKAGES_POSTFIX, FAILED_PACKAGES_PREFIX
FAILED_PACKAGES_REGEX = re.compile(f'{re.escape(FAILED_PACKAGES_PREFIX)}(.*){re.escape(FAILED_PACKAGES_POSTFIX)}')
if TYPE_CHECKING:
    from lumina.sdk.launch.agent.job_status_tracker import JobAndRunStatusTracker
_GIT_URI_REGEX = re.compile('^[^/|^~|^\\.].*(git|bitbucket|dev\\.azure\\.com|\\.visualstudio\\.com)')
_VALID_IP_REGEX = '^https?://[0-9]+(?:\\.[0-9]+){3}(:[0-9]+)?'
_VALID_PIP_PACKAGE_REGEX = '^[a-zA-Z0-9_.-]+$'
_VALID_WANDB_REGEX = '^https?://(api.)?wandb'
_WANDB_URI_REGEX = re.compile('|'.join([_VALID_WANDB_REGEX, _VALID_IP_REGEX]))
_WANDB_QA_URI_REGEX = re.compile('^https?://ap\\w.qa.wandb')
_WANDB_DEV_URI_REGEX = re.compile('^https?://ap\\w.wandb.test')
_WANDB_LOCAL_DEV_URI_REGEX = re.compile('^https?://localhost')
API_KEY_REGEX = 'WANDB_API_KEY=\\w+(-\\w+)?'
MACRO_REGEX = re.compile('\\$\\{(\\w+)\\}')
AZURE_CONTAINER_REGISTRY_URI_REGEX = re.compile('^(?:https://)?([\\w]+)\\.azurecr\\.io/(?P<repository>[\\w\\-]+):?(?P<tag>.*)')
ELASTIC_CONTAINER_REGISTRY_URI_REGEX = re.compile('^(?:https://)?(?P<account>[\\w-]+)\\.dkr\\.ecr\\.(?P<region>[\\w-]+)\\.amazonaws\\.com/(?P<repository>[\\.\\/\\w-]+):?(?P<tag>.*)$')
GCP_ARTIFACT_REGISTRY_URI_REGEX = re.compile('^(?:https://)?(?P<region>[\\w-]+)-docker\\.pkg\\.dev/(?P<project>[\\w-]+)/(?P<repository>[\\w-]+)/?(?P<image_name>[\\w-]+)?(?P<tag>:.*)?$', re.IGNORECASE)
S3_URI_RE = re.compile('s3://([^/]+)(/(.*))?')
GCS_URI_RE = re.compile('gs://([^/]+)(?:/(.*))?')
AZURE_BLOB_REGEX = re.compile('^https://([^\\.]+)\\.blob\\.core\\.windows\\.net/([^/]+)/?(.*)$')
ARN_PARTITION_RE = re.compile('^arn:([^:]+):[^:]*:[^:]*:[^:]*:[^:]*$')
...
def set_project_entity_defaults(uri: str | None, job: str | None, api: Api, project: str | None, entity: str | None, launch_config: dict[str, Any] | None) -> tuple[str | None, str]:
    ...
    if platform.system() != 'Windows' and sys.stdout.encoding == 'UTF-8':
        prefix = '🚀 '
    lumina.termlog(f"{LOG_PREFIX}{prefix}Launching run into {entity}{('/' + project if project else '')}")
...
def strip_resource_args_and_template_vars(launch_spec: dict[str, Any]) -> None:
    if launch_spec.get('resource_args') and launch_spec.get('template_variables'):
        lumina.termwarn('Launch spec contains both resource_args and template_variables, only one can be set. Using template_variables.')
...
def validate_launch_spec_source(launch_spec: dict[str, Any]) -> None:
    ...
    if bool(job) == bool(docker_image):
        raise LaunchError('Exactly one of job or docker_image must be specified in the launch spec.')
_DOCKER_ALLOWED_RESOURCE_ARGS = {'command', 'cpu', 'cpu-period', 'cpu-quota', 'cpu-shares', 'cpus', 'cpuset-cpus', 'cpuset-mems', 'e', 'env', 'gpus', 'm', 'memory', 'memory-reservation', 'memory-swap', 'name', 'pids-limit', 'platform', 'shm-size', 'ulimit', 'w', 'workdir'}
_DOCKER_REPEATABLE_RESOURCE_ARGS = {'e', 'env', 'ulimit'}
K8S_RESTRICTED_SECURITY_CONTEXT: dict[str, Any] = {'allowPrivilegeEscalation': False, 'capabilities': {'drop': ['ALL']}, 'seccompProfile': {'type': 'RuntimeDefault'}}
_K8S_REFUSED_PRESENT_KEYS = {'securityContext': 'the agent manages the pod/container security context and it cannot be overridden', 'hostPath': 'hostPath volumes are not permitted', 'serviceAccountToken': 'projected service account tokens are not permitted'}
_K8S_REFUSED_HOST_NAMESPACE_KEYS = ('hostPID', 'hostIPC', 'hostNetwork', 'shareProcessNamespace', 'hostUsers')
...
def _raise_unsafe_resource_arg(resource: str, field: str, reason: str) -> None:
    raise LaunchError(f"Unsafe resource_args.{resource} option '{field}' is not allowed: {reason}")
...
def _validate_local_container_resource_value(name: str, value: Any) -> None:
    ...
    if name in _DOCKER_REPEATABLE_RESOURCE_ARGS and isinstance(value, (list, tuple)):
        if not value:
            raise LaunchError(f"resource_args.local-container option '{name}' requires a value.")
        for item in value:
            if not _is_safe_docker_value(item):
                raise LaunchError(f"resource_args.local-container option '{name}' only accepts string or numeric values.")
        return
    if not _is_safe_docker_value(value):
        raise LaunchError(f"resource_args.local-container option '{name}' must be a single string or numeric value.")
...
def validate_local_container_resource_args(docker_args: dict[str, Any]) -> None:
    ...
    for raw_name, value in docker_args.items():
        ...
        if name not in _DOCKER_ALLOWED_RESOURCE_ARGS:
            ...
            raise LaunchError(f"Unsupported resource_args.local-container option '{name}'. Supported options are: {supported}")
    ...
    if isinstance(node, dict):
        ...
        if isinstance(annotations, dict):
            for key, value in annotations.items():
                ...
                if ('apparmor' in normalized_key or 'seccomp' in normalized_key) and 'unconfined' in normalized_value:
                    _raise_unsafe_resource_arg('kubernetes', f'metadata.annotations.{key}', 'unconfined apparmor/seccomp profiles are not permitted')
    ...
    if isinstance(node, dict):
        ...
        for key in _K8S_REFUSED_HOST_NAMESPACE_KEYS:
            if _is_truthy_resource_value(node.get(key)):
                _raise_unsafe_resource_arg('kubernetes', key, 'host namespace sharing is not permitted')
        for key in _K8S_REFUSED_SERVICE_ACCOUNT_KEYS:
            if _is_present_resource_value(node.get(key)):
                _raise_unsafe_resource_arg('kubernetes', key, 'overriding the pod service account is not permitted')
        if _is_truthy_resource_value(node.get('automountServiceAccountToken')):
            _raise_unsafe_resource_arg('kubernetes', 'automountServiceAccountToken', 'mounting service account tokens is not permitted')
        if _is_truthy_resource_value(node.get('hostPort')):
            _raise_unsafe_resource_arg('kubernetes', 'hostPort', 'binding host ports is not permitted')
        if str(node.get('mountPropagation', '')).strip() == 'Bidirectional':
            _raise_unsafe_resource_arg('kubernetes', 'mountPropagation', 'bidirectional mount propagation is not permitted')
        for value in node.values():
        ...
        for container_key in _K8S_CONTAINER_KEYS:
            for container in pod_spec.get(container_key) or []:
                if isinstance(container, dict):
                    container['securityContext'] = copy.deepcopy(K8S_RESTRICTED_SECURITY_CONTEXT)
...
def get_local_python_deps(dir: str, filename: str='requirements.local.txt') -> str | None:
    try:
        ...
        lumina.termerror(f'Command failed: {e}')
        return None
...
def apply_patch(patch_string: str, dst_dir: str) -> None:
    ...
    try:
        ...
        raise lumina.Error('Failed to apply diff.patch associated with run.')
...
def convert_jupyter_notebook_to_script(fname: str, project_dir: str) -> str:
    nbconvert = lumina.util.get_module('nbconvert', 'nbformat and nbconvert are required to use launch with notebooks')
    nbformat = lumina.util.get_module('nbformat', 'nbformat and nbconvert are required to use launch with notebooks')
...
async def get_kube_context_and_api_client(kubernetes: Any, resource_args: dict[str, Any]) -> tuple[Any, Any]:
    config_file = resource_args.get('configFile')
    context = None
    if config_file is not None or os.path.exists(os.path.expanduser('~/.kube/config')):
        all_contexts, active_context = kubernetes.config.list_kube_config_contexts(config_file)
        context = None
        ...
        util.get_module('awscli', 'awscli is required to load a kubernetes context from eks. Please run `pip install wandb[launch]` to install it.')
        await kubernetes.config.load_kube_config(config_file, context['name'])
        api_client = await kubernetes.config.new_client_from_config(config_file, context=context['name'])
