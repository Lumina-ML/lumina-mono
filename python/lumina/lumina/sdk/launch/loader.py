...
from lumina.docker import is_docker_installed
...
WANDB_RUNNERS = {'local-container', 'local-process', 'kubernetes', 'vertex', 'sagemaker'}
...
def registry_from_config(config: dict[str, Any] | None, environment: AbstractEnvironment) -> AbstractRegistry:
    ...
    lumina.termwarn('The `registry` block of the launch agent config is being deprecated. Please specify an image repository URI under the `builder.destination` key of your launch agent config. See https://docs.wandb.ai/platform/launch/setup-agent-advanced#agent-configuration for more information.')
