...
class RunQueueItemFileSaver:
    ...
    def save_contents(self, contents: str, fname: str, file_sub_type: FileSubtypes) -> list[str] | None:
        if not isinstance(self.run, lumina.Run):
            lumina.termwarn('Not saving file contents because agent has no run')
            return None
        ...
        if isinstance(res, list):
            ...
            lumina.termwarn(f'Failed to save files for run queue item: {self.run_queue_item_id}')
            return None
