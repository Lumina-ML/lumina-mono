...
from lumina.sdk.launch._launch_add import launch_add
from lumina.sdk.launch.runner.local_container import LocalSubmittedRun
from lumina.sdk.launch.runner.local_process import LocalProcessRunner
from lumina.sdk.launch.sweeps.scheduler import Scheduler
from lumina.sdk.launch.utils import LAUNCH_CONFIG_FILE, resolve_build_and_registry_config
from lumina.sdk.lib import runid
from .. import loader
from .._project_spec import LaunchProject
from ..errors import LaunchDockerError, LaunchError
from ..utils import LAUNCH_DEFAULT_PROJECT, LOG_PREFIX, PROJECT_SYNCHRONOUS, event_loop_thread_exec, validate_launch_resource_args
from .job_status_tracker import JobAndRunStatusTracker
from .run_queue_item_file_saver import RunQueueItemFileSaver
AGENT_POLLING_INTERVAL = 10
RECEIVED_JOB_POLLING_INTERVAL = 0.0
...
if _env_timeout:
    try:
        ...
        raise LaunchError(f'Invalid value for WANDB_LAUNCH_START_TIMEOUT: {_env_timeout}')
else:
    RUN_START_TIMEOUT = 60 * 30
...
class InternalAgentLogger:
    ...
    def error(self, message: str):
        if self._print_to_terminal:
            lumina.termerror(f'{LOG_PREFIX}{message}')
        _logger.error(f'{LOG_PREFIX}{message}')

    def warn(self, message: str):
        if self._print_to_terminal:
            lumina.termwarn(f'{LOG_PREFIX}{message}')
        _logger.warning(f'{LOG_PREFIX}{message}')

    def info(self, message: str):
        if self._print_to_terminal:
            lumina.termlog(f'{LOG_PREFIX}{message}')
...
class LaunchAgent:
    ...
    def __init__(self, api: Api, config: dict[str, Any]):
        ...
        self._stopped_run_timeout = config.get('stopped_run_timeout', DEFAULT_STOPPED_RUN_TIMEOUT)
        self._known_warnings: list[str] = []
        self.version: str = 'wandb@' + lumina.__version__
        ...
        create_response = self._api.create_launch_agent(self._entity, self._project, self._queues, sent_config, self.version)
        self._id = create_response['launchAgentId']
        if self._api.entity_is_team(self._entity):
            lumina.termwarn(f'{LOG_PREFIX}Agent is running on team entity ({self._entity}). Members of this team will be able to run code on this device.')
    ...
    def _is_scheduler_job(self, run_spec: dict[str, Any]) -> bool:
        ...
        if not run_spec:
            self._internal_logger.debug('Received runSpec in _is_scheduler_job that was empty')
    ...
    async def fail_run_queue_item(self, run_queue_item_id: str, message: str, phase: str, files: list[str] | None=None) -> None:
    ...
    def _init_agent_run(self) -> None:
        settings = lumina.Settings(silent=True, disable_git=True, disable_job_creation=True)
        self._wandb_run = lumina.init(project=self._project, entity=self._entity, settings=settings, id=self._name, job_type=HIDDEN_AGENT_RUN_TYPE)
    ...
    async def pop_from_queue(self, queue: str) -> Any:
        ...
        try:
            ...
            ups = await pop(queue, entity=self._entity, project=self._project, agent_id=self._id)
            return ups
    ...
    def print_status(self) -> None:
        ...
        lumina.termlog(f'{LOG_PREFIX}{output_str}')
    ...
    async def update_status(self, status: str) -> None:
        ...
        if not update_ret['success']:
            lumina.termerror(f'{LOG_PREFIX}Failed to update agent status to {status}')
    ...
    def _check_run_exists_and_inited(self, entity: str, project: str, run_id: str, rqi_id: str) -> bool:
        ...
        try:
            ...
            self._internal_logger.info(f'Run {entity}/{project}/{run_id} with rqi id: {rqi_id} did not have associated run')
        return False

    async def finish_thread_id(self, thread_id: int, exception: Exception | LaunchDockerError | None=None) -> None:
        """Removes the job from our list for now."""
        with self._jobs_lock:
            job_and_run_status = self._jobs[thread_id]
        if job_and_run_status.entity is not None and job_and_run_status.entity != self._entity:
            self._internal_logger.info('Skipping check for completed run status because run is on a different entity than agent')
        elif exception is not None:
            tb_str = traceback.format_exception(type(exception), value=exception, tb=exception.__traceback__)
            fnames = job_and_run_status.saver.save_contents(''.join(tb_str), 'error.log', 'error')
            await self.fail_run_queue_item(job_and_run_status.run_queue_item_id, str(exception), job_and_run_status.err_stage, fnames)
        elif job_and_run_status.project is None or job_and_run_status.run_id is None:
            self._internal_logger.info(f'called finish_thread_id on thread whose tracker has no project or run id. RunQueueItemID: {job_and_run_status.run_queue_item_id}')
            lumina.termerror('Missing project or run id on thread called finish thread id')
            await self.fail_run_queue_item(job_and_run_status.run_queue_item_id, 'submitted job was finished without assigned project or run id', 'agent')
        elif job_and_run_status.run is not None:
            called_init = False
            logs = None
            interval = 1
            while True:
                called_init = self._check_run_exists_and_inited(self._entity, job_and_run_status.project, job_and_run_status.run_id, job_and_run_status.run_queue_item_id)
            ...
            if not called_init:
                ...
                if logs:
                    fnames = job_and_run_status.saver.save_contents(logs, 'error.log', 'error')
                await self.fail_run_queue_item(job_and_run_status.run_queue_item_id, _msg, 'run', fnames)
        else:
            self._internal_logger.info(f'Finish thread id {thread_id} had no exception and no run')
            get_sentry().exception('launch agent called finish thread id on thread without run or exception')
        with self._jobs_lock:
            del self._jobs[thread_id]
        if len(self.thread_ids) == 0:
            await self.update_status(AGENT_POLLING)

    async def run_job(self, job: dict[str, Any], queue: str, file_saver: RunQueueItemFileSaver) -> None:
        ...
        asyncio.create_task(self.task_run_job(launch_spec, job, self.default_config, self._api, job_tracker))
    ...
    async def loop(self) -> None:
        ...
        try:
            while True:
                ...
                if self.num_running_jobs < self._max_jobs:
                    ...
                    if job_and_queue is not None:
                        ...
                        try:
                            file_saver = RunQueueItemFileSaver(self._wandb_run, job['runQueueItemId'])
                            if self._is_scheduler_job(job.get('runSpec', {})) and self.num_running_schedulers >= self._max_schedulers:
                                lumina.termwarn(f'{LOG_PREFIX}Agent already running the maximum number of sweep schedulers: {self._max_schedulers}. To set this value use `max_schedulers` key in the agent config')
                                continue
                            await self.run_job(job, queue, file_saver)
                        except Exception as e:
                            lumina.termerror(f'{LOG_PREFIX}Error running job: {traceback.format_exc()}')
                            get_sentry().exception(e)
                            files = file_saver.save_contents(contents=traceback.format_exc(), fname='error.log', file_sub_type='error')
                            await self.fail_run_queue_item(run_queue_item_id=job['runQueueItemId'], message=str(e), phase='agent', files=files)
                if self._ticks % 2 == 0:
                    if len(self.thread_ids) == 0:
                        await self.update_status(AGENT_POLLING)
                    else:
            ...
            lumina.termlog(f'{LOG_PREFIX}Shutting down, active jobs:')
            self.print_status()
        finally:
            self._jobs_event.clear()

    async def task_run_job(self, launch_spec: dict[str, Any], job: dict[str, Any], default_config: dict[str, Any], api: Api, job_tracker: JobAndRunStatusTracker) -> None:
        ...
        try:
            ...
            await self._task_run_job(launch_spec, job, default_config, api, rqi_id, job_tracker)
        except LaunchDockerError as e:
            lumina.termerror(f'{LOG_PREFIX}agent {self._name} encountered an issue while starting Docker, see above output for details.')
            exception = e
            get_sentry().exception(e)
        except LaunchError as e:
            lumina.termerror(f'{LOG_PREFIX}Error running job: {e}')
            exception = e
            get_sentry().exception(e)
        except Exception as e:
            lumina.termerror(f'{LOG_PREFIX}Error running job: {traceback.format_exc()}')
    ...
    async def _task_run_job(self, launch_spec: dict[str, Any], job: dict[str, Any], default_config: dict[str, Any], api: Api, thread_id: int, job_tracker: JobAndRunStatusTracker) -> None:
        ...
        backend_config: dict[str, Any] = {PROJECT_SYNCHRONOUS: False}
        ...
        environment = loader.environment_from_config(default_config.get('environment', {}))
        registry = loader.registry_from_config(registry_config, environment)
        builder = loader.builder_from_config(build_config, environment, registry)
        backend = loader.runner_from_config(resource, api, backend_config, environment, registry)
        if not (project.docker_image or project.job_base_image or isinstance(backend, LocalProcessRunner)):
            from lumina.sdk.launch.builder.noop import NoOpBuilder
            if isinstance(builder, NoOpBuilder) and project.job_source_type in ('artifact', 'repo'):
                base_image = project._resource_args_build.get('base_image') or _DEFAULT_BASE_IMAGE
                lumina.termwarn(f'{LOG_PREFIX}No builder configured. Using base image: {base_image}')
        ...
        if self._is_scheduler_job(launch_spec):
            ...
            lumina.termlog(f'{LOG_PREFIX}Preparing to run sweep scheduler ({self.num_running_schedulers}/{self._max_schedulers})')
        if not run:
            with self._jobs_lock:
        ...
        while self._jobs_event.is_set():
            ...
            if state == 'starting' and RUN_START_TIMEOUT > 0 and (time.time() - start_time > RUN_START_TIMEOUT):
                await run.cancel()
                raise LaunchError(f'Run failed to start within {RUN_START_TIMEOUT} seconds. If you want to increase this timeout, set WANDB_LAUNCH_START_TIMEOUT to a larger value.')
            if await self._check_run_finished(job_tracker, launch_spec):
                return
            if await job_tracker.check_wandb_run_stopped(self._api):
                if stopped_time is None:
                    stopped_time = time.time()
                elif time.time() - stopped_time > self._stopped_run_timeout:
                    await run.cancel()
            await asyncio.sleep(AGENT_POLLING_INTERVAL)
    ...
    async def check_sweep_state(self, launch_spec: dict[str, Any], api: Api) -> None:
        ...
        if launch_spec.get('sweep_id'):
            try:
                ...
                state = await get_sweep_state(sweep=launch_spec['sweep_id'], entity=launch_spec['entity'], project=launch_spec['project'])
            except Exception as e:
                self._internal_logger.debug(f'Fetch sweep state error: {e}')
                state = None
            if state != 'RUNNING' and state != 'PAUSED':
                raise LaunchError(f"Launch agent picked up sweep job, but sweep ({launch_spec['sweep_id']}) was in a terminal state ({state})")

    async def _check_run_finished(self, job_tracker: JobAndRunStatusTracker, launch_spec: dict[str, Any]) -> bool:
        ...
        try:
            ...
            for warning in status.messages:
                if warning not in self._known_warnings:
                    ...
                    success = self._api.update_run_queue_item_warning(job_tracker.run_queue_item_id, warning, 'Kubernetes', [])
                    if not success:
                        _logger.warning(f'Error adding warning {warning} to run queue item {job_tracker.run_queue_item_id}')
            ...
            if state == 'preempted' and job_tracker.entity == self._entity:
                ...
                if config['_resume_count'] > MAX_RESUME_COUNT:
                    lumina.termlog(f'{LOG_PREFIX}Run {job_tracker.run_id} has already resumed {MAX_RESUME_COUNT} times.')
                    return True
                lumina.termlog(f'{LOG_PREFIX}Run {job_tracker.run_id} was preempted, requeuing...')
                if 'sweep_id' in config:
                    del config['sweep_id']
                launch_add(config=config, project_queue=self._project, queue_name=job_tracker.queue)
                return True
            if state in ['stopped', 'failed', 'finished', 'preempted']:
                if job_tracker.is_scheduler:
                    lumina.termlog(f'{LOG_PREFIX}Scheduler finished with ID: {run.id}')
                    if state == 'failed':
                        try:
                            self._api.set_sweep_state(sweep=job_tracker.run_id, entity=job_tracker.entity, project=job_tracker.project, state='CANCELED')
                        except Exception as e:
                            raise LaunchError(f'Failed to update sweep state: {e}')
                else:
                    lumina.termlog(f'{LOG_PREFIX}Job finished with ID: {run.id}')
                with self._jobs_lock:
                    job_tracker.completed_status = state
                return True
            return False
        except LaunchError as e:
            lumina.termerror(f'{LOG_PREFIX}Terminating job {run.id} because it failed to start: {str(e)}')
            known_error = True
            with self._jobs_lock:
                job_tracker.failed_to_start = True
        except Exception as e:
            lumina.termerror(f'{LOG_PREFIX}Error getting status for job {run.id}')
            lumina.termerror(traceback.format_exc())
            _logger.info('---')
