...
from lumina.sdk.launch.builder.build import build_image_from_project
from lumina.sdk.launch.errors import LaunchError
from lumina.sdk.launch.utils import LAUNCH_DEFAULT_PROJECT, LOG_PREFIX, construct_launch_spec, validate_launch_spec_source
from ._project_spec import LaunchProject

def push_to_queue(api: Api, queue_name: str, launch_spec: dict[str, Any], template_variables: dict | None, project_queue: str, priority: int | None=None) -> Any:
    return api.push_to_run_queue(queue_name, launch_spec, template_variables, project_queue, priority)

def launch_add(uri: str | None=None, job: str | None=None, config: dict[str, Any] | None=None, template_variables: dict[str, float | int | str] | None=None, project: str | None=None, entity: str | None=None, queue_name: str | None=None, resource: str | None=None, entry_point: list[str] | None=None, name: str | None=None, version: str | None=None, docker_image: str | None=None, project_queue: str | None=None, resource_args: dict[str, Any] | None=None, run_id: str | None=None, build: bool | None=False, repository: str | None=None, sweep_id: str | None=None, author: str | None=None, priority: int | None=None) -> public.QueuedRun:
    ...
    """
    api = Api()
    return _launch_add(api, job, config, template_variables, project, entity, queue_name, resource, entry_point, name, version, docker_image, project_queue, resource_args, run_id=run_id, build=build, repository=repository, sweep_id=sweep_id, author=author, priority=priority)

def _launch_add(api: Api, job: str | None, config: dict[str, Any] | None, template_variables: dict | None, project: str | None, entity: str | None, queue_name: str | None, resource: str | None, entry_point: list[str] | None, name: str | None, version: str | None, docker_image: str | None, project_queue: str | None, resource_args: dict[str, Any] | None=None, run_id: str | None=None, build: bool | None=False, repository: str | None=None, sweep_id: str | None=None, author: str | None=None, priority: int | None=None) -> public.QueuedRun:
    launch_spec = construct_launch_spec(None, job, api, name, project, entity, docker_image, resource, entry_point, version, resource_args, config, run_id, repository, author, sweep_id)
    if build:
        ...
        if launch_spec.get('job') is not None:
            lumina.termwarn("Build doesn't support setting a job. Overwriting job.")
        ...
        run = lumina.run or lumina.init(project=launch_spec['project'], entity=launch_spec['entity'], job_type='launch_job')
    ...
    if project_queue == LAUNCH_DEFAULT_PROJECT:
        lumina.termlog(f'{LOG_PREFIX}Added run to queue {queue_name}.')
    else:
        lumina.termlog(f'{LOG_PREFIX}Added run to queue {project_queue}/{queue_name}.')
    lumina.termlog(f'{LOG_PREFIX}Launch spec:\n{pprint.pformat(launch_spec)}\n')
