...
from lumina.sdk.launch.utils import get_entrypoint_file
...
def _get_accelerator_base_image(resource_args_build: dict[str, Any]) -> str | None:
    ...
    for accelerator_key in ('accelerator', 'cuda'):
        ...
        if not isinstance(base_image, str):
            raise LaunchError(f'Invalid accelerator base image. Expected builder.{accelerator_key}.base_image to be a string Docker image reference, got {type(base_image).__name__}.')
        return base_image
    return None
...
class LaunchProject:
    ...
    def __init__(self, uri: str | None, job: str | None, api: Api, launch_spec: dict[str, Any], target_entity: str, target_project: str, name: str | None, docker_config: dict[str, Any], git_info: dict[str, str], overrides: dict[str, Any], resource: str, resource_args: dict[str, Any], run_id: str | None, sweep_id: str | None=None):
        self.uri = uri
        self.job = job
        if job is not None:
            lumina.termlog(f'{LOG_PREFIX}Launching job: {job}')
        ...
        self._resource_args_build = resource_args_copy.get(resource, {}).pop('builder', {})
        ...
        self.accelerator_base_image: str | None = _get_accelerator_base_image(self._resource_args_build)
        self.docker_image: str | None = docker_config.get('docker_image') or launch_spec.get('image_uri')
        self.docker_user_id = docker_config.get('user_id', 1000)
        self._entry_point: EntryPoint | None = None
    ...
    def change_project_dir(self, new_dir: str) -> None:
        ...
        if old_dir is not None:
            shutil.copytree(old_dir, new_dir, symlinks=True, dirs_exist_ok=True, ignore=shutil.ignore_patterns('fsmonitor--daemon.ipc', '.git'))
    ...
    def init_overrides(self, overrides: dict[str, Any]) -> None:
        ...
        if override_entrypoint:
            ...
            self.override_entrypoint = EntryPoint(name=get_entrypoint_file(override_entrypoint), command=override_entrypoint)
        override_working_dir = overrides.get('working_dir')
        self.resolved_working_dir: str = f'{CODE_MOUNT_DIR}/{override_working_dir}' if override_working_dir else CODE_MOUNT_DIR
    ...
    def from_spec(cls, launch_spec: dict[str, Any], api: Api) -> LaunchProject:
        ...
        return LaunchProject(launch_spec.get('uri'), launch_spec.get('job'), api, launch_spec, launch_spec['entity'], launch_spec['project'], name, launch_spec.get('docker', {}), launch_spec.get('git', {}), launch_spec.get('overrides', {}), launch_spec.get('resource'), launch_spec.get('resource_args', {}), launch_spec.get('run_id'), launch_spec.get('sweep_id', {}))
    ...
    def image_name(self) -> str:
        ...
        if self.docker_image is not None:
            ...
            return lumina.util.make_docker_image_name_safe(shortened_uri)
        else:
            assert self.job is not None
            return lumina.util.make_docker_image_name_safe(self.job.split(':')[0])
    ...
    def _fetch_job(self) -> None:
        ...
        """
        public_api = lumina.apis.public.Api()
    ...
    def get_env_vars_dict(self, api: Api, max_env_length: int) -> dict[str, str]:
        ...
        if self.queue_name:
            env_vars[lumina.env.LAUNCH_QUEUE_NAME] = self.queue_name
        if self.queue_entity:
            env_vars[lumina.env.LAUNCH_QUEUE_ENTITY] = self.queue_entity
        if self.run_queue_item_id:
            env_vars[lumina.env.LAUNCH_TRACE_ID] = self.run_queue_item_id
        ...
        artifacts = {}
        if self.job:
            artifacts = {lumina.util.LAUNCH_JOB_ARTIFACT_SLOT_NAME: self.job}
        env_vars['WANDB_ARTIFACTS'] = json.dumps({**artifacts, **self.override_artifacts})
        return env_vars
            ...
            with open(base_requirements) as f2:
                for line in f2:
                    ...
                    try:
                        ...
                        _logger.warning('Unable to parse line %s in requirements.txt', line, exc_info=True)
                        continue
...
def _inject_file_overrides_env_vars(overrides: dict[str, Any], env_dict: dict[str, Any], maximum_env_length: int) -> None:
    ...
    if len(str_overrides) <= maximum_env_length:
        ...
        return
    chunks = [str_overrides[i:i + maximum_env_length] for i in range(0, len(str_overrides), maximum_env_length)]
    overrides_chunks_dict = {f'WANDB_LAUNCH_FILE_OVERRIDES_{i}': chunk for i, chunk in enumerate(chunks)}
