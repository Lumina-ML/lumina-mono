...
from lumina.sdk.launch.utils import AZURE_CONTAINER_REGISTRY_URI_REGEX
from lumina.util import get_module
from .abstract import AbstractRegistry
if TYPE_CHECKING:
    from azure.containerregistry import ContainerRegistryClient
    from azure.core.exceptions import ResourceNotFoundError
ContainerRegistryClient = get_module('azure.containerregistry', required='The azure-containerregistry package is required to use launch with Azure. Please install it with `pip install azure-containerregistry`.').ContainerRegistryClient
ResourceNotFoundError = get_module('azure.core.exceptions', required='The azure-core package is required to use launch with Azure. Please install it with `pip install azure-core`.').ResourceNotFoundError

class AzureContainerRegistry(AbstractRegistry):
    """Helper for accessing Azure Container Registry resources."""

    def __init__(self, uri: str | None=None, registry_name: str | None=None, repo_name: str | None=None):
        """Initialize an AzureContainerRegistry."""
        if uri is not None:
            self.uri = uri
            if any((x is not None for x in (registry_name, repo_name))):
                raise LaunchError('Please specify either a registry name and repo name or a registry URI.')
            if self.uri.startswith('https://'):
                self.uri = self.uri[len('https://'):]
            match = AZURE_CONTAINER_REGISTRY_URI_REGEX.match(self.uri)
            if match is None:
                raise LaunchError(f'Unable to parse Azure Container Registry URI: {self.uri}')
            self.registry_name = match.group(1)
            self.repo_name = match.group(2)
        else:
            if any((x is None for x in (registry_name, repo_name))):
                raise LaunchError('Please specify both a registry name and repo name or a registry URI.')
            self.registry_name = registry_name
            self.repo_name = repo_name
            self.uri = f'{self.registry_name}.azurecr.io/{self.repo_name}'

    @classmethod
    def from_config(cls, config: dict) -> AzureContainerRegistry:
        ...
        uri = config.get('uri')
        if uri is None:
            raise LaunchError('Please specify a registry name to use under the registry.uri.')
        return cls(uri=uri)
    ...
    async def check_image_exists(self, image_uri: str) -> bool:
        ...
        if match is None:
            raise LaunchError(f'Unable to parse Azure Container Registry URI: {image_uri}')
        ...
        try:
            ...
            raise LaunchError(f'Unable to check if image exists in Azure Container Registry: {e}') from e
