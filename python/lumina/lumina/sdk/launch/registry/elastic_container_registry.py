...
from lumina.sdk.launch.utils import ELASTIC_CONTAINER_REGISTRY_URI_REGEX, event_loop_thread_exec
from lumina.util import get_module
_logger = logging.getLogger(__name__)
botocore = get_module('botocore', required='The boto3 package is required to use launch with AWS. Please install it with `pip install wandb[launch]`.')
boto3 = get_module('boto3', required='The boto3 package is required to use launch with AWS. Please install it with `pip install wandb[launch]`.')

class ElasticContainerRegistry(AbstractRegistry):
    """Elastic Container Registry class."""

    def __init__(self, uri: str | None=None, account_id: str | None=None, region: str | None=None, repo_name: str | None=None) -> None:
        ...
        if uri:
            ...
            if any([account_id, region, repo_name]):
                raise LaunchError("Could not create ElasticContainerRegistry from config. Either 'uri' or 'account_id', 'region', and 'repo_name' are required.")
            match = ELASTIC_CONTAINER_REGISTRY_URI_REGEX.match(self.uri)
            if not match:
                raise LaunchError(f'Could not create ElasticContainerRegistry from config. The uri {self.uri} is invalid.')
        ...
        if self.account_id is None:
            raise LaunchError("Could not create ElasticContainerRegistry from config. Either 'uri' or 'account_id' is required.")
        if self.region is None:
            raise LaunchError("Could not create ElasticContainerRegistry from config. Either 'uri' or 'region' is required.")
        if self.repo_name is None:
            raise LaunchError("Could not create ElasticContainerRegistry from config. Either 'uri' or 'repository' is required.")

    @classmethod
    def from_config(cls, config: dict[str, str]) -> ElasticContainerRegistry:
        ...
        """
        acceptable_keys = {'uri', 'type', 'account_id', 'region', 'repo_name'}
        unsupported_keys = set(config.keys()) - acceptable_keys
        if unsupported_keys:
            raise LaunchError(f'The Elastic Container Registry config contains unsupported keys: {unsupported_keys}. Please remove these keys. The acceptable keys are: {acceptable_keys}.')
        return cls(uri=config.get('uri'), account_id=config.get('account_id'), region=config.get('region'), repo_name=config.get('repository'))
    ...
    async def get_username_password(self) -> tuple[str, str]:
        ...
        try:
            ...
            username, password = base64.standard_b64decode(response['authorizationData'][0]['authorizationToken']).split(b':')
    ...
    async def check_image_exists(self, image_uri: str) -> bool:
        ...
        if ':' not in image_uri:
            ...
            if uri != repo_uri:
                raise LaunchError(f'Image uri {image_uri} does not match Elastic Container Registry uri {repo_uri}.')
        _logger.debug(f'Checking if image tag {tag} exists in repository {self.uri}')
        try:
            session = boto3.Session(region_name=self.region)
            client = await event_loop_thread_exec(session.client)('ecr')
            response = await event_loop_thread_exec(client.describe_images)(repositoryName=self.repo_name, imageIds=[{'imageTag': tag}])
