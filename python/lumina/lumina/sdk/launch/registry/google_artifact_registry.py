...
import logging
import google.auth
import google.cloud.artifactregistry
from lumina.sdk.launch.errors import LaunchError
from lumina.sdk.launch.utils import GCP_ARTIFACT_REGISTRY_URI_REGEX, event_loop_thread_exec
from lumina.util import get_module
from .abstract import AbstractRegistry
_logger = logging.getLogger(__name__)
google = get_module('google', required='The google package is required to use launch with Google. Please install it with `pip install wandb[launch]`.')
google.auth = get_module('google.auth', required='The google-auth package is required to use launch with Google. Please install it with `pip install wandb[launch]`.')
google.cloud.artifactregistry = get_module('google.cloud.artifactregistry', required='The google-cloud-artifactregistry package is required to use launch with Google. Please install it with `pip install wandb[launch]`.')
...
class GoogleArtifactRegistry(AbstractRegistry):
    ...
    def __init__(self, uri: str | None=None, repository: str | None=None, image_name: str | None=None, project: str | None=None, region: str | None=None) -> None:
        ...
        """
        _logger.info(f'Initializing Google Artifact Registry with repository {repository} and image name {image_name}')
        if uri is not None:
            self.uri = uri
            if any([repository, image_name, project, region]):
                raise LaunchError('The Google Artifact Registry must be specified with either the uri key or the repository, image-name, project and region keys, but not both.')
            match = GCP_ARTIFACT_REGISTRY_URI_REGEX.match(self.uri)
            if not match:
                raise LaunchError(f'The Google Artifact Registry uri {self.uri} is invalid. Please provide a uri of the form REGION-docker.pkg.dev/PROJECT/REPOSITORY/IMAGE_NAME.')
            ...
            if any((x is None for x in (repository, region, image_name))):
                raise LaunchError('The Google Artifact Registry must be specified with either the uri key or the repository, image-name, project and region keys.')
            self.project = project
            self.region = region
            self.repository = repository
            self.image_name = image_name
            self.uri = f'{self.region}-docker.pkg.dev/{self.project}/{self.repository}/{self.image_name}'
        _missing_kwarg_msg = 'The Google Artifact Registry is missing the {} kwarg. Please specify it by name or as part of the uri argument.'
        if not self.region:
    ...
    @classmethod
    def from_config(cls, config: dict) -> GoogleArtifactRegistry:
        ...
        """
        acceptable_keys = {'uri', 'type', 'repository', 'image-name', 'region', 'project'}
        unacceptable_keys = set(config.keys()) - acceptable_keys
        if unacceptable_keys:
            raise LaunchError(f'The Google Artifact Registry config contains unacceptable keys: {unacceptable_keys}. Please remove these keys. The acceptable keys are: {acceptable_keys}.')
        return cls(uri=config.get('uri'), repository=config.get('repository'), image_name=config.get('image-name'), project=config.get('project'), region=config.get('region'))
    ...
    async def get_repo_uri(self) -> str:
        ...
        Returns:
            The repository URI.
        """
        return f'{self.region}-docker.pkg.dev/{self.project}/{self.repository}/{self.image_name}'
    ...
    async def check_image_exists(self, image_uri: str) -> bool:
        ...
        if repo_uri != self_repo_uri:
            raise LaunchError(f'The image {image_uri} does not match to the image uri repository {self.uri}.')
        parent = f'projects/{self.project}/locations/{self.region}/repositories/{self.repository}'
        artifact_registry_client = event_loop_thread_exec(google.cloud.artifactregistry.ArtifactRegistryClient)
        ...
        try:
            ...
            raise LaunchError(f'The Google Artifact Registry repository {self.repository} does not exist. Please create it or modify your registry configuration.') from e
        return False
