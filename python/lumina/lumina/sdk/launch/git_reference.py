...
from lumina.sdk.lib.gitlib import GitCommandError, run_git
PREFIX_HTTPS = 'https://'
PREFIX_SSH = 'git@'
SUFFIX_GIT = '.git'
GIT_COMMIT_REGEX = re.compile('[0-9a-f]{40}')
_SCP_LIKE_REMOTE = re.compile('^[\\w.+-]+@[\\w.-]+:.+', re.ASCII)
_PROTOCOL_HARDENING = ('-c', 'protocol.file.allow=never', '-c', 'protocol.ext.allow=never')
...
def _validate_uri(uri: str) -> None:
    ...
    if stripped.startswith('-'):
        raise LaunchError(f'Refusing to fetch git remote {uri!r}: only https://, ssh://, and git@host:path remotes are allowed.')
    if stripped.lower().startswith((PREFIX_HTTPS, 'ssh://')) or _SCP_LIKE_REMOTE.match(stripped):
        return
    raise LaunchError(f'Refusing to fetch git remote {uri!r}: only https://, ssh://, and git@host:path remotes are allowed.')
...
class GitReference:
    ...
    def fetch(self, dst_dir: str) -> None:
        ...
        try:
            run_git(*_PROTOCOL_HARDENING, 'submodule', 'update', '--init', '--recursive', cwd=dst_dir)
        except GitCommandError as e:
            raise LaunchError(f'Unable to update submodules for git repository {self.url}:\n{e}')
