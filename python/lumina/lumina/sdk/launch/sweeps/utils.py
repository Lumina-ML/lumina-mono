...
def load_sweep_config(sweep_config_path: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        lumina.termerror(f"Couldn't open sweep file: {sweep_config_path}")
        return None
    try:
        config: dict[str, Any] | None = yaml.safe_load(yaml_file)
    except yaml.YAMLError as err:
        lumina.termerror(f'Error in configuration file: {err}')
        return None
    if not config:
        lumina.termerror('Configuration file is empty')
        return None
    return config
...
def construct_scheduler_args(sweep_config: dict[str, Any], queue: str, project: str, author: str | None=None, return_job: bool=False) -> list[str] | dict[str, str] | None:
    ...
    if not job and (not image_uri):
        lumina.termerror("No 'job' nor 'image_uri' top-level key found in sweep config, exactly one is required for a launch-sweep")
        return None
    elif job and image_uri:
        lumina.termerror("Sweep config has both 'job' and 'image_uri' but a launch-sweep can use only one")
        return None
    if return_job:
        args_dict: dict[str, str] = {'sweep_id': 'WANDB_SWEEP_ID', 'queue': queue, 'project': project}
        if job:
            args_dict['job'] = job
        elif image_uri:
        ...
        return args_dict
    args = ['--queue', f'{queue!r}', '--project', f'{project!r}']
    if author:
        args += ['--author', f'{author!r}']
    if job:
        args += ['--job', f'{job!r}']
    elif image_uri:
...
def create_sweep_command_args(command: dict) -> dict[str, Any]:
    ...
    return {'args': flags, 'args_no_equals': args_no_equals, 'args_no_hyphens': flags_no_hyphens, 'args_no_boolean_flags': flags_no_booleans, 'args_json': [json.dumps(flags_dict)], 'args_dict': flags_dict, 'args_append_hydra': flags_append_hydra, 'args_override_hydra': flags_override_hydra}
...
def check_job_exists(public_api: PublicApi, job: str | None) -> bool:
    ...
    try:
        ...
        lumina.termerror(f'Failed to load job. {e}')
        return False
    return True
