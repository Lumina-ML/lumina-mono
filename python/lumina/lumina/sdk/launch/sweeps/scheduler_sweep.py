...
from lumina.sdk.launch.sweeps.scheduler import LOG_PREFIX, RunState, Scheduler, SweepRun
...
class SweepScheduler(Scheduler):
    ...
    def _get_next_sweep_run(self, worker_id: int) -> SweepRun | None:
        ...
        for command in commands:
            ...
            if _run_id in self._runs:
                lumina.termlog(f'{LOG_PREFIX}Skipping duplicate run: {_run_id}')
                continue
            return SweepRun(id=_run_id, state=RunState.PENDING, args=command.get('args', {}), logs=command.get('logs', []), worker_id=worker_id)
        return None
    ...
    def _get_sweep_commands(self, worker_id: int) -> list[dict[str, Any]]:
        ...
        try:
            ...
            lumina.termerror(f'{LOG_PREFIX}Sweep was deleted or agent was not found. Stopping sweep.')
            self.stop_sweep()
            return []
