...
from lumina.sdk.launch.sweeps import SchedulerError
from lumina.sdk.launch.sweeps.utils import create_sweep_command_args, make_launch_sweep_entrypoint
from lumina.sdk.launch.utils import event_loop_thread_exec, strip_resource_args_and_template_vars
from lumina.sdk.lib.runid import generate_id
if TYPE_CHECKING:
    import lumina.apis.public as public
    from lumina.apis.internal import Api
    from lumina.apis.public import QueuedRun, Run
...
class Scheduler(ABC):
    ...
    def __init__(self, api: Api, *args: Any | None, polling_sleep: float | None=None, sweep_id: str | None=None, entity: str | None=None, project: str | None=None, project_queue: str | None=None, num_workers: int | str | None=None, **kwargs: Any | None):
        import yaml
        from lumina.apis.public import Api as PublicApi
        self._api = api
        self._public_api = PublicApi()
        self._entity = entity or os.environ.get('WANDB_ENTITY') or api.settings('entity') or api.default_entity
        self._project = project or os.environ.get('WANDB_PROJECT') or api.settings('project')
        ...
        try:
            ...
            if self._num_runs_launched > 0:
                lumina.termlog(f'{LOG_PREFIX}Found {self._num_runs_launched} previous valid runs for sweep {self._sweep_id}')
        except Exception as e:
            raise SchedulerError(f'{LOG_PREFIX}Exception when finding sweep ({sweep_id}) {e}')
        self._kwargs: dict[str, Any] = kwargs
        self._runs: dict[str, SweepRun] = {}
        self._threading_lock: threading.Lock = threading.Lock()
        self._polling_sleep = polling_sleep if polling_sleep is not None else DEFAULT_POLLING_SLEEP
        ...
        self._settings_config: dict[str, Any] = self._wandb_run.config.get('settings', {})
    ...
    @property
    def is_alive(self) -> bool:
        return self.state not in [SchedulerState.COMPLETED, SchedulerState.FAILED, SchedulerState.STOPPED, SchedulerState.CANCELLED]
    ...
    def _init_wandb_run(self) -> lumina.Run:
        """Controls resume or init logic for a scheduler wandb run."""
        settings = lumina.Settings(disable_job_creation=True)
        run: lumina.Run = lumina.init(name=f'Scheduler.{self._sweep_id}', resume='allow', config=self._kwargs, settings=settings)
        return run
    ...
    def start(self) -> None:
        ...
        lumina.termlog(f'{LOG_PREFIX}Scheduler starting.')
        if not self.is_alive:
            lumina.termerror(f'{LOG_PREFIX}Sweep already in end state ({self.state.name.lower()}). Exiting...')
            self.exit()
            return
        self._state = SchedulerState.STARTING
        if not self._try_load_executable():
            lumina.termerror(f"{LOG_PREFIX}No 'job' or 'image_uri' loaded from sweep config.")
            self.exit()
            return
        self._load_state()
    ...
    def run(self) -> None:
        """Main run function."""
        lumina.termlog(f'{LOG_PREFIX}Scheduler running')
        ...
        try:
            while True:
                ...
                if not self.is_alive:
                    break
                lumina.termlog(f'{LOG_PREFIX}Polling for new runs to launch')
                self._update_run_states()
                self._poll()
                if self.state == SchedulerState.FLUSH_RUNS:
                    if self.num_active_runs == 0:
                        lumina.termlog(f'{LOG_PREFIX}Done polling on runs, exiting')
                        break
                    time.sleep(self._polling_sleep)
                    continue
                for worker_id in self.available_workers:
                    if self.at_runcap:
                        lumina.termlog(f'{LOG_PREFIX}Sweep at run_cap ({self._num_runs_launched})')
                    ...
                    try:
                        ...
                        lumina.termerror(f'{LOG_PREFIX}Failed to get next sweep run: {e}')
            ...
            lumina.termwarn(f'{LOG_PREFIX}Scheduler received KeyboardInterrupt. Exiting')
            self.state = SchedulerState.STOPPED
            self.exit()
            return
        except Exception as e:
            lumina.termlog(f'{LOG_PREFIX}Scheduler failed with exception {e}')
            self.state = SchedulerState.FAILED
            self.exit()
            raise
        else:
    ...
    def exit(self) -> None:
        ...
        try:
            ...
            lumina.termerror(f'{LOG_PREFIX}Failed to save state: {traceback.format_exc()}')
        status = ''
        if self.state == SchedulerState.FLUSH_RUNS:
            ...
            self._set_sweep_state('CANCELED')
            status = 'cancelled'
            self._stop_runs()
        else:
            self.state = SchedulerState.FAILED
            ...
            status = 'crashed'
            self._stop_runs()
        lumina.termlog(f'{LOG_PREFIX}Scheduler {status}')
    ...
    def _try_load_executable(self) -> bool:
        ...
        if self._kwargs.get('job'):
            try:
                ...
                lumina.termlog(f'{LOG_PREFIX}Successfully loaded job ({_job_artifact.name}) in scheduler')
            except Exception:
                lumina.termerror(f'{LOG_PREFIX}{traceback.format_exc()}')
                return False
            return True
    ...
    async def _register_agents(self) -> None:
        ...
        for worker_id in range(self._num_workers):
            ...
            try:
                worker = register_agent(f'{socket.gethostname()}-{worker_id}', sweep_id=self._sweep_id, project_name=self._project, entity=self._entity)
                tasks.append(worker)
            except Exception as e:
        ...
        for idx, agent_config in enumerate(finished_tasks):
            self._workers[idx] = _Worker(agent_config=agent_config, agent_id=agent_config['id'])
    ...
    def _cleanup_runs(self, runs_to_remove: list[str]) -> None:
        ...
        with self._threading_lock:
            for run_id in runs_to_remove:
                lumina.termlog(f'{LOG_PREFIX}Cleaning up finished run ({run_id})')
    ...
    def _stop_runs(self) -> None:
        ...
        for run_id in to_delete:
            lumina.termlog(f'{LOG_PREFIX}Stopping run ({run_id})')
            if not self._stop_run(run_id):
                lumina.termwarn(f'{LOG_PREFIX}Failed to stop run ({run_id})')
    ...
    def _stop_run(self, run_id: str) -> bool:
        ...
        try:
            ...
            if success:
                lumina.termlog(f'{LOG_PREFIX}Stopped run {run_id}.')
                return True
        except Exception as e:
    ...
    def _get_run_state(self, run_id: str, prev_run_state: RunState=RunState.UNKNOWN) -> RunState:
        ...
        try:
            ...
            if prev_run_state == RunState.UNKNOWN:
                lumina.termwarn(f'Failed to get runstate for run ({run_id}). Error: {traceback.format_exc()}')
                run_state = RunState.FAILED
            else:
                run_state = RunState.UNKNOWN
        except (AttributeError, ValueError):
            lumina.termwarn(f'Bad state ({run_state}) for run ({run_id}). Error: {traceback.format_exc()}')
            run_state = RunState.UNKNOWN
        return run_state
    ...
    def _create_run(self) -> dict[str, Any]:
        ...
        try:
            server_run, inserted = self._api.upsert_run(project=self._project, entity=self._entity, sweep_name=self._sweep_id)
            return server_run
        except Exception as e:
    ...
    def _set_sweep_state(self, state: str) -> None:
        lumina.termlog(f'{LOG_PREFIX}Updating sweep state to: {state.lower()}')
        try:
    ...
    def _encode(self, _id: str) -> str:
        return base64.b64decode(bytes(_id.encode('utf-8'))).decode('utf-8').split(':')[2]

    def _make_entry_and_launch_config(self, run: SweepRun) -> tuple[list[str] | None, dict[str, dict[str, Any]]]:
        ...
        if entry_point:
            ...
            if unresolved:
                lumina.termwarn(f'{LOG_PREFIX}Sweep command contains unresolved macros: {unresolved}, see launch docs for supported macros.')
    ...
    def _add_to_launch_queue(self, run: SweepRun) -> bool:
        ...
        if entry_point:
            lumina.termwarn(f"{LOG_PREFIX}Sweep command {entry_point} will override {('job' if _job else 'image_uri')} entrypoint")
        ...
        lumina.termlog(f"{LOG_PREFIX}Added run ({run_id}) to queue ({self._kwargs.get('queue')})")
        return True
