...
from ..utils import LOG_PREFIX, event_loop_thread_exec, warn_failed_packages_from_build_logs
...
class DockerBuilder(AbstractBuilder):
    ...
    def __init__(self, environment: AbstractEnvironment, registry: AbstractRegistry, config: dict[str, Any]):
    ...
    @classmethod
    def from_config(cls, config: dict[str, Any], environment: AbstractEnvironment, registry: AbstractRegistry) -> DockerBuilder:
        ...
        if image_uri:
            if registry is not None:
                lumina.termwarn(f'{LOG_PREFIX}Overriding registry from registry config with {image_uri} from builder config.')
    ...
    async def build_image(self, launch_project: LaunchProject, entrypoint: EntryPoint, job_tracker: JobAndRunStatusTracker | None=None) -> str:
        ...
        if not launch_project.build_required() and await self.registry.check_image_exists(image_uri):
            return image_uri
        _logger.info(f'image {image_uri} does not already exist in repository, building.')
        try:
            output = await event_loop_thread_exec(docker.build)(tags=[image_uri], file=dockerfile, context_path=build_ctx_path, platform=self.config.get('platform'))
            warn_failed_packages_from_build_logs(output, image_uri, launch_project.api, job_tracker)
        ...
        if repository:
            ...
            lumina.termlog(f'{LOG_PREFIX}Pushing image {image_uri}')
            ...
            elif launch_project.resource == 'sagemaker' and f'The push refers to repository [{repository}]' not in push_resp:
