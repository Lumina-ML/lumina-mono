...
import lumina
import lumina.env
from lumina import docker
from lumina.apis.internal import Api
from lumina.sdk.launch.loader import builder_from_config, environment_from_config, registry_from_config
from lumina.util import docker_image_regex, get_module
from .._project_spec import EntryPoint, LaunchProject
from ..errors import ExecutionError, LaunchError
from ..utils import LOG_PREFIX, event_loop_thread_exec
from .templates.dockerfile import ACCELERATOR_SETUP_TEMPLATE, ENTRYPOINT_TEMPLATE, PIP_TEMPLATE, PYTHON_SETUP_TEMPLATE, USER_CREATE_TEMPLATE
_logger = logging.getLogger(__name__)
_WANDB_DOCKERFILE_NAME = 'Dockerfile.wandb'
_DOCKER_IMAGE_REF_UNSAFE_CHARS = re.compile('[\\s#]')
_DOCKER_IMAGE_SHA256_DIGEST = re.compile('sha256:[0-9a-fA-F]{64}')
...
def _validate_accelerator_base_image(base_image: str) -> str:
    ...
    if _DOCKER_IMAGE_REF_UNSAFE_CHARS.search(base_image):
        raise LaunchError("Invalid accelerator base image. Docker image references cannot contain whitespace or '#'.")
    image_name, digest_sep, digest = base_image.partition('@')
    if digest_sep and (not _DOCKER_IMAGE_SHA256_DIGEST.fullmatch(digest)):
        raise LaunchError('Invalid accelerator base image. Expected digest to match sha256:<64 hex characters>.')
    if not image_name or not docker_image_regex(image_name):
        raise LaunchError('Invalid accelerator base image. Expected a valid Docker image reference name.')
    return base_image
...
async def validate_docker_installation() -> None:
    ...
    if not await find_exec('docker'):
        raise ExecutionError('Could not find Docker executable. Ensure Docker is installed as per the instructions at https://docs.docker.com/install/overview/.')
...
async def build_image_from_project(launch_project: LaunchProject, api: Api, launch_config: dict[str, Any]) -> str:
    ...
    if not isinstance(env_config, dict):
        ...
        raise LaunchError(f'Invalid environment config: {env_config} of type {wrong_type} loaded from launch config. Expected dict.')
    ...
    if not isinstance(registry_config, dict):
        ...
        raise LaunchError(f'Invalid registry config: {registry_config} of type {wrong_type} loaded from launch config. Expected dict.')
    ...
    if not isinstance(builder_config, dict):
        ...
        raise LaunchError(f'Invalid builder config: {builder_config} of type {wrong_type} loaded from launch config. Expected dict.')
    ...
    entry_point = launch_project.get_job_entry_point() or launch_project.override_entrypoint
    assert entry_point is not None
    lumina.termlog(f'{LOG_PREFIX}Building docker image from uri source')
...
def get_base_setup(launch_project: LaunchProject, py_version: str, py_major: str) -> str:
    ...
    if launch_project.accelerator_base_image:
        accelerator_base_image = _validate_accelerator_base_image(launch_project.accelerator_base_image)
        ...
        base_setup = ACCELERATOR_SETUP_TEMPLATE.format(accelerator_base_image=accelerator_base_image, python_packages=' \\\n'.join(python_packages), py_version=py_version)
    else:
        python_packages = ['python3-dev', 'gcc']
        base_setup = PYTHON_SETUP_TEMPLATE.format(py_base_image=python_base_image)
    return base_setup

def get_requirements_section(launch_project: LaunchProject, build_context_dir: str, builder_type: str) -> str:
    if builder_type == 'docker':
        buildx_installed = docker.is_buildx_installed()
        if not buildx_installed:
            lumina.termwarn('Docker BuildX is not installed, for faster builds upgrade docker: https://github.com/docker/buildx#installing')
    ...
    if (base_path / 'src' / 'requirements.txt').exists():
        ...
        if not any(['wandb' in r for r in requirements]):
            lumina.termwarn(f'{LOG_PREFIX}wandb is not present in requirements.txt.')
        return PIP_TEMPLATE.format(buildx_optional_prefix=prefix, requirements_files=' '.join(requirements_files), pip_install=deps_install_line)
    elif (base_path / 'src' / 'pyproject.toml').exists():
        tomli = get_module('tomli')
        if tomli is None:
            lumina.termwarn('pyproject.toml found but tomli could not be loaded. To install dependencies from pyproject.toml please run `pip install tomli` and try again.')
        else:
            ...
            if project_deps:
                if not any(['wandb' in d for d in project_deps]):
                    lumina.termwarn(f'{LOG_PREFIX}wandb is not present as a dependency in pyproject.toml.')
                with open(base_path / 'src' / 'requirements.txt', 'w') as f:
                    f.write('\n'.join(project_deps))
                requirements_files += ['src/requirements.txt']
                deps_install_line = 'pip install uv && uv pip install -r requirements.txt'
                return PIP_TEMPLATE.format(buildx_optional_prefix=prefix, requirements_files=' '.join(requirements_files), pip_install=deps_install_line)
    if not deps_install_line and (base_path / 'src' / 'requirements.frozen.txt').exists():
        requirements_files += ['src/requirements.frozen.txt', '_wandb_bootstrap.py']
        deps_install_line = launch_project.parse_existing_requirements() + 'python _wandb_bootstrap.py'
        ...
        with open(base_path / 'src' / 'requirements.frozen.txt') as f:
            requirements = f.readlines()
        if not any(['wandb' in r for r in requirements]):
            lumina.termwarn(f'{LOG_PREFIX}wandb is not present in requirements.frozen.txt.')
        return PIP_TEMPLATE.format(buildx_optional_prefix=prefix, requirements_files=' '.join(requirements_files), pip_install=deps_install_line)
    else:
        requirements_line = 'RUN mkdir -p env/'
        lumina.termwarn('No requirements file found. No packages will be installed.')
