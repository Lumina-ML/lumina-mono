...
if len(ONLY_INCLUDE) == 0:
    ...
    OPTS.append('--force')
TORCH_DEP_REGEX = 'torch(vision|audio)?==\\d+\\.\\d+\\.\\d+(\\+(?:cu[\\d]{2,3})|(?:\\+cpu))?'

def install_deps(deps: list[str], failed: set[str] | None=None, extra_index: str | None=None, opts: list[str] | None=None) -> set[str] | None:
...
def main() -> None:
    ...
    if os.path.exists('requirements.frozen.txt'):
        with open('requirements.frozen.txt') as f:
            ...
            for req in f:
                if len(ONLY_INCLUDE) == 0 or req in ONLY_INCLUDE or req.split('=')[0].lower() in ONLY_INCLUDE:
                    if req.startswith('wandb==') and 'dev1' in req:
                        req = 'wandb'
                    match = re.match(TORCH_DEP_REGEX, req)
                    if match:
            ...
            if len(failed) > 0:
                sys.stderr.write(FAILED_PACKAGES_PREFIX + ','.join(failed) + FAILED_PACKAGES_POSTFIX)
                sys.stderr.flush()
...
def get_current_package(line: str, deps: list[str], current_pkg: str | None) -> str | None:
