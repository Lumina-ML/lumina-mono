...
from lumina.sdk.launch.builder.build import image_tag_from_dockerfile_and_source
from lumina.sdk.launch.errors import LaunchError
from lumina.sdk.launch.utils import get_current_python_version
from .build import _WANDB_DOCKERFILE_NAME, get_base_setup, get_docker_user, get_entrypoint_setup, get_requirements_section, get_user_setup
...
class BuildContextManager:
    ...
    def _generate_dockerfile(self, builder_type: str) -> str:
        ...
        requirements_section = get_requirements_section(launch_project, self._directory, builder_type)
    ...
    def create_build_context(self, builder_type: str) -> tuple[str, str]:
        ...
        """
        entrypoint = self._launch_project.get_job_entry_point() or self._launch_project.override_entrypoint
        ...
        if override_dockerfile:
            full_path = os.path.join(build_context_root_dir, override_dockerfile)
            if not os.path.exists(full_path):
                raise LaunchError(f'Dockerfile does not exist at {full_path}')
            shutil.copytree(build_context_root_dir, self._directory, symlinks=True, dirs_exist_ok=True, ignore=shutil.ignore_patterns('fsmonitor--daemon.ipc'))
            shutil.copy(full_path, os.path.join(self._directory, _WANDB_DOCKERFILE_NAME))
            return (self._directory, image_tag_from_dockerfile_and_source(self._launch_project, open(full_path).read()))
        ...
        if job_dockerfile:
            ...
            shutil.copytree(build_context_root_dir, self._directory, symlinks=True, dirs_exist_ok=True, ignore=shutil.ignore_patterns('fsmonitor--daemon.ipc'))
            shutil.copy(dockerfile_path, os.path.join(self._directory, _WANDB_DOCKERFILE_NAME))
            return (self._directory, image_tag_from_dockerfile_and_source(self._launch_project, open(dockerfile_path).read()))
        ...
        if os.path.exists(path):
            shutil.copytree(os.path.dirname(path), self._directory, symlinks=True, dirs_exist_ok=True, ignore=shutil.ignore_patterns('fsmonitor--daemon.ipc'))
            if entrypoint_dir:
            ...
            return (self._directory, image_tag_from_dockerfile_and_source(self._launch_project, docker_file_contents))
        dst_path = os.path.join(self._directory, 'src')
        assert self._launch_project.project_dir is not None
        shutil.copytree(src=self._launch_project.project_dir, dst=dst_path, symlinks=True, ignore=shutil.ignore_patterns('fsmonitor--daemon.ipc'))
        shutil.copy(os.path.join(os.path.dirname(__file__), 'templates', '_wandb_bootstrap.py'), os.path.join(self._directory))
        ...
        image_tag = image_tag_from_dockerfile_and_source(self._launch_project, docker_file_contents)
