...
class AbstractRun(ABC):
    ...
    def _run_cmd(self, cmd: list[str], output_only: bool | None=False) -> subprocess.Popen[bytes] | bytes | None:
...
class AbstractRunner(ABC):
    ...
    def verify(self) -> bool:
        ...
        if self._api.api_key is None:
            lumina.termerror("Couldn't find W&B api key, run wandb login or set WANDB_API_KEY")
            sys.exit(1)
        return True
