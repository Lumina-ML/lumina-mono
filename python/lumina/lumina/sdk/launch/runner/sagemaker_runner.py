...
from lumina.sdk.launch.environment.aws_environment import AwsEnvironment
from lumina.sdk.launch.errors import LaunchError
from .._project_spec import EntryPoint, LaunchProject
from ..registry.abstract import AbstractRegistry
from ..utils import LOG_PREFIX, MAX_ENV_LENGTHS, PROJECT_SYNCHRONOUS, event_loop_thread_exec, to_camel_case
    ...
    async def get_logs(self) -> str | None:
        ...
        try:
            ...
            if len(describe_res['logStreams']) == 0:
                lumina.termwarn(f'Failed to get logs for training job: {self.training_job_name}')
                return None
            ...
            lumina.termwarn(f'Failed to handle logs for training job: {self.training_job_name} with error {str(e)}')
            return None

    async def wait(self) -> bool:
        while True:
            status_state = (await self.get_status()).state
            lumina.termlog(f'{LOG_PREFIX}Training job {self.training_job_name} status: {status_state}')
...
class SageMakerRunner(AbstractRunner):
    ...
    async def run(self, launch_project: LaunchProject, image_uri: str) -> AbstractRun | None:
        ...
        role_arn = get_role_arn(given_sagemaker_args, self.backend_config, account_id, partition)
        sagemaker_client = session.client('sagemaker')
        log_client = None
        try:
            log_client = session.client('logs')
        except Exception as e:
            lumina.termwarn(f'Failed to connect to cloudwatch logs with error {str(e)}, logs will not be available')
        ...
        if command_args:
            ...
            lumina.termlog(f'{LOG_PREFIX}Launching run on sagemaker with entrypoint: {command_str}')
        else:
            lumina.termlog(f'{LOG_PREFIX}Launching run on sagemaker with user-provided entrypoint in image')
...
def build_sagemaker_args(launch_project: LaunchProject, api: Api, role_arn: str, entry_point: EntryPoint | None, args: list[str] | None, max_env_length: int, image_uri: str, default_output_path: str | None=None) -> dict[str, Any]:
    ...
    if sagemaker_args.get('ResourceConfig') is None:
        raise LaunchError('Sagemaker launcher requires a ResourceConfig resource argument')
    if sagemaker_args.get('StoppingCondition') is None:
        raise LaunchError('Sagemaker launcher requires a StoppingCondition resource argument')
...
async def launch_sagemaker_job(launch_project: LaunchProject, sagemaker_args: dict[str, Any], sagemaker_client: boto3.Client, log_client: boto3.Client | None=None) -> SagemakerSubmittedRun:
    ...
    lumina.termlog(f"{LOG_PREFIX}Run job submitted with arn: {resp.get('TrainingJobArn')}")
    ...
    lumina.termlog(f'{LOG_PREFIX}See training job status at: {url}')
    return run

def get_role_arn(sagemaker_args: dict[str, Any], backend_config: dict[str, Any], account_id: str, partition: str) -> str:
