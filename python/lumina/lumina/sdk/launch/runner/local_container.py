...
from ..utils import CODE_MOUNT_DIR, LOG_PREFIX, MAX_ENV_LENGTHS, PROJECT_SYNCHRONOUS, _is_wandb_dev_uri, _is_wandb_local_uri, docker_image_exists, event_loop_thread_exec, pull_docker_image, sanitize_wandb_api_key, validate_local_container_resource_args
...
class LocalContainerRunner(AbstractRunner):
    ...
    def _populate_docker_args(self, launch_project: LaunchProject, image_uri: str) -> dict[str, Any]:
        submitter_docker_args: dict[str, Any] = launch_project.fill_macros(image_uri).get('local-container', {})
        ...
        if base_image is not None:
            ...
            docker_args['volume'].append(f'{launch_project.project_dir}:{CODE_MOUNT_DIR}')
    ...
    async def run(self, launch_project: LaunchProject, image_uri: str) -> AbstractRun | None:
        ...
        if launch_project.docker_image or launch_project.job_base_image:
            try:
                ...
                lumina.termwarn(f'Error attempting to pull docker image {image_uri}')
...
def _run_entry_point(command: str, work_dir: str | None) -> AbstractRun:
    ...
    thread = threading.Thread(target=_thread_process_runner, args=(run, _shell_command(command), work_dir, env))
...
def _shell_command(command: str) -> list[str]:
    ...
    if shell is None:
        raise LaunchError('Could not launch command: no compatible shell found (expected bash or sh).')
    ...
    if docker_args:
        for name, value in docker_args.items():
            ...
            if isinstance(value, dict):
                raise LaunchError(f"Invalid docker argument '{name}': mapping values are not supported.")
