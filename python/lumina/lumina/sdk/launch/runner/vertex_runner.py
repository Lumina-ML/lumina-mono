...
class VertexRunner(AbstractRunner):
    ...
    async def run(self, launch_project: LaunchProject, image_uri: str) -> AbstractRun | None:
        ...
        env_vars = launch_project.get_env_vars_dict(api=self._api, max_env_length=MAX_ENV_LENGTHS[self.__class__.__name__])
        worker_specs = spec_args.get('worker_pool_specs', [])
        if not worker_specs:
            raise LaunchError('Vertex requires at least one worker pool spec. Please specify a worker pool spec in resource arguments under the key `vertex.spec.worker_pool_specs`.')
        for spec in worker_specs:
            if not spec.get('container_spec'):
                raise LaunchError('Vertex requires a container spec for each worker pool spec. Please specify a container spec in resource arguments under the key `vertex.spec.worker_pool_specs[].container_spec`.')
        ...
        if not spec_args.get('staging_bucket'):
            raise LaunchError('Vertex requires a staging bucket. Please specify a staging bucket in resource arguments under the key `vertex.spec.staging_bucket`.')
        _logger.info('Launching Vertex job...')
        submitted_run = await launch_vertex_job(launch_project, spec_args, run_args, self.environment, synchronous)
        return submitted_run

async def launch_vertex_job(launch_project: LaunchProject, spec_args: dict[str, Any], run_args: dict[str, Any], environment: GcpEnvironment, synchronous: bool=False) -> VertexSubmittedRun:
    try:
        await environment.verify()
        aiplatform = get_module('google.cloud.aiplatform', 'VertexRunner requires google.cloud.aiplatform to be installed')
        init = event_loop_thread_exec(aiplatform.init)
        await init(project=environment.project, location=environment.region, staging_bucket=spec_args.get('staging_bucket'), credentials=await environment.get_credentials())
        labels = spec_args.get('labels', {})
        labels[WANDB_RUN_ID_KEY] = launch_project.run_id
        job = aiplatform.CustomJob(display_name=launch_project.name, worker_pool_specs=spec_args.get('worker_pool_specs'), base_output_dir=spec_args.get('base_output_dir'), encryption_spec_key_name=spec_args.get('encryption_spec_key_name'), labels=labels)
        execution_kwargs = dict(timeout=run_args.get('timeout'), service_account=run_args.get('service_account'), network=run_args.get('network'), enable_web_access=run_args.get('enable_web_access', False), experiment=run_args.get('experiment'), experiment_run=run_args.get('experiment_run'), tensorboard=run_args.get('tensorboard'), restart_job_on_worker_restart=run_args.get('restart_job_on_worker_restart', False))
