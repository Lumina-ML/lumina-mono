...
from kubernetes_asyncio.client import ApiException, BatchV1Api, CoreV1Api, CustomObjectsApi, V1Pod, V1PodStatus
import lumina
from lumina.sdk.launch.agent import LaunchAgent
from lumina.sdk.launch.errors import LaunchError
from lumina.sdk.launch.runner.abstract import State, Status
from lumina.sdk.launch.utils import get_kube_context_and_api_client
...
CRD_STATE_DICT: dict[str, State] = {'created': 'starting', 'pending': 'starting', 'running': 'running', 'completing': 'running', 'succeeded': 'finished', 'completed': 'finished', 'failed': 'failed', 'aborted': 'failed', 'timeout': 'failed', 'terminated': 'failed', 'terminating': 'stopping'}
...
def _log_err_task_callback(task: asyncio.Task) -> None:
    ...
    if exec is not None:
        if isinstance(exec, asyncio.CancelledError):
            lumina.termlog(f'Task {task.get_name()} was cancelled')
            return
        name = task.get_name()
        lumina.termerror(f'Exception in task {name}')
        tb = exec.__traceback__
        tb_str = ''.join(traceback.format_tb(tb))
        lumina.termerror(tb_str)
...
class LaunchKubernetesMonitor:
    ...
    def __init__(self, core_api: CoreV1Api, batch_api: BatchV1Api, custom_api: CustomObjectsApi, label_selector: str):
        ...
        self._monitor_tasks: dict[tuple[str, str | CustomResource], asyncio.Task] = dict()
    ...
    @classmethod
    def monitor_namespace(cls, namespace: str, custom_resource: CustomResource | None=None) -> None:
    ...
    def __monitor_namespace(self, namespace: str, custom_resource: CustomResource | None=None) -> None:
        """Start monitoring a namespaces for resources."""
    ...
    def _add_status_message(self, job_name: str, message: str) -> None:
        ...
        lumina.termwarn(f'Warning from Kubernetes for job {job_name}: {message}')
    ...
    async def _monitor_jobs(self, namespace: str) -> None:
        ...
        async for event in watcher.stream(self._batch_api.list_namespaced_job, namespace=namespace, label_selector=self._label_selector):
            ...
            if event.get('type') == 'DELETED' and self._job_states.get(job_name) != Status('finished'):
                self._set_status_state(job_name, 'failed')
    ...
    async def stream(self, func: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        while True:
            try:
                ...
                lumina.termwarn(f'Broken event stream: {e}, attempting to recover')
                ...
                lumina.termerror(f'Unknown exception in event stream of type {exc_type}: {E}, attempting to recover. Stack trace: {stack_trace}')
