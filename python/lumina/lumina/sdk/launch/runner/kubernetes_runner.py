...
from lumina.sdk.launch.agent.agent import LaunchAgent
from lumina.sdk.launch.environment.abstract import AbstractEnvironment
from lumina.sdk.launch.registry.abstract import AbstractRegistry
from lumina.sdk.launch.registry.azure_container_registry import AzureContainerRegistry
from lumina.sdk.launch.registry.local_registry import LocalRegistry
from lumina.sdk.launch.runner.abstract import Status
from lumina.sdk.launch.runner.kubernetes_monitor import WANDB_K8S_LABEL_AGENT, WANDB_K8S_LABEL_AUXILIARY_RESOURCE, WANDB_K8S_LABEL_MONITOR, WANDB_K8S_RUN_ID, CustomResource, LaunchKubernetesMonitor
from lumina.sdk.launch.utils import recursive_macro_sub, sanitize_identifiers_for_k8s, yield_containers
from lumina.sdk.lib.retry import ExponentialBackoff, retry_async
from lumina.util import get_module
from .._project_spec import EntryPoint, LaunchProject
from ..errors import LaunchError
from ..utils import CODE_MOUNT_DIR, LOG_PREFIX, MAX_ENV_LENGTHS, PROJECT_SYNCHRONOUS, get_kube_context_and_api_client, inject_restricted_security_context, make_k8s_label_safe, make_name_dns_safe, validate_kubernetes_resource_args
from .abstract import AbstractRun, AbstractRunner
get_module('kubernetes_asyncio', required='Kubernetes runner requires the kubernetes package. Please install it with `pip install wandb[launch]`.')
...
class KubernetesSubmittedRun(AbstractRun):
    ...
    def __init__(self, batch_api: BatchV1Api, core_api: CoreV1Api, apps_api: AppsV1Api, network_api: NetworkingV1Api, name: str, namespace: str | None='default', secret: V1Secret | None=None, auxiliary_resource_label_key: str | None=None) -> None:
    ...
    async def get_logs(self) -> str | None:
        try:
            ...
            if not pod_names:
                lumina.termwarn(f'Found no pods for kubernetes job: {self.name}')
                return None
            logs = await self.core_api.read_namespaced_pod_log(name=pod_names[0], namespace=self.namespace)
            if logs:
                return str(logs)
            else:
                lumina.termwarn(f'No logs for kubernetes pod(s): {pod_names}')
            return None
        except Exception as e:
            lumina.termerror(f'{LOG_PREFIX}Failed to get pod logs: {e}')
            return None
    ...
    async def wait(self) -> bool:
        ...
        while True:
            ...
            lumina.termlog(f'{LOG_PREFIX}Job {self.name} status: {status.state}')
    ...
    async def _delete_auxiliary_resources_by_label(self) -> None:
        if self.auxiliary_resource_label_key is None:
            return
        label_selector = f'{WANDB_K8S_LABEL_AUXILIARY_RESOURCE}={self.auxiliary_resource_label_key}'
        try:
            resource_cleanups = [(self.core_api, 'service'), (self.batch_api, 'job'), (self.core_api, 'pod'), (self.core_api, 'secret'), (self.apps_api, 'deployment'), (self.network_api, 'network_policy')]
            for api_client, resource_type in resource_cleanups:
                try:
                    ...
                    resources = await list_method(namespace=self.namespace, label_selector=label_selector)
                    for resource in resources.items:
                        await delete_method(name=resource.metadata.name, namespace=self.namespace)
                except (AttributeError, ApiException) as e:
                    lumina.termwarn(f'Could not clean up {resource_type}: {e}')
        except Exception as e:
            lumina.termwarn(f'Failed to clean up some auxiliary resources: {e}')
...
class CrdSubmittedRun(AbstractRun):
    ...
    async def get_logs(self) -> str | None:
        ...
        try:
            ...
            lumina.termwarn(f'Failed to get logs for {self.name}: {str(e)}')
            return None
        if not logs:
            return None
    ...
    async def wait(self) -> bool:
        ...
        while True:
            status = await self.get_status()
            lumina.termlog(f'{LOG_PREFIX}Job {self.name} status: {status}')
            if status.state in ['finished', 'failed', 'preempted']:
                return status.state == 'finished'
            await asyncio.sleep(5)
...
class KubernetesRunner(AbstractRunner):
    ...
    async def _inject_defaults(self, resource_args: dict[str, Any], launch_project: LaunchProject, image_uri: str, namespace: str, core_api: CoreV1Api) -> tuple[dict[str, Any], V1Secret | None]:
        ...
        entry_point = launch_project.override_entrypoint or launch_project.get_job_entry_point()
        ...
        env_vars = launch_project.get_env_vars_dict(self._api, MAX_ENV_LENGTHS[self.__class__.__name__])
        api_key_secret = None
            ...
            for key, value in env_vars.items():
                if key == 'WANDB_API_KEY' and value and (LaunchAgent.initialized() or self.backend_config[PROJECT_SYNCHRONOUS]):
                    ...
                    def handle_exception(e):
                        lumina.termwarn(f'Exception when ensuring Kubernetes API key secret: {e}. Retrying...')
                    api_key_secret = await retry_async(backoff=ExponentialBackoff(initial_sleep=datetime.timedelta(seconds=1), max_sleep=datetime.timedelta(minutes=1), max_retries=API_KEY_SECRET_MAX_RETRIES), fn=ensure_api_key_secret, on_exc=handle_exception, core_api=core_api, secret_name=secret_name, namespace=namespace, api_key=value)
        ...
        if launch_project.job_base_image:
            if SOURCE_CODE_PVC_NAME and SOURCE_CODE_PVC_MOUNT_PATH:
                apply_code_mount_configuration(job, launch_project)
            else:
                apply_code_mount_configuration_emptydir(job, launch_project, self._api)
        if LaunchAgent.initialized():
    ...
    async def _wait_for_resource_ready(self, api_client: kubernetes_asyncio.client.ApiClient, config: dict[str, Any], namespace: str, timeout_seconds: int=300) -> None:
        ...
        if not resource_kind or not resource_name:
            lumina.termerror(f'{LOG_PREFIX}Cannot wait for resource without kind or name')
            return
        lumina.termlog(f"{LOG_PREFIX}Waiting for {resource_kind} '{resource_name}' to be ready...")
        start_time = time.time()
        if resource_kind == 'Deployment':
            await self._wait_for_deployment_ready(api_client, resource_name, namespace, timeout_seconds)
        elif resource_kind == 'Service':
            await self._wait_for_service_ready(api_client, resource_name, namespace, timeout_seconds)
        elif resource_kind == 'Pod':
            await self._wait_for_pod_ready(api_client, resource_name, namespace, timeout_seconds)
        else:
            lumina.termlog(f'{LOG_PREFIX}No specific readiness check for {resource_kind}, waiting 5 seconds...')
            await asyncio.sleep(5)
        elapsed = time.time() - start_time
        lumina.termlog(f"{LOG_PREFIX}{resource_kind} '{resource_name}' is ready after {elapsed:.1f}s")

    async def _wait_for_deployment_ready(self, api_client: kubernetes_asyncio.client.ApiClient, name: str, namespace: str, timeout_seconds: int) -> None:
        """Wait for a Deployment to be ready."""
        apps_api = kubernetes_asyncio.client.AppsV1Api(api_client)

        async def check_deployment_ready():
            deployment = await apps_api.read_namespaced_deployment(name=name, namespace=namespace)
    ...
    async def _wait_for_service_ready(self, api_client: kubernetes_asyncio.client.ApiClient, name: str, namespace: str, timeout_seconds: int) -> None:
        """Wait for a Service to have endpoints."""
        core_api = kubernetes_asyncio.client.CoreV1Api(api_client)

        async def check_service_ready():
            endpoints = await core_api.read_namespaced_endpoints(name=name, namespace=namespace)
    ...
    async def _wait_for_pod_ready(self, api_client: kubernetes_asyncio.client.ApiClient, name: str, namespace: str, timeout_seconds: int) -> None:
    ...
    async def _wait_with_timeout(self, check_func, timeout_seconds: int, name: str) -> None:
        ...
        while time.time() - start_time < timeout_seconds:
            try:
                ...
                if e.status == 404:
                    ...
                    lumina.termerror(f"{LOG_PREFIX}Error waiting for resource '{name}': {e}")
                    raise
            except Exception as e:
                lumina.termerror(f"{LOG_PREFIX}Error waiting for resource '{name}': {e}")
                raise
            await asyncio.sleep(2)
        raise LaunchError(f"Resource '{name}' not ready within {timeout_seconds} seconds")

    async def _prepare_resource(self, api_client: kubernetes_asyncio.client.ApiClient, config: dict[str, Any], namespace: str, run_id: str, launch_project: LaunchProject, api_key_secret: V1Secret | None=None, wait_for_ready: bool=True, wait_timeout: int=300, auxiliary_resource_label_value: str | None=None) -> None:
        ...
        if auxiliary_resource_label_value:
            config['metadata']['labels'][WANDB_K8S_LABEL_AUXILIARY_RESOURCE] = auxiliary_resource_label_value
        env_vars = launch_project.get_env_vars_dict(self._api, MAX_ENV_LENGTHS[self.__class__.__name__])
        wandb_config_env = {'WANDB_CONFIG': env_vars.get('WANDB_CONFIG', '{}')}
        add_wandb_env(config, wandb_config_env)
        if auxiliary_resource_label_value:
            add_label_to_pods(config, WANDB_K8S_LABEL_AUXILIARY_RESOURCE, auxiliary_resource_label_value)
        if api_key_secret:
            for cont in yield_containers(config):
                env = cont.setdefault('env', [])
                env.append({'name': 'WANDB_API_KEY', 'valueFrom': {'secretKeyRef': {'name': api_key_secret.metadata.name, 'key': 'password'}}})
                cont['env'] = env
        validate_kubernetes_resource_args(config)
        inject_restricted_security_context(config)
        try:
            sanitize_identifiers_for_k8s(config)
            await kubernetes_asyncio.utils.create_from_dict(api_client, config, namespace=namespace)
            if wait_for_ready:
                await self._wait_for_resource_ready(api_client, config, namespace, wait_timeout)
        except Exception as e:
            lumina.termerror(f'{LOG_PREFIX}Failed to create Kubernetes resource: {e}')
            raise LaunchError(f'Failed to create Kubernetes resource: {e}')

    async def run(self, launch_project: LaunchProject, image_uri: str) -> AbstractRun | None:
        ...
        if not resource_args:
            lumina.termlog(f'{LOG_PREFIX}Note: no resource args specified. Add a Kubernetes yaml spec or other options in a json file with --resource-args <json>.')
        ...
        if launch_project.job_base_image is not None:
            if SOURCE_CODE_PVC_NAME and SOURCE_CODE_PVC_MOUNT_PATH:
                ...
                launch_project.change_project_dir(os.path.join(SOURCE_CODE_PVC_MOUNT_PATH, code_subdir))
            else:
        ...
        if api_version not in ['batch/v1', 'batch/v1beta1']:
            ...
            if LaunchAgent.initialized():
                add_label_to_pods(resource_args, WANDB_K8S_LABEL_AGENT, LaunchAgent.name())
                resource_args['metadata']['labels'][WANDB_K8S_LABEL_AGENT] = LaunchAgent.name()
            if launch_project.job_base_image:
                if use_emptydir_code_mount:
                    apply_code_mount_configuration_emptydir(resource_args, launch_project, self._api)
                else:
        ...
        update_dict = {'project_name': launch_project.target_project, 'entity_name': launch_project.target_entity, 'run_id': launch_project.run_id, 'run_name': launch_project.name, 'image_uri': image_uri, 'author': launch_project.author}
        update_dict.update(os.environ)
        additional_services: list[dict[str, Any]] = recursive_macro_sub(launch_project.launch_spec.get('additional_services', []), update_dict)
        auxiliary_resource_label_value = make_k8s_label_safe(f'aux-{launch_project.target_entity}-{launch_project.target_project}-{launch_project.run_id}')
        if additional_services:
            lumina.termlog(f'{LOG_PREFIX}Creating additional services: {additional_services}')
            wait_for_ready = resource_args.get('wait_for_ready', True)
            wait_timeout = resource_args.get('wait_timeout', 300)
            await asyncio.gather(*[self._prepare_resource(api_client, resource.get('config', {}), namespace, launch_project.run_id, launch_project, secret, wait_for_ready, wait_timeout, auxiliary_resource_label_value) for resource in additional_services if resource.get('config', {})])
        ...
        submitted_job = KubernetesSubmittedRun(batch_api, core_api, apps_api, network_api, job_name, namespace, secret, auxiliary_resource_label_value)
        if self.backend_config[PROJECT_SYNCHRONOUS]:
...
def inject_entrypoint_and_args(containers: list[dict], entry_point: EntryPoint | None, override_args: list[str], should_override_entrypoint: bool) -> None:
    ...
    for i in range(len(containers)):
        ...
        if entry_point and (not containers[i].get('command') or should_override_entrypoint):
            containers[i]['command'] = entry_point.command

async def ensure_api_key_secret(core_api: CoreV1Api, secret_name: str, namespace: str, api_key: str) -> V1Secret:
    ...
    secret = client.V1Secret(data=secret_data, metadata=client.V1ObjectMeta(name=secret_name, namespace=namespace, labels=labels), kind='Secret', type='kubernetes.io/basic-auth')
    try:
        try:
            return await core_api.create_namespaced_secret(namespace, secret)
        except ApiException as e:
            if e.status == 409:
                existing_secret = await core_api.read_namespaced_secret(name=secret_name, namespace=namespace)
                if existing_secret.data != secret_data:
                    if existing_secret.metadata.labels.get('wandb.ai/created-by') == 'launch-agent':
                        await core_api.delete_namespaced_secret(name=secret_name, namespace=namespace)
                        return await core_api.create_namespaced_secret(namespace, secret)
                    else:
...
async def maybe_create_imagepull_secret(core_api: CoreV1Api, registry: AbstractRegistry, run_id: str, namespace: str) -> V1Secret | None:
    ...
    creds_info = {'auths': {registry.uri: {'auth': base64.b64encode(f'{uname}:{token}'.encode()).decode(), 'email': 'deprecated@wandblaunch.com'}}}
    secret_data = {'.dockerconfigjson': base64.b64encode(json.dumps(creds_info).encode()).decode()}
    secret = client.V1Secret(data=secret_data, metadata=client.V1ObjectMeta(name=f'regcred-{run_id}', namespace=namespace), kind='Secret', type='kubernetes.io/dockerconfigjson')
    try:
        try:
            return await core_api.create_namespaced_secret(namespace, secret)
        except ApiException as e:
            if e.status == 409:
                return await core_api.read_namespaced_secret(name=f'regcred-{run_id}', namespace=namespace)
            raise
    except Exception as e:
...
def _set_container_command_with_dep_install(container: dict, working_dir: str, requirements_path: str) -> None:
    ...
    original_cmd_str = ' '.join((shlex.quote(c) for c in original_command + original_args))
    if not original_cmd_str:
        return
    user_requirements = f'{working_dir}/requirements.txt'
    pyproject = f'{working_dir}/pyproject.toml'
    install_prefix = f"if [ -f {shlex.quote(user_requirements)} ]; then pip install -r {shlex.quote(user_requirements)}; elif [ -f {shlex.quote(pyproject)} ]; then pip install {shlex.quote(working_dir)}; elif [ -f {shlex.quote(requirements_path)} ]; then pip install -r {shlex.quote(requirements_path)}; else echo 'No requirements file found'; fi"
    container['command'] = ['/bin/sh', '-c']
    container['args'] = [f'{install_prefix} && exec {original_cmd_str}']

def apply_code_mount_configuration(manifest: dict | list, project: LaunchProject) -> None:
    ...
    for pod in yield_pods(manifest):
        for container in yield_containers(pod):
            ...
            container['volumeMounts'].append({'name': 'wandb-source-code-volume', 'mountPath': CODE_MOUNT_DIR, 'subPath': source_dir})
            container['workingDir'] = project.resolved_working_dir
            if project._auto_default_base_image:
                _set_container_command_with_dep_install(container, project.resolved_working_dir, f'{CODE_MOUNT_DIR}/.job/requirements.frozen.txt')
        spec = pod['spec']
        if 'volumes' not in spec:
            spec['volumes'] = []
        spec['volumes'].append({'name': 'wandb-source-code-volume', 'persistentVolumeClaim': {'claimName': SOURCE_CODE_PVC_NAME}})

def _build_code_fetch_script(source_type: str, source_info: dict, install_deps: bool, job_dir: str) -> str:
    ...
    if source_type == 'artifact':
        ...
        return f'git clone {shlex.quote(git_remote)} {CODE_MOUNT_DIR} && git config --global --add safe.directory {CODE_MOUNT_DIR} && cd {CODE_MOUNT_DIR} && git checkout {shlex.quote(git_commit)}' + fetch_job_artifact + chmod_suffix

def _build_source_init_container(fetch_script: str, base_url: str, api_key_env: dict | None) -> dict:
    ...
    return {'name': 'wandb-source-code-init', 'image': 'wandb/launch-agent:0.25.1', 'volumeMounts': [{'name': 'wandb-source-code-volume', 'mountPath': CODE_MOUNT_DIR}], 'env': init_env, 'command': ['/bin/sh', '-c', fetch_script]}

def _configure_containers_for_code_mount(pod: dict, project: LaunchProject, install_deps: bool, job_dir: str) -> dict | None:
    ...
    api_key_env = None
    for container in yield_containers(pod):
        container.setdefault('volumeMounts', []).append({'name': 'wandb-source-code-volume', 'mountPath': CODE_MOUNT_DIR})
        container['workingDir'] = project.resolved_working_dir
        if install_deps:
            _set_container_command_with_dep_install(container, project.resolved_working_dir, f'{job_dir}/requirements.frozen.txt')
...
def apply_code_mount_configuration_emptydir(manifest: dict | list, project: LaunchProject, api: Api) -> None:
    ...
    if source_type not in ('artifact', 'repo'):
        raise LaunchError(f'Cannot use emptyDir code mount for unknown source type: {source_type!r}')
    job_dir = f'{CODE_MOUNT_DIR}/.job'
    for pod in yield_pods(manifest):
        spec = pod['spec']
        spec.setdefault('volumes', []).append({'name': 'wandb-source-code-volume', 'emptyDir': {}})
        api_key_env = _configure_containers_for_code_mount(pod, project, install_deps, job_dir)
        fetch_script = _build_code_fetch_script(source_type, source_info, install_deps, job_dir)
        init_container = _build_source_init_container(fetch_script, base_url, api_key_env)
