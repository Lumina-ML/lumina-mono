...
from .utils import LAUNCH_CONFIG_FILE, PROJECT_SYNCHRONOUS, construct_launch_spec, validate_launch_spec_source
...
def set_launch_logfile(logfile: str) -> None:
    ...
    if logfile == '-':
        ...
        try:
            ...
            lumina.termerror(f'Could not open {logfile} for writing logs. Please check the path and permissions.\nError: {e}')
            return
    lumina.termlog(f"Internal agent logs printing to {('stdout' if logfile == '-' else logfile)}. ")
...
def resolve_agent_config(entity: str | None, max_jobs: int | None, queues: tuple[str] | None, config: str | None, verbosity: int | None) -> tuple[dict[str, Any], Api]:
    ...
    """
    import yaml
    defaults = {'max_jobs': 1, 'max_schedulers': 1, 'queues': [], 'registry': {}, 'builder': {}, 'verbosity': 0}
    ...
    if resolved_config.get('queue'):
        if isinstance(resolved_config.get('queue'), str):
            ...
            msg = "Invalid launch agent config for key 'queue' with type: {type(resolved_config.get('queue'))} (expected str). Specify multiple queues with the 'queues' key"
...
def create_and_run_agent(api: Api, config: dict[str, Any]) -> None:
    try:
        from lumina.sdk.launch.agent import config as agent_config
...
async def _launch(api: Api, job: str | None=None, name: str | None=None, project: str | None=None, entity: str | None=None, docker_image: str | None=None, entry_point: list[str] | None=None, version: str | None=None, resource: str | None=None, resource_args: dict[str, Any] | None=None, launch_config: dict[str, Any] | None=None, synchronous: bool | None=None, run_id: str | None=None, repository: str | None=None) -> AbstractRun:
    """Helper that delegates to the project-running method corresponding to the passed-in backend."""
    if launch_config is None:
        launch_config = {}
    if resource is None:
        resource = 'local-container'
    launch_spec = construct_launch_spec(None, job, api, name, project, entity, docker_image, resource, entry_point, version, resource_args, launch_config, run_id, repository, author=None)
    ...
    image_uri = launch_project.docker_image or launch_project.job_base_image
    ...
    backend = loader.runner_from_config(resource, api, runner_config, environment, registry)
    if backend:
        assert image_uri
        ...
        raise ExecutionError(f"Unavailable backend {resource}, available backends: {', '.join(loader.WANDB_RUNNERS)}")
...
def launch(api: Api, job: str | None=None, entry_point: list[str] | None=None, version: str | None=None, name: str | None=None, resource: str | None=None, resource_args: dict[str, Any] | None=None, project: str | None=None, entity: str | None=None, docker_image: str | None=None, config: dict[str, Any] | None=None, synchronous: bool | None=True, run_id: str | None=None, repository: str | None=None) -> AbstractRun:
    ...
    Raises:
        ...
        is unsuccessful.
    """
    submitted_run_obj = asyncio.run(_launch(job=job, name=name, project=project, entity=entity, docker_image=docker_image, entry_point=entry_point, version=version, resource=resource, resource_args=resource_args, launch_config=config, synchronous=synchronous, api=api, run_id=run_id, repository=repository))
