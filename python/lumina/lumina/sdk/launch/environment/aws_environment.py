...
boto3 = get_module('boto3', required='AWS environment requires boto3 to be installed. Please install it with `pip install wandb[launch]`.')
botocore = get_module('botocore', required='AWS environment requires botocore to be installed. Please install it with `pip install wandb[launch]`.')
_logger = logging.getLogger(__name__)

class AwsEnvironment(AbstractEnvironment):
    """AWS environment."""

    def __init__(self, region: str, access_key: str, secret_key: str, session_token: str) -> None:
    ...
    def from_default(cls, region: str | None=None) -> AwsEnvironment:
        ...
        try:
            ...
            if not credentials:
                raise LaunchError('Could not create AWS environment from default environment. Please verify that your AWS credentials are configured correctly.')
            access_key = credentials.access_key
            secret_key = credentials.secret_key
            session_token = credentials.token
        except botocore.client.ClientError as e:
            raise LaunchError(f'Could not create AWS environment from default environment. Please verify that your AWS credentials are configured correctly. {e}')
        if not region:
            raise LaunchError('Could not create AWS environment from default environment. Region not specified.')
        return cls(region=region, access_key=access_key, secret_key=secret_key, session_token=session_token)

    @classmethod
    def from_config(cls, config: dict[str, str]) -> AwsEnvironment:
        """Create an AWS environment from the default AWS environment.
        ...
        if not region:
            raise LaunchError('Could not create AWS environment from config. Region not specified.')
        return cls.from_default(region=region)
    ...
    async def get_partition(self) -> str:
        ...
        try:
            ...
            if not arn:
                raise LaunchError('Could not set partition for AWS environment. ARN not found.')
            matched_partition = ARN_PARTITION_RE.match(arn)
            if not matched_partition:
                raise LaunchError(f'Could not set partition for AWS environment. ARN {arn} is not valid.')
            partition = matched_partition.group(1)
            return partition
        except botocore.exceptions.ClientError as e:
            raise LaunchError(f'Could not set partition for AWS environment. {e}') from e
    ...
    async def verify(self) -> None:
        ...
        try:
            ...
            raise LaunchError(f'Could not verify AWS environment. Please verify that your AWS credentials are configured correctly. {e}') from e
    ...
    async def get_session(self) -> boto3.Session:
        ...
        try:
            ...
            return await session(region_name=self._region, aws_access_key_id=self._access_key, aws_secret_access_key=self._secret_key, aws_session_token=self._session_token)
        except botocore.exceptions.ClientError as e:
    ...
    async def upload_file(self, source: str, destination: str) -> None:
        ...
        if not match:
            raise LaunchError(f'{_err_prefix}: Destination {destination} is not a valid s3 URI.')
        ...
        try:
            ...
            raise LaunchError(f'{_err_prefix}: botocore error attempting to copy {source} to {destination}. {e}')
        ...
        try:
            ...
            for path, _, files in os.walk(source):
                for file in files:
                    ...
                    key_path = abs_path.replace(source, '').replace('\\', '/').lstrip('/')
                    client.upload_file(abs_path, bucket, key_path)
        except botocore.exceptions.ClientError as e:
            raise LaunchError(f'{_err_prefix}: botocore error attempting to copy {source} to {destination}. {e}') from e
        except Exception as e:
            raise LaunchError(f'{_err_prefix}: Unexpected error attempting to copy {source} to {destination}. {e}') from e
    ...
    async def verify_storage_uri(self, uri: str) -> None:
        ...
        if not match:
            raise LaunchError(f'Failed to validate storage uri: {uri} is not a valid s3 URI.')
        bucket = match.group(1)
        try:
            session = await self.get_session()
            client = await event_loop_thread_exec(session.client)('s3')
            ...
            if e.response['Error']['Code'] == '404':
                raise LaunchError(f'Could not verify AWS storage uri {uri}. Bucket {bucket} does not exist.')
            if e.response['Error']['Code'] == '403':
                raise LaunchError(f'Could not verify AWS storage uri {uri}. Bucket {{bucket}} is not accessible. Please check that this client is authenticated with permission to access the bucket.')
            raise LaunchError(f'Failed to verify AWS storage uri {uri}. Response: {e.response} Please verify that your AWS credentials are configured correctly.')
