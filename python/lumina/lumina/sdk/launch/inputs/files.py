...
def config_path_is_valid(path: str) -> None:
    ...
    if os.path.isabs(path):
        raise LaunchError(f'Invalid config path: {path}. Please provide a relative path.')
    if '..' in path:
        raise LaunchError(f'Invalid config path: {path}. Please provide a relative path without any upward path traversal, e.g. `../config.json`.')
    ...
    if not any((path.endswith(ext) for ext in ['.json', '.yaml', '.yml'])):
        raise LaunchError(f'Invalid config path: {path}. Only JSON and YAML files are supported.')
