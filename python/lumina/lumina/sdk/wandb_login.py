...
import click
import lumina
from lumina.errors import AuthenticationError, term
from lumina.sdk import wandb_setup
from lumina.sdk.lib import settings_file, wbauth
...
def login(key: str | None=None, relogin: bool | None=None, host: str | None=None, force: bool | None=None, timeout: int | None=None, verify: bool=False, referrer: str | None=None, anonymous: DoNotSet=UNSET) -> bool:
    ...
    if anonymous is not UNSET:
        term.termwarn('The anonymous parameter to wandb.login() has no effect and will' + ' be removed in future versions.', repeat=False)
    ...
    if host:
        host = host.rstrip('/')
    _update_system_settings(global_settings.read_system_settings(), host=host)
    logged_in, _ = _login(key=key, relogin=relogin, host=host, force=force, timeout=timeout, verify=verify, referrer=referrer or 'models')
    return logged_in

def _update_system_settings(system_settings: settings_file.SettingsFiles, *, host: str | None) -> None:
...
def _login(*, key: str | None=None, relogin: bool | None=None, host: str | None=None, force: bool | None=None, timeout: float | None=None, verify: bool=False, referrer: str='models', update_api_key: bool=True, _silent: bool | None=None) -> tuple[bool, str | None]:
    """Log in to W&B.
    ...
    if lumina.util._is_kaggle() and (not lumina.util._has_internet()):
        term.termerror('To use W&B in kaggle you must enable internet in the settings' + ' panel on the right.')
        return (False, None)
    if key:
        auth: wbauth.Auth | None = _use_explicit_key(key, host=host_url, settings=settings, update_api_key=update_api_key, silent=_silent)
    else:
        auth = _find_or_prompt_for_key(settings, host=host_url, force=force, relogin=relogin, referrer=referrer, input_timeout=timeout)
...
def _find_or_prompt_for_key(settings: lumina.Settings, *, host: wbauth.HostUrl, force: bool, relogin: bool, referrer: str, input_timeout: float | None) -> wbauth.Auth | None:
    ...
    try:
        auth = wbauth.authenticate_session(host=host, source='wandb.login()', no_offline=force, no_create=force, referrer=referrer, input_timeout=input_timeout, relogin=relogin)
    except TimeoutError:
        timed_out = True
    if not auth:
        if timed_out:
...
def _verify_login(key: str, base_url: str) -> None:
    ...
    api = InternalApi(api_key=key, default_settings={'base_url': base_url})
    try:
        is_api_key_valid = api.validate_api_key()
    except ConnectionError as e:
        raise AuthenticationError(f'Unable to connect to {base_url} to verify API token.') from e
    except Exception as e:
        raise AuthenticationError('An error occurred while verifying the API key.') from e
    if not is_api_key_valid:
        raise AuthenticationError(f'API key verification failed for host {base_url}.' + ' Make sure your API key is valid.')
...
def _print_logged_in_message(settings: wandb.Settings, *, host: str) -> None:
    ...
    term.termlog(f'{login_state_str}. {login_info_str}', repeat=False)
