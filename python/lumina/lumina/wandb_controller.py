...
from lumina.sdk import wandb_sweep
from lumina.sdk.launch.sweeps.utils import handle_sweep_config_violations, sweep_config_err_text_from_jsonschema_violations
from lumina.util import get_module
sweeps = get_module('sweeps', required='wandb[sweeps] is required to use the local controller. Please run `pip install wandb[sweeps]`.')
...
class _WandbController:
    ...
    def __init__(self, sweep_id_or_config=None, entity=None, project=None):
        ...
        self._create: dict = {}
        self._custom_search: Callable[[dict | sweeps.SweepConfig, list[sweeps.SweepRun]], sweeps.SweepRun | None] | None = None
        self._custom_stopping: Callable[[dict | sweeps.SweepConfig, list[sweeps.SweepRun]], list[sweeps.SweepRun]] | None = None
        ...
        if isinstance(sweep_id_or_config, str):
            ...
            for config_key, controller_attr in zip(['method', 'early_terminate'], ['_custom_search', '_custom_stopping'], strict=True):
        ...
        for r in sweep_obj['runs']:
            ...
            if 'history' in rr:
                if isinstance(rr['history'], list):
                    ...
                    raise ValueError('Invalid history value: expected list of json strings: {}'.format(rr['history']))
    ...
    def done(self) -> bool:
        ...
        return state not in [s.upper() for s in (sweeps.RunState.preempting.value, SWEEP_INITIAL_RUN_STATE.value, sweeps.RunState.running.value)]
    ...
    def _stopping(self) -> list[sweeps.SweepRun]:
        ...
        debug_lines = [' '.join([f'{k}={v}' for k, v in run.early_terminate_info.items()]) for run in stop_runs if run.early_terminate_info is not None]
        if debug_lines:
...
def _sweep_status(sweep_obj: dict, sweep_conf: dict | sweeps.SweepConfig, sweep_runs: list[sweeps.SweepRun]) -> str:
