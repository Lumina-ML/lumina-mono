...
from lumina.filesync import step_upload
from lumina.sdk.lib import filesystem, runid
    ...
    def _thread_body(self) -> None:
        while True:
            ...
            if isinstance(req, RequestUpload):
                path = req.path
                if req.copy:
                    path = os.path.join(self._tempdir.name, f'{runid.generate_id()}-{req.save_name}')
                ...
                self._output_queue.put(step_upload.RequestUpload(path, req.save_name, None, None, req.copy, None, None))
            elif isinstance(req, RequestStoreManifestFiles):
                for entry in req.manifest.entries.values():
                    if entry.local_path:
                        self._stats.init_file(entry.local_path, cast(int, entry.size), is_artifact_file=True)
                        self._output_queue.put(step_upload.RequestUpload(entry.local_path, entry.path, req.artifact_id, entry.digest, False, functools.partial(req.save_fn, entry), entry.digest))
            elif isinstance(req, RequestCommitArtifact):
                self._output_queue.put(step_upload.RequestCommitArtifact(req.artifact_id, req.finalize, req.before_commit, req.result_future))
