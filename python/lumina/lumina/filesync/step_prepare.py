...
if TYPE_CHECKING:
    from lumina.sdk.internal.internal_api import Api, CreateArtifactFileSpecInput, CreateArtifactFilesResponseFile
...
class StepPrepare:
    ...
    def _thread_body(self) -> None:
        while True:
            finish, batch = gather_batch(request_queue=self._request_queue, batch_time=self._batch_time, inter_event_time=self._inter_event_time, max_batch_size=self._max_batch_size)
            if batch:
    ...
    def prepare(self, file_spec: CreateArtifactFileSpecInput) -> queue.Queue[ResponsePrepare]:
