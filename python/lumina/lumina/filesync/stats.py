...
from lumina.sdk.lib import filenames
...
class Stats:
    ...
    def update_uploaded_file(self, save_name: str, total_uploaded: int) -> None:
        with self._lock:
            self._stats[save_name] = self._stats[save_name]._replace(uploaded=total_uploaded)
    ...
    def summary(self) -> Summary:
        ...
        return Summary(uploaded_bytes=sum((f.uploaded for f in stats)), total_bytes=sum((f.total for f in stats)), deduped_bytes=sum((f.total for f in stats if f.deduped)))
    ...
    def file_counts_by_category(self) -> FileCountsByCategory:
        ...
        for save_name, stats in file_stats:
            if stats.artifact_file:
                ...
                other_files += 1
        return FileCountsByCategory(artifact=artifact_files, wandb=wandb_files, media=media_files, other=other_files)
